...
Emails and phone numbers have separate formatting rules. Email masking lowercases everything and keeps only first/last name characters. Phone masking strips all punctuation, keeps the last four digits, and masks the optional country code length.

Data structure: a digit list extracts normalized phone digits.

Edge cases: email names have at least two characters by constraints. Phone country code can have length 0 to 3, which controls the leading '+***-' prefix.

Complexity: O(n) time and O(n) space for normalization.
"""
