#
# @lc app=leetcode id=2722 lang=python3
#
...
# Testcase Example:  "[{\"id\": 1,\"x\": 1},{\"id\": 2,\"x\": 9}]\n[{\"id\": 3,\"x\": 5}]"
#
# Given two arrays arr1 and arr2, return a new array joinedArray. All the
# objects in each of the two inputs arrays will contain an id field that has an
# integer value.
#
...
#     {"id": 3, "x": 5}
# ]
# Explanation: There are no duplicate ids so arr1 is simply concatenated with
# arr2.
#
# Example 2:
#
...
class Solution:
    def join(self, arr1: List[Dict[str, Any]], arr2: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        JavaScript problem (Python analog): join two object arrays on id; arr2 keys
        overwrite arr1 on conflict; return sorted by id.

        Algorithm:
        - Index by id from arr1 then merge arr2; sort values by id.
