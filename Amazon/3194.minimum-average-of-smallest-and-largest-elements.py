#
# @lc app=leetcode id=3194 lang=python3
#
# [3194] Minimum Average of Smallest and Largest Elements
#
# https://leetcode.com/problems/minimum-average-of-smallest-and-largest-elements/description/
#
# algorithms
# Easy (85.52%)
# Likes:    220
# Dislikes: 19
# Total Accepted:    116.5K
# Total Submissions: 136.3K
# Testcase Example:  "[7,8,3,4,15,13,4,1]"
#
#
# You have an array of floating point numbers averages which is initially
# empty. You are given an array nums of n integers where n is even.
#
# You repeat the following procedure n / 2 times:
#
# Remove the smallest element, minElement, and the largest element
# maxElement, from nums.
#
# Add (minElement + maxElement) / 2 to averages.
#
# Return the minimum element in averages.
#
# Example 1:
#
# Input: nums = [7,8,3,4,15,13,4,1]
#
# Output: 5.5
#
# Explanation:
#
#                         step
#                         nums
#                         averages
#
#                         0
#                         [7,8,3,4,15,13,4,1]
#                         []
#
#                         1
#                         [7,8,3,4,13,4]
#                         [8]
#
#                         2
#                         [7,8,4,4]
#                         [8,8]
#
#                         3
#                         [7,4]
#                         [8,8,6]
#
#                         4
#                         []
#                         [8,8,6,5.5]
#
# The smallest element of averages, 5.5, is returned.
#
# Example 2:
#
# Input: nums = [1,9,8,3,10,5]
#
# Output: 5.5
#
# Explanation:
#
#                         step
#                         nums
#                         averages
#
#                         0
#                         [1,9,8,3,10,5]
#                         []
#
#                         1
#                         [9,8,3,5]
#                         [5.5]
#
#                         2
#                         [8,5]
#                         [5.5,6]
#
#                         3
#                         []
#                         [5.5,6,6.5]
#
# Example 3:
#
# Input: nums = [1,2,3,7,8,9]
#
# Output: 5.0
#
# Explanation:
#
#                         step
#                         nums
#                         averages
#
#                         0
#                         [1,2,3,7,8,9]
#                         []
#
#                         1
#                         [2,3,7,8]
#                         [5]
#
#                         2
#                         [3,7]
#                         [5,5]
#
#                         3
#                         []
#                         [5,5,5]
#
# Constraints:
#
# 2 <= n == nums.length <= 50
#
# n is even.
#
# 1 <= nums[i] <= 50
#

# @lc code=start

from typing import List


class Solution:
    def minimumAverage(self, nums: List[int]) -> float:
        """
        Interview explanation:
        Repeatedly remove min and max, record their average; return the minimum
        of those averages. Sorting makes each pair nums[i]+nums[n-1-i].

        Algorithm:
        - Sort; for i in 0..n/2-1 track min of (nums[i]+nums[n-1-i])/2.

        Complexity: O(n log n) time, O(n) or O(1) extra depending on sort.
        """
        nums.sort()
        n = len(nums)
        return min((nums[i] + nums[n - 1 - i]) / 2 for i in range(n // 2))

    def minimumAverage_two_pointer(self, nums: List[int]) -> float:
        """
        Interview explanation:
        Same sorted pairing with explicit two pointers.

        Algorithm:
        - Sort; l,r = 0,n-1; shrink while tracking minimum average.

        Complexity: O(n log n) time, O(1) extra space.
        """
        nums.sort()
        l, r = 0, len(nums) - 1
        ans = float("inf")
        while l < r:
            ans = min(ans, (nums[l] + nums[r]) / 2)
            l += 1
            r -= 1
        return ans
# @lc code=end
