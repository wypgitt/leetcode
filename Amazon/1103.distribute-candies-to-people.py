...
class Solution:
    def distributeCandies(self, candies: int, num_people: int) -> List[int]:
        ...
        Give 1,2,3,... candies in round-robin until candies run out; last gift
        may be partial. Simulate with a counter.

        Algorithm (simulation):
        - ans = [0]*n; give=1; i=0.
        - While candies: take=min(give,candies); ans[i%n]+=take; candies-=take; give++.

        Complexity: O(√candies) gifts since Σk ~ candies, O(n) space.
        """
        ans = [0] * num_people
        give = 1
        i = 0
