...
# = [17].
# There is only one stick left, so you are done. The total cost is 4 + 9 +
# 17 = 30.
#
# Example 3:
#
...
class Solution:
    def connectSticks(self, sticks: List[int]) -> int:
        ...
        Premium. Cost to connect two sticks = sum of lengths; total cost to
        merge all into one. Always merge two shortest (Huffman / greedy heap).

        Algorithm (heap):
        - Min-heap all sticks; while >1: pop two, push sum, add sum to cost.
