...
# integer array edges, where edges[i] = [u_i, v_i] indicates there is a message
# channel between servers u_i and v_i, and they can pass any number of messages
...
# processed. Starting from second 1, at the beginning of every second, each data
# server will check if it has received a reply to the message it sent (including
# any newly arrived replies) from the master server:
#
#
# If it has not, it will resend the message periodically. The data server i will
# resend the message every patience[i] second(s), i.e., the data server i will
# resend the message if patience[i] second(s) have elapsed since the last time
# the message was sent from this server.
#
#
# Otherwise, no more resending will occur from this server.
#
# The network becomes idle when there are no messages passing between servers or
# arriving at servers.
#
...
# since this server has sent the message, therefore it resends the message
# (denoted 2B).
#
# At second 2,
...
class Solution:
    def networkBecomesIdle(self, edges: List[List[int]], patience: List[int]) -> int:
        ...
        Server 0 is master. Each data server sends to 0 along shortest path;
        RTT = 2*dist. Retransmits every patience[i] until first reply arrives.
        Idle when last reply arrives; return that time + 1.

        Algorithm:
        - BFS distances from 0. For each node i>0: last send =
          ((RTT-1)//patience[i])*patience[i]; finish = last+RTT; max+1.
        ...
        for i in range(1, n):
            rtt = 2 * dist[i]
            last = ((rtt - 1) // patience[i]) * patience[i]
            ans = max(ans, last + rtt)
        return ans + 1
# @lc code=end
