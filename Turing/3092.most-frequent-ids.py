...
#
# Return an array ans of length n, where ans[i] represents the count of
# the most frequent ID in the collection after the i^th step. If the
...
#
# After step 1, we have 3 IDs with the value of 2 and 2 IDs with the value
# of 3. So ans[1] = 3.
#
# After step 2, we have 2 IDs with the value of 3. So ans[2] = 2.
#
# After step 3, we have 2 IDs with the value of 3 and 1 ID with the value
# of 1. So ans[3] = 2.
#
# Example 2:
#
...
class Solution:
    def mostFrequentIDs(self, nums: List[int], freq: List[int]) -> List[int]:
        ...
        Stream of +freq/-freq updates on IDs; after each step report the max
        frequency in the multiset (0 if empty).

        Algorithm:
        - Maintain counts; push (-count, id) onto a max-heap.
        - Lazy-delete stale heap tops that disagree with current counts.
        ...
        for id_, f in zip(nums, freq):
            cnt[id_] += f
            heapq.heappush(heap, (-cnt[id_], id_))
