...
# divisible by k, then replace the substring of length k starting at i
# with the substring of length k starting at j. That is, replace the
# substring word[i..i + k - 1] with the substring word[j..j + k - 1].
#
...
class Solution:
    def minimumOperationsToMakeKPeriodic(self, word: str, k: int) -> int:
        ...
        Word length n is divisible by k. An operation copies any k-block onto
        another k-aligned block. Make the string k-periodic (all blocks equal).

        Algorithm:
        - Count frequencies of each length-k block; keep the most common.
        - Operations = (n/k) - max_frequency.
