#
# @lc app=leetcode id=410 lang=python3
#
# [410] Split Array Largest Sum
#
# https://leetcode.com/problems/split-array-largest-sum/description/
#
# algorithms
# Hard (61.09%)
# Likes:    11581
# Dislikes: 274
# Total Accepted:    694K
# Total Submissions: 1.1M
# Testcase Example:  "[7,2,5,10,8]"
#
# Given an integer array nums and an integer k, split nums into k non-empty
# subarrays such that the largest sum of any subarray is minimized.
#
# Return the minimized largest sum of the split.
#
# A subarray is a contiguous part of the array.
#
# Example 1:
#
# Input: nums = [7,2,5,10,8], k = 2
# Output: 18
# Explanation: There are four ways to split nums into two subarrays.
# The best way is to split it into [7,2,5] and [10,8], where the largest sum
# among the two subarrays is only 18.
#
# Example 2:
#
# Input: nums = [1,2,3,4,5], k = 2
# Output: 9
# Explanation: There are four ways to split nums into two subarrays.
# The best way is to split it into [1,2,3] and [4,5], where the largest sum
# among the two subarrays is only 9.
#
# Constraints:
#
# 1 <= nums.length <= 1000
#
# 0 <= nums[i] <= 10^6
#
# 1 <= k <= min(50, nums.length)
#

# @lc code=start

from typing import List


class Solution:
    def splitArray(self, nums: List[int], k: int) -> int:
        """
        Interview explanation:
        Binary search the minimized largest subarray sum. Feasible(mid): can we
        split into ≤k contiguous parts each summing ≤ mid?

        Algorithm:
        - lo = max(nums), hi = sum(nums).
        - While lo<hi: mid=(lo+hi)//2; if feasible(mid) hi=mid else lo=mid+1.
        - feasible: greedy count parts needed for capacity mid.

        Complexity: O(n log S) time, O(1) space.
        """
        def can_split(cap: int) -> bool:
            parts = 1
            cur = 0
            for x in nums:
                if cur + x > cap:
                    parts += 1
                    cur = 0
                    if parts > k:
                        return False
                cur += x
            return True

        lo, hi = max(nums), sum(nums)
        while lo < hi:
            mid = (lo + hi) // 2
            if can_split(mid):
                hi = mid
            else:
                lo = mid + 1
        return lo

    def splitArrayDP(self, nums: List[int], k: int) -> int:
        """
        Interview explanation:
        Alternate DP: dp[i][j] = min largest-sum splitting first i elements into
        j parts. Transition over last cut position.

        Algorithm:
        - prefix sums; dp[i][1]=prefix[i]; for j=2..k fill over cut p.

        Complexity: O(n^2 k) time, O(nk) space.
        """
        n = len(nums)
        prefix = [0] * (n + 1)
        for i, x in enumerate(nums):
            prefix[i + 1] = prefix[i] + x
        INF = 10**18
        dp = [[INF] * (k + 1) for _ in range(n + 1)]
        for i in range(1, n + 1):
            dp[i][1] = prefix[i]
        for j in range(2, k + 1):
            for i in range(j, n + 1):
                for p in range(j - 1, i):
                    dp[i][j] = min(dp[i][j], max(dp[p][j - 1], prefix[i] - prefix[p]))
        return dp[n][k]
# @lc code=end
