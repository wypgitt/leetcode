...
# crashTime is the time (in seconds) when the program crashed and memory1_crash
# and memory2_crash are the available bits of memory in the first and second
# sticks respectively.
#
# Example 1:
#
...
class Solution:
    def memLeak(self, memory1: int, memory2: int) -> List[int]:
        ...
        Each second i allocates i bits to the stick with ≥ memory (prefer
        memory1 on ties). Continue until neither can take i. Return
        [crash_second, mem1, mem2].

        Algorithm (simulation):
        - i=1; while True: pick larger (or mem1); if <i break; subtract i; i++.

        Complexity: O(sqrt(memory1+memory2)) time, O(1) space.
        """
        i = 1
        while True:
            if memory1 >= memory2:
