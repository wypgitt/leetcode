...
class Solution {
    long long geom(long long base, int power, long long limit) {
        long long total = 1, term = 1;
        for (int i = 0; i < power; ++i) {
            if (term > limit / base) return limit + 1;
            term *= base;
            total += term;
...
public:
    string smallestGoodBase(string n) {
        ...
        int maxPower = 63 - __builtin_clzll(num);
        for (int power = maxPower; power > 1; --power) {
            long long l = 2, r = num - 1;
            while (l <= r) {
                long long mid = (l + r) / 2;
                long long s = geom(mid, power, num);
