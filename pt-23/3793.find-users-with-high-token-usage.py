#
# @lc app=leetcode id=3793 lang=python3
#
...
# Included in the result
#
# The Results table is ordered by avg_tokens in descending order, then by
...
class Solution:
    def solve(self) -> str:
        ...
        Per-user prompt stats: count and average tokens; keep users with >= 3
        prompts and at least one prompt above their own average (max > avg).

        Algorithm:
        - GROUP BY user_id with COUNT and ROUND(AVG(tokens), 2).
        - HAVING prompt_count >= 3 AND MAX(tokens) > avg_tokens.
        - ORDER BY avg_tokens DESC, user_id ASC.

        Complexity: O(N) aggregation over prompts.
        """
        self.sql = """
SELECT
    user_id,
