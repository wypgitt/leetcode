#
# @lc app=leetcode id=3626 lang=python3
#
...
# +----------+----------------+-------------+------------------+--------------------+------------------+
# | 3        | City Center    | Los Angeles | Tablet           | Stylus
# | 40.00            |
# | 2        | Suburb Mall    | Chicago     | Phone            | Case
# | 25.00            |
# | 1        | Downtown Tech  | New York    | Laptop           | Mouse
# | 10.00            |
# +----------+----------------+-------------+------------------+--------------------+------------------+
#
# Explanation:
...
# meet minimum 3 products requirement
#
# The Results table is ordered by imbalance ratio in descending order,
# then by store name in ascending order
#
...
class Solution:
    def solve(self) -> str:
        ...
        Find stores with >=3 products where the most expensive item has less
        stock than the cheapest; report imbalance_ratio = cheap_qty / exp_qty.

        Algorithm:
        - Window ranks: most expensive (price DESC, qty DESC) and cheapest
          (price ASC, qty DESC) per store; COUNT products per store.
        - Keep stores with cnt>=3 and exp.qty < cheap.qty.
        - ROUND(cheap.qty / exp.qty, 2); ORDER BY ratio DESC, store_name ASC.

        Complexity: O(N log N) with window sorts over inventory rows.
        """
        self.sql = """
WITH
    T AS (
...
FROM P1 p1
JOIN P2 p2
    ON p1.store_id = p2.store_id AND p1.quantity < p2.quantity
JOIN stores s
    ON p1.store_id = s.store_id
