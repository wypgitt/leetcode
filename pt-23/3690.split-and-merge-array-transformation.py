#
# @lc app=leetcode id=3690 lang=python3
#
...
class Solution:
    def minSplitMerge(self, nums1: List[int], nums2: List[int]) -> int:
        ...
        n <= 6, so BFS over permutations under the split-and-move operation
        finds the shortest transformation.

        Algorithm:
        - State = tuple(array). From each state, try every subarray cut and
          every reinsertion index; BFS until nums2.
        - Alternate: bidirectional BFS (same idea, fewer states explored).

        Complexity: O(n! * n^3) time in worst case (tiny n).
        """
        start = tuple(nums1)
        target = tuple(nums2)
        if start == target:
