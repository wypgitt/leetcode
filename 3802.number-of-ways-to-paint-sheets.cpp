...
class Solution {
    ...
    long long countAtLeast(const vector<int>& limits, int need) {
        return limits.end() - lower_bound(limits.begin(), limits.end(), need);
    }
...
public:
    int numberOfWays(int n, vector<int>& limit) {
        ...
        set<int> bounds{1, n};
        for (int v : limit) for (int p : {v, v + 1, n - v, n - v + 1}) if (1 <= p && p <= n) bounds.insert(p);
        vector<int> pts(bounds.begin(), bounds.end());
        long long ans = 0;
        for (int i = 0; i + 1 < (int)pts.size(); ++i) {
            int left = pts[i], right = pts[i + 1];
            if (left > n - 1) break;
            int length = min(right, n) - left;
            if (length <= 0) continue;
            long long first = countAtLeast(limit, left);
            long long second = countAtLeast(limit, n - left);
            long long same = countAtLeast(limit, max(left, n - left));
            long long per = first * second - same;
            ans = (ans + per % MOD * length) % MOD;
        }
        return ans;
    }
};
