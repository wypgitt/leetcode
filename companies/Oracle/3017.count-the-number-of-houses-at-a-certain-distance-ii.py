...
class Solution:
    def countOfPairs(self, n: int, x: int, y: int) -> List[int]:
        ...
        Same distance-histogram as I, but n <= 1e5. For each source, distances on
        the path-plus-chord are piecewise-linear; encode contributions with a
        difference array so each house updates O(1) range endpoints.

        Algorithm:
        - 0-index x,y and ensure x <= y.
        - For each i, add +2 at dist 0 (two directed rays), fork +1 toward the
          chord endpoints, and -1 where each ray dies (line ends / meeting on
          the chord). Prefix-sum the difference array.
