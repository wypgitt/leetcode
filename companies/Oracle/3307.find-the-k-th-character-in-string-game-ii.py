...
# operations, where operations[i] represents the type of the i^th
# operation.
#
...
#
# The input is generated such that word has at least k characters after
# all operations.
#
...
class Solution:
    def kthCharacter(self, k: int, operations: List[int]) -> str:
        ...
        Ops double the string: type 0 copies, type 1 appends next-letters.
        k can be 1e14, so walk backwards through the halves that contain k.

        Algorithm:
        - Work with 0-index k - 1. While in the right half of op j, subtract 2^j
          and add one shift if operations[j] == 1.
        - Answer 'a' + (shift mod 26).
