...
/**
 * Definition for a binary tree node (also supplied by LeetCode; keep for local compilation).
 */
class TreeNode {
    int val;
    TreeNode left;
...
class Solution {
    ...
    private TreeNode dfs(int[] preorder, int hi) {
        if (i == preorder.length || preorder[i] > hi) {
            return null;
        }
        int v = preorder[i++];
        TreeNode root = new TreeNode(v);
        root.left = dfs(preorder, v);
        root.right = dfs(preorder, hi);
        return root;
    }
}
// @lc code=end
