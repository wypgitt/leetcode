...
# If a customer checks in at time t_1 then checks out at time t_2, then t_1 <
# t_2. All events happen in chronological order.
#
# Example 1:
#
# Input
...
class UndergroundSystem:
    def __init__(self):
        ...
        Track check-ins by id and aggregate total time/count per station pair
        for average travel time queries.

        Algorithm:
        - checkin: id → (station, t); trips: (start,end) → [total, count]

        Complexity: O(1) init.
        """
        self.inp: Dict[int, Tuple[str, int]] = {}
        self.stats: Dict[Tuple[str, str], list] = {}

    def checkIn(self, id: int, stationName: str, t: int) -> None:
        """
        Interview explanation:
        Record passenger check-in station and time (one active trip per id).

        Algorithm:
        - inp[id] = (stationName, t)

        Complexity: O(1).
        """
        self.inp[id] = (stationName, t)

    def checkOut(self, id: int, stationName: str, t: int) -> None:
        """
        Interview explanation:
        Complete trip; add duration to (start,end) aggregates.

        Algorithm:
        - start,t0=inp.pop(id); stats[(start,end)] += (t-t0, 1)

        Complexity: O(1).
        """
        start, t0 = self.inp.pop(id)
    ...
    def getAverageTime(self, startStation: str, endStation: str) -> float:
        """
        Interview explanation:
        Average travel time for completed trips between the two stations.

        Algorithm:
        - total/count for (start,end)
