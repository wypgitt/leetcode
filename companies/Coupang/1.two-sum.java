 ...
 *     position is some earlier value y with y = target − x. A HashMap maps each
 ...
 * =============================================================================
 *
 * Java HashMap<Integer, Integer>: hash map / unordered map — average O(1) get
 * and put for integer keys. Keys are numbers from nums; values are their indices.
 *
...
class Solution {
    /**
     * Return two distinct indices {@code i < j} such that {@code nums[i] + nums[j] == target}.
     *
     * <p>Strategy: hash map from value → index for elements already seen. At index
    ...
    public int[] twoSum(int[] nums, int target) {
        ...
        return new int[0]; // unreachable when exactly one solution exists (LeetCode guarantee)
    }
}
// @lc code=end
