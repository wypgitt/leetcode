...
class Solution:
    def minOperationsMaxProfit(self, customers: List[int], boardingCost: int, runningCost: int) -> int:
        ...
        Wheel boards up to 4 waiting customers per rotation; rotations continue
        until queue empty and no future customers. Profit = boarded*boardingCost
        - rotations*runningCost. Return min rotations to reach max profit, or -1
        if never positive.

        Algorithm (simulate):
        - wait=0; i=0; rot=0; profit=0; best=-1; best_rot=-1.
        - While wait or i < len: if i<len wait+=customers[i]; i+=1;
          board=min(4,wait); wait-=board; rot+=1; profit+=board*bc-rc;
          update best.

        Complexity: O(n + total_customers/4) ~ O(sum customers) rotations.
        """
        wait = 0
        i = 0
        rot = 0
