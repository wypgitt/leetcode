/*
 * @lc app=leetcode id=1088 lang=java
 *
...
public class Solution {
    ...
    private static final int[] ROT = {0, 1, -1, -1, -1, -1, 9, -1, 8, 6};
    private String s;

    private int rotatedValue(int x) {
        int y = 0;
        int t = x;
        while (t > 0) {
            int v = t % 10;
    ...
    private boolean isConfusing(int x) {
        return x != rotatedValue(x);
    }

    private int dfs(int pos, boolean tight, int value) {
        if (pos == s.length()) {
            return isConfusing(value) ? 1 : 0;
        }
