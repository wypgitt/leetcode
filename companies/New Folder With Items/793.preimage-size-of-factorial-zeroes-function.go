...
// PreimageSizeFZF793 returns 5 if k is in the image of Z(n)=trailing_zeros(n!), otherwise 0.
func PreimageSizeFZF793(k int) int {
	trailingZerosFactorial := func(n int) int {
		z := 0
		for n > 0 {
			n /= 5
			z += n
	...
	if trailingZerosFactorial(lo) == k {
		return 5
	}
	return 0
}
