...
class Solution {
    bool canFinishAfter(int index, int missing, vector<int>& last) {
        for (int ch = 0; ch < 26; ++ch) if ((missing >> ch) & 1 && last[ch] <= index) return false;
        return true;
    }
...
public:
    string lexSmallestAfterDeletion(string s) {
        ...
        for (int i = 0; i < (int)s.size(); ++i) positions[s[i] - 'a'].push_back(i);
        vector<int> last(26, -1), ptr(26);
        int missing = 0;
        for (int ch = 0; ch < 26; ++ch) if (!positions[ch].empty()) {
            last[ch] = positions[ch].back();
            missing |= 1 << ch;
        }
        int start = 0;
        string ans;
        while (missing) {
            for (int ch = 0; ch < 26; ++ch) {
                auto& v = positions[ch];
                while (ptr[ch] < (int)v.size() && v[ptr[ch]] < start) ++ptr[ch];
                if (ptr[ch] == (int)v.size()) continue;
                int idx = v[ptr[ch]];
                int nextMissing = missing & ~(1 << ch);
                if (canFinishAfter(idx, nextMissing, last)) {
