...
public:
    ...
    void add(int i, int delta) { for (++i; i <= n; i += i & -i) tree[i] += delta; }
    int sum(int i) { int total = 0; for (++i; i > 0; i -= i & -i) total += tree[i]; return total; }
    int rangeSum(int l, int r) { return l > r ? 0 : sum(r) - (l > 0 ? sum(l - 1) : 0); }
};
...
public:
    vector<int> minDeletions(string s, vector<vector<int>>& queries) {
        ...
        int n = chars.size();
        FenwickTree bit(max(1, n - 1));
        auto edge = [&](int i) { return chars[i] != chars[i + 1] ? 1 : 0; };
        for (int i = 0; i + 1 < n; ++i) if (edge(i)) bit.add(i, 1);
        auto refresh = [&](int i, int oldValue) {
            if (0 <= i && i + 1 < n) {
                int nv = edge(i);
                if (nv != oldValue) bit.add(i, nv - oldValue);
            }
        };
        vector<int> ans;
        for (auto& q : queries) {
            if (q[0] == 1) {
                int i = q[1];
                int leftOld = i > 0 ? edge(i - 1) : 0;
                int rightOld = i + 1 < n ? edge(i) : 0;
                chars[i] = chars[i] == 'A' ? 'B' : 'A';
                refresh(i - 1, leftOld);
                refresh(i, rightOld);
            } else {
                int l = q[1], r = q[2];
                int transitions = bit.rangeSum(l, r - 1);
                int len = r - l + 1;
                ans.push_back(len - (transitions + 1));
            }
        }
        return ans;
    }
