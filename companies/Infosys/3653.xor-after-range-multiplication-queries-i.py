...
class Solution:
    def xorAfterQueries(self, nums: List[int], queries: List[List[int]]) -> int:
        ...
        Apply each stepped range multiply modulo 1e9+7, then XOR the array.

        Algorithm:
        - For each [l,r,k,v], idx=l; while idx<=r: nums[idx]*=v % MOD; idx+=k.
        - Fold XOR over nums.

        Complexity: O(n + Σ (r-l)/k) ≤ O(n*q) with n,q ≤ 1000.
        """
        MOD = 10**9 + 7
