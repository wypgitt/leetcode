#
# @lc app=leetcode id=1484 lang=python3
#
...
class Solution:
    def solve(self) -> str:
        ...
        For each sell_date: count distinct products and list their names
        lexicographically joined by comma. Order by sell_date.

        Algorithm:
        - GROUP BY sell_date; COUNT(DISTINCT product); GROUP_CONCAT(DISTINCT
          product ORDER BY product SEPARATOR ',').

        Complexity: O(N log N) with sorts inside aggregation.
        """
        return self.sql
