...
class Solution:
    def shortestPathWithHops(self, n: int, edges: List[List[int]], s: int, d: int, k: int) -> int:
        ...
        Undirected weighted graph; up to k edges may be taken as cost 0. Min cost s→d.

        Algorithm:
        - Dijkstra on states (node, hops_used). From (u,t) pay w to (v,t), or if t<k
          hop to (v,t+1) at cost 0.

        Complexity: O((n+m)k log(nk)) time, O(nk + m) space.
        """
        g = [[] for _ in range(n)]
        for u, v, w in edges:
            g[u].append((v, w))
        ...
        while pq:
            ...
            for v, w in g[u]:
                ...
                if t < k and du < dist[v][t + 1]:
                    dist[v][t + 1] = du
                    heapq.heappush(pq, (du, v, t + 1))
        return min(dist[d])

    def shortestPathWithHops_dijkstra(self, n: int, edges: List[List[int]], s: int, d: int, k: int) -> int:
        """
        Interview explanation:
        Alternate naming for layered-graph Dijkstra with hop resource.
        ...
        Complexity: O((n+m)k log(nk)) time, O(nk + m) space.
        """
        return self.shortestPathWithHops(n, edges, s, d, k)
# @lc code=end
