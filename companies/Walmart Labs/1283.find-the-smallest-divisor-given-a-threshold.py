...
class Solution:
    def smallestDivisor(self, nums: List[int], threshold: int) -> int:
        ...
        Find minimal divisor d such that sum(ceil(num/d)) <= threshold.
        Monotonic in d → binary search on answer.

        Algorithm:
        - lo=1, hi=max(nums); while lo<hi: mid; if sum_ceil<=threshold: hi=mid
