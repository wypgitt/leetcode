...
class Solution:
    def getKth(self, lo: int, hi: int, k: int) -> int:
        ...
        Power value = Collatz steps to reach 1. Sort [lo,hi] by power then value;
        return k-th (1-indexed).

        Algorithm:
        - Memoized power(x); sorted(range(lo,hi+1), key=power)[k-1]

        Complexity: O((hi-lo) * L + n log n) with L Collatz steps bound.
        """
        @lru_cache(None)
        def power(x: int) -> int:
            if x == 1:
