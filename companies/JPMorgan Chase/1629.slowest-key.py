...
# releaseTimes[i - 1], and the 0^th keypress had a duration of releaseTimes[0].
#
...
class Solution:
    def slowestKey(self, releaseTimes: List[int], keysPressed: str) -> str:
        ...
        Duration of key i is releaseTimes[i]-releaseTimes[i-1] (0 for first).
        Return longest duration key; tie → lexicographically largest key.

        Algorithm (linear scan):
        - Track best duration and best key while scanning.

        Complexity: O(n) time, O(1) space.
        """
        best_d = releaseTimes[0]
        best_k = keysPressed[0]
    ...
    def slowestKey_enumerate(self, releaseTimes: List[int], keysPressed: str) -> str:
        """
        Interview explanation:
        Alternate: compute all durations then max with key tie-break.

        Algorithm:
        - durations list; max by (duration, key).

        Complexity: O(n) time, O(n) space.
        """
        durs = [releaseTimes[0]] + [
            releaseTimes[i] - releaseTimes[i - 1] for i in range(1, len(releaseTimes))
        ]
        best_i = max(range(len(durs)), key=lambda i: (durs[i], keysPressed[i]))
        return keysPressed[best_i]
# @lc code=end
