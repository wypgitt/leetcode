...
#
# You are given an array rectangles where rectangles[i] = [l_i, w_i] represents
# the i^th rectangle of length l_i and width w_i.
#
# You can cut the i^th rectangle to form a square with a side length of k if
# both k <= l_i and k <= w_i. For example, if you have a rectangle [4,6], you
...
class Solution:
    def countGoodRectangles(self, rectangles: List[List[int]]) -> int:
        ...
        Largest square from rectangle (l,w) has side min(l,w). Count how many
        rectangles achieve the global maximum side length.

        Algorithm:
        - sides = min(l,w); maxside = max(sides); count equals.
