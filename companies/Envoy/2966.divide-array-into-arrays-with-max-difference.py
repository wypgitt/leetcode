...
# the conditions, return an empty array. And if there are multiple
...
#
# Because there are four 2s there will be an array with the elements 2 and
# 5 no matter how we divide it. since 5 - 2 = 3 > k, the condition is not
...
class Solution:
    def divideArray(self, nums: List[int], k: int) -> List[List[int]]:
        ...
        Partition into triples where max-min <= k. Greedy after sorting is optimal.

        Algorithm:
        - Sort nums. Take consecutive triples; if nums[i+2]-nums[i] > k, impossible.
