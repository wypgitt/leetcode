...
class Solution:
    def lengthLongestPath(self, input: str) -> int:
        ...
        Paths are encoded with newlines and tab-depth. Stack[depth] stores
        absolute path length up to that directory depth; when we see a file
        (contains '.'), update the max absolute length.

        Algorithm:
        - Split by '\\n'; depth = count of leading '\\t'.
        - name length without tabs; parent_len = stack[depth] (or 0).
        - cur = parent_len + len(name) + (1 if depth else 0) for the '/'.
        - stack[depth+1] = cur; if file, ans = max(ans, cur).

        Complexity: O(n) time, O(depth) space.
        """
        max_len = 0
        # stack[d] = length of path prefix at depth d (including trailing '/')
        stack = {0: 0}
