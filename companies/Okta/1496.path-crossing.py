#
# @lc app=leetcode id=1496 lang=python3
#
# [1496] Path Crossing
#
# https://leetcode.com/problems/path-crossing/description/
#
# algorithms
# Easy (62.65%)
# Likes:    1568
# Dislikes: 50
# Total Accepted:    200K
# Total Submissions: 320K
# Testcase Example:  "\"NES\""
#
# Given a string path, where path[i] = 'N', 'S', 'E' or 'W', each representing
# moving one unit north, south, east, or west, respectively. You start at the
# origin (0, 0) on a 2D plane and walk on the path specified by path.
#
# Return true if the path crosses itself at any point, that is, if at any time
# you are on a location you have previously visited. Return false otherwise.
#
# Example 1:
#
# Input: path = "NES"
# Output: false
# Explanation: Notice that the path doesn't cross any point more than once.
#
# Example 2:
#
# Input: path = "NESWW"
# Output: true
# Explanation: Notice that the path visits the origin twice.
#
# Constraints:
#
# 1 <= path.length <= 10^4
#
# path[i] is either 'N', 'S', 'E', or 'W'.
#

# @lc code=start
class Solution:
    def isPathCrossing(self, path: str) -> bool:
        """
        Interview explanation:
        Walk N/S/E/W from origin; return true if any position visited twice.

        Algorithm:
        - Set of (x,y); start (0,0); for each move update and check membership.

        Complexity: O(n) time/space.
        """
        x = y = 0
        seen = {(0, 0)}
        move = {"N": (0, 1), "S": (0, -1), "E": (1, 0), "W": (-1, 0)}
        for c in path:
            dx, dy = move[c]
            x += dx
            y += dy
            if (x, y) in seen:
                return True
            seen.add((x, y))
        return False

    def isPathCrossing_encode(self, path: str) -> bool:
        """
        Interview explanation:
        Alternate: encode (x,y) as a single int (e.g. x*offset+y) in a set.

        Algorithm:
        - Same walk; seen.add(x*20001+y) with offset for negative coords.

        Complexity: O(n) time/space.
        """
        x = y = 0
        seen = {0}
        move = {"N": (0, 1), "S": (0, -1), "E": (1, 0), "W": (-1, 0)}
        for c in path:
            dx, dy = move[c]
            x += dx
            y += dy
            key = x * 20001 + y
            if key in seen:
                return True
            seen.add(key)
        return False

# @lc code=end
