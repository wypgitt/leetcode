...
class Solution:
    def poorPigs(self, buckets: int, minutesToDie: int, minutesToTest: int) -> int:
        ...
        With T = minutesToTest // minutesToDie tests, each pig has (T+1)
        states (dies on test 1..T, or never). Information: (T+1)^pigs >=
        buckets. Minimal pigs = ceil(log_(T+1)(buckets)).

        Algorithm:
        - states = minutesToTest // minutesToDie + 1
        - Return smallest p with states^p >= buckets.
