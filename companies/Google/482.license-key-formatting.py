...
class Solution:
    def licenseKeyFormatting(self, s: str, k: int) -> str:
        ...
        Remove dashes, uppercase, then regroup from the right into groups of
        size k (first group may be shorter).

        Algorithm:
        - chars = [c.upper() for c in s if c != '-']
        - Build from the end in chunks of k; reverse join with '-'.
        ...
        i = len(chars)
        while i > 0:
            groups.append("".join(chars[max(0, i - k) : i]))
            i -= k
