...
class Solution:
    def goodSubsetofBinaryMatrix(self, grid: List[List[int]]) -> List[int]:
        ...
        Binary matrix (n<=5). Pick a row subset so every column sum <= floor(k/2).
        Only k=1 (all-zero row) or k=2 (bitwise AND of masks == 0) can work.

        Algorithm:
        - Pack each row as a bit mask; return [i] if mask 0; else find two masks
          with AND 0 and return their indices sorted.

        Complexity: O(m * n + 4^n) time with n<=5, O(2^n) space.
        """
        first = {}
    ...
    def goodSubsetofBinaryMatrix_bit(self, grid: List[List[int]]) -> List[int]:
        """
        Interview explanation:
        Alternate bit-mask naming for the k∈{1,2} search.

        Algorithm:
        - Same mask map + pairwise AND check.

        Complexity: O(m n + 4^n) time, O(2^n) space.
        """
        return self.goodSubsetofBinaryMatrix(grid)
# @lc code=end
