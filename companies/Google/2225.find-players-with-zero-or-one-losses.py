...
class Solution:
    def findWinners(self, matches: List[List[int]]) -> List[List[int]]:
        ...
        From match winners/losers, return [players with 0 losses, players with
        exactly 1 loss], each sorted ascending. Only players who played.

        Algorithm:
        - Count losses; collect players with loss 0 and 1.
        ...
        for w, l in matches:
            ...
            played.add(l)
            losses[l] += 1
        zero = sorted(p for p in played if losses[p] == 0)
        one = sorted(p for p in played if losses[p] == 1)
        return [zero, one]
# @lc code=end
