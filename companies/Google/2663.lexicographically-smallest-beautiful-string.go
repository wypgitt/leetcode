...
//
// @lc app=leetcode id=2663 lang=golang
//
...
// SmallestBeautifulString2663 returns the lexicographically smallest beautiful string
// strictly larger than s, or "" if none exists.
func SmallestBeautifulString2663(s string, k int) string {
	b := []byte(s)
	n := len(b)

	for i := n - 1; i >= 0; i-- {
		for j := int(b[i]-'a') + 1; j < k; j++ {
			c := byte('a' + j)
			if i > 0 && c == b[i-1] {
				continue
			}
