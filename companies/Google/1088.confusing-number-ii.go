...
func ConfusingNumberII1088(n int) int {
	// rot[d] = image of digit d after 180° rotation; -1 means invalid in any confusing number
	rot := [10]int{0, 1, -1, -1, -1, -1, 9, -1, 8, 6}
	s := strconv.Itoa(n)

	rotatedValue := func(x int) int {
		if x == 0 {
			return 0
		}
		y := 0
	...
	isConfusing := func(x int) bool {
		return x != rotatedValue(x)
	}

	var dfs func(pos int, tight bool, value int) int
	dfs = func(pos int, tight bool, value int) int {
		if pos == len(s) {
			if isConfusing(value) {
				return 1
			}
			return 0
		}
