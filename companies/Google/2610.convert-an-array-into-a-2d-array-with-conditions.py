...
class Solution:
    def findMatrix(self, nums: list[int]) -> list[list[int]]:
        ...
        Pack nums into the minimum number of rows so each row has distinct values.

        Algorithm:
        - Frequency of a value is the row index it must reach (0-based).
        - Place the k-th occurrence of a value into row k-1; grow rows as needed.
