#
# @lc app=leetcode id=185 lang=python3
#
...
class Solution:
    def solve(self) -> str:
        ...
        Top three unique salaries per department — DENSE_RANK so tied salaries
        share a rank and the next distinct salary gets the next integer.

        Algorithm:
        - Window: DENSE_RANK() OVER (PARTITION BY departmentId ORDER BY salary DESC).
        - Keep rnk <= 3; join Department for names.

        Complexity: O(E log E) for the window sort per partition; O(E) space
        for ranked rows.
        """
        return self.sql
