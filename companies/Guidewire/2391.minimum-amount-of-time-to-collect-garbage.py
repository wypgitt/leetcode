...
# the assortment of garbage at the i^th house. garbage[i] consists only of the
...
class Solution:
    def garbageCollection(self, garbage: List[str], travel: List[int]) -> int:
        ...
        Three trucks (M/P/G) start at house 0; pick their garbage type; travel[i]
        is time between house i and i+1. Each piece costs 1. Return total time
        for all trucks (they only go as far as last house with their type).

        Algorithm:
        - Sum all garbage lengths; for each type add travel prefix to last index
          containing that type.

        Complexity: O(n + total garbage chars) time, O(1) space.
        """
        ans = 0
        last = {'M': 0, 'P': 0, 'G': 0}
    ...
    def garbageCollection_prefix(self, garbage: List[str], travel: List[int]) -> int:
        """
        Interview explanation:
        Alternate: precompute travel prefix; for each type find last house.

        Algorithm:
        - Same idea with explicit prefix array.
