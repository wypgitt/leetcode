...
class Solution:
    def oddEvenJumps(self, arr: List[int]) -> int:
        ...
        From i, an odd jump goes to the least value >= arr[i] (smallest index
        on ties); even jump to the greatest value <= arr[i]. Reachability is
        DP: higher[i] = can reach end starting with an odd jump from i;
        lower[i] similarly for even. higher[i] = lower[next_odd[i]];
        lower[i] = higher[next_even[i]]. Count indices with higher[i] True
        (first jump is always odd). Build next jumps via monotonic stack on
        value-sorted indices.

        Algorithm (DP + stack):
        - Sort indices by value (then index) for odd next; reverse for even.
        - Monotonic increasing index stack: next[i] = first larger index in
          that order (ceiling / floor jump targets).
        - DP from the end: higher[-1]=lower[-1]=True; propagate via next.
        - Answer = number of True higher[i].

        Complexity: O(n log n) time (sort), O(n) space.
        """
        n = len(arr)

        def make_next(order: List[int]) -> List[int]:
            nxt = [-1] * n
            stack: List[int] = []
            for i in order:
        ...
        odd_next = make_next(odd_order)
        even_next = make_next(even_order)
        ...
        for i in range(n - 2, -1, -1):
            if odd_next[i] != -1:
                higher[i] = lower[odd_next[i]]
            if even_next[i] != -1:
                lower[i] = higher[even_next[i]]
        return sum(higher)

    def oddEvenJumps_sortedmap(self, arr: List[int]) -> int:
        """
        Interview explanation:
        Alternate: process indices right-to-left, maintaining a SortedMap of
        value -> index for positions to the right. Odd jump = ceiling key;
        even jump = floor key. Same DP as above. Here simulated with a sorted
        list of (value, index) and bisect (TreeMap / SortedDict equivalent).

        Algorithm:
        - higher[-1]=lower[-1]=True; insert arr[-1] into sorted structure.
        - For i from n-2..0: ceiling/floor via bisect; set higher/lower; insert.
        ...
        lower = [False] * n
        higher[-1] = lower[-1] = True
        # sorted list of values seen to the right; map value -> latest index
        vals: List[int] = [arr[-1]]
        idx_of = {arr[-1]: n - 1}

        for i in range(n - 2, -1, -1):
            v = arr[i]
            # odd: smallest value >= v
            j = bisect.bisect_left(vals, v)
            if j < len(vals):
                higher[i] = lower[idx_of[vals[j]]]
            # even: largest value <= v
            j2 = bisect.bisect_right(vals, v) - 1
            if j2 >= 0:
                lower[i] = higher[idx_of[vals[j2]]]
            pos = bisect.bisect_left(vals, v)
