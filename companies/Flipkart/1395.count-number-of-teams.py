#
# @lc app=leetcode id=1395 lang=python3
#
# [1395] Count Number of Teams
#
# https://leetcode.com/problems/count-number-of-teams/description/
#
# algorithms
# Medium (70.18%)
# Likes:    3472
# Dislikes: 237
# Total Accepted:    247K
# Total Submissions: 353K
# Testcase Example:  "[2,5,3,4,1]"
#
# There are n soldiers standing in a line. Each soldier is assigned a unique
# rating value.
#
# You have to form a team of 3 soldiers amongst them under the following rules:
#
# Choose 3 soldiers with index (i, j, k) with rating (rating[i], rating[j],
# rating[k]).
#
# A team is valid if: (rating[i] < rating[j] < rating[k]) or (rating[i] >
# rating[j] > rating[k]) where (0 <= i < j < k < n).
#
# Return the number of teams you can form given the conditions. (soldiers can
# be part of multiple teams).
#
# Example 1:
#
# Input: rating = [2,5,3,4,1]
# Output: 3
# Explanation: We can form three teams given the conditions. (2,3,4), (5,4,1),
# (5,3,1).
#
# Example 2:
#
# Input: rating = [2,1,3]
# Output: 0
# Explanation: We can't form any team given the conditions.
#
# Example 3:
#
# Input: rating = [1,2,3,4]
# Output: 4
#
# Constraints:
#
# n == rating.length
#
# 3 <= n <= 1000
#
# 1 <= rating[i] <= 10^5
#
# All the integers in rating are unique.
#

# @lc code=start

from typing import List


class Solution:
    def numTeams(self, rating: List[int]) -> int:
        """
        Interview explanation:
        Count increasing/decreasing triplets. For each middle index j, count
        left smaller/larger and right larger/smaller; add products.

        Algorithm:
        - For j in 1..n-2: count L<, L>, R<, R>; ans += Ls*Rg + Lg*Rs

        Complexity: O(n^2) time, O(1) space.
        """
        n = len(rating)
        ans = 0
        for j in range(1, n - 1):
            ls = lg = rs = rg = 0
            for i in range(j):
                if rating[i] < rating[j]:
                    ls += 1
                elif rating[i] > rating[j]:
                    lg += 1
            for k in range(j + 1, n):
                if rating[k] < rating[j]:
                    rs += 1
                elif rating[k] > rating[j]:
                    rg += 1
            ans += ls * rg + lg * rs
        return ans

    def numTeams_brute(self, rating: List[int]) -> int:
        """
        Interview explanation:
        Alternate O(n^3) enumeration of all triplets (acceptable for n≤1000? no —
        constraints n≤1000 so O(n^3) TLE; kept for clarity on tiny n).

        Algorithm:
        - Triple loop count strict mono triplets

        Complexity: O(n^3) time, O(1) space.
        """
        n = len(rating)
        ans = 0
        for i in range(n):
            for j in range(i + 1, n):
                for k in range(j + 1, n):
                    if rating[i] < rating[j] < rating[k] or rating[i] > rating[j] > rating[k]:
                        ans += 1
        return ans
# @lc code=end
