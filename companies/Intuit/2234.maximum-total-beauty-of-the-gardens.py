...
#
# You are given a 0-indexed integer array flowers of size n, where flowers[i] is
# the number of flowers already planted in the i^th garden. Flowers that are
# already planted cannot be removed. You are then given another integer
# newFlowers, which is the maximum number of flowers that Alice can additionally
# plant. You are also given the integers target, full, and partial.
#
...
class Solution:
    def maximumBeauty(self, flowers: List[int], newFlowers: int, target: int, full: int, partial: int) -> int:
        ...
        Complete garden: flowers >= target (worth `full`). Incomplete gardens
        contribute partial * min(incomplete). Plant <= newFlowers extras; max beauty.

        Algorithm:
        (enumerate complete count + binary search)
        - Sort flowers. Start from already-complete count; successively fill the
          next-richest incomplete garden to target. With remaining flowers, binary
          search how far we can level the left incomplete prefix, then distribute
          extras evenly; clamp min to target-1.
