...
class Solution:
    def watchedVideosByFriends(
        ...
        Friends form an undirected graph. Find people exactly `level` away from
        id (BFS), tally their watched videos, return videos sorted by frequency
        then lexicographically.

        Algorithm (BFS):
        - BFS from id for `level` layers; count videos of that layer; sort.

        Complexity: O(n + e + v log v) time.
        """
        n = len(friends)
        q = deque([id])
        seen = {id}
