...
class Solution:
    def destroyTargets(self, nums: List[int], space: int) -> int:
        ...
        Destroying x also destroys all nums[i] == x + c*space. Choose x in nums
        maximizing destroyed count; ties -> smallest x.

        Algorithm:
        - Group by residue mod space; pick residue with max count, then min value.
        ...
        for vals in groups.values():
            c = len(vals)
            m = min(vals)
            if c > best_cnt or (c == best_cnt and m < best):
                best_cnt = c
                best = m
        return best
# @lc code=end
