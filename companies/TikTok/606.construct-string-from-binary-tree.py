...
# necessary to indicate the absence of a left child for 2 and the presence of a
# right child.
#
# Constraints:
#
...
class Solution:
    def tree2str(self, root: Optional[TreeNode]) -> str:
        ...
        Preorder DFS serialization: omit empty () pairs except keep left empty
        parentheses when a right child exists.

        Algorithm:
        - DFS(node): value + optional "(left)" + optional "(right)".
        - If only right: include "()" for missing left.
        - If only left: omit right ().

        Complexity: O(N) time and space (output + recursion).
        """
        if not root:
            return ""
