...
class Solution:
    def maxRunTime(self, n: int, batteries: List[int]) -> int:
        ...
        n computers run simultaneously; batteries can be moved freely. Max
        minutes all n run together.

        Algorithm:
        - Feasible T if sum(min(b, T) for b in batteries) >= n*T
          (no battery can contribute more than T to the simultaneous run).

        Complexity: O(m log S) time, O(1) space.
        """
    ...
    def maxRunTime_greedy(self, n: int, batteries: List[int]) -> int:
        """
        Interview explanation:
        Alternate greedy: sort; if largest batteries already > average target,
        assign them dedicated and reduce n.

        Algorithm:
        - Sort ascending; while batteries[-1] > total//n, remove it as dedicated PC.

        Complexity: O(m log m) time, O(1)/O(m) space.
        """
