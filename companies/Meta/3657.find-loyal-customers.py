#
# @lc app=leetcode id=3657 lang=python3
#
...
# calculated as the number of refund transactions divided by the total
# number of transactions (purchases plus refunds).
#
...
class Solution:
    def solve(self) -> str:
        ...
        Loyal customers: ≥3 purchases, active ≥30 days, refund rate < 20%.

        Algorithm:
        - GROUP BY customer_id with purchase count, refund count, date span.
        - HAVING purchases ≥ 3, DATEDIFF(max,min) ≥ 30, refunds/total < 0.2.
        - ORDER BY customer_id.

        Complexity: O(T) over transactions.
        """
        self.sql = """
SELECT
    customer_id
