...
class Solution:
    def minBitwiseArray(self, nums: List[int]) -> List[int]:
        ...
        Same bit identity as I, but nums[i] up to 1e9 so scanning is impossible.
        x | (x + 1) fills x's trailing zeros; invert by clearing the top trailing 1.

        Algorithm:
        - Even -> -1. Else answer = n XOR (1 << (trailing_ones(n) - 1)).
