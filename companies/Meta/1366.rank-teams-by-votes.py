...
class Solution:
    def rankTeams(self, votes: List[str]) -> str:
        ...
        Rank teams by position-vote counts: more 1st-place votes wins, then
        2nd, etc.; ties broken by team letter ascending.

        Algorithm:
        - count[team][pos] +=1; sort teams by (-count tuple, team)

        Complexity: O(V*T + T^2 log T) with T teams, V votes.
        """
        teams = list(votes[0])
        m = len(teams)
