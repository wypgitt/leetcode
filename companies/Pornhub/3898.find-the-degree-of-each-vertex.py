...
# adjacency matrix of an undirected graph with n vertices labeled from 0
# to n - 1.
#
# matrix[i][j] = 1 indicates that there is an edge between vertices i and
# j.
#
# matrix[i][j] = 0 indicates that there is no edge between vertices i and
# j.
#
# The degree of a vertex is the number of edges connected to it.
#
# Return an integer array ans of size n where ans[i] represents the degree
# of vertex i.
#
# Example 1:
#
...
class Solution:
    def findDegrees(self, matrix: list[list[int]]) -> list[int]:
        ...
        Degree of vertex i is the number of 1s in row i of the adjacency matrix.

        Algorithm:
        - Return [sum(row) for row in matrix].

        Complexity: O(n^2) time, O(n) space for the answer.
        """
        return [sum(row) for row in matrix]
# @lc code=end
