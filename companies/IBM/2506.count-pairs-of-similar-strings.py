...
# and 'b'.
# - i = 3 and j = 4 : both words[3] and words[4] only consist of characters 'a',
# 'b', and 'c'.
#
# Example 2:
#
...
class Solution:
    def similarPairs(self, words: List[str]) -> int:
        ...
        Count pairs of words that use exactly the same set of characters.

        Algorithm:
        - Map each word to frozenset(chars); for each word add prior count of same
          key, then increment (combination counting).

        Complexity: O(total chars) time, O(n) space.
        """
        cnt: Counter = Counter()
        ans = 0
        for w in words:
    ...
    def similarPairs_bit(self, words: List[str]) -> int:
        """
        Interview explanation:
        Same set-of-chars similarity via 26-bit masks.

        Algorithm:
        - OR bit (c-'a') per word; count pairs with identical masks.
