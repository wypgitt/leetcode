...
class Solution:
    def nextGreaterElement(self, nums1: List[int], nums2: List[int]) -> List[int]:
        ...
        Monotonic decreasing stack on nums2: for each value, pop smaller tops
        and record them as having this value as next greater. Map nums1 via
        the precomputed next-greater table.

        Algorithm:
        - stack = []; nxt = {}
        - For x in nums2: while stack and stack[-1] < x: nxt[stack.pop()] = x
          push x.
        - For remaining: nxt = -1; answer = [nxt.get(v,-1) for v in nums1].
