...
class Solution:
    def firstDayBeenInAllRooms(self, nextVisit: List[int]) -> int:
        ...
        Rooms 0..n-1. Odd visit to i jumps to nextVisit[i]; even visit goes to
        i+1. DP: dp[i] = first day we reach i (0-indexed days). To reach i+1 we
        need two visits to i; after first arrival at i we bounce to nextVisit[i]
        and must return.

        Algorithm:
        - dp[0]=0; dp[i] = (2*dp[i-1] - dp[nextVisit[i-1]] + 2) mod MOD.
        - Return dp[n-1].
    ...
    def firstDayBeenInAllRooms_clarity(self, nextVisit: List[int]) -> int:
        """
        Interview explanation:
        Alternate same DP with explicit +MOD before mod for negative intermediates.
