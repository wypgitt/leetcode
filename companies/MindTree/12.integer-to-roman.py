...
class Solution:
    def intToRoman(self, num: int) -> str:
        ...
        Roman numerals are naturally greedy when we include the subtractive
        forms (900, 400, 90, 40, 9, 4) in the value table. At each step, append
        the largest symbol whose value fits in the remaining number. This works
        because Roman notation is positional by decreasing value groups.

        Algorithm:
        - Store value/symbol pairs from largest to smallest.
        - For each pair, append symbol count = num // value times.
        - Reduce num by the used amount.

        Edge cases and tests:
        - 3 -> III exercises repetition.
        - 4, 9, 40, 90, 400, 900 exercise subtractive notation.
        - 3999 -> MMMCMXCIX is the maximum constraint value.

        Complexity: O(1) time and O(1) space because the input range is fixed.
        """
        values = [
