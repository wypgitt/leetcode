...
# For the prefix [1, 1, 2], the conversion array is [2, 2, 4] hence the score is
# 8
# For the prefix [1, 1, 2, 4], the conversion array is [2, 2, 4, 8] hence the
# score is 16
# For the prefix [1, 1, 2, 4, 8], the conversion array is [2, 2, 4, 8, 16] hence
# the score is 32
...
class Solution:
    def findPrefixScore(self, nums: List[int]) -> List[int]:
        ...
        For each prefix, score is the prefix sum of conversion values, where
        conver[i] = nums[i] + max(nums[0..i]).

        Algorithm:
        - Track running max (conver partner) and cumulative score; append each
          prefix score.
