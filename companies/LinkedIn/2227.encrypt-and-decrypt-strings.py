...
# array dictionary that contains all permitted original strings after
# decryption. You should implement a data structure that can encrypt or decrypt
# a 0-indexed string.
#
...
#                            // 'a' maps to "ei", 'b' maps to "zf", 'c' maps to
# "ei", and 'd' maps to "am".
# encrypter.decrypt("eizfeiam"); // return 2.
#                               // "ei" can map to 'a' or 'c', "zf" maps to 'b',
# and "am" maps to 'd'.
#                               // Thus, the possible strings after decryption
# are "abad", "cbad", "abcd", and "cbcd".
#                               // 2 of those strings, "abad" and "abcd", appear
# in dictionary, so the answer is 2.
#
#
#
# Constraints:
...
class Encrypter:
    def __init__(self, keys: List[str], values: List[str], dictionary: List[str]):
        ...
        Map each key char to a 2-letter value. encrypt maps chars via keys;
        decrypt counts dictionary words whose encryption equals word2.

        Algorithm:
        - Store key->value; precompute encryption frequency of dictionary words.

        Complexity: init O(|dict| * L), encrypt O(L), decrypt O(1).
        """
    ...
    def encrypt(self, word1: str) -> str:
        """
        Interview explanation:
        Replace each character by its 2-letter mapping; return "" if any unmapped.

        Algorithm:
        - Concatenate mapped digraphs.

        Complexity: O(L) time, O(L) space.
        """
        out = []
        for ch in word1:
            if ch not in self.mp:
                return ""
            out.append(self.mp[ch])
        return "".join(out)

    def decrypt(self, word2: str) -> int:
        """
        Interview explanation:
        Count dictionary words that encrypt to word2 (many-to-one mappings allowed).

        Algorithm:
        - Lookup precomputed frequency map.
