...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        Cars going to target cannot pass; a slower car ahead catches into a
        fleet. Sort by position descending; compute time to target; stack of
        fleet times — a car joins previous fleet if time ≤ fleet ahead.

        Algorithm:
        - Sort (pos,speed) by pos desc; times=(target-pos)/speed; stack of times.
