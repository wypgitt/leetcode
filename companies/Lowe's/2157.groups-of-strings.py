...
# of words.
#
# Two strings s1 and s2 are said to be connected if the set of letters of s2 can
# be obtained from the set of letters of s1 by any one of the following
# operations:
#
#
...
class Solution:
    def groupStrings(self, words: List[str]) -> List[int]:
        ...
        Words are unique-letter sets. Connect if one set is obtained from another
        by adding, deleting, or replacing exactly one letter. Return
        [number_of_groups, size_of_largest_group] (counting duplicate words).

        Algorithm:
        (Union-Find on bitmasks)
        - Map each word to a bitmask; identical masks start in one group.
        - For each mask x and bit i: union with x^(1<<i) (add/delete).
        - If bit i set: for unset j, union with (x^(1<<i))|(1<<j) (replace).
        - Track component count and max size (sum of word frequencies).

        Complexity: O(n * 26^2 * α) time, O(n) space.
        """
        p = {}
        size = Counter()
        n = len(words)
