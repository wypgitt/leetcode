...
class Solution:
    def findReplaceString(
        ...
        Simultaneous replacements: for each (index, source, target), replace
        only if s[index:].startswith(source). Apply from left using a map of
        index → (source_len, target).

        Algorithm:
        - Build dict index→(len(src), tgt) for matches; scan i from 0..n:
          if i in map append tgt and skip len; else append s[i].

        Complexity: O(n + sum |sources|) time, O(n) space.
        """
        replace = {}
        for idx, src, tgt in zip(indices, sources, targets):
            if s.startswith(src, idx):
                replace[idx] = (len(src), tgt)
        out = []
        i = 0
        n = len(s)
        while i < n:
