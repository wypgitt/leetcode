...
# represents the i^th person of height h_i with exactly k_i other people in
# front who have a height greater than or equal to h_i.
#
# Reconstruct and return the queue that is represented by the input array
# people. The returned queue should be formatted as an array queue, where
# queue[j] = [h_j, k_j] is the attributes of the j^th person in the queue
...
class Solution:
    def reconstructQueue(self, people: List[List[int]]) -> List[List[int]]:
        ...
        Sort by height descending, then by k ascending. Insert each person at
        index k into a list: taller people already placed don't affect shorter
        people's counts, so inserting at k leaves exactly k taller-or-equal ahead.

        Algorithm:
        - Sort people by (-h, k).
        - For each [h,k], insert into result at position k.

        Complexity: O(n^2) time (list inserts), O(n) space.
        """
