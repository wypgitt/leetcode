...
class Solution:
    def sortJumbled(self, mapping: List[int], nums: List[int]) -> List[int]:
        ...
        Map each digit of numbers via mapping[d]; sort nums by mapped value
        ascending, keeping original relative order for ties (stable sort).

        Algorithm:
        - Define mapped(x); sorted(nums, key=mapped) is stable in Python.

        Complexity: O(n log n * d) time, O(n) space.
        """
        def mapped(x: int) -> int:
            ...
            res = 0
            mul = 1
            # build from least significant digit
            y = x
            digits = []
            while y:
                digits.append(mapping[y % 10])
                y //= 10
