...
#
#
# int freeMemory(int mID) Free all memory units with the id mID. Return the
# number of memory units you have freed.
#
#
#
# Example 1:
...
# loc.freeMemory(2); // Free all memory units with mID 2. The memory array
# becomes [1,_, 3,_,_,_,_,_,_,_]. We return 1 since there is only 1 unit with
# mID 2.
...
# loc.freeMemory(1); // Free all memory units with mID 1. The memory array
# becomes [_,_,3,4,4,4,_,_,_,_]. We return 3 since there are 3 units with mID 1.
# loc.allocate(10, 2); // We can not find any free block with 10 consecutive
# free memory units, so we return -1.
# loc.freeMemory(7); // Free all memory units with mID 7. The memory array
# remains the same since there is no memory unit with mID 7. We return 0.
#
#
#
# Constraints:
...
class Allocator:
    def __init__(self, n: int):
        ...
        Simulate a size-n memory array; allocate leftmost contiguous free block
        with an mID; freeMemory clears all units with that mID.

        Algorithm:
        - mem[i]=0 free else mID; scan for consecutive frees / clear by id.

        Complexity: init O(n) time, O(n) space.
        """
        self.n = n
        self.mem = [0] * n
    ...
    def allocate(self, size: int, mID: int) -> int:
        """
        Interview explanation:
        Find leftmost run of `size` free units; mark with mID; return start index
        or -1.

        Algorithm:
        - Linear scan counting consecutive zeros; on hit, write mID over the block.
    ...
    def freeMemory(self, mID: int) -> int:
        """
        Interview explanation:
        Free every unit currently tagged mID; return how many units were freed.

        Algorithm:
        - Scan array; zero matching cells and count.
