...
class Solution:
    def maxAverageRatio(self, classes: List[List[int]], extraStudents: int) -> float:
        ...
        Extra brilliant students each boost one class's pass ratio. Greedy:
        always assign the next student to the class with the largest marginal
        gain (pass+1)/(total+1) − pass/total. Max-heap of gains.

        Algorithm:
        - gain(p,t) = (p+1)/(t+1) - p/t
        - Heap (-gain, p, t); pop, add one student, push updated, repeat.

        Complexity: O((n + extra) log n) time, O(n) space.
        """
