...
class Solution:
    def minCost(self, n: int, cuts: List[int]) -> int:
        ...
        Classic stick-cutting DP: cost of a cut = current stick length.
        Sort cuts with sentinels 0 and n; dp[i][j] = min cost to cut open
        interval between cuts[i] and cuts[j] (all intermediate cuts).

        Algorithm:
        - arr=sorted(cuts+[0,n]); dp(i,j)=0 if j=i+1 else min over k in (i,j)
          dp(i,k)+dp(k,j) + (arr[j]-arr[i]).

        Complexity: O(m^3) time, O(m^2) space for m=len(cuts)+2.
        """
        arr = sorted(cuts + [0, n])
        m = len(arr)
        ...
        def dp(i: int, j: int) -> int:
            ...
            return best + arr[j] - arr[i]

        return dp(0, m - 1)

    def minCost_bottomup(self, n: int, cuts: List[int]) -> int:
        """
        Interview explanation:
        Alternate bottom-up interval DP on the same sorted cut array.

        Algorithm:
        - For length=2..m-1, for i, j=i+length: dp[i][j]=min_k + arr[j]-arr[i].
        ...
        for length in range(2, m):
            for i in range(m - length):
                j = i + length
                dp[i][j] = min(dp[i][k] + dp[k][j] for k in range(i + 1, j)) + arr[j] - arr[i]
        return dp[0][m - 1]
# @lc code=end
