...
class Solution:
    def totalStrength(self, strength: List[int]) -> int:
        ...
        Total strength = sum over all subarrays of (min * sum). MOD 1e9+7.

        Algorithm:
        - For each i as strict min of range (left,right) via mono stack, sum
          contributions using prefix-of-prefix sums:
          strength[i] * (rightSum*leftLen - leftSum*rightLen).
        ...
        for i in range(n):
            while stack and strength[stack[-1]] > strength[i]:
                right[stack.pop()] = i
            stack.append(i)
        pref = list(itertools.accumulate(itertools.accumulate(strength), initial=0))
        ans = 0
        for i, (l, r) in enumerate(zip(left, right)):
            left_sum = pref[i] - pref[max(0, l)]
            right_sum = pref[r] - pref[i]
            left_len = i - l
            right_len = r - i
            ans += strength[i] * (right_sum * left_len - left_sum * right_len) % MOD
        return ans % MOD
# @lc code=end
