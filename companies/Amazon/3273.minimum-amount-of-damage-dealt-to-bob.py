...
# the enemies that is still alive and deals power points of damage to
# them.
#
# Determine the minimum total amount of damage points that will be dealt
# to Bob before all n enemies are dead.
#
# Example 1:
#
...
#
# Attack enemy 0 in the next second, after which enemy 0 will go down, the
# number of damage points dealt to Bob is 3 points.
#
...
class Solution:
    def minDamage(self, power: int, damage: List[int], health: List[int]) -> int:
        ...
        Enemy i takes time[i] = ceil(health[i]/power) seconds to kill and deals
        damage[i] each second while alive. Total damage depends on kill order;
        optimal order compares time[i]*damage[j] vs time[j]*damage[i].

        Algorithm:
        - time[i] = (health[i] + power - 1) // power.
        - Sort enemies by comparator time_a * dmg_b < time_b * dmg_a (kill a first).
        - Accumulate: remaining_dps starts as sum(damage); for each enemy in order,
          ans += remaining_dps * time; then remaining_dps -= damage.

        Complexity: O(n log n) time, O(n) space.
        """
        n = len(damage)
        time = [(health[i] + power - 1) // power for i in range(n)]
        order = list(range(n))
