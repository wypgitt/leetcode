#
# @lc app=leetcode id=1855 lang=python3
#
# [1855] Maximum Distance Between a Pair of Values
#
# https://leetcode.com/problems/maximum-distance-between-a-pair-of-values/description/
#
# algorithms
# Medium (61.24%)
# Likes:    1601
# Dislikes: 37
# Total Accepted:    168K
# Total Submissions: 274K
# Testcase Example:  "[55,30,5,4,2]"
#
# You are given two non-increasing 0-indexed integer arrays nums1 and nums2.
#
# A pair of indices (i, j), where 0 <= i < nums1.length and 0 <= j <
# nums2.length, is valid if both i <= j and nums1[i] <= nums2[j]. The distance
# of the pair is j - i.
#
# Return the maximum distance of any valid pair (i, j). If there are no valid
# pairs, return 0.
#
# An array arr is non-increasing if arr[i-1] >= arr[i] for every 1 <= i <
# arr.length.
#
# Example 1:
#
# Input: nums1 = [55,30,5,4,2], nums2 = [100,20,10,10,5]
# Output: 2
# Explanation: The valid pairs are (0,0), (2,2), (2,3), (2,4), (3,3), (3,4),
# and (4,4).
# The maximum distance is 2 with pair (2,4).
#
# Example 2:
#
# Input: nums1 = [2,2,2], nums2 = [10,10,1]
# Output: 1
# Explanation: The valid pairs are (0,0), (0,1), and (1,1).
# The maximum distance is 1 with pair (0,1).
#
# Example 3:
#
# Input: nums1 = [30,29,19,5], nums2 = [25,25,25,25,25]
# Output: 2
# Explanation: The valid pairs are (2,2), (2,3), (2,4), (3,3), and (3,4).
# The maximum distance is 2 with pair (2,4).
#
# Constraints:
#
# 1 <= nums1.length, nums2.length <= 10^5
#
# 1 <= nums1[i], nums2[j] <= 10^5
#
# Both nums1 and nums2 are non-increasing.
#

# @lc code=start
from typing import List
import bisect


class Solution:
    def maxDistance(self, nums1: List[int], nums2: List[int]) -> int:
        """
        Interview explanation:
        Both arrays non-increasing. Maximize j-i with i≤j and nums1[i]≤nums2[j].
        Two pointers: advance j while nums2[j] ≥ nums1[i], then try next i.

        Algorithm (two pointers):
        - i=j=0; while both in range: if nums1[i]≤nums2[j]: ans=max(j-i); j++
          else i++.

        Complexity: O(n+m) time, O(1) space.
        """
        i = j = ans = 0
        n, m = len(nums1), len(nums2)
        while i < n and j < m:
            if nums1[i] <= nums2[j]:
                ans = max(ans, j - i)
                j += 1
            else:
                i += 1
        return ans

    def maxDistance_binary_search(self, nums1: List[int], nums2: List[int]) -> int:
        """
        Interview explanation:
        Alternate: for each i, binary search rightmost j ≥ i with nums2[j]≥nums1[i]
        using negated nums2 as non-decreasing.

        Algorithm:
        - neg = [-x for x in nums2]; j = bisect_right(neg, -v)-1; update ans.

        Complexity: O(n log m) time.
        """
        ans = 0
        neg = [-x for x in nums2]
        for i, v in enumerate(nums1):
            j = bisect.bisect_right(neg, -v) - 1
            if j >= i:
                ans = max(ans, j - i)
        return ans
# @lc code=end
