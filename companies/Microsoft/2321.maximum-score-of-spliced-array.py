...
# swap the subarray nums1[left...right] with the subarray nums2[left...right].
#
#
...
class Solution:
    def maximumsSplicedArray(self, nums1: List[int], nums2: List[int]) -> int:
        ...
        One subarray swap between nums1 and nums2; maximize max(sum1, sum2)
        after the splice.

        Algorithm:
        - Swapping a subarray of nums2 into nums1 adds sum(diff[l..r]) where
          diff[i]=nums2[i]-nums1[i]. Max gain is Kadane on diff; similarly for
          the other direction. Answer = max(sum1+gain12, sum2+gain21).

        Complexity: O(n) time, O(1) space.
        """
        def kadane(diff: List[int]) -> int:
            best = cur = 0
            for x in diff:
                cur = max(0, cur + x)
                best = max(best, cur)
        ...
        d12 = [b - a for a, b in zip(nums1, nums2)]
        d21 = [-x for x in d12]
        return max(s1 + kadane(d12), s2 + kadane(d21))

    def maximumsSplicedArray_dp(self, nums1: List[int], nums2: List[int]) -> int:
        """
        Interview explanation:
        Kadane / max-subarray alternate of the splice gain.

        Algorithm:
        - Max subarray of diffs for each direction.

        Complexity: O(n) time, O(1) space.
        """
        return self.maximumsSplicedArray(nums1, nums2)
# @lc code=end
