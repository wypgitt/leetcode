...
# Function 0 resumes execution at the beginning of time 7 and executes for 1
# unit of time.
...
class Solution:
    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
        ...
        Single-threaded call stack: exclusive time = wall time not spent in
        callees. Stack of function ids; on start/end adjust with timestamps.

        Algorithm:
        - Stack of (id); prev timestamp.
        - start: add (t - prev) to top; push; prev = t.
        - end: add (t - prev + 1) to top; pop; prev = t + 1.

        Complexity: O(L) time, O(L) stack space.
        """
        ans = [0] * n
        stack = []
        prev = 0
