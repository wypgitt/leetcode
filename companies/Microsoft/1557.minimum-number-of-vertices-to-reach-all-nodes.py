...
# an array edges where edges[i] = [from_i, to_i] represents a directed edge
# from node from_i to node to_i.
#
...
class Solution:
    def findSmallestSetOfVertices(self, n: int, edges: List[List[int]]) -> List[int]:
        ...
        DAG: any node with indegree > 0 is reachable from somewhere. Smallest
        set that can reach all nodes = all nodes with indegree 0.

        Algorithm (indegree):
        - Mark nodes that appear as destinations; return unmarked 0..n-1.
    ...
    def findSmallestSetOfVertices_set(self, n: int, edges: List[List[int]]) -> List[int]:
        """
        Interview explanation:
        Alternate set difference: all nodes minus those with an incoming edge.

        Algorithm:
        - return sorted(set(range(n)) - {v for _,v in edges})
