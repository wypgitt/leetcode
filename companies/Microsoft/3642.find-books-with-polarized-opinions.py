#
# @lc app=leetcode id=3642 lang=python3
#
...
# receive both very high ratings and very low ratings from different
# readers.
#
...
#
# Only include books where polarization score ≥ 0.6 (at least 60% extreme
# ratings)
#
...
# +---------+------------------+---------------+-----------+-------+---------------+--------------------+
# | 1       | The Great Gatsby | F. Scott      | Fiction   | 180   | 4
# | 1.00               |
# | 3       | 1984             | George Orwell | Dystopian | 328   | 4
# | 1.00               |
# +---------+------------------+---------------+-----------+-------+---------------+--------------------+
#
# Explanation:
...
# Books not included:
#
# To Kill a Mockingbird (book_id = 2): All ratings are 4-5, no low ratings
# (≤2)
#
...
class Solution:
    def solve(self) -> str:
        ...
        Polarized books need both high (≥4) and low (≤2) ratings, enough
        sessions, and mostly extreme ratings.

        Algorithm:
        - Aggregate reading_sessions by book_id.
        - Require COUNT >= 5, MAX >= 4, MIN <= 2.
        - rating_spread = MAX - MIN; polarization_score = extreme/total.
        - Keep score >= 0.6; join books; ORDER BY score DESC, title DESC.

        Complexity: O(S + B) over sessions and books.
        """
        self.sql = """
SELECT
    b.book_id,
    ...
    ROUND(
        SUM(r.session_rating <= 2 OR r.session_rating >= 4) / COUNT(*),
        2
