#
# @lc app=leetcode id=3994 lang=python3
#
# [3994] Minimum Adjacent Swaps to Partition Array
#
# https://leetcode.com/problems/minimum-adjacent-swaps-to-partition-array/description/
#
# algorithms
# Medium (57.23%)
# Likes:    62
# Dislikes: 2
# Total Accepted:    18.5K
# Total Submissions: 32.2K
# Testcase Example:  "[1,3,2,4,5,6]\r\n3\r\n4\r"
#
#
# You are given an integer array nums and two integers a and b such that a
# < b.
#
# An array is called good if it can be split into three contiguous parts,
# in this order, such that:
#
# Every element in the first part is less than a.
#
# Every element in the second part is in the range [a, b] inclusive.
#
# Every element in the third part is greater than b.
#
# Any of the three parts may be empty.
#
# In one adjacent swap, you may swap two neighboring elements of nums.
#
# Return the minimum number of adjacent swaps required to make nums good.
# Since the answer may be very large, return it modulo 10^9 + 7.
#
# Example 1:
#
# Input: nums = [1,3,2,4,5,6], a = 3, b = 4
#
# Output: 1
#
# Explanation:
#
# Swap nums[1] and nums[2]. The array becomes [1, 2, 3, 4, 5, 6].
#
# This array is good because it can be split into [1, 2], [3, 4], and [5,
# 6].
#
# Example 2:
#
# Input: nums = [9,7,5,3], a = 4, b = 8
#
# Output: 5
#
# Explanation:
#
# One sequence of optimal swaps is as follows:
#
# Swap nums[2] and nums[3]. The array becomes [9, 7, 3, 5].
#
# Swap nums[1] and nums[2]. The array becomes [9, 3, 7, 5].
#
# Swap nums[0] and nums[1]. The array becomes [3, 9, 7, 5].
#
# Swap nums[1] and nums[2]. The array becomes [3, 7, 9, 5].
#
# Swap nums[2] and nums[3]. The array becomes [3, 7, 5, 9].
#
# This array is good because it can be split into [3], [7, 5], and [9].
#
# Example 3:
#
# Input: nums = [3,7,5,9], a = 4, b = 8
#
# Output: 0
#
# Explanation:
#
# The array is already good. No swaps are needed.
#
# Constraints:
#
# 1 <= nums.length <= 10^5
#
# ​​​​​​​1 <= nums[i] <= 10^9
#
# 1 <= a < b <= 10^9​​​​​​​
#

# @lc code=start
class Solution:
    def minAdjacentSwaps(self, nums: list[int], a: int, b: int) -> int:
        """
        Interview explanation:
        Label each value as type 0 (<a), 1 (in [a,b]), or 2 (>b). A good array
        is types in sorted order. Min adjacent swaps equals the number of
        inversions in the type sequence.

        Algorithm:
        - Map nums to types 0/1/2.
        - Count inversions with a Fenwick tree over type values while scanning
          right to left (or merge-sort). Return count mod 10^9+7.

        Complexity: O(n log n) time, O(n) space.
        """
        MOD = 10**9 + 7
        types = [0 if x < a else 1 if x <= b else 2 for x in nums]
        # Fenwick on types 0..2, scan left to right counting larger-before
        # via querying how many already seen are greater... easier: inversions
        # = pairs i<j with types[i] > types[j].
        bit = [0] * 4

        def add(i: int, v: int) -> None:
            i += 1
            while i < len(bit):
                bit[i] += v
                i += i & -i

        def sum_prefix(i: int) -> int:
            i += 1
            s = 0
            while i > 0:
                s += bit[i]
                i -= i & -i
            return s

        ans = 0
        seen = 0
        for t in types:
            # numbers already placed that are > t
            ans += seen - sum_prefix(t)
            add(t, 1)
            seen += 1
        return ans % MOD
# @lc code=end
