...
# opponent is denoted by energy[i] and experience[i] respectively. When you face
# an opponent, you need to have both strictly greater experience and energy to
...
class Solution:
    def minNumberOfHours(
        ...
        Defeat opponents in order: need energy > opponent energy and experience
        > opponent experience; then energy -= opp.energy, experience += opp.exp.
        Train energy/experience beforehand; min total training hours.

        Algorithm:
        - Energy: need sum(energy)+1 total start energy => train max(0, that - initial).
        - Experience: simulate; whenever exp <= opp, train the deficit+1.

        Complexity: O(n) time, O(1) space.
        """
        need_energy = max(0, sum(energy) + 1 - initialEnergy)
