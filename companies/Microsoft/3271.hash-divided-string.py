...
class Solution:
    def stringHash(self, s: str, k: int) -> str:
        ...
        Split s into blocks of length k; each block hashes to (sum of letter
        indices) mod 26 mapped back to a lowercase letter.

        Algorithm:
        - For i = 0, k, 2k, ...: h = sum(ord(c)-97) % 26; append chr(97+h).

        Complexity: O(n) time, O(n/k) space for the answer.
        """
        res = []
