...
//   - Go: map[int]int (same role as Python dict for this problem)
...
// TwoSum1 returns two distinct indices i < j such that nums[i] + nums[j] == target.
//
...
func TwoSum1(nums []int, target int) []int {
	...
	return nil // unreachable when exactly one solution exists (LeetCode guarantee)
}
