...
class Solution:
    def minOperations(self, nums: List[int], x: int, y: int) -> int:
        ...
        Each op picks i: subtract x from nums[i] and y from every other entry.
        Minimize ops so all values become <= 0.

        Algorithm:
        - Binary search T. For fixed T, need_j = ceil(max(0, v - T*y) / (x-y));
          feasible iff sum need_j <= T (remainder ops can be spent anywhere).

        Complexity: O(n log M) time, O(1) space (M = max(nums)).
        """
        diff = x - y
    ...
    def minOperations_binary_search(self, nums: List[int], x: int, y: int) -> int:
        """
        Interview explanation:
        Alternate name for the same binary-search feasibility approach.

        Algorithm:
        - Identical check(T) predicate and binary search on T.

        Complexity: O(n log M) time, O(1) space.
        """
        return self.minOperations(nums, x, y)
# @lc code=end
