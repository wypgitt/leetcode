...
#
# You are also given an integer k, which denotes the total number of moves to be
# made.
#
...
class Solution:
    def maximumTop(self, nums: List[int], k: int) -> int:
        ...
        Pile with nums[0] on top. Move: remove top, or put one previously removed
        number back on top. After exactly k moves, maximize the top element
        (-1 if empty).

        Algorithm:
        - n==1: empty after odd k, else nums[0].
        - Else top can be max of first min(n,k-1) (remove then put max back), or
          nums[k] if k<n (remove k without putting back).
