...
class Solution:
    def minAvailableDuration(self, slots1: List[List[int]], slots2: List[List[int]], duration: int) -> List[int]:
        ...
        Premium. Earliest common free slot of length >= duration between two
        people. Sort both slot lists; two-pointer scan intersections.

        Algorithm:
        - Sort slots; i=j=0; intersect [max(start), min(end)]; if >=duration return
