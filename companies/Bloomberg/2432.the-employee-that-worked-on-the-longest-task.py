...
#
#
# leaveTime_i is the time at which the employee finished the i^th task. All the
# values leaveTime_i are unique.
#
...
# The task with the longest time is task 3 and the employee with id 1 is the one
# that worked on it, so we return 1.
#
# Example 2:
#
...
class Solution:
    def hardestWorker(self, n: int, logs: List[List[int]]) -> int:
        ...
        Tasks run sequentially from time 0; logs give [employeeId, leaveTime].
        Return employee with longest single task; ties -> smallest id.

        Algorithm:
        - Scan durations leave-prev; track best duration and id.
        ...
        for emp, leave in logs:
            dur = leave - prev
            if dur > best_t or (dur == best_t and emp < best_id):
                best_t, best_id = dur, emp
            prev = leave
        return best_id
# @lc code=end
