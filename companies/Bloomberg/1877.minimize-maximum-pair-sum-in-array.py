...
class Solution:
    def minPairSum(self, nums: List[int]) -> int:
        ...
        Pair n/2 pairs; minimize the maximum pair sum. Sort and pair smallest
        with largest.

        Algorithm:
        - Sort; ans = max(nums[i]+nums[n-1-i]) for i in 0..n/2.

        Complexity: O(n log n) time, O(n) or in-place sort.
        """
        nums.sort()
        n = len(nums)
