#
# @lc app=leetcode id=3764 lang=python3
#
...
# for this table.
# Each row represents a completed course by a user with their rating (1-5
# scale).
#
...
# students.
#
# Return the pair frequency, identifying which course transitions are most
...
#
# +---------+-----------+------------------+-----------------+---------------+
# | user_id | course_id | course_name      | completion_date |
# course_rating |
# +---------+-----------+------------------+-----------------+---------------+
...
# Explanation:
#
# User 1: Completed 6 courses with average rating 4.5 (qualifies as top
# performer)
#
# User 2: Completed 5 courses with average rating 4.4 (qualifies as top
# performer)
#
...
#
# User 1: Python Basics → SQL Fundamentals → JavaScript → React Basics →
# Node.js → Docker
#
# User 2: Python Basics → React Basics → Node.js → Docker → AWS
# Fundamentals
#
# Most common transitions: Node.js → Docker (2 times), React Basics →
# Node.js (2 times)
#
# Results are ordered by transition_count in descending order, then by
# first_course in ascending order, and then by second_course in ascending
# order.
#
...
class Solution:
    def solve(self) -> str:
        ...
        Restrict to top performers (>=5 courses, avg rating >= 4), then count
        consecutive course transitions in completion order.

        Algorithm:
        - CTE for qualifying users.
        - LEAD(course_name) per user ordered by completion_date.
        - GROUP BY pair; ORDER BY count DESC, names ASC.

        Complexity: O(N log N) with sorting/windowing.
        """
        self.sql = """
