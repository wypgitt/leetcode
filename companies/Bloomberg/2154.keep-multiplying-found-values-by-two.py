...
class Solution:
    def findFinalValue(self, nums: List[int], original: int) -> int:
        ...
        While original is in nums, replace original with 2*original. Return the
        final value when it is no longer found.

        Algorithm:
        (hash set)
        - Put nums in a set; while original in set: original *= 2; return it.

        Complexity: O(n + log(max/original)) time, O(n) space.
        """
        s = set(nums)
        while original in s:
            original *= 2
