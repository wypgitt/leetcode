...
# assign edge weights in the path from node 1 to x such that its total
# cost is odd.
#
...
#
# For example, the path from Node 1 to Node 4 consists of two edges (1 → 3
# and 3 → 4).
#
...
class Solution:
    def assignEdgeWeights(self, edges: List[List[int]]) -> int:
        ...
        Only the root-to-deepest path matters. With d edges each weighted 1 or 2,
        the path sum is odd for exactly half of the 2^d assignments when d ≥ 1,
        i.e. 2^{d-1} (mod 1e9+7).

        Algorithm:
        - BFS/DFS from node 1 to find maximum depth d (edge count).
