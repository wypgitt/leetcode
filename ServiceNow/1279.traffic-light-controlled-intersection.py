...
# Example 1:
#
# Input: cars = [1,3,5,2,4], directions = [2,1,2,4,3], arrivalTimes =
# [10,20,30,40,50]
# Output: [
# "Car 1 Has Passed Road A In Direction 2",    // Traffic light on road A
# is green, car 1 can cross the intersection.
...
# intersection as the light is still green.
# "Traffic Light On Road B Is Green",          // Car 2 requests green
# light for road B.
# "Car 2 Has Passed Road B In Direction 4",    // Car 2 crosses as the
# light is green on road B now.
...
# Example 2:
#
# Input: cars = [1,2,3,4,5], directions = [2,4,3,3,1], arrivalTimes =
# [10,20,30,40,40]
# Output: [
# "Car 1 Has Passed Road A In Direction 2",    // Traffic light on road A
# is green, car 1 can cross the intersection.
...
# light is green on road B now.
# "Traffic Light On Road A Is Green",          // Car 5 requests green
# light for road A.
# "Car 5 Has Passed Road A In Direction 1",    // Car 5 crosses as the
# light is green on road A now.
# "Traffic Light On Road B Is Green",          // Car 4 requests green
# light for road B. Car 4 blocked until car 5 crosses and then traffic
# light is green on road B.
...
class TrafficLight:
    def __init__(self):
        ...
        Premium concurrency. Cars on roads 1/2 arrive; only one road's green
        light at a time. Serialize with a lock; track which road is green;
        when a car on the other road arrives, flip the light then cross.

        Algorithm:
        - lock + green_road initially 1.
        - carArrived: with lock, if road != green: turnGreen(); set green;
          then crossCar().

        Complexity: O(1) per car arrival (critical section).
        """
    ...
    def carArrived(
        ...
        """
        Interview explanation:
        Under the mutual-exclusion lock, ensure this car's road is green
        (calling turnGreen if needed), then let the car cross.

        Algorithm:
        - Acquire lock.
        - If roadId != self.green: turnGreen(); self.green = roadId.
        - crossCar(); release lock.

        Complexity: O(1) per call excluding waiting on lock.
        """
        with self.lock:
