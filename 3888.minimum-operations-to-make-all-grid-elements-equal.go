...
//
// @lc app=leetcode id=3888 lang=golang
//
// [3888] Minimum Operations to Make All Grid Elements Equal
//
...
// MinOperations3888 returns minimum k×k increment stamps to equalize the grid, or -1.
func MinOperations3888(grid [][]int, k int) int {
	m, n := len(grid), len(grid[0])
	mx := grid[0][0]
	...
	check := func(target int) int {
		...
		for i, row := range grid {
			...
			for j, val := range row {
				jj := j + 1
				diff[ii][jj] += diff[ii-1][jj] + diff[ii][jj-1] - diff[ii-1][jj-1]

				curVal := val + diff[ii][jj]
