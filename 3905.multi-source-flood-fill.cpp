...
public:
    vector<vector<int>> colorGrid(int n, int m, vector<vector<int>>& sources) {
        vector<vector<int>> grid(n, vector<int>(m)), dist(n, vector<int>(m, -1));
        ...
        int time = 0;
        while (!q.empty()) {
            unordered_map<int, int> proposal;
            int sz = q.size();
            while (sz--) {
                auto [r, c] = q.front();
                q.pop();
                ...
                for (int d = 0; d < 4; ++d) {
                    ...
                    if (0 <= nr && nr < n && 0 <= nc && nc < m && dist[nr][nc] == -1) {
                        int key = nr * m + nc;
                        proposal[key] = max(proposal[key], color);
                    }
                }
            }
            ++time;
            for (auto [key, color] : proposal) {
                int r = key / m, c = key % m;
                if (dist[r][c] == -1) {
                    dist[r][c] = time;
                    grid[r][c] = color;
