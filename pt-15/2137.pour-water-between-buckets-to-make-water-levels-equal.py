...
# represented by a 0-indexed integer array buckets, where the i^th bucket
# contains buckets[i] gallons of water. You are also given an integer
# loss.
#
# You want to make the amount of water in each bucket equal. You can pour
# any amount of water from one bucket to another bucket (not necessarily
# an integer). However, every time you pour k gallons of water, you spill
# loss percent of k.
#
...
# 0.5 * 50% = 0.25 gallons are spilled and buckets[0] only receives 0.5 -
# 0.25 = 0.25 gallons of water.
# Now, buckets = [2.25, 3.5, 6].
# Pour 2.5 gallons of water from buckets[2] to buckets[0].
# 2.5 * 50% = 1.25 gallons are spilled and buckets[0] only receives 2.5 -
# 1.25 = 1.25 gallons of water.
...
class Solution:
    def equalizeWater(self, buckets: List[int], loss: int) -> float:
        ...
        Premium: pour between buckets; lose `loss`% of poured water. Equalize
        all to same level; maximize that level.

        Algorithm:
        - Check target t: excess above t, after (100-loss)% efficiency, covers deficit.

        Complexity: O(n log (max/eps)) time, O(1) space.
        """
        pct = (100 - loss) / 100.0
