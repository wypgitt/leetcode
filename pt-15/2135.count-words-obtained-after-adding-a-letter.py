...
class Solution:
    def wordCount(self, startWords: List[str], targetWords: List[str]) -> int:
        ...
        A target is reachable if by adding one letter to some start word and
        rearranging we get target. Equiv: target's letter-set minus one letter
        equals some start's letter-set.

        Algorithm:
        - Store bitmasks of start words. For each target mask, try clearing each
          set bit; check membership in starts.

        Complexity: O(L) over total letters, O(|starts|) space.
        """
        starts = set()
        for w in startWords:
            mask = 0
