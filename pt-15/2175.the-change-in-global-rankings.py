#
# @lc app=leetcode id=2175 lang=python3
#
...
# Testcase Example:  "{\"headers\": {\"TeamPoints\": [\"team_id\", \"name\", \"points\"], \"PointsChange\": [\"team_id\", \"points_change\"]}, \"rows\": {\"TeamPoints\": [[3, \"Algeria\", 1431], [1, \"Senegal\", 2132], [2, \"New Zealand\", 1402], [4, \"Croatia\", 1817]], \"PointsChange\": [[3, 399], [2, 0], [4, 13], [1, -22]]}}"
#
#
# Table: TeamPoints
#
...
# points, we break the tie by sorting them by their name in
# lexicographical order.
#
# The points of each national team should be updated based on its
# corresponding points_change value.
#
# Write a solution to calculate the change in the global rankings after
...
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL. TeamPoints(team_id, name, points), PointsChange(team_id,
        points_change). Rank by points DESC, name ASC. After applying point
        changes, compute rank_diff = old_rank - new_rank for each team.

        Algorithm:
        - JOIN changes; RANK() before and after (points+delta); subtract ranks.
    ...
    SELECT
        t.team_id,
        t.name,
        CAST(RANK() OVER (ORDER BY t.points DESC, t.name) AS SIGNED)
          - CAST(RANK() OVER (ORDER BY (t.points + p.delta) DESC, t.name) AS SIGNED)
          AS rank_diff
    FROM TeamPoints t
    JOIN P p USING (team_id);
    """
# @lc code=end
