...
# pyramid, and its height be h. Then, the plot comprises of cells (i, j) where r
# <= i <= r + h - 1 and c - (i - r) <= j <= c + (i - r).
#
...
class Solution:
    def countPyramids(self, grid: List[List[int]]) -> int:
        ...
        Count pyramids and inverse pyramids of fertile cells (1s). A pyramid of
        height h≥2 has an apex and successively wider rows of 1s beneath (or
        above for inverse). Height-1 (single cell) is not counted.

        Algorithm:
        - dp[i][j] = max height with apex at (i,j) growing toward increasing i:
          if grid[i][j]=1: 1+min(dp[i+1][j-1], dp[i+1][j], dp[i+1][j+1]).
        - Sum (dp-1); repeat on vertically reversed grid for inverse pyramids.
