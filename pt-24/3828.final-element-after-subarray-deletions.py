#
# @lc app=leetcode id=3828 lang=python3
#
# [3828] Final Element After Subarray Deletions
#
# https://leetcode.com/problems/final-element-after-subarray-deletions/description/
#
# algorithms
# Medium (41.16%)
# Likes:    119
# Dislikes: 24
# Total Accepted:    33.5K
# Total Submissions: 81.3K
# Testcase Example:  "[1,5,2]"
#
#
# You are given an integer array nums.
#
# Two players, Alice and Bob, play a game in turns, with Alice playing
# first.
#
# In each turn, the current player chooses any subarray nums[l..r] such
# that r - l + 1 < m, where m is the current length of the array.
#
# The selected subarray is removed, and the remaining elements are
# concatenated to form the new array.
#
# The game continues until only one element remains.
#
# Alice aims to maximize the final element, while Bob aims to minimize it.
# Assuming both play optimally, return the value of the final remaining
# element.
#
# Example 1:
#
# Input: nums = [1,5,2]
#
# Output: 2
#
# Explanation:
#
# One valid optimal strategy:
#
# Alice removes [1], array becomes [5, 2].
#
# Bob removes [5], array becomes [2]​​​​​​​. Thus, the answer is 2.
#
# Example 2:
#
# Input: nums = [3,7]
#
# Output: 7
#
# Explanation:
#
# Alice removes [3], leaving the array [7]. Since Bob cannot play a turn
# now, the answer is 7.
#
# Constraints:
#
# 1 <= nums.length <= 10^5
#
# 1 <= nums[i] <= 10^5
#

# @lc code=start
from typing import List


class Solution:
    def finalElement(self, nums: List[int]) -> int:
        """
        Interview explanation:
        Alice and Bob delete proper subarrays until one value remains. Alice
        maximizes; Bob minimizes. Only the endpoints can survive optimal play.

        Algorithm:
        - Alice can clear the middle in one move, leaving two ends for Bob, so
          she can force max(nums[0], nums[-1]).
        - Bob can always delete any middle value Alice tries to protect.
        - Therefore the answer is exactly max of the two endpoints.

        Complexity: O(1) time, O(1) space.
        """
        return max(nums[0], nums[-1])
# @lc code=end
