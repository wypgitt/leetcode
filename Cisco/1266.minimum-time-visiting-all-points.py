...
#
# On a 2D plane, there are n points with integer coordinates points[i] = [x_i,
# y_i]. Return the minimum time in seconds to visit all the points in the order
# given by points.
#
...
class Solution:
    def minTimeToVisitAllPoints(self, points: List[List[int]]) -> int:
        ...
        Moving diagonally costs the same as axis-aligned (Chebyshev distance).
        Time between consecutive points = max(|dx|,|dy|); sum them.

        Algorithm:
        - ans=0; for each consecutive pair: ans += max(abs(dx), abs(dy)).
