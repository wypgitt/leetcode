...
public:
    vector<string> braceExpansionII(string expression) {
        set<string> words;
        function<void(string)> dfs = [&](string exp) {
            size_t j = exp.find('}');
            if (j == string::npos) {
                words.insert(exp);
                return;
            ...
            string prefix = exp.substr(0, i), suffix = exp.substr(j + 1);
            string inside = exp.substr(i + 1, j - i - 1), alt;
            stringstream ss(inside);
            while (getline(ss, alt, ',')) dfs(prefix + alt + suffix);
        };
        dfs(expression);
