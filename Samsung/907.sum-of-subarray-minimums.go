...
const mod907 = 1_000_000_007

// SumSubarrayMins907 returns sum of min(subarray) over all contiguous subarrays, modulo 10^9+7.
//
...
func SumSubarrayMins907(arr []int) int {
	n := len(arr)
	left := make([]int, n)
	right := make([]int, n)
	...
	ans := 0
	for i := 0; i < n; i++ {
		term := int64(arr[i]) * int64(left[i]) % mod907
		term = term * int64(right[i]) % mod907
		ans = (ans + int(term)) % mod907
	}
	return ans
}
