...
#
#
# You are controlling a robot that is located somewhere in a room. The
# room is modeled as an m x n binary grid where 0 represents a wall and 1
...
# every empty cell in the room). The robot with the four given APIs can
# move forward, turn left, or turn right. Each turn is 90 degrees.
#
# When the robot tries to move into a wall cell, its bumper sensor detects
...
#   // returns false if next cell is obstacle and robot stays on the
# current cell.
#   boolean move();
#
...
# you must control the robot using only the four mentioned APIs without
...
# LeetCode provides Robot globally. Local stub for py_compile only.
try:
    Robot  # type: ignore[name-defined]
except NameError:
    class Robot:
        def move(self) -> bool:
            return False
...
class Solution:
    def cleanRoom(self, robot: "Robot") -> None:
        ...
        Premium. Spiral DFS backtracking on an unknown grid. Clean current cell,
        try 4 directions; if move succeeds, recurse then backtrack (two turns +
        move + two turns) to restore pose. Mark visited in relative coordinates.

        Algorithm:
        - dirs = [(-1,0),(0,1),(1,0),(0,-1)]  # up,right,down,left
        - DFS(r,c,d): clean; visited.add; for each turn 0..3: try move;
          if moved: DFS(nr,nc,nd); goBack(); always turnRight to next dir.
        - goBack: turnRight twice, move, turnRight twice.
