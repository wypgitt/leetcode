 ...
 *
 * Naive approach
 * For each **`x` from 1 to `n`**, test substring membership — correct, **O(n · |s|)** substring scans worst-case (Java
 * `String.contains` searches linear in **`|s|`** per check).
 *
 ...
 *
 * Algorithm
 * For **`i`** from **`n/2 + 1`** to **`n`**, if binary string for **`i`** not contained in **`s`**, return **`False`**. Otherwise return **`True`**.
 *
 * Data structures
 * No extra structures — only string containment (substring search). Optional: build a **suffix automaton** or
...
class Solution {
    public boolean queryString(String s, int n) {
        for (int x = n / 2 + 1; x <= n; x++) {
            String pat = Integer.toBinaryString(x);
            if (!s.contains(pat)) {
                return false;
            }
        }
