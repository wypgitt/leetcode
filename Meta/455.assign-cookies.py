...
class Solution:
    def findContentChildren(self, g: List[int], s: List[int]) -> int:
        ...
        Greedy: sort greed and cookie sizes; always give the smallest cookie
        that can satisfy the current least-greedy child.

        Algorithm:
        - Sort g, s; two pointers i (child), j (cookie).
        - If s[j] >= g[i]: content++, i++; always j++.

        Complexity: O(n log n + m log m) time, O(1)/O(n) space.
        """
        g.sort()
        s.sort()
        i = j = 0
