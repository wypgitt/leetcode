...
class Solution:
    def minimizedMaximum(self, n: int, quantities: List[int]) -> int:
        ...
        Distribute product types to n stores (a store gets at most one type);
        minimize the max products any store receives. Splitting type x into
        stores needs ceil(x/m) stores if each gets ≤ m.

        Algorithm:
        - Feasible if sum(ceil(q/m)) <= n; minimize m.

        Complexity: O(m_types * log max(q)) time, O(1) space.
        """
        lo, hi = 1, max(quantities)
        while lo < hi:
            mid = (lo + hi) // 2
            need = sum((q + mid - 1) // mid for q in quantities)
            if need <= n:
                hi = mid
            else:
                lo = mid + 1
