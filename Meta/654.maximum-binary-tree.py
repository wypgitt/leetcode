...
class Solution:
    def constructMaximumBinaryTree(self, nums: List[int]) -> Optional[TreeNode]:
        ...
        Root is max of nums; left subtree from left subarray, right from right.
        Recurse (or stack O(N) cartesian tree).

        Algorithm:
        - Find max index m; TreeNode(nums[m]); recurse nums[:m], nums[m+1:].

        Complexity: O(N^2) naive / O(N) with stack; O(N) space.
        """
        def build(arr: List[int]) -> Optional[TreeNode]:
            if not arr:
                return None
            m = max(range(len(arr)), key=lambda i: arr[i])
            node = TreeNode(arr[m])
