...
class Solution:
    def getIndex(self, reader: "ArrayReader") -> int:
        ...
        Premium. Array has all equal except one larger element. ArrayReader
        compareSub(l,r,x,y) compares sums of two subarrays. Binary search:
        compare left half vs right half to find which side has the large one.

        Algorithm:
        - lo,hi=0,n-1; while lo<hi: mid=(lo+hi)//2; if length even compare
          [lo,mid] vs [mid+1,hi] (equal lengths); if odd compare [lo,mid-1] vs
          [mid+1,hi] and mid alone if equal.

        Complexity: O(log n) compares, O(1) space.
        """
        n = reader.length()
        lo, hi = 0, n - 1
        while lo < hi:
            ...
            if length % 2 == 0:
                mid = (lo + hi) // 2
                # compare [lo..mid] vs [mid+1..hi] same length
                ...
                mid = (lo + hi) // 2
                # compare [lo..mid-1] vs [mid+1..hi]; mid is middle
                if lo == hi:
                    break
