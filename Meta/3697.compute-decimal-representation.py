...
#
# A positive integer is a base-10 component if it is the product of a
# single digit from 1 to 9 and a non-negative power of 10. For example,
...
class Solution:
    def decimalRepresentation(self, n: int) -> List[int]:
        ...
        Fewest base-10 components is exactly the non-zero positional digits
        (digit * 10^p), already minimal.

        Algorithm:
        - Peel digits from low to high; collect non-zero digit*place; reverse
          for descending order.
        - Alternate: convert to string and emit digit * 10^(len-1-i).
