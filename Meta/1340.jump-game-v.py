...
class Solution:
    def maxJumps(self, arr: List[int], d: int) -> int:
        ...
        From i jump to j in (i, i+d] or [i-d, i) if arr[i] > all between and
        arr[i] > arr[j]. Maximize visits (indices). DFS + memo: longest path
        in the implicit DAG.

        Algorithm (DFS memo):
        - dp[i] = 1 + max(dp[j] over valid jumps); try left/right until blocked
          by taller/equal.
