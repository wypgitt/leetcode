...
class Solution:
    def numOfUnplacedFruits(self, fruits: List[int], baskets: List[int]) -> int:
        ...
        Place each fruit into the leftmost unused basket with capacity >= fruit.
        Count fruits that find no basket (n <= 100).

        Algorithm:
        - Mark baskets used; for each fruit scan baskets left to right.
        - Increment answer when no basket fits.
        ...
        for fruit in fruits:
            ...
            for j, capacity in enumerate(baskets):
                if not used[j] and capacity >= fruit:
                    used[j] = True
                    placed = True
                    break
            if not placed:
