...
class Solution {
    ...
    long long value(Line line, long long x) { return line.first * x + line.second; }
    bool bad(Line a, Line b, Line c) {
        return (__int128)(b.second - a.second) * (b.first - c.first) >= (__int128)(c.second - b.second) * (a.first - b.first);
    }
    void addLine(vector<Line>& hull, long long m, long long b) {
        Line line{m, b};
        while (hull.size() >= 2 && bad(hull[hull.size() - 2], hull.back(), line)) hull.pop_back();
        hull.push_back(line);
    }
...
public:
    long long minPartitionScore(vector<int>& nums, int k) {
        ...
        for (int part = 1; part <= k; ++part) {
            vector<long long> cur(n + 1, INF), hullX;
            vector<Line> hull;
            int head = 0;
            for (int i = part; i <= n; ++i) {
                int cut = i - 1;
                addLine(hull, -2 * pref[cut], prev[cut] + pref[cut] * pref[cut]);
                long long x = pref[i];
                while (head + 1 < (int)hull.size() && value(hull[head + 1], x) <= value(hull[head], x)) ++head;
                cur[i] = x * x + value(hull[head], x);
            }
            prev.swap(cur);
        }
