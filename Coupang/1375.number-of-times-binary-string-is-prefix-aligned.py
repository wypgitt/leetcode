...
class Solution:
    def numTimesAllBlue(self, flips: List[int]) -> int:
        ...
        After each flip, string is prefix-aligned iff the first k bulbs are on
        and the rest off — equivalent to max flipped index so far == step count.

        Algorithm:
        - mx=0; for i,f in enumerate(flips,1): mx=max(mx,f); ans+=(mx==i)
