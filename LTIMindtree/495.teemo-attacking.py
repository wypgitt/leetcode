...
class Solution:
    def findPoisonedDuration(self, timeSeries: List[int], duration: int) -> int:
        ...
        Each attack poisons for `duration` seconds; overlapping attacks merge.
        Total = sum of gaps capped by duration, plus final duration.

        Algorithm:
        - ans = 0; for consecutive attacks t[i], t[i+1]:
          ans += min(duration, t[i+1]-t[i])
        - ans += duration for the last attack.
