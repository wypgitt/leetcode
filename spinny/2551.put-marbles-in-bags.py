...
class Solution:
    def putMarbles(self, weights: List[int], k: int) -> int:
        ...
        Splitting into k contiguous bags adds pair costs at the k-1 cut boundaries.
        Ends of the array always contribute; maximize/minimize sum of chosen cuts.

        Algorithm:
        - Build adjacent pair sums weights[i]+weights[i+1].
        - Sort them; answer is sum of largest (k-1) minus sum of smallest (k-1).
        ...
        pairs.sort()
        m = k - 1
        return sum(pairs[-m:]) - sum(pairs[:m])
# @lc code=end
