...
Simulate flow row by row. A glass keeps at most 1 cup; any excess is split equally to the two glasses below. Only the previous row is needed to compute the next row.

Data structure: a one-dimensional DP array stores the current row amounts.

Edge cases: return is capped at 1 because a queried glass may contain more in the simulation amount before capping. poured=0 naturally propagates zeros.
