...
public:
    bool isEscapePossible(vector<vector<int>>& blocked, vector<int>& source, vector<int>& target) {
        const int N = 1000000;
        int lim = (int)blocked.size() * (int)blocked.size() / 2;
        set<pair<int, int>> blockedSet;
        for (auto& b : blocked) blockedSet.insert({b[0], b[1]});
        int dirs[5] = {-1, 0, 1, 0, -1};
        auto walk = [&](int sx, int sy, int tx, int ty) {
            vector<pair<int, int>> st{{sx, sy}};
            ...
            while (!st.empty()) {
                ...
                seen.insert({x, y});
                if ((int)seen.size() > lim || (x == tx && y == ty)) return true;
                for (int d = 0; d < 4; ++d) {
                    int nx = x + dirs[d], ny = y + dirs[d + 1];
                    pair<int, int> p{nx, ny};
                    if (0 <= nx && nx < N && 0 <= ny && ny < N && !blockedSet.count(p) && !seen.count(p)) {
                        st.push_back(p);
                    }
                }
            }
