...
public:
    int numberOfRoutes(vector<string>& grid, int d) {
        ...
        int horizontal = d;
        int upward = (int)floor(sqrt(1LL * d * d - 1));
        auto rangeSums = [&](const vector<int>& values, int radius, const string& row) {
            vector<int> pref(cols + 1), res(cols);
            for (int c = 0; c < cols; ++c) pref[c + 1] = (pref[c] + values[c]) % MOD;
            for (int c = 0; c < cols; ++c) {
                if (row[c] == '#') continue;
                int l = max(0, c - radius), r = min(cols - 1, c + radius);
                res[c] = (pref[r + 1] - pref[l] + MOD) % MOD;
            }
            return res;
        };
        vector<int> enter(cols);
        for (int c = 0; c < cols; ++c) enter[c] = grid.back()[c] == '.';
        for (int r = rows - 1; r >= 0; --r) {
            vector<int> after = rangeSums(enter, horizontal, grid[r]);
            if (r == 0) return accumulate(after.begin(), after.end(), 0LL) % MOD;
            enter = rangeSums(after, upward, grid[r - 1]);
        }
        return 0;
    }
};
