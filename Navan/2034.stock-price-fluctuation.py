...
class StockPrice:
    ...
    def __init__(self):
        ...
        Track stock prices by timestamp with updates; query current/max/min.

        Algorithm:
        - Map timestamp->price; max/min heaps with lazy deletion of stale entries.

        Complexity: O(1) extra beyond heaps/map.
        """
        self.time_price = {}
        self.latest_time = 0
        self.maxh = []
    ...
    def update(self, timestamp: int, price: int) -> None:
        """
        Interview explanation:
        Set / correct price at timestamp.

        Algorithm:
        - Update map and latest time; push (price)/( -price) into heaps.
    ...
    def maximum(self) -> int:
        """
        Interview explanation:
        Current maximum price among all timestamps.

        Algorithm:
        - Pop max-heap until top matches map; return price.

        Complexity: Amortized O(log n).
        """
        while True:
            p, t = self.maxh[0]
            if -p == self.time_price.get(t):
                return -p
    ...
    def minimum(self) -> int:
        """
        Interview explanation:
        Current minimum price among all timestamps.

        Algorithm:
        - Pop min-heap until top matches map; return price.
