...
class Solution:
    def numOfBurgers(self, tomatoSlices: int, cheeseSlices: int) -> List[int]:
        ...
        Jumbo=4 tomatoes+1 cheese, Small=2 tomatoes+1 cheese. Solve
        4j+2s=T, j+s=C => j=(T-2C)/2, s=C-j. Nonneg integers required.

        Algorithm:
        - If T odd or T<2C or T>4C: [].
        - j=(T-2*C)//2; s=C-j; return [j,s].

        Complexity: O(1) time/space.
        """
        if (
            tomatoSlices % 2
            or tomatoSlices < 2 * cheeseSlices
            or tomatoSlices > 4 * cheeseSlices
        ):
            return []
