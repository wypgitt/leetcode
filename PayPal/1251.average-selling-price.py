#
# @lc app=leetcode id=1251 lang=python3
#
...
class Solution:
    def solve(self) -> str:
        ...
        Average selling price per product = sum(price*units)/sum(units), rounded
        to 2 decimals. Join UnitsSold to Prices on product_id and date range;
        products with no sales get 0 via LEFT JOIN from Prices.

        Algorithm:
        - LEFT JOIN Prices to UnitsSold where purchase_date between start/end.
        - GROUP BY product_id; ROUND(IFNULL(SUM(price*units)/SUM(units),0),2).

        Complexity: O(P + U) with hash join conceptually.
        """
        return self.sql
