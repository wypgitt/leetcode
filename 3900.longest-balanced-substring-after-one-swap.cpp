...
class Solution {
    int bestWithCap(const vector<int>& starts, int right, int cap) {
        if (cap <= 0) return 0;
        auto it = lower_bound(starts.begin(), starts.end(), right - cap);
        if (it != starts.end() && *it < right) return right - *it;
        return 0;
    }
...
public:
    int longestBalanced(string s) {
        ...
        for (int i = 0; i < (int)s.size(); ++i) {
            bal += s[i] == '1' ? 1 : -1;
            prefix.push_back(bal);
            positions[bal].push_back(i + 1);
        }
        int ans = 0;
        unordered_map<int, int> earliest;
        for (int i = 0; i < (int)prefix.size(); ++i) {
            int b = prefix[i];
            if (earliest.count(b)) ans = max(ans, i - earliest[b]);
            else earliest[b] = i;
        }
        int capOnes = 2 * zeros, capZeros = 2 * ones;
        for (int right = 1; right < (int)prefix.size(); ++right) {
            int b = prefix[right];
            ans = max(ans, bestWithCap(positions[b - 2], right, capOnes));
            ans = max(ans, bestWithCap(positions[b + 2], right, capZeros));
        }
        return ans;
    }
};
