...
// 
//         def removed_evens_upto(l: int, r: int, t: int) -> int:
//             p = bisect.bisect_right(nums, t) - 1
//             if p < l:
//                 return 0
//             p = min(p, r)
//             return even_prefix[p + 1] - even_prefix[l]
// 
//         def kth_for_query(l: int, r: int, k: int) -> int:
//             hi = 2 * (k + n)
//             lo = 2
//             while lo < hi:
//                 mid = (lo + hi) // 2
//                 rem = mid // 2 - removed_evens_upto(l, r, mid)
//                 if rem >= k:
//                     hi = mid
//                 else:
//                     lo = mid + 1
...
public:
    vector<int> kthRemainingInteger(vector<int>& nums, vector<vector<int>>& queries) {
        int n = nums.size();
        vector<int> evenPrefix(n + 1);
        for (int i = 0; i < n; ++i) evenPrefix[i + 1] = evenPrefix[i] + (nums[i] % 2 == 0);
        auto removed = [&](int l, int r, int t) {
            int p = upper_bound(nums.begin(), nums.end(), t) - nums.begin() - 1;
            if (p < l) return 0;
            p = min(p, r);
