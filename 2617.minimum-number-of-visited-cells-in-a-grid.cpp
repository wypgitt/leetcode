...
public:
    _SegTree(int n, int inf) : inf(inf), size(1) {
        while (size < n) size <<= 1;
    ...
    void update(int pos, int val) {
        int i = pos + size;
        tree[i] = val;
        for (i >>= 1; i; i >>= 1) tree[i] = min(tree[i << 1], tree[i << 1 | 1]);
    }
...
public:
    int minimumVisitedCells(vector<vector<int>>& grid) {
        ...
        const int INF = 1e9;
        vector<_SegTree> rows, cols;
        for (int i = 0; i < m; ++i) rows.emplace_back(n, INF);
        for (int j = 0; j < n; ++j) cols.emplace_back(m, INF);
        rows[m - 1].update(n - 1, 1);
        cols[n - 1].update(m - 1, 1);
        for (int i = m - 1; i >= 0; --i) {
            for (int j = n - 1; j >= 0; --j) {
                if (grid[i][j] == 0) continue;
                int rmax = min(n - 1, j + grid[i][j]);
                int dmax = min(m - 1, i + grid[i][j]);
                int mr = rows[i].query(j + 1, rmax);
                int md = cols[j].query(i + 1, dmax);
                int cur = min(rows[i].query(j, j), cols[j].query(i, i));
                int nv = (mr >= INF && md >= INF) ? cur : min(cur, min(mr, md) + 1);
                rows[i].update(j, nv);
                cols[j].update(i, nv);
            }
        }
