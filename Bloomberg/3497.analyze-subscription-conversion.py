#
# @lc app=leetcode id=3497 lang=python3
#
...
# activity_duration is the number of minutes the user spent on the
# platform that day.
# Each row represents a user's activity on a specific date.
#
# A subscription service wants to analyze user behavior patterns. The
# company offers a 7-day free trial, after which users can subscribe to a
# paid plan or cancel. Write a solution to:
#
# Find users who converted from free trial to paid subscription
#
# Calculate each user's average daily activity duration during their free
# trial period (rounded to 2 decimal places)
#
# Calculate each user's average daily activity duration during their paid
# subscription period (rounded to 2 decimal places)
#
...
#
# Had 3 days of paid subscription with durations of 75, 90, and 65
# minutes.
#
...
#
# Did not convert to a paid subscription (only had free_trial and
...
#
# Had 3 days of paid subscription with durations of 50, 55, and 85
# minutes.
#
...
class Solution:
    def solve(self) -> str:
        ...
        Find users with both free_trial and paid activity; report average daily
        durations for trial and paid periods (2 decimals), ordered by user_id.

        Algorithm:
        - Aggregate AVG(duration) per user for free_trial and paid.
        - Inner-join converters; ROUND averages; ORDER BY user_id.

        Complexity: O(N) scan/group.
        """
        self.sql = """
WITH
FreeTrial AS (
...
SELECT
    f.user_id,
    ROUND(f.avg_trial, 2) AS trial_avg_duration,
    ROUND(p.avg_paid, 2) AS paid_avg_duration
FROM FreeTrial f
