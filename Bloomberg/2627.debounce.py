#
# @lc app=leetcode id=2627 lang=python3
#
# [2627] Debounce
#
...
# within that window of time. The debounced function should also receive the
# passed parameters.
#
# For example, let's say t = 50ms, and the function was called at 30ms, 60ms,
# and 100ms.
#
...
#
# The 1st call is cancelled by the 2nd call because the 2nd call occurred before
# 100ms
...
def debounce(fn: Callable[..., Any], t: int) -> Callable[..., None]:
    """
    Interview explanation:
    Return a debounced function: each call resets a timer of t ms; only the
    latest call's args run after the quiet period.

    Algorithm:
    - Keep a threading.Timer; on each call cancel any pending timer and
      schedule fn(*args) after t/1000 seconds.

    Complexity: O(1) per call excluding timer scheduling.
    """
    ...
    def debounced(*args: Any) -> None:
        """
        Interview explanation:
        Schedule or reschedule the underlying function.

        Algorithm:
        - Cancel previous Timer if any, then start a new one for fn(*args).

        Complexity: O(1).
        """
        if timer["t"] is not None:
            timer["t"].cancel()
        timer["t"] = threading.Timer(t / 1000.0, fn, args=args)
        timer["t"].daemon = True
        timer["t"].start()
...
class Solution:
    def debounce(self, fn: Callable[..., Any], t: int) -> Callable[..., None]:
        """
        Interview explanation:
        Thin Solution wrapper for debounce.

        Algorithm:
        - Delegate to debounce(fn, t).
