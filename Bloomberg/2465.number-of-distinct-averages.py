...
class Solution:
    def distinctAverages(self, nums: List[int]) -> int:
        ...
        Repeatedly remove min and max, record their average; count distinct
        averages until empty.

        Algorithm:
        - Sort; pair ends inward; put averages in a set.
        ...
        i, j = 0, len(a) - 1
        while i < j:
            s.add(a[i] + a[j])  # 2*avg; same distinctness
            i += 1
            j -= 1
        return len(s)
# @lc code=end
