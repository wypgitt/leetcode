#
# @lc app=leetcode id=1555 lang=python3
#
...
# Testcase Example:  "{\"headers\": {\"Users\": [\"user_id\", \"user_name\", \"credit\"], \"Transactions\": [\"trans_id\", \"paid_by\", \"paid_to\", \"amount\", \"transacted_on\"]}, \"rows\": {\"Users\": [[1, \"Moustafa\", 100], [2, \"Jonathan\", 200], [3, \"Winston\", 10000], [4, \"Luis\", 800]], \"Transactions\": [[1, 1, 3, 400, \"2020-08-01\"], [2, 3, 2, 500, \"2020-08-02\"], [3, 2, 1, 200, \"2020-08-03\"]]}}"
#
#
# Table: Users
#
...
# Each row of this table contains information about the transaction in the
# bank.
...
# out the current balance of all users and check whether they have
...
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL. Users(user_id, user_name, credit) and
        Transactions(trans_id, paid_by, paid_to, amount, transacted_on).
        Apply all transfers then report credit and whether credit_limit_breached
        ('Yes' if credit < 0 else 'No').

        Algorithm:
        - Net change = SUM received - SUM paid per user (LEFT JOIN both roles).
        - credit' = credit + net; breached flag via CASE.

        Complexity: O(U + T) aggregation.
        """
        return self.sql
    ...
    SELECT
        u.user_id,
        u.user_name,
        u.credit + IFNULL(t.chg, 0) AS credit,
        CASE
            WHEN u.credit + IFNULL(t.chg, 0) < 0 THEN 'Yes'
            ELSE 'No'
        END AS credit_limit_breached
    FROM Users u
    LEFT JOIN (
        SELECT user_id, SUM(delta) AS chg
        FROM (
