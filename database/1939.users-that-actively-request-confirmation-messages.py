#
# @lc app=leetcode id=1939 lang=python3
#
...
# either confirmed ('confirmed') or expired without confirming
# ('timeout').
#
# Write a solution to find the IDs of the users that requested a
...
# User 6 requested two messages within 23 hours, 59 minutes, and 59
# seconds of each other, so we include them.
# User 7 requested two messages within 24 hours and 1 second of each
# other, so we exclude them from the answer.
#
# @lc code=start
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL. Users who requested two confirmation messages within 24 hours
        (same user, two distinct timestamps with difference ≤ 24h).

        Algorithm:
        - Self-join Confirmations on user_id where c1.time < c2.time and
          TIMESTAMPDIFF(SECOND, c1, c2) <= 86400; DISTINCT user_id.

        Complexity: O(C^2) join worst-case.
        """
        return self.sql
