#
# @lc app=leetcode id=3150 lang=python3
#
...
# |
# +----------+-----------------------------------------------------------------------------------+
#   | 1        | Traveling, exploring, and living my best life @JaneSmith
# @SaraJohnson @LisaTaylor |
#   |          | @MikeBrown #Foodie #Fitness #Learning
# |
#   | 2        | Just had the best dinner with friends! #Foodie #Friends
# #Fun                      |
#   | 4        | Working hard on my new project #Work #Goals #Productivity
# #Fun                    |
# +----------+-----------------------------------------------------------------------------------+
#
# Output:
...
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL: Tweets(tweet_id, content). Invalid if length > 140, more
        than 3 '@' mentions, or more than 3 '#' hashtags. Order by tweet_id.

        Algorithm:
        - Filter with LENGTH and (LENGTH - LENGTH(REPLACE(...))) counts.
        - ORDER BY tweet_id ASC.

        Complexity: O(N * L) for string scans.
        """
        self.sql = """
SELECT tweet_id
FROM Tweets
