#
# @lc app=leetcode id=1875 lang=python3
#
...
# Testcase Example:  "{\"headers\":{\"Employees\":[\"employee_id\",\"name\",\"salary\"]},\"rows\":{\"Employees\":[[2,\"Meir\",3000],[3,\"Michael\",3000],[7,\"Addilyn\",7400],[8,\"Juan\",6100],[9,\"Kannon\",7400]]}}"
#
#
# Table: Employees
#
...
# Juan (employee_id=8) is not included in any team because their salary of
# 6100 is unique (i.e. no other employee has the same salary).
# The team IDs are assigned as follows (based on salary ranking, lowest
# first):
# - team_id=1: Meir and Michael, a salary of 3000
# - team_id=2: Addilyn and Kannon, a salary of 7400
# Juan's salary of 6100 is not included in the ranking because they are
# not on a team.
#
# @lc code=start
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL: Employees(employee_id, name, salary). Assign team_id =
        dense rank of salary among salaries that appear at least twice.
        Employees with unique salary are excluded. Order by team_id, employee_id.

        Algorithm:
        - Filter salaries with COUNT>=2; DENSE_RANK() OVER (ORDER BY salary).
    ...
    FROM Employees e
    JOIN dup d ON e.salary = d.salary
    ORDER BY team_id, employee_id;
    """

    def solve_rank_filter(self) -> str:
        """
        Interview explanation:
        Alternate: window COUNT(*) OVER (PARTITION BY salary); keep cnt>=2 then
        dense-rank salaries.

        Algorithm:
        - CTE with cnt; filter; DENSE_RANK by salary.
