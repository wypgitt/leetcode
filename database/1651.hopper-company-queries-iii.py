#
# @lc app=leetcode id=1651 lang=python3
#
...
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL. For each month in 2020, report the working average ride
        distance and duration over that month and the previous two months
        (3-month rolling window of accepted rides requested in those months).
        Average = total / 3 even if some months have no rides. Round to 2 decimals.

        Algorithm:
        - Build months 1..12; join accepted rides in 2020 aggregated by month.
        - For month m, sum distance/duration over months m-2..m (clip to >=1);
          divide by 3; ROUND(..., 2).

        Complexity: O(R + A) after grouping.
        """
        return self.sql
    ...
    SELECT m.month,
           ROUND(IFNULL((
               SELECT SUM(a2.distance) FROM accepted a2
               WHERE a2.month BETWEEN m.month - 2 AND m.month
           ), 0) / 3, 2) AS average_ride_distance,
           ROUND(IFNULL((
               SELECT SUM(a2.duration) FROM accepted a2
               WHERE a2.month BETWEEN m.month - 2 AND m.month
           ), 0) / 3, 2) AS average_ride_duration
    FROM months m
    ORDER BY m.month;
    """
# @lc code=end
