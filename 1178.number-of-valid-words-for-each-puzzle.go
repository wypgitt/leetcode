...
// FindNumOfValidWords1178 returns, for each puzzle, the number of valid words.
func FindNumOfValidWords1178(words []string, puzzles []string) []int {
	cnt := make(map[int]int, len(words))
	for _, w := range words {
		m := bitmask1178(w)
