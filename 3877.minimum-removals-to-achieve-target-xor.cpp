...
public:
    int minRemovals(vector<int>& nums, int target) {
        int maxXor = 1 << 14, bad = -1000000000;
        vector<int> dp(maxXor, bad);
        dp[0] = 0;
        for (int v : nums) {
            vector<int> nxt = dp;
            for (int x = 0; x < maxXor; ++x) if (dp[x] != bad) nxt[x ^ v] = max(nxt[x ^ v], dp[x] + 1);
            dp.swap(nxt);
        }
        if (target >= maxXor || dp[target] == bad) return -1;
