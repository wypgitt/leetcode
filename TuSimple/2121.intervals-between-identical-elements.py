...
# between their indices. More formally, the interval between arr[i] and arr[j]
# is |i - j|.
#
...
class Solution:
    def getDistances(self, arr: List[int]) -> List[int]:
        ...
        For each index i, sum |i-j| over all j with arr[j]==arr[i].

        Algorithm:
        - Group indices by value. For sorted positions p[0..m):
          prefix contribution: ans[p[i]] = p[i]*i - sum(p[:i])
          + sum(p[i+1:]) - p[i]*(m-i-1).
