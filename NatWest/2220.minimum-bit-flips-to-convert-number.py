...
class Solution:
    def minBitFlips(self, start: int, goal: int) -> int:
        ...
        Flip bits of start to become goal; min flips is Hamming distance.

        Algorithm:
        - Bit-count of start XOR goal.

        Complexity: O(1) / O(bits) time, O(1) space.
        """
        return (start ^ goal).bit_count()

    def minBitFlips_bit(self, start: int, goal: int) -> int:
        """
        Interview explanation:
        Alternate: Brian Kernighan clear lowest set bit loop.

        Algorithm:
        - x = start^goal; while x: x &= x-1; count++.

        Complexity: O(bits set) time, O(1) space.
        """
        x = start ^ goal
        ans = 0
        while x:
