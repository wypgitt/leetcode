...
class Solution {
    pair<int, int> direction(int dx, int dy) {
...
public:
    long long countTrapezoids(vector<vector<int>>& points) {
        map<pair<int, int>, map<long long, long long>> lines;
        map<pair<int, int>, pair<long long, map<pair<int, int>, long long>>> mids;
        int n = points.size();
        for (int i = 0; i < n; ++i) {
            auto [x1, y1] = pair<int, int>{points[i][0], points[i][1]};
            for (int j = i + 1; j < n; ++j) {
                int x2 = points[j][0], y2 = points[j][1];
                auto dir = direction(x2 - x1, y2 - y1);
                long long c = 1LL * dir.second * x1 - 1LL * dir.first * y1;
                ++lines[dir][c];
        ...
        for (auto& [_, lineCounts] : lines) {
            long long prev = 0;
            for (auto& [__, cnt] : lineCounts) {
                parallel += prev * cnt;
                prev += cnt;
            }
        }
        long long parallelograms = 0;
        for (auto& [_, data] : mids) {
            long long total = data.first;
            long long cur = total * (total - 1) / 2;
            for (auto& [__, same] : data.second) cur -= same * (same - 1) / 2;
            parallelograms += cur;
        }
