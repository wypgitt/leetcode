...
class Solution:
    def shoppingOffers(
        ...
        Min cost to buy exact needs, using special offers (bundles) any number
        of times or individual prices. DFS + memo over remaining needs tuple.

        Algorithm:
        - DFS(remain): try each special if it fits; also base = sum(p_i*remain_i).
        - Memoize on tuple(remain).

        Complexity: O(S * product(needs_i+1)) states/transitions roughly.
        """
        n = len(price)

        @lru_cache(None)
        def dfs(remain: Tuple[int, ...]) -> int:
            cost = sum(price[i] * remain[i] for i in range(n))
