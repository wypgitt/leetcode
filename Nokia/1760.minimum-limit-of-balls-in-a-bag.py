...
class Solution:
    def minimumSize(self, nums: List[int], maxOperations: int) -> int:
        ...
        Binary search the minimum achievable max bag size m. Bag x needs
        (x-1)//m split operations. Feasible iff total ops ≤ maxOperations.

        Algorithm:
        - lo=1, hi=max(nums); binary search smallest m with can(m).
    ...
    def minimumSize_ceil(self, nums: List[int], maxOperations: int) -> int:
        """
        Interview explanation:
        Alternate same binary search using ceil(x/m)−1 = (x+m−1)//m − 1.

        Algorithm:
        - ops_needed(m) = sum((x+m-1)//m - 1); search minimal m with ops ≤ limit.

        Complexity: O(n log max(nums)).
        """
        def ops_needed(m: int) -> int:
            return sum((x + m - 1) // m - 1 for x in nums)
