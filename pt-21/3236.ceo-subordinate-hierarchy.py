#
# @lc app=leetcode id=3236 lang=python3
#
...
# Testcase Example:  "{\"headers\":{\"Employees\":[\"employee_id\",\"employee_name\",\"manager_id\",\"salary\"]},\"rows\":{\"Employees\":[[1,\"Alice\",null,150000],[2,\"Bob\",1,120000],[3,\"Charlie\",1,110000],[4,\"David\",2,105000],[5,\"Eve\",2,100000],[6,\"Frank\",3,95000],[7,\"Grace\",3,98000],[8,\"Helen\",5,90000]]}}"
#
#
# Table: Employees
#
...
# the CEO's salary
#
# Return the result table ordered by hierarchy_level ascending, and then
# by subordinate_id ascending.
#
# The query result format is in the following example.
#
# Example:
...
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL: list every non-CEO employee with hierarchy depth from the CEO
        and salary - CEO_salary. Order by level, then id.

        Algorithm:
        - Recursive CTE from CEO (manager_id IS NULL, level 0).
        - Join CEO salary; filter level > 0; order by hierarchy_level, subordinate_id.

        Complexity: O(N) recursion/BFS over the org tree.
        """
        self.sql = """
WITH RECURSIVE
    T AS (
