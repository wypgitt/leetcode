...
//         """
//         Return all words in words1 that contain every words2 string as a letter-multiset subset.
// 
...
//         req = [0] * 26
//         for b in words2:
//             cnt_b = self._letter_counts(b)
//             for i in range(26):
//                 req[i] = max(req[i], cnt_b[i])
// 
//         ans: List[str] = []
//         for a in words1:
//             cnt_a = self._letter_counts(a)
//             if self._covers(cnt_a, req):
//                 ans.append(a)
//         return ans
// 
//     @staticmethod
//     def _letter_counts(s: str) -> List[int]:
//         c = [0] * 26
//         base = ord("a")
//         for ch in s:
//             c[ord(ch) - base] += 1
...
// 
//     @staticmethod
//     def _covers(cnt_a: List[int], req: List[int]) -> bool:
//         for i in range(26):
//             if cnt_a[i] < req[i]:
//                 return False
//         return True
...
class Solution {
    ...
    bool covers(vector<int>& a, vector<int>& req) {
        for (int i = 0; i < 26; ++i) if (a[i] < req[i]) return false;
        return true;
    }
...
public:
    vector<string> wordSubsets(vector<string>& words1, vector<string>& words2) {
        ...
        for (auto& b : words2) {
            auto cb = counts(b);
            for (int i = 0; i < 26; ++i) req[i] = max(req[i], cb[i]);
        }
        vector<string> ans;
        for (auto& a : words1) {
            auto ca = counts(a);
            if (covers(ca, req)) ans.push_back(a);
        }
        return ans;
    }
};
