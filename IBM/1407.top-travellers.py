#
# @lc app=leetcode id=1407 lang=python3
#
# [1407] Top Travellers
#
...
class Solution:
    def solve(self) -> str:
        ...
        SQL: report each user's total travelled distance (0 if no rides).
        Order by distance DESC, then name ASC.

        Algorithm:
        - LEFT JOIN Users to Rides on id=user_id; SUM(distance) with COALESCE;
          GROUP BY user; ORDER BY travelled_distance DESC, name ASC.

        Complexity: O(U+R) with proper indexes / hash aggregation.
        """
        return self.sql
