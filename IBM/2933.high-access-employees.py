...
# name of an employee, and access_times[i][1] represents the access time
...
# However, "e" has just one access time, so it can not be in the answer
# and the final answer is ["c","d"].
#
# Example 3:
#
# Input: access_times =
...
# "cd" has also three access times in the one-hour period of [10:25,
# 11:24] which are 10:25, 10:46, and 10:55.
...
class Solution:
    def findHighAccessEmployees(self, access_times: List[List[str]]) -> List[str]:
        ...
        High-access: some employee has 3+ accesses within any one-hour window
        (strictly less than 60 minutes from first to third in a triple).

        Algorithm:
        - Group times by name; sort minutes; if times[i+2]-times[i] < 60, mark.

        Complexity: O(m log m) time, O(m) space.
        """
        by = defaultdict(list)
        for name, t in access_times:
            by[name].append(int(t[:2]) * 60 + int(t[2:]))
        ans = []
