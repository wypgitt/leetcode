#
# @lc app=leetcode id=1321 lang=python3
#
...
# window (i.e., current day + 6 days before). average_amount should be rounded
...
class Solution:
    def solve(self) -> str:
        ...
        Customer(customer_id, name, visited_on, amount). Compute 7-day moving
        sum and average (2 decimals) for each day that has a full prior window
        (day and 6 days before). There is at least one customer every day.

        Algorithm:
        - Daily totals CTE; self-join/window over 7 days; filter days with
          COUNT=7; ROUND(avg,2); order by visited_on.

        Complexity: O(D) over distinct days.
        """
        return self.sql
