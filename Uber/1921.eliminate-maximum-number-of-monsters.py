...
class Solution:
    def eliminateMaximum(self, dist: List[int], speed: List[int]) -> int:
        ...
        Weapon charges 1/min starting at t=0; kill one monster per minute.
        Monster i arrives at ceil(dist/speed). Kill in order of arrival time.

        Algorithm:
        - arrival = ceil(d/s); sort; kill if arrival[i] > i (minute i).
