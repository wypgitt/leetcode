...
class Solution:
    def countPairs(self, root: Optional[TreeNode], distance: int) -> int:
        ...
        Good leaf pair: path length between leaves <= distance. Postorder DFS
        returns distances of leaves in subtree; combine left/right counts.

        Algorithm:
        - dfs returns list of leaf depths from node; for a in left, b in right
          if a+b<=distance ans++; return [d+1 for d in left+right if d+1<distance].

        Complexity: O(n * distance^2) time, O(h*distance) space.
        """
        self.ans = 0
