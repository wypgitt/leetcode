...
# In all the subarrays, the maximum element is equal to k = 4, so nums is
# now beautiful.
...
# In all the subarrays, the maximum element is equal to k = 5, so nums is
# now beautiful.
...
# Output: 0
# Explanation: The only subarray with a size of 3 or more in this example
# is [1,1,2].
# The maximum element, 2, is already greater than k = 1, so we don't need
# any increment operation.
...
class Solution:
    def minIncrementOperations(self, nums: List[int], k: int) -> int:
        ...
        Array is beautiful iff every length-3 subarray has max >= k (implies
        all longer windows too). Min total +1 operations.

        Algorithm:
        - Rolling DP of 3 states: min cost where we "cover" via raising the
          element at the current / prev / prev2 index to >= k.
        - Transition: new_cost = min(prev three) + max(0, k - nums[i]).

        Complexity: O(n) time, O(1) space.
        """
        a = b = c = 0
        for x in nums:
            a, b, c = b, c, min(a, b, c) + max(0, k - x)
        return min(a, b, c)
# @lc code=end
