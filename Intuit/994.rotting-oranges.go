...
type cell994 struct{ r, c int }

// OrangesRotting994 returns the minimum minutes until all fresh oranges rot, or -1.
func OrangesRotting994(grid [][]int) int {
	m := len(grid)
	n := len(grid[0])

	q := make([]cell994, 0, m*n)
	fresh := 0
	for i := 0; i < m; i++ {
		for j := 0; j < n; j++ {
			if grid[i][j] == 1 {
	...
	head := 0
	for head < len(q) {
		levelCount := len(q) - head
		for t := 0; t < levelCount; t++ {
			cur := q[head]
			head++
			for _, d := range dirs {
				ni := cur.r + d[0]
