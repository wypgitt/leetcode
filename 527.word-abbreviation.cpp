...
class Solution {
    string abbreviate(const string& word, int pref) {
        int omitted = word.size() - pref - 1;
        string abbr = word.substr(0, pref) + to_string(omitted) + word.back();
...
public:
    vector<string> wordsAbbreviation(vector<string>& words) {
        ...
        while (true) {
            ...
            for (int i = 0; i < n; ++i) groups[abbreviate(words[i], pref[i])].push_back(i);
            bool conflict = false;
            for (auto& [_, ids] : groups) if (ids.size() > 1) {
                conflict = true;
                for (int id : ids) ++pref[id];
            }
            if (!conflict) break;
        }
        vector<string> ans;
        for (int i = 0; i < n; ++i) ans.push_back(abbreviate(words[i], pref[i]));
        return ans;
    }
};
