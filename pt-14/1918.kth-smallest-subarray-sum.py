...
class Solution:
    def kthSmallestSubarraySum(self, nums: List[int], k: int) -> int:
        ...
        Premium. k-th smallest among all contiguous subarray sums.
        Binary search on sum X; count how many subarrays have sum ≤ X via
        sliding window (nums positive).

        Algorithm:
        - lo=min(nums), hi=sum(nums). mid; count ≤ mid with two pointers;
