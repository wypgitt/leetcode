...
# m respectively. servers[i] is the weight of the i^th server, and tasks[j] is
# the time needed to process the j^th task in seconds.
#
...
class Solution:
    def assignTasks(self, servers: List[int], tasks: List[int]) -> List[int]:
        ...
        Assign each task (arrival at time i, duration tasks[i]) to free server
        with smallest weight then smallest index; if none free, wait until one
        frees. Return server index per task.

        Algorithm (two heaps):
        - free: (weight, index); busy: (free_time, weight, index).
        - For each task time t=i: release busy with free_time≤t (or ≤next if empty).
        - Pop free, push busy with free_time = cur_time + tasks[i].
