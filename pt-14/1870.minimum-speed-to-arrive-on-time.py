...
class Solution:
    def minSpeedOnTime(self, dist: List[int], hour: float) -> int:
        ...
        Trains between n cities; wait to integer hour between trains except last.
        Binary search minimum integer speed to finish by `hour`.

        Algorithm (binary search):
        - lo=1, hi=1e7. time = sum(ceil(d/mid) for all but last)+last/mid.
        - If time≤hour feasible → search lower else higher.
    ...
    def minSpeedOnTime_linear(self, dist: List[int], hour: float) -> int:
        """
        Interview explanation:
        Alternate: try speeds from 1 upward until feasible (same check). Only for
        discussion; binary search is the optimal search over monotone predicate.

        Algorithm:
        - For speed in 1..1e7: if ok(speed) return speed; else -1.

        Complexity: O(n*V) time.
        """
        n = len(dist)
        if hour <= n - 1:
            return -1
        ...
        def ok(speed: int) -> bool:
            t = sum(math.ceil(dist[i] / speed) for i in range(n - 1)) + dist[-1] / speed
            return t <= hour
