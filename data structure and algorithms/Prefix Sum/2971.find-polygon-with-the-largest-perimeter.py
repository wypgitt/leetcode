...
# ..., a_k where a_1 <= a_2 <= a_3 <= ... <= a_k and a_1 + a_2 + a_3 + ...
# + a_k-1 > a_k, then there always exists a polygon with k sides whose
# lengths are a_1, a_2, a_3, ..., a_k.
#
...
# from nums has 5 sides: 1, 1, 2, 3, and 5. The perimeter is 1 + 1 + 2 + 3
# + 5 = 12.
# We cannot have a polygon with either 12 or 50 as the longest side
# because it is not possible to include 2 or more smaller sides that have
...
class Solution:
    def largestPerimeter(self, nums: List[int]) -> int:
        ...
        Polygon sides (sorted) need sum of all but the longest > longest; maximize sum.

        Algorithm:
        - Sort ascending. Prefix sum; from the right, find largest prefix of length >= 3
          where sum - max > max. Return that sum or -1.
        ...
        for i in range(len(nums) - 1, 1, -1):
            total_without = total - nums[i]
            if total_without > nums[i]:
                return total
            total -= nums[i]
        return -1

    def largestPerimeter_prefix(self, nums: List[int]) -> int:
        """
        Interview explanation:
        Alternate: explicit prefix sums after sorting.

        Algorithm:
        - Build prefix; scan i from n-1 down to 2; if pref[i] > nums[i] return pref[i+1].
