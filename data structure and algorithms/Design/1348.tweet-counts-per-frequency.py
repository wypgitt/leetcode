...
class TweetCounts:
    def __init__(self):
        ...
        Store tweet timestamps per name (sorted). recordTweet inserts a time;
        getTweetCountsPerFrequency buckets [start,end] by minute/hour/day.

        Algorithm:
        - defaultdict(list); bisect.insort on record; on query compute chunk
          size, allocate buckets, bisect range and increment chunk indices.

        Complexity: record O(n); query O(n) over tweets in range / O(k+occ).
        """
    ...
    def recordTweet(self, tweetName: str, time: int) -> None:
        """
        Interview explanation:
        Insert timestamp in sorted order for tweetName.

        Algorithm:
        - bisect.insort into list.

        Complexity: O(n) for insert into list.
        """
    ...
    def getTweetCountsPerFrequency(
        ...
        """
        Interview explanation:
        Split [startTime, endTime] into consecutive intervals of length
        60 / 3600 / 86400; count tweets in each chunk.

        Algorithm:
        - delta by freq; size = (end-start)//delta + 1; for each time in range
          via bisect, idx=(t-start)//delta; increment.

        Complexity: O(occ + chunks) time.
        """
