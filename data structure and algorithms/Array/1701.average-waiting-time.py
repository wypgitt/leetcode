...
#
# arrival_i is the arrival time of the i^th customer. The arrival times are
...
class Solution:
    def averageWaitingTime(self, customers: List[List[int]]) -> float:
        ...
        Single chef processes customers in order. Track when the chef becomes free;
        each customer's finish = max(arrival, free) + cook time; wait = finish - arrival.

        Algorithm:
        - free = 0; total = 0
        - For each (arr, t): start = max(free, arr); finish = start + t; total += finish - arr; free = finish
        - Return total / n
