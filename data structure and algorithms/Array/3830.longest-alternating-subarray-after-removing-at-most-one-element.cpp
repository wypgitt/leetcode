/*
 * @lc app=leetcode id=3830 lang=cpp
 *
 * [3830] Longest Alternating Subarray After Removing At Most One Element
 */
// Translated from 3830.longest-alternating-subarray-after-removing-at-most-one-element.py.
// Original Python source and explanation are preserved below as comments.
// #
// # lc-original app=leetcode id=3830 lang=python3
// #
// # [3830] Longest Alternating Subarray After Removing At Most One Element
// #
// #
...
public:
    int longestAlternating(vector<int>& nums) {
        ...
        if (n <= 1) return n;
        vector<int> sign(n - 1);
        for (int i = 0; i + 1 < n; ++i) sign[i] = cmp(nums[i], nums[i + 1]);
        vector<int> endLen(n, 1), startLen(n, 1);
        for (int i = 1; i < n; ++i) {
            if (sign[i - 1] == 0) endLen[i] = 1;
            else if (i >= 2 && sign[i - 2] == -sign[i - 1]) endLen[i] = endLen[i - 1] + 1;
            else endLen[i] = 2;
        }
        for (int i = n - 2; i >= 0; --i) {
            if (sign[i] == 0) startLen[i] = 1;
            else if (i + 2 < n && sign[i] == -sign[i + 1]) startLen[i] = startLen[i + 1] + 1;
            else startLen[i] = 2;
        }
        int ans = *max_element(endLen.begin(), endLen.end());
        for (int removed = 1; removed + 1 < n; ++removed) {
