...
# half-closed segment [start_i, end_i) with color_i as the color.
#
...
# describes a half-closed segment [left_j, right_j) with the mixed color sum of
# mix_j.
#
...
class Solution:
    def splitPainting(self, segments: List[List[int]]) -> List[List[int]]:
        ...
        Mix colors on half-open segments; return mixed painting as [start,end,sum]
        for maximal stretches with constant mix (sum>0).

        Algorithm:
        - Sweep: diff[start]+=color, diff[end]-=color; scan sorted points; emit
          segments between consecutive points while running sum > 0.
        ...
        for i in range(len(points) - 1):
            run += diff[points[i]]
            if run > 0:
                ans.append([points[i], points[i + 1], run])
        return ans
# @lc code=end
