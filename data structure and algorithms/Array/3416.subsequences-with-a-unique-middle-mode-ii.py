#
# @lc app=leetcode id=3416 lang=python3
#
# [3416] Subsequences with a Unique Middle Mode II
#
# https://leetcode.com/problems/subsequences-with-a-unique-middle-mode-ii/description/
#
# algorithms
# Hard (15.65%)
# Likes:    2
...
# from this list, and it has a unique middle mode of 1.
#
# Example 2:
#
...
# number at index 2 has the greatest frequency in the subsequence. [1, 2,
# 2, 3, 3] does not have a unique middle mode because 2 and 3 both appear
# twice in the subsequence.
#
# Example 3:
#
...
# Explanation:
#
# There does not exist a subsequence of length 5 with a unique middle
# mode.
#
# Constraints:
#
...
class Solution:
    def subsequencesWithMiddleMode(self, nums: List[int]) -> int:
        ...
        Same as Unique Middle Mode I but n <= 1e5: count length-5
        subsequences whose middle element is the unique mode. Fix the
        middle index; total C(L,2)*C(R,2) minus configurations where some
        other value ties/beats the middle's frequency (combinatorial
        casework with aggregate frequency sums).

        Algorithm:
        - Maintain left/right frequency maps while scanning the middle.
        - Keep aggregate sums (sum left[x]^2, left[x]*right[x], ...)
          excluding the middle value to subtract invalid patterns in O(1).
