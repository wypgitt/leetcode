#
# @lc app=leetcode id=304 lang=python3
#
# [304] Range Sum Query 2D - Immutable
#
# https://leetcode.com/problems/range-sum-query-2d-immutable/description/
#
# algorithms
# Medium (58.75%)
# Likes:    5457
# Dislikes: 373
# Total Accepted:    501K
# Total Submissions: 852K
# Testcase Example:  "[\"NumMatrix\",\"sumRegion\",\"sumRegion\",\"sumRegion\"]"
#
# Given a 2D matrix matrix, handle multiple queries of the following type:
#
# Calculate the sum of the elements of matrix inside the rectangle defined by
# its upper left corner (row1, col1) and lower right corner (row2, col2).
#
# Implement the NumMatrix class:
#
# NumMatrix(int[][] matrix) Initializes the object with the integer matrix
# matrix.
#
# int sumRegion(int row1, int col1, int row2, int col2) Returns the sum of the
# elements of matrix inside the rectangle defined by its upper left corner
# (row1, col1) and lower right corner (row2, col2).
#
# You must design an algorithm where sumRegion works on O(1) time complexity.
#
# Example 1:
#
# Input
# ["NumMatrix", "sumRegion", "sumRegion", "sumRegion"]
# [[[[3, 0, 1, 4, 2], [5, 6, 3, 2, 1], [1, 2, 0, 1, 5], [4, 1, 0, 1, 7], [1, 0,
# 3, 0, 5]]], [2, 1, 4, 3], [1, 1, 2, 2], [1, 2, 2, 4]]
# Output
# [null, 8, 11, 12]
#
# Explanation
# NumMatrix numMatrix = new NumMatrix([[3, 0, 1, 4, 2], [5, 6, 3, 2, 1], [1, 2,
# 0, 1, 5], [4, 1, 0, 1, 7], [1, 0, 3, 0, 5]]);
# numMatrix.sumRegion(2, 1, 4, 3); // return 8 (i.e sum of the red rectangle)
# numMatrix.sumRegion(1, 1, 2, 2); // return 11 (i.e sum of the green
# rectangle)
# numMatrix.sumRegion(1, 2, 2, 4); // return 12 (i.e sum of the blue rectangle)
#
# Constraints:
#
# m == matrix.length
#
# n == matrix[i].length
#
# 1 <= m, n <= 200
#
# -10^4 <= matrix[i][j] <= 10^4
#
# 0 <= row1 <= row2 < m
#
# 0 <= col1 <= col2 < n
#
# At most 10^4 calls will be made to sumRegion.
#

# @lc code=start
from typing import List


class NumMatrix:
    def __init__(self, matrix: List[List[int]]):
        """
        Interview explanation:
        2D prefix sums: dp[i+1][j+1] = sum of submatrix (0,0)..(i,j).
        Region sum via inclusion-exclusion of four corners.

        Algorithm:
        - dp[i+1][j+1] = matrix[i][j] + dp[i][j+1] + dp[i+1][j] - dp[i][j].

        Complexity: O(mn) build, O(1) query, O(mn) space.
        """
        if not matrix or not matrix[0]:
            self.dp = [[0]]
            return
        m, n = len(matrix), len(matrix[0])
        self.dp = [[0] * (n + 1) for _ in range(m + 1)]
        for i in range(m):
            for j in range(n):
                self.dp[i + 1][j + 1] = (
                    matrix[i][j]
                    + self.dp[i][j + 1]
                    + self.dp[i + 1][j]
                    - self.dp[i][j]
                )

    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        """
        Interview explanation:
        Rectangle sum from 2D prefix via inclusion-exclusion of four corners.

        Algorithm:
        - dp[r2+1][c2+1] - dp[r1][c2+1] - dp[r2+1][c1] + dp[r1][c1].

        Complexity: O(1) time, O(1) space.
        """
        dp = self.dp
        return (
            dp[row2 + 1][col2 + 1]
            - dp[row1][col2 + 1]
            - dp[row2 + 1][col1]
            + dp[row1][col1]
        )


# Your NumMatrix object will be instantiated and called as such:
# obj = NumMatrix(matrix)
# param_1 = obj.sumRegion(row1,col1,row2,col2)
# @lc code=end

