#
# @lc app=leetcode id=3496 lang=python3
#
...
#
# Remove the first and last elements (2 + 1) = 3. The remaining array is
# [4].
#
...
# The possible operations are:
#
# Remove the first and last elements (5 + 2) = 7. The remaining array is
# [-1, 4].
#
# Remove the first two elements (5 + -1) = 4. The remaining array is [4,
# 2].
#
# Remove the last two elements (4 + 2) = 6. The remaining array is [5,
# -1].
#
...
class Solution:
    def maxScore(self, nums: List[int]) -> int:
        ...
        Each op removes two endpoints of the current array. The leftover when
        we stop (≤ 2 elements) must be a contiguous segment of the original.
        Maximize removed sum ≡ minimize leftover sum.

        Algorithm:
        - Odd n: leave the single minimum element.
        - Even n: leave the adjacent pair with minimum sum.
        - Score = total sum − leftover.
        ...
        return total - min(a + b for a, b in pairwise(nums))
# @lc code=end
