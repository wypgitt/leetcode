...
class Solution:
    def maxIncreasingCells(self, mat: List[List[int]]) -> int:
        ...
        Move in same row/col to a strictly greater value; maximize path length.

        Algorithm:
        - Bucket cells by value ascending. For each value, dp = 1+max(rowMax,colMax)
          using frozen maxima from smaller values; then update row/col maxima.
        ...
        for _, pos in sorted(g.items()):
            mx = [1 + max(row_max[i], col_max[j]) for i, j in pos]
            ans = max(ans, max(mx))
    ...
    def maxIncreasingCells_dp(self, mat: List[List[int]]) -> int:
        """
        Interview explanation:
        Alternate DP naming for the value-ordered row/col maxima approach.

        Algorithm:
        - Same batched DP by increasing values.

        Complexity: O(mn log(mn)) time, O(mn) space.
        """
        return self.maxIncreasingCells(mat)
# @lc code=end
