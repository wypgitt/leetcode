...
# =[5,3,2,1,4].
# We can see that for each index i, nums1[i] != nums2[i]. The cost required here
# is 10.
# Note that there are other ways to swap values, but it can be proven that it is
# not possible to obtain a cost less than 10.
#
# Example 2:
#
...
class Solution:
    def minimumTotalCost(self, nums1: List[int], nums2: List[int]) -> int:
        ...
        Make nums1[i] != nums2[i] for all i by swapping values in nums1 (cost
        = sum of indices). Minimize total cost, or -1.

        Algorithm:
        - All equal positions must be involved in swaps (add their indices).
          If some value dominates equals (2*freq > #equals), borrow enough
          unequal positions where both sides != that value (cheapest first).
