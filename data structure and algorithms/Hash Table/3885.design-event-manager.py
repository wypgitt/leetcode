...
class EventManager:
    ...
    * Randomized testing against a simple reference dictionary that scans all
    ...
    def updatePriority(self, eventId: int, newPriority: int) -> None:
        """
        Interview explanation:
        Change an active event's priority (lazy heap update).

        Algorithm:
        - Overwrite active_priority[eventId]; push new (-priority, id) onto heap.
    ...
    def pollHighest(self) -> int:
        """
        Interview explanation:
        Remove and return the highest-priority active event id, or -1.

        Algorithm:
        - Pop heap until top matches active_priority (discard stale entries).
