...
class Solution:
    def profitableSchemes(self, n: int, minProfit: int, group: List[int], profit: List[int]) -> int:
        ...
        Count subsets of crimes with members used <= n and profit >= minProfit.
        DP: dp[members][profit] = ways (cap profit at minProfit).

        Algorithm (DP):
        - dp[0][0]=1. For each crime (g,p), update backward:
          dp[j+g][min(minProfit, k+p)] += dp[j][k].
        - Sum dp[*][minProfit].

        Complexity: O(len(group)*n*minProfit) time/space.
        """
        MOD = 10**9 + 7
            ...
            for j in range(n - g, -1, -1):
                for k in range(minProfit, -1, -1):
                    if dp[j][k]:
                        ...
                        dp[j + g][nk] = (dp[j + g][nk] + dp[j][k]) % MOD
