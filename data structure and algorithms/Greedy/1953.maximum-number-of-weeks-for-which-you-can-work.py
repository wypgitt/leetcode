...
class Solution:
    def numberOfWeeks(self, milestones: List[int]) -> int:
        ...
        One milestone/week, never same project two weeks in a row. With total S
        and max M: finish all S iff M <= S-M+1; else only 2*(S-M)+1 weeks.

        Algorithm:
        - S=sum, M=max; return S if M <= S-M+1 else 2*(S-M)+1.
    ...
    def numberOfWeeks_clarity(self, milestones: List[int]) -> int:
        """
        Interview explanation:
        Alternate wording of the same greedy bound.

        Algorithm:
        - Same closed form with explicit others = total - largest.
        ...
        return total if largest <= others + 1 else others * 2 + 1
# @lc code=end
