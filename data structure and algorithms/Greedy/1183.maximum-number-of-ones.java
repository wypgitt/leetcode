/*
 * @lc app=leetcode id=1183 lang=java
 *
...
public class Solution {
    ...
    private static int rowOrColCount(int dim, int residue, int L) {
        if (residue >= dim) {
            return 0;
        }
    ...
    public int maximumNumberOfOnes(int width, int height, int sideLength, int maxOnes) {
        ...
        for (int a = 0; a < L; a++) {
            int ra = rowOrColCount(height, a, L);
            for (int b = 0; b < L; b++) {
                int rb = rowOrColCount(width, b, L);
                freq.add(ra * rb);
            }
        }
        ...
        int sum = 0;
        int take = Math.min(maxOnes, freq.size());
