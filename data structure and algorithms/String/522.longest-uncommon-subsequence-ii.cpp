...
class Solution {
    bool isSubseq(const string& small, const string& large) {
        int p = 0;
        for (char c : large) if (p < (int)small.size() && small[p] == c) ++p;
        return p == (int)small.size();
    }

public:
    int findLUSlength(vector<string>& strs) {
        vector<int> order(strs.size());
        ...
        for (int idx : order) {
            ...
            for (int j = 0; j < (int)strs.size(); ++j) {
                if (j == idx) continue;
                if (strs[j].size() >= strs[idx].size() && isSubseq(strs[idx], strs[j])) {
                    uncommon = false;
                    break;
                }
            }
            if (uncommon) return strs[idx].size();
        }
        return -1;
    }
};
