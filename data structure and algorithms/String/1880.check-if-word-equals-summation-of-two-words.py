...
class Solution:
    def isSumEqual(self, firstWord: str, secondWord: str, targetWord: str) -> bool:
        ...
        Map 'a'→0 ...'j'→9; interpret words as base-10 numbers; check
        value(first)+value(second)==value(target).

        Algorithm:
        - def val(w): int(''.join(str(ord(c)-97) for c in w))
        - return val(first)+val(second)==val(target).
