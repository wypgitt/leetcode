...
class Solution:
    def maximumNumber(self, num: str, change: List[int]) -> str:
        ...
        Mutate one contiguous substring by mapping digit d→change[d]. Choose
        substring to maximize resulting number (lexicographic).

        Algorithm:
        - Scan left-to-right; start mutating at first d where change[d]>d; continue
          while change[d]>=d; stop at first decrease. At most one segment.
