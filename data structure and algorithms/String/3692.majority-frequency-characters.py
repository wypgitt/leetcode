...
class Solution:
    def majorityFrequencyGroup(self, s: str) -> str:
        ...
        Group characters by occurrence count k; pick the largest group, ties
        broken by larger k.

        Algorithm:
        - Count frequencies; bucket chars by k.
        - Choose bucket maximizing (size, k); join its characters.

        Complexity: O(|s|) time, O(1) space over alphabet.
        """
        freq = Counter(s)
