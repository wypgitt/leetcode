...
class Solution:
    def reductionOperations(self, nums: List[int]) -> int:
        ...
        Each op: pick largest element that isn't unique-largest? Reduce a largest
        value to the next strictly smaller present value. Count ops to equalize.
        Equals: for each value, how many steps down to the minimum.

        Algorithm (sort):
        - Sort; walk unique tiers: each new larger tier adds (count of elements
          at/above) when tier increases → accumulate index of distinct rank.
