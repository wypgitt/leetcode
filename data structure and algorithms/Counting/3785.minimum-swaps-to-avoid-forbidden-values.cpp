...
public:
    int minSwaps(vector<int>& nums, vector<int>& forbidden) {
        int n = nums.size();
        unordered_map<int, int> cn, cf;
        for (int x : nums) ++cn[x];
        for (int x : forbidden) ++cf[x];
        unordered_set<int> keys;
        for (auto [x, _] : cn) keys.insert(x);
        for (auto [x, _] : cf) keys.insert(x);
        for (int x : keys) if (cn[x] + cf[x] > n) return -1;
        unordered_map<int, int> bad;
        int total = 0, maxBad = 0;
        for (int i = 0; i < n; ++i) if (nums[i] == forbidden[i]) {
            maxBad = max(maxBad, ++bad[nums[i]]);
            ++total;
        }
        if (total == 0) return 0;
