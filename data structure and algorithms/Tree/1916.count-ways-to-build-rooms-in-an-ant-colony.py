#
# @lc app=leetcode id=1916 lang=python3
#
# [1916] Count Ways to Build Rooms in an Ant Colony
#
# https://leetcode.com/problems/count-ways-to-build-rooms-in-an-ant-colony/description/
#
# algorithms
# Hard (51.82%)
# Likes:    528
# Dislikes: 57
# Total Accepted:    11.9K
# Total Submissions: 22.9K
# Testcase Example:  "[-1,0,1]"
#
# You are an ant tasked with adding n new rooms numbered 0 to n-1 to your
# colony. You are given the expansion plan as a 0-indexed integer array of
# length n, prevRoom, where prevRoom[i] indicates that you must build room
# prevRoom[i] before building room i, and these two rooms must be connected
# directly. Room 0 is already built, so prevRoom[0] = -1. The expansion plan is
# given such that once all the rooms are built, every room will be reachable
# from room 0.
#
# You can only build one room at a time, and you can travel freely between
# rooms you have already built only if they are connected. You can choose to
# build any room as long as its previous room is already built.
#
# Return the number of different orders you can build all the rooms in. Since
# the answer may be large, return it modulo 10^9 + 7.
#
# Example 1:
#
# Input: prevRoom = [-1,0,1]
# Output: 1
# Explanation: There is only one way to build the additional rooms: 0 → 1 → 2
#
# Example 2:
#
# Input: prevRoom = [-1,0,0,1,2]
# Output: 6
# Explanation:
# The 6 ways are:
# 0 → 1 → 3 → 2 → 4
# 0 → 2 → 4 → 1 → 3
# 0 → 1 → 2 → 3 → 4
# 0 → 1 → 2 → 4 → 3
# 0 → 2 → 1 → 3 → 4
# 0 → 2 → 1 → 4 → 3
#
# Constraints:
#
# n == prevRoom.length
#
# 2 <= n <= 10^5
#
# prevRoom[0] == -1
#
# 0 <= prevRoom[i] < n for all 1 <= i < n
#
# Every room is reachable from room 0 once all the rooms are built.
#

# @lc code=start
from typing import List


class Solution:
    def waysToBuildRooms(self, prevRoom: List[int]) -> int:
        """
        Interview explanation:
        Tree of room prerequisites (prevRoom[i] parent of i). Ways to order
        builds = multinomial over children subtree sizes: for node, interleave
        children's topological sequences.

        Algorithm:
        - MOD=1e9+7. DFS returns (ways, size). ways = product(child.ways) *
          C(sum sizes; child sizes) multinomial.
        - Precompute factorials / inv factorials for nCk mod MOD.

        Complexity: O(n) time with O(n) fact, O(n) space.
        """
        MOD = 10**9 + 7
        n = len(prevRoom)
        g = [[] for _ in range(n)]
        for i in range(1, n):
            g[prevRoom[i]].append(i)

        fact = [1] * (n + 1)
        for i in range(1, n + 1):
            fact[i] = fact[i - 1] * i % MOD
        invfact = [1] * (n + 1)
        invfact[n] = pow(fact[n], MOD - 2, MOD)
        for i in range(n, 0, -1):
            invfact[i - 1] = invfact[i] * i % MOD

        def nCk(a: int, b: int) -> int:
            return fact[a] * invfact[b] % MOD * invfact[a - b] % MOD

        def dfs(u: int):
            ways, size = 1, 0
            for v in g[u]:
                w, s = dfs(v)
                ways = ways * w % MOD * nCk(size + s, s) % MOD
                size += s
            return ways, size + 1

        return dfs(0)[0]
# @lc code=end
