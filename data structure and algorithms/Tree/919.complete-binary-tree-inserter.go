...
// The front of the queue is always the parent for the next insert.
type CBTInserter919 struct {
	root *TreeNode
	// Nodes that do not yet have two children; next parent is at candHead.
	candidates []*TreeNode
	candHead   int
}

func NewCBTInserter919(root *TreeNode) *CBTInserter919 {
	c := &CBTInserter919{root: root}
	if root == nil {
		return c
	}
	bfs := []*TreeNode{root}
...
func (c *CBTInserter919) Insert919(val int) int {
	parent := c.candidates[c.candHead]
...
func (c *CBTInserter919) GetRoot919() *TreeNode {
	return c.root
}
