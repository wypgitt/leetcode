...
#
# Remove the empty prefix and the suffix [4, 8, 16, 32]. nums becomes [1,
# 2].
#
# Remove the empty prefix and the suffix [2, 4, 8, 16, 32]. nums becomes
# [1].
#
...
class Solution:
    def resultArray(self, nums: List[int], k: int) -> List[int]:
        ...
        Keeping a contiguous middle after removing prefix/suffix means counting
        subarray products modulo k for every residue.

        Algorithm:
        - For each ending index, maintain counts of products of subarrays ending there.
        - Extend previous endings by * nums[i], plus the singleton [nums[i]].
        - Accumulate into result[x].
