...
class Solution:
    def sumOfPower(self, nums: List[int]) -> int:
        ...
        For every non-empty subsequence, power = max^2 * min. Sum powers mod 1e9+7.

        Algorithm:
        - Sort ascending. Sweep x as max: contrib = x^2 * (x + F_prev) where F is sum of mins over
          nonempty subsets of earlier elements; F <- 2F + x.
