...
# length m representing the DAG, where edges[i] = [u_i, v_i] indicates
# that there is a directed edge from team u_i to team v_i in the graph.
#
...
# Notes
#
# A cycle is a series of nodes a_1, a_2, ..., a_n, a_n+1 such that node
# a_1 is the same node as node a_n+1, the nodes a_1, a_2, ..., a_n are
# distinct, and there is a directed edge from the node a_i to node a_i+1
...
class Solution:
    def findChampion(self, n: int, edges: List[List[int]]) -> int:
        ...
        Edge u->v means u is stronger than v. A champion has in-degree 0 (nobody
        stronger). Return it only if unique.

        Algorithm:
        - Mark nodes with incoming edges; collect zeros; return sole candidate else -1.
        ...
        for _, v in edges:
            weaker[v] = True
        champs = [i for i in range(n) if not weaker[i]]
        return champs[0] if len(champs) == 1 else -1

    def findChampion_indegree(self, n: int, edges: List[List[int]]) -> int:
        """
        Interview explanation:
        Alternate: explicit in-degree counts; scan for the unique zero.

        Algorithm:
        - Count in-degrees; track the only node with degree 0.
