...
# [k_i, trim_i]. For each queries[i], you need to:
#
#
# Trim each number in nums to its rightmost trim_i digits.
#
#
# Determine the index of the k_i^th smallest trimmed number in nums. If two
# trimmed numbers are equal, the number with the lower index is considered to be
# smaller.
#
#
...
class Solution:
    def smallestTrimmedNumbers(self, nums: List[str], queries: List[List[int]]) -> List[int]:
        ...
        For query (k, trim), consider each nums[i] trimmed to last `trim`
        digits; return original index of the k-th smallest (ties by index).

        Algorithm:
        - Group queries by trim (or answer each); sort indices by trimmed
          string then index; pick k-1.

        Complexity: O(t * n log n * L) worst; with t unique trims up to L.
        """
        n = len(nums)
        ans = []
        # cache sorts by trim length
        cache = {}
        for k, trim in queries:
            if trim not in cache:
                cache[trim] = sorted(range(n), key=lambda i: (nums[i][-trim:], i))
            ans.append(cache[trim][k - 1])
        return ans
# @lc code=end
