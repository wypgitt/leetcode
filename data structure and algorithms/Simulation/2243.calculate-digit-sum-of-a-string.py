...
# Output: "135"
# Explanation:
# - For the first round, we divide s into groups of size 3: "111", "112", "222",
# and "23".
...
class Solution:
    def digitSum(self, s: str, k: int) -> str:
        ...
        While len(s) > k, split into groups of size k (last may be shorter),
        replace each group by digit-sum as decimal string, concatenate.

        Algorithm:
        - Simulate until length <= k.

        Complexity: O(n log_k value) rounds; each O(n).
        """
        while len(s) > k:
            nxt = []
            for i in range(0, len(s), k):
                nxt.append(str(sum(int(ch) for ch in s[i : i + k])))
            s = "".join(nxt)
        return s
# @lc code=end
