...
class Solution:
    def buildArray(self, nums: List[int]) -> List[int]:
        ...
        Return ans[i] = nums[nums[i]]. Trivial O(n) space; follow-up encodes
        both values in-place with modular packing.

        Algorithm:
        - ans = [nums[nums[i]] for i].
    ...
    def buildArray_inplace(self, nums: List[int]) -> List[int]:
        """
        Interview explanation:
        Classic alternate in-place: store old+new in one int via n-base encoding.

        Algorithm:
        - nums[i] += n * (nums[nums[i]] % n); then nums[i] //= n.
