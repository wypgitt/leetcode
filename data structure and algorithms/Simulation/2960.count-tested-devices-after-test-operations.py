...
# devices, and batteryPercentages becomes [1,0,1,0,1].
# At device 3, batteryPercentages[3] == 0, so we move to the next device
# without testing.
# At device 4, batteryPercentages[4] > 0, so there are now 3 tested
...
class Solution:
    def countTestedDevices(self, batteryPercentages: List[int]) -> int:
        ...
        Test device i if battery > 0 (after prior tests' -1 penalties); each
        test decrements all later batteries by 1 (floor 0).

        Algorithm:
        - Let tested = count so far; device i tests iff battery[i] > tested
          (equivalent to applying tested prior decrements).
    ...
    def countTestedDevices_sim(self, batteryPercentages: List[int]) -> int:
        """
        Interview explanation:
        Alternate direct simulation mutating the array.

        Algorithm:
        - For each i with battery>0, increment and decrement suffix.

        Complexity: O(n^2) time, O(1) extra.
        """
        arr = batteryPercentages[:]
        ans = 0
        n = len(arr)
        for i in range(n):
            if arr[i] > 0:
