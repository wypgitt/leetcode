#
# @lc app=leetcode id=3126 lang=python3
#
...
# Each row of this table contains server_id, status_time, and
# session_status.
#
...
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL: Servers(server_id, status_time, session_status). Sessions
        alternate start/stop per server. Sum all running intervals and return
        floor(total_seconds / 86400) as total_uptime_days.

        Algorithm:
        - LEAD(status_time) OVER (PARTITION BY server_id ORDER BY status_time)
          pairs each start with its stop; TIMESTAMPDIFF seconds; FLOOR sum/86400.
...
SELECT
  FLOOR(
    SUM(TIMESTAMPDIFF(SECOND, status_time, stop_time)) / 86400
