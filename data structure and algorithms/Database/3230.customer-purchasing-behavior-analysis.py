#
# @lc app=leetcode id=3230 lang=python3
#
...
# Testcase Example:  "{\"headers\":{\"Transactions\":[\"transaction_id\",\"customer_id\",\"product_id\",\"transaction_date\",\"amount\"],\"Products\":[\"product_id\",\"category\",\"price\"]},\"rows\":{\"Transactions\":[[1,101,1,\"2023-01-01\",100.00],[2,101,2,\"2023-01-15\",150.00],[3,102,1,\"2023-01-01\",100.00],[4,102,3,\"2023-01-22\",200.00],[5,101,3,\"2023-02-10\",200.00]],\"Products\":[[1,\"A\",100.00],[2,\"B\",150.00],[3,\"C\",200.00]]}}"
#
#
# Table: Transactions
#
...
#
# Top category: C (Customer 101 made 1 purchase each in categories A, B,
# and C. Since the count is the same for all categories, we choose the
# most recent transaction, which is category C on 2023-02-10)
#
...
#
# Top category: C (Customer 102 made 1 purchase each in categories A and
# C. Since the count is the same for both categories, we choose the most
# recent transaction, which is category C on 2023-01-22)
#
# Loyalty score: (2 * 10) + (300.00 / 100) = 23.00
#
# Note: The output is ordered by loyalty_score in descending order, then
...
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL: per customer, totals / counts / unique categories / average,
        top category (most purchases, tie-break latest date), and loyalty score.

        Algorithm:
        - Join Transactions with Products.
        - Rank categories per customer by COUNT DESC, MAX(date) DESC.
        - Aggregate metrics; loyalty = txn_count*10 + total_amount/100.
        - Order by loyalty_score DESC, customer_id ASC.

        Complexity: O(N log N) with sorting/window.
        """
        self.sql = """
WITH
    T AS (
...
SELECT
    ...
    ROUND(COUNT(1) * 10 + SUM(amount) / 100, 2) AS loyalty_score
FROM T t
