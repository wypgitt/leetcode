#
# @lc app=leetcode id=3601 lang=python3
#
...
#
# Calculate the efficiency improvement as (second_half_avg -
# first_half_avg)
#
...
# +-----------+---------------+------------------+-------------------+------------------------+
# | 2         | Bob Smith     | 11.24            | 13.33             |
# 2.10                   |
# | 1         | Alice Johnson | 11.97            | 14.02             |
# 2.05                   |
# +-----------+---------------+------------------+-------------------+------------------------+
#
# Explanation:
...
#
# First half trips (Jan-Jun): Feb 15 (120.5/10.2 = 11.81), Mar 20
# (200.0/16.5 = 12.12)
#
# First half average efficiency: (11.81 + 12.12) / 2 = 11.97
#
# Second half trips (Jul-Dec): Aug 10 (150.0/11.0 = 13.64), Sep 25
# (180.0/12.5 = 14.40)
#
...
#
# First half trips: Jan 10 (100.0/9.0 = 11.11), Apr 15 (250.0/22.0 =
# 11.36)
#
...
#
# Efficiency improvement: 13.33 - 11.24 = 2.10 (rounded to 2 decimal
# places)
#
...
#
# The output table is ordered by efficiency improvement in descending
...
class Solution:
    def solve(self) -> str:
        ...
        Compare each driver's average fuel efficiency (distance/fuel) in H1
        (Jan–Jun) vs H2 (Jul–Dec); keep drivers who improved.

        Algorithm:
        - Aggregate AVG(distance_km / fuel_consumed) per half-year.
        - Inner-join halves so both periods exist; require H2 > H1.
        - ROUND averages and improvement to 2 decimals; order by
          improvement DESC, name ASC.

        Complexity: O(T + D) aggregation/joins over trips and drivers.
        """
        self.sql = """
WITH first_half AS (
    SELECT
        driver_id,
        AVG(distance_km / fuel_consumed) AS avg_eff
    FROM trips
