...
The player starts at (0,0) and all parties move one Manhattan unit per turn. If any ghost can reach the target in no more moves than the player, it can wait there and catch the player. Otherwise the player can go straight to the target safely.

Data structure: none beyond scalar distance calculations.

Edge cases: equality is losing because the ghost arrives at the same time. Ghost interactions elsewhere do not matter; target interception is sufficient and necessary.

Complexity: O(g) time for g ghosts and O(1) space.
"""
