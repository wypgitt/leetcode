...
class Solution:
    def getMoneyAmount(self, n: int) -> int:
        ...
        Minimax DP: we pay the guessed number x, then recursively face the
        worse of the two remaining intervals. Choose x minimizing that worst
        cost. dp[i][j] = min money to guarantee a win for range [i, j].

        Algorithm:
        - dp[i][i] = 0 (single number known).
        - For length L from 2..n, for each [lo, hi]:
          dp[lo][hi] = min over x in [lo,hi] of
            x + max(dp[lo][x-1], dp[x+1][hi]) (missing side = 0).
        ...
        for length in range(2, n + 1):
            for lo in range(1, n - length + 2):
                ...
                for x in range(lo, hi + 1):
                    left = dp[lo][x - 1] if x > lo else 0
                    right = dp[x + 1][hi] if x < hi else 0
                    best = min(best, x + max(left, right))
                dp[lo][hi] = best
        return dp[1][n]
# @lc code=end
