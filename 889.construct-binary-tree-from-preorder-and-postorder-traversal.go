...
// =============================================================================
//
// LeetCode note: Do **not** redefine `TreeNode` in the submission — the judge injects it.
// This repo uses shared `TreeNode` in `leetcode_common_types.go` for local compilation.
//
// =============================================================================

// @lc code=start

// ConstructFromPrePost889 builds any binary tree consistent with preorder and postorder (unique values).
//
// Map each value to its postorder index for O(1) left-subtree size (L).
// Recurse on index ranges without slicing arrays.
func ConstructFromPrePost889(preorder, postorder []int) *TreeNode {
	pos := make(map[int]int, len(postorder))
