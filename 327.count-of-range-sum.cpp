...
public:
    int countRangeSum(vector<int>& nums, int lower, int upper) {
        ...
        function<long long(int, int)> merge = [&](int lo, int hi) -> long long {
            if (hi - lo <= 1) return 0;
            int mid = (lo + hi) / 2;
            long long cnt = merge(lo, mid) + merge(mid, hi);
            int i = mid, j = mid;
            for (int left = lo; left < mid; ++left) {
                while (i < hi && pref[i] - pref[left] < lower) ++i;
                while (j < hi && pref[j] - pref[left] <= upper) ++j;
                cnt += j - i;
            }
            vector<long long> merged;
            int p = lo, q = mid;
            while (p < mid && q < hi) merged.push_back(pref[p] <= pref[q] ? pref[p++] : pref[q++]);
            while (p < mid) merged.push_back(pref[p++]);
            while (q < hi) merged.push_back(pref[q++]);
            copy(merged.begin(), merged.end(), pref.begin() + lo);
            return cnt;
        };
