...
# charges a specific amount to swap places. The cost to swap with person i
# is given by cost[i].
#
...
#
# i = 2. We can swap with person 1 for a cost of 3, then swap with person
# 2 for free.
#
# i = 3. We can swap with person 3 for a cost of 1.
#
# i = 4. We can swap with person 3 for a cost of 1, then swap with person
# 4 for free.
#
# i = 5. We can swap with person 3 for a cost of 1, then swap with person
# 5 for free.
#
# Example 2:
#
...
class Solution:
    def minCosts(self, cost: List[int]) -> List[int]:
        ...
        Reaching position i costs the cheapest person in [0..i]: pay that person
        once, then swap freely with anyone behind you.

        Algorithm:
        - answer[i] = min(cost[0..i]) via a running prefix minimum.

        Complexity: O(n) time, O(1) extra space.
        Alternate: answer[i] = min(answer[i-1], cost[i]) for i > 0.
        """
        ans = []
        best = cost[0]
        for c in cost:
