# System Design: Error tracking like Sentry
...
|-----------|----------|----------|
| Job | Design **Error tracking like Sentry** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Error tracking like Sentry | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Error tracking like Sentry.
...
Domain note: tailor 2–3 cases specifically to **Error tracking like Sentry** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Error tracking like Sentry.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Error tracking like Sentry**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Error tracking like Sentry".
...
Core components for **Error tracking like Sentry**:
- **agents/collectors**
- **ingest pipeline**
- **TSDB/log index**
- **alert evaluator**
...
flowchart LR
  ...
  GW --> Svc[Stateless errortrackingsentry Service]
...
**Failure modes checklist for Error tracking like Sentry**
...
### 5.4 Problem-specific deep notes for Error tracking like Sentry
...
1. **Invariants** for Error tracking like Sentry (what must never break).
...
| 16 | Which algorithms apply to Error tracking like Sentry? | Expect discussion of: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Error tracking like Sentry**: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate.
...
*Category: Observability, reliability, security, and operations · Slug: `error-tracking-sentry` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Error tracking like Sentry
...
| Business SLI for Error tracking like Sentry | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Error tracking like Sentry
...
*Enriched for interview drill · `error-tracking-sentry`*
