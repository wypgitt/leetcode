# System Design: Electricity-grid telemetry
...
|-----------|----------|----------|
| Job | Design **Electricity-grid telemetry** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Electricity-grid telemetry | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Electricity-grid telemetry.
...
Domain note: tailor 2–3 cases specifically to **Electricity-grid telemetry** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Electricity-grid telemetry.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Electricity-grid telemetry**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Electricity-grid telemetry".
...
Core components for **Electricity-grid telemetry**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless electricitygridtelem Service]
...
**Failure modes checklist for Electricity-grid telemetry**
...
### 5.4 Problem-specific deep notes for Electricity-grid telemetry
...
1. **Invariants** for Electricity-grid telemetry (what must never break).
...
| 16 | Which algorithms apply to Electricity-grid telemetry? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Electricity-grid telemetry**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `electricity-grid-telemetry` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Electricity-grid telemetry
...
| Business SLI for Electricity-grid telemetry | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Electricity-grid telemetry
...
*Enriched for interview drill · `electricity-grid-telemetry`*
