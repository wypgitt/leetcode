# System Design: Electronic health-record system
...
|-----------|----------|----------|
| Job | Design **Electronic health-record system** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Electronic health-record system | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Electronic health-record system.
...
Domain note: tailor 2–3 cases specifically to **Electronic health-record system** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Electronic health-record system.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Electronic health-record system**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Electronic health-record system".
...
Core components for **Electronic health-record system**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless electronichealthreco Service]
...
**Failure modes checklist for Electronic health-record system**
...
### 5.4 Problem-specific deep notes for Electronic health-record system
...
1. **Invariants** for Electronic health-record system (what must never break).
...
| 16 | Which algorithms apply to Electronic health-record system? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Electronic health-record system**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `electronic-health-record` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Electronic health-record system
...
| Business SLI for Electronic health-record system | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Electronic health-record system
...
*Enriched for interview drill · `electronic-health-record`*
