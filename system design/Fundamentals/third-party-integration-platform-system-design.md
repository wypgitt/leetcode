# System Design: Third-party integration platform
...
|-----------|----------|----------|
| Job | Design **Third-party integration platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Third-party integration platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Third-party integration platform.
...
Domain note: tailor 2–3 cases specifically to **Third-party integration platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Third-party integration platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Third-party integration platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Third-party integration platform".
...
Core components for **Third-party integration platform**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless thirdpartyintegratio Service]
...
**Failure modes checklist for Third-party integration platform**
...
### 5.4 Problem-specific deep notes for Third-party integration platform
...
1. **Invariants** for Third-party integration platform (what must never break).
...
| 16 | Which algorithms apply to Third-party integration platform? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Third-party integration platform**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `third-party-integration-platform` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Third-party integration platform
...
| Business SLI for Third-party integration platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Third-party integration platform
...
*Enriched for interview drill · `third-party-integration-platform`*
