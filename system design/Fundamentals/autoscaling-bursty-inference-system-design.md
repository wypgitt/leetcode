# System Design: Autoscaling for expensive, bursty inference
...
|-----------|----------|----------|
| Job | Design **Autoscaling for expensive, bursty inference** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Autoscaling for expensive, bursty inference | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Autoscaling for expensive, bursty inference.
...
Domain note: tailor 2–3 cases specifically to **Autoscaling for expensive, bursty inference** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Autoscaling for expensive, bursty inference.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Autoscaling for expensive, bursty inference**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Autoscaling for expensive, bursty inference".
...
Core components for **Autoscaling for expensive, bursty inference**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless autoscalingburstyinf Service]
...
**Failure modes checklist for Autoscaling for expensive, bursty inference**
...
### 5.4 Problem-specific deep notes for Autoscaling for expensive, bursty inference
...
1. **Invariants** for Autoscaling for expensive, bursty inference (what must never break).
...
| 16 | Which algorithms apply to Autoscaling for expensive, bursty inference? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Autoscaling for expensive, bursty inference**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `autoscaling-bursty-inference` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Autoscaling for expensive, bursty inference
...
| Business SLI for Autoscaling for expensive, bursty inference | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Autoscaling for expensive, bursty inference
...
*Enriched for interview drill · `autoscaling-bursty-inference`*
