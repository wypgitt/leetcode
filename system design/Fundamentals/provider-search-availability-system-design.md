# System Design: Provider search and availability
...
|-----------|----------|----------|
| Job | Design **Provider search and availability** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Provider search and availability | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Provider search and availability.
...
Domain note: tailor 2–3 cases specifically to **Provider search and availability** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Provider search and availability.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Provider search and availability**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Provider search and availability".
...
Core components for **Provider search and availability**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless providersearchavaila Service]
...
**Failure modes checklist for Provider search and availability**
...
### 5.4 Problem-specific deep notes for Provider search and availability
...
1. **Invariants** for Provider search and availability (what must never break).
...
| 16 | Which algorithms apply to Provider search and availability? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Provider search and availability**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `provider-search-availability` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Provider search and availability
...
| Business SLI for Provider search and availability | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Provider search and availability
...
*Enriched for interview drill · `provider-search-availability`*
