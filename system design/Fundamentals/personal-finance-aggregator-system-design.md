# System Design: Personal-finance aggregator
...
|-----------|----------|----------|
| Job | Design **Personal-finance aggregator** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Personal-finance aggregator | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Personal-finance aggregator.
...
Domain note: tailor 2–3 cases specifically to **Personal-finance aggregator** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Personal-finance aggregator.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Personal-finance aggregator**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Personal-finance aggregator".
...
Core components for **Personal-finance aggregator**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless personalfinanceaggre Service]
...
**Failure modes checklist for Personal-finance aggregator**
...
### 5.4 Problem-specific deep notes for Personal-finance aggregator
...
1. **Invariants** for Personal-finance aggregator (what must never break).
...
| 16 | Which algorithms apply to Personal-finance aggregator? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Personal-finance aggregator**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `personal-finance-aggregator` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Personal-finance aggregator
...
| Business SLI for Personal-finance aggregator | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Personal-finance aggregator
...
*Enriched for interview drill · `personal-finance-aggregator`*
