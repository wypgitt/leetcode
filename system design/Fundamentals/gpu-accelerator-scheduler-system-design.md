# System Design: GPU/accelerator scheduler
...
|-----------|----------|----------|
| Job | Design **GPU/accelerator scheduler** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for GPU/accelerator scheduler | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for GPU/accelerator scheduler.
...
Domain note: tailor 2–3 cases specifically to **GPU/accelerator scheduler** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to GPU/accelerator scheduler.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **GPU/accelerator scheduler**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "GPU/accelerator scheduler".
...
Core components for **GPU/accelerator scheduler**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless gpuacceleratorschedu Service]
...
**Failure modes checklist for GPU/accelerator scheduler**
...
### 5.4 Problem-specific deep notes for GPU/accelerator scheduler
...
1. **Invariants** for GPU/accelerator scheduler (what must never break).
...
| 16 | Which algorithms apply to GPU/accelerator scheduler? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **GPU/accelerator scheduler**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `gpu-accelerator-scheduler` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for GPU/accelerator scheduler
...
| Business SLI for GPU/accelerator scheduler | Product truth |
...
- Cost model at 100×
- Abuse cases unique to GPU/accelerator scheduler
...
*Enriched for interview drill · `gpu-accelerator-scheduler`*
