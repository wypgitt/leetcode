...
|-----------|----------|----------|
| Job | Design **Disaster recovery** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Disaster recovery | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Disaster recovery.
...
Domain note: tailor 2–3 cases specifically to **Disaster recovery** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Disaster recovery.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Disaster recovery**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Disaster recovery".
...
Core components for **Disaster recovery**:
- **agents/collectors**
- **ingest pipeline**
- **TSDB/log index**
- **alert evaluator**
...
flowchart LR
  ...
  GW --> Svc[Stateless disasterrecovery Service]
...
**Failure modes checklist for Disaster recovery**
...
### 5.4 Problem-specific deep notes for Disaster recovery
...
1. **Invariants** for Disaster recovery (what must never break).
...
| 16 | Which algorithms apply to Disaster recovery? | Expect discussion of: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Disaster recovery**: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate.
...
*Category: Observability, reliability, security, and operations · Slug: `disaster-recovery` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Disaster recovery
...
| Business SLI for Disaster recovery | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Disaster recovery
...
*Enriched for interview drill · `disaster-recovery`*
