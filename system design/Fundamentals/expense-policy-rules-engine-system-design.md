# System Design: Expense-policy rules engine
...
|-----------|----------|----------|
| Job | Design **Expense-policy rules engine** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Expense-policy rules engine | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Expense-policy rules engine.
...
Domain note: tailor 2–3 cases specifically to **Expense-policy rules engine** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Expense-policy rules engine.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Expense-policy rules engine**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Expense-policy rules engine".
...
Core components for **Expense-policy rules engine**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless expensepolicyrulesen Service]
...
**Failure modes checklist for Expense-policy rules engine**
...
### 5.4 Problem-specific deep notes for Expense-policy rules engine
...
1. **Invariants** for Expense-policy rules engine (what must never break).
...
| 16 | Which algorithms apply to Expense-policy rules engine? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Expense-policy rules engine**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `expense-policy-rules-engine` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Expense-policy rules engine
...
| Business SLI for Expense-policy rules engine | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Expense-policy rules engine
...
*Enriched for interview drill · `expense-policy-rules-engine`*
