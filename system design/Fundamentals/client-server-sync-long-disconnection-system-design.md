# System Design: Client/server synchronization after long disconnection

> **Focus areas:** device identity · telemetry · OTA · offline sync · edge
> **Style:** End-to-end design with progressive scale (10× → 100× → 1,000×)
> **Category:** Devices, mobile, edge, and IoT
...
|-----------|----------|----------|
| Job | Design **Client/server synchronization after long disconnection** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Client/server synchronization after long disconnection | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Client/server synchronization after long disconnection.
...
Domain note: tailor 2–3 cases specifically to **Client/server synchronization after long disconnection** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Client/server synchronization after long disconnection.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Client/server synchronization after long disconnection**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Client/server synchronization after long disconnection".
...
Section family `11-devices-iot` emphasizes: device identity · telemetry · OTA · offline sync · edge.
...
Core components for **Client/server synchronization after long disconnection**:
- **device auth**
- **telemetry ingest**
- **command plane**
- **OTA rollout**
...
**Algorithms / structures likely discussed:** exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching

### 3.5 Deal-breakers for this problem family
- chatty always-on GPS
- OTA without staged rollout/rollback
...
flowchart LR
  ...
  GW --> Svc[Stateless clientserversynclong Service]
...
**Failure modes checklist for Client/server synchronization after long disconnection**
...
- Key algorithms: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching
...
### 5.4 Problem-specific deep notes for Client/server synchronization after long disconnection
...
1. **Invariants** for Client/server synchronization after long disconnection (what must never break).
...
| 16 | Which algorithms apply to Client/server synchronization after long disconnection? | Expect discussion of: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Client/server synchronization after long disconnection**: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching.
...
*Category: Devices, mobile, edge, and IoT · Slug: `client-server-sync-long-disconnection` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Client/server synchronization after long disconnection
...
| Business SLI for Client/server synchronization after long disconnection | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Client/server synchronization after long disconnection
...
*Enriched for interview drill · `client-server-sync-long-disconnection`*
