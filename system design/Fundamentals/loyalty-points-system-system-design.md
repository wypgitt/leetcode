# System Design: Loyalty-points system
...
|-----------|----------|----------|
| Job | Design **Loyalty-points system** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Loyalty-points system | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Loyalty-points system.
...
Domain note: tailor 2–3 cases specifically to **Loyalty-points system** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Loyalty-points system.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Loyalty-points system**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Loyalty-points system".
...
Core components for **Loyalty-points system**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless loyaltypointssystem Service]
...
**Failure modes checklist for Loyalty-points system**
...
### 5.4 Problem-specific deep notes for Loyalty-points system
...
1. **Invariants** for Loyalty-points system (what must never break).
...
| 16 | Which algorithms apply to Loyalty-points system? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Loyalty-points system**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `loyalty-points-system` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Loyalty-points system
...
| Business SLI for Loyalty-points system | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Loyalty-points system
...
*Enriched for interview drill · `loyalty-points-system`*
