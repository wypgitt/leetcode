...
|-----------|----------|----------|
| Job | Design **Cryptocurrency exchange** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Cryptocurrency exchange | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Cryptocurrency exchange.
...
Domain note: tailor 2–3 cases specifically to **Cryptocurrency exchange** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Cryptocurrency exchange.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Cryptocurrency exchange**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Cryptocurrency exchange".
...
Core components for **Cryptocurrency exchange**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless cryptocurrencyexchan Service]
...
**Failure modes checklist for Cryptocurrency exchange**
...
### 5.4 Problem-specific deep notes for Cryptocurrency exchange
...
1. **Invariants** for Cryptocurrency exchange (what must never break).
...
| 16 | Which algorithms apply to Cryptocurrency exchange? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Cryptocurrency exchange**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `cryptocurrency-exchange` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Cryptocurrency exchange
...
| Business SLI for Cryptocurrency exchange | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Cryptocurrency exchange
...
*Enriched for interview drill · `cryptocurrency-exchange`*
