# System Design: Online examination platform
...
|-----------|----------|----------|
| Job | Design **Online examination platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Online examination platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Online examination platform.
...
Domain note: tailor 2–3 cases specifically to **Online examination platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Online examination platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Online examination platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Online examination platform".
...
Core components for **Online examination platform**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless onlineexaminationpla Service]
...
**Failure modes checklist for Online examination platform**
...
### 5.4 Problem-specific deep notes for Online examination platform
...
1. **Invariants** for Online examination platform (what must never break).
...
| 16 | Which algorithms apply to Online examination platform? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Online examination platform**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `online-examination-platform` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Online examination platform
...
| Business SLI for Online examination platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Online examination platform
...
*Enriched for interview drill · `online-examination-platform`*
