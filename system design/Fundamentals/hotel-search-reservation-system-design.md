# System Design: Hotel Search and Reservation

> **Focus areas:** Hotel search · Room inventory · Rates · GDS/OTAs · Holds · Overbooking policies  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Hotel Search and Reservation**  
...
Goal: search hotels and reserve rooms at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to search hotels and reserve rooms | Unrelated product surface |
| Core noun | `hotel reservation` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | rate/inventory, provider aggregation, holds, overbook policy | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **rate/inventory**.
2. Deliver a production-quality path for **provider aggregation**.
3. Deliver a production-quality path for **holds**.
4. Deliver a production-quality path for **overbook policy**.
5. Durable `hotel reservation` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | search < 400ms (cache); book < 1s + provider |
...
1. User/system initiates flow to search hotels and reserve rooms → validation → core `hotel reservation` created.
...
|--------|----------|-----|------|--------|
| Hotel search QPS | 4K | 40K | 400K | 4M |
...
> Design **Hotel Search and Reservation** to search hotels and reserve rooms, emphasizing rate/inventory, provider aggregation, holds, overbook policy. Start from baseline ~4K hotel search qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 4K search
Entities (order-of): hotels+room types
Write class focus: ARIQ updates
Typical payload: rates
...
Use rates as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **provider cache**, admission control, and key sharding.
...
```text
search cache/providers → rate shop → hold allotment → book → confirm
```
...
**Hotel Search and Reservation** interview signal: speak fluently about **rate/inventory** and **provider aggregation**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Hotel Search and Reservation**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **rate/inventory** as a first-class subsystem.
2. Invest in **provider aggregation** as a first-class subsystem.
3. Invest in **holds** as a first-class subsystem.
4. Invest in **overbook policy** as a first-class subsystem.
...
"For Hotel Search and Reservation, I'd guarantee **rate/inventory** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Hotel Search and Reservation?
...
For Hotel Search and Reservation: emphasize rate/inventory, provider aggregation, holds; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Hotel Search and Reservation
baseline: {'Primary QPS': '4K search', 'Entities': 'hotels+room types', 'Write QPS': 'ARIQ updates', 'Payload': 'rates', 'Hot structure': 'provider cache'}
