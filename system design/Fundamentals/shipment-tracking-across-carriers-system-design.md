# System Design: Shipment tracking across carriers
...
|-----------|----------|----------|
| Job | Design **Shipment tracking across carriers** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Shipment tracking across carriers | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Shipment tracking across carriers.
...
Domain note: tailor 2–3 cases specifically to **Shipment tracking across carriers** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Shipment tracking across carriers.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Shipment tracking across carriers**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Shipment tracking across carriers".
...
Core components for **Shipment tracking across carriers**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless shipmenttrackingacro Service]
...
**Failure modes checklist for Shipment tracking across carriers**
...
### 5.4 Problem-specific deep notes for Shipment tracking across carriers
...
1. **Invariants** for Shipment tracking across carriers (what must never break).
...
| 16 | Which algorithms apply to Shipment tracking across carriers? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Shipment tracking across carriers**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `shipment-tracking-across-carriers` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Shipment tracking across carriers
...
| Business SLI for Shipment tracking across carriers | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Shipment tracking across carriers
...
*Enriched for interview drill · `shipment-tracking-across-carriers`*
