# System Design: Route Planning

> **Focus areas:** Graph routing · Contraction hierarchies · ETA · Multi-stop · Traffic-aware · Alternatives  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Route Planning**  
...
Goal: compute paths and ETAs at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to compute paths and ETAs | Unrelated product surface |
| Core noun | `route request` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | road graph, CH/HL algorithms, traffic weights, alternative routes | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **road graph**.
2. Deliver a production-quality path for **CH/HL algorithms**.
3. Deliver a production-quality path for **traffic weights**.
4. Deliver a production-quality path for **alternative routes**.
5. Durable `route request` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | p99 route < 200ms regional |
...
1. User/system initiates flow to compute paths and ETAs → validation → core `route request` created.
...
| Domain-specific contention | Serialize on route request keys; lease/TTL patterns |
...
> Design **Route Planning** to compute paths and ETAs, emphasizing road graph, CH/HL algorithms, traffic weights, alternative routes. Start from baseline ~3K directions qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 3K directions
Entities (order-of): 50M edges/region
Write class focus: traffic 10K
Typical payload: 8–20 KB route
...
Use 8–20 KB route as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **CH prep graphs**, admission control, and key sharding.
...
```text
bidirectional Dijkstra/A* on CH or hub labels; traffic overlays
```
...
**Route Planning** interview signal: speak fluently about **road graph** and **CH/HL algorithms**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Route Planning**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **road graph** as a first-class subsystem.
2. Invest in **CH/HL algorithms** as a first-class subsystem.
3. Invest in **traffic weights** as a first-class subsystem.
4. Invest in **alternative routes** as a first-class subsystem.
...
"For Route Planning, I'd guarantee **road graph** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Route Planning?
...
For Route Planning: emphasize road graph, CH/HL algorithms, traffic weights; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
baseline: {'Primary QPS': '3K directions', 'Entities': '50M edges/region', 'Write QPS': 'traffic 10K', 'Payload': '8–20 KB route', 'Hot structure': 'CH prep graphs'}
