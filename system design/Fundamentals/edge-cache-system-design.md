...
|-----------|----------|----------|
| Job | Design **Edge cache** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Edge cache | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Edge cache.
...
Domain note: tailor 2–3 cases specifically to **Edge cache** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Edge cache.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Edge cache**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Edge cache".
...
Core components for **Edge cache**:
- **device auth**
- **telemetry ingest**
- **command plane**
- **OTA rollout**
...
flowchart LR
  ...
  GW --> Svc[Stateless edgecache Service]
...
**Failure modes checklist for Edge cache**
...
### 5.4 Problem-specific deep notes for Edge cache
...
1. **Invariants** for Edge cache (what must never break).
...
| 16 | Which algorithms apply to Edge cache? | Expect discussion of: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Edge cache**: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching.
...
*Category: Devices, mobile, edge, and IoT · Slug: `edge-cache` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Edge cache
...
| Business SLI for Edge cache | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Edge cache
...
*Enriched for interview drill · `edge-cache`*
