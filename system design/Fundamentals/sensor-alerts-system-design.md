...
|-----------|----------|----------|
| Job | Design **Sensor alerts** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Sensor alerts | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Sensor alerts.
...
Domain note: tailor 2–3 cases specifically to **Sensor alerts** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Sensor alerts.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Sensor alerts**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Sensor alerts".
...
Core components for **Sensor alerts**:
- **device auth**
- **telemetry ingest**
- **command plane**
- **OTA rollout**
...
flowchart LR
  ...
  GW --> Svc[Stateless sensoralerts Service]
...
**Failure modes checklist for Sensor alerts**
...
### 5.4 Problem-specific deep notes for Sensor alerts
...
1. **Invariants** for Sensor alerts (what must never break).
...
| 16 | Which algorithms apply to Sensor alerts? | Expect discussion of: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Sensor alerts**: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching.
...
*Category: Devices, mobile, edge, and IoT · Slug: `sensor-alerts` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Sensor alerts
...
| Business SLI for Sensor alerts | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Sensor alerts
...
*Enriched for interview drill · `sensor-alerts`*
