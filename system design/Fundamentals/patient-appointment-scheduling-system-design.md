# System Design: Patient appointment scheduling
...
|-----------|----------|----------|
| Job | Design **Patient appointment scheduling** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Patient appointment scheduling | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Patient appointment scheduling.
...
Domain note: tailor 2–3 cases specifically to **Patient appointment scheduling** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Patient appointment scheduling.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Patient appointment scheduling**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Patient appointment scheduling".
...
Core components for **Patient appointment scheduling**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless patientappointmentsc Service]
...
**Failure modes checklist for Patient appointment scheduling**
...
### 5.4 Problem-specific deep notes for Patient appointment scheduling
...
1. **Invariants** for Patient appointment scheduling (what must never break).
...
| 16 | Which algorithms apply to Patient appointment scheduling? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Patient appointment scheduling**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `patient-appointment-scheduling` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Patient appointment scheduling
...
| Business SLI for Patient appointment scheduling | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Patient appointment scheduling
...
*Enriched for interview drill · `patient-appointment-scheduling`*
