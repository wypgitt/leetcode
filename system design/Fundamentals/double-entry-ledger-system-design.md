<!-- Adapted into Fundamentals bank from Stripe/merchant-ledger-system-design.md for cross-company prep. -->

# System Design: Merchant Ledger Service

> **Focus areas:** Merchant balances · Payout gates · Reserves · Statements  
...
|---|---|---|---|
| F1 | Visibility? | Available pending | Derived from internal |
| F2 | Writes? | Internal only | No merchant journal post |
| F3 | Payout gate? | Available-reserves | RYW balance |
| F4 | Statements? | Monthly export | Immutable PDF+data |
| F5 | Multi-currency? | Per currency view | Separate balances |
| F6 | Disputes? | Reserve holds | Reflect pending |
| F7 | API? | GET balance transactions | Read mostly |
| F8 | Idempotency? | On internal mirror | Sync jobs |
| F9 | Recon? | Mirror vs core ledger | Alert drift |
| F10 | Caching? | Short TTL+version | Never stale payout |
| F11 | Timezone? | Statement boundaries | Merchant local |
| F12 | Audit? | Merchant-visible history | Append only |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Merchant Ledger Service
4. Admin recon/replay hooks for Merchant Ledger Service
...
1. Visibility? → Available pending → durable write + audit
2. Writes? → Internal only → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Multi-currency? → safe degradation documented
...
> Design merchant-visible ledger ~200 post/s backed by internal double-entry.
...
### 3.11 Domain model (Merchant Ledger Service)
...
### 4.1 End-to-end (Merchant Ledger Service)
...
```text
API Edge → Merchant Ledger Service → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of merchant ledger service system design.*

### 8.19 Merchant Ledger Service — rollout checklist
