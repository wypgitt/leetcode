...
|-----------|----------|----------|
| Job | Design **Product catalog** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Product catalog | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Product catalog.
...
Domain note: tailor 2–3 cases specifically to **Product catalog** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Product catalog.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Product catalog**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Product catalog".
...
Core components for **Product catalog**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless productcatalog Service]
...
**Failure modes checklist for Product catalog**
...
### 5.4 Problem-specific deep notes for Product catalog
...
1. **Invariants** for Product catalog (what must never break).
...
| 16 | Which algorithms apply to Product catalog? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Product catalog**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `product-catalog` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Product catalog
...
| Business SLI for Product catalog | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Product catalog
...
*Enriched for interview drill · `product-catalog`*
