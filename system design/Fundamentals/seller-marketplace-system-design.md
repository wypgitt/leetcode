# System Design: Seller marketplace
...
|-----------|----------|----------|
| Job | Design **Seller marketplace** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Seller marketplace | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Seller marketplace.
...
Domain note: tailor 2–3 cases specifically to **Seller marketplace** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Seller marketplace.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Seller marketplace**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Seller marketplace".
...
Core components for **Seller marketplace**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless sellermarketplace Service]
...
**Failure modes checklist for Seller marketplace**
...
### 5.4 Problem-specific deep notes for Seller marketplace
...
1. **Invariants** for Seller marketplace (what must never break).
...
| 16 | Which algorithms apply to Seller marketplace? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Seller marketplace**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `seller-marketplace` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Seller marketplace
...
| Business SLI for Seller marketplace | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Seller marketplace
...
*Enriched for interview drill · `seller-marketplace`*
