# System Design: Batched Delivery Assignment

> **Focus areas:** Batching window · Multi-pickup · Constraints · Optimization · Fairness · Realtime  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Batched Delivery Assignment**  
...
Goal: assign multiple orders to couriers at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to assign multiple orders to couriers | Unrelated product surface |
| Core noun | `delivery batch` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | batch window, constraint solver, reoptimization, SLA risk | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **batch window**.
2. Deliver a production-quality path for **constraint solver**.
3. Deliver a production-quality path for **reoptimization**.
4. Deliver a production-quality path for **SLA risk**.
5. Durable `delivery batch` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | plan within batch window budget |
...
1. User/system initiates flow to assign multiple orders to couriers → validation → core `delivery batch` created.
...
| Domain-specific contention | Serialize on delivery batch keys; lease/TTL patterns |
...
|--------|----------|-----|------|--------|
| Orders optimized/s | 500 | 5K | 50K | 500K |
...
> Design **Batched Delivery Assignment** to assign multiple orders to couriers, emphasizing batch window, constraint solver, reoptimization, SLA risk. Start from baseline ~500 orders optimized/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 500 assigns
Entities (order-of): orders in window
Write class focus: assignments
Typical payload: batch plans
...
Use batch plans as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **optimizer workers**, admission control, and key sharding.
...
```text
window collect → score bundles → assign exclusive → optional reoptimize
```
...
**Batched Delivery Assignment** interview signal: speak fluently about **batch window** and **constraint solver**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Batched Delivery Assignment**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **batch window** as a first-class subsystem.
2. Invest in **constraint solver** as a first-class subsystem.
3. Invest in **reoptimization** as a first-class subsystem.
4. Invest in **SLA risk** as a first-class subsystem.
...
"For Batched Delivery Assignment, I'd guarantee **batch window** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Batched Delivery Assignment?
...
For Batched Delivery Assignment: emphasize batch window, constraint solver, reoptimization; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Batched Delivery Assignment
baseline: {'Primary QPS': '500 assigns', 'Entities': 'orders in window', 'Write QPS': 'assignments', 'Payload': 'batch plans', 'Hot structure': 'optimizer workers'}
