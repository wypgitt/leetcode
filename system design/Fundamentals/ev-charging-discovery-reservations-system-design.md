# System Design: EV Charging-Station Discovery and Reservations

> **Focus areas:** Charger geo · Availability · Reservations · Sessions · Roaming · Payments · OCPI  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **EV Charging-Station Discovery and Reservations**  
...
Goal: discover chargers and reserve sessions at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to discover chargers and reserve sessions | Unrelated product surface |
| Core noun | `charge reservation` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | charger availability, reservation holds, roaming, session billing | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **charger availability**.
2. Deliver a production-quality path for **reservation holds**.
3. Deliver a production-quality path for **roaming**.
4. Deliver a production-quality path for **session billing**.
5. Durable `charge reservation` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | nearby < 150ms; reserve < 300ms |
...
| N5 | Freshness? | Live location/ETA/status SLOs | connector status < 30–60s |
...
1. User/system initiates flow to discover chargers and reserve sessions → validation → core `charge reservation` created.
...
> Design **EV Charging-Station Discovery and Reservations** to discover chargers and reserve sessions, emphasizing charger availability, reservation holds, roaming, session billing. Start from baseline ~2K discovery qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 2K discovery
Entities (order-of): chargers
Write class focus: status 5K
Typical payload: station docs
...
Use station docs as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **availability index**, admission control, and key sharding.
...
```text
geo discover → show availability → reserve connector → session → bill
```
...
**EV Charging-Station Discovery and Reservations** interview signal: speak fluently about **charger availability** and **reservation holds**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **EV Charging-Station Discovery and Reservations**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **charger availability** as a first-class subsystem.
2. Invest in **reservation holds** as a first-class subsystem.
3. Invest in **roaming** as a first-class subsystem.
4. Invest in **session billing** as a first-class subsystem.
...
"For EV Charging-Station Discovery and Reservations, I'd guarantee **charger availability** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in EV Charging-Station Discovery and Reservations?
...
For EV Charging-Station Discovery and Reservations: emphasize charger availability, reservation holds, roaming; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: EV Charging-Station Discovery and Reservations
baseline: {'Primary QPS': '2K discovery', 'Entities': 'chargers', 'Write QPS': 'status 5K', 'Payload': 'station docs', 'Hot structure': 'availability index'}
...
| Baseline | 2K Discovery QPS |
| Invariant | no double-allocate / no silent money loss |
| Scale endgame | cell mesh + edge + tiered stores |

---

*End of EV Charging-Station Discovery and Reservations system design prep doc.*
