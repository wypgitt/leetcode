...
|-----------|----------|----------|
| Job | Design **Code search** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Code search | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Code search.
...
Domain note: tailor 2–3 cases specifically to **Code search** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Code search.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Code search**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Code search".
...
Core components for **Code search**:
- **API/control plane**
- **job scheduler**
- **artifact store**
- **runners/workers**
...
flowchart LR
  ...
  GW --> Svc[Stateless codesearch Service]
...
**Failure modes checklist for Code search**
...
### 5.4 Problem-specific deep notes for Code search
...
1. **Invariants** for Code search (what must never break).
...
| 16 | Which algorithms apply to Code search? | Expect discussion of: content-addressed storage, DAG scheduling, fair share / DRF, Merkle trees, copy-on-write. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Code search**: content-addressed storage, DAG scheduling, fair share / DRF, Merkle trees, copy-on-write.
...
*Category: Developer tools and cloud platforms · Slug: `code-search` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Code search
...
| Business SLI for Code search | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Code search
...
*Enriched for interview drill · `code-search`*
