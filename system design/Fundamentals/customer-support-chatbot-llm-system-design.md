# System Design: Customer-support chatbot using a third-party LLM
...
|-----------|----------|----------|
| Job | Design **Customer-support chatbot using a third-party LLM** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Customer-support chatbot using a third-party LLM | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Customer-support chatbot using a third-party LLM.
...
Domain note: tailor 2–3 cases specifically to **Customer-support chatbot using a third-party LLM** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Customer-support chatbot using a third-party LLM.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Customer-support chatbot using a third-party LLM**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Customer-support chatbot using a third-party LLM".
...
Core components for **Customer-support chatbot using a third-party LLM**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless customersupportchatb Service]
...
**Failure modes checklist for Customer-support chatbot using a third-party LLM**
...
### 5.4 Problem-specific deep notes for Customer-support chatbot using a third-party LLM
...
1. **Invariants** for Customer-support chatbot using a third-party LLM (what must never break).
...
| 16 | Which algorithms apply to Customer-support chatbot using a third-party LLM? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Customer-support chatbot using a third-party LLM**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `customer-support-chatbot-llm` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Customer-support chatbot using a third-party LLM
...
| Business SLI for Customer-support chatbot using a third-party LLM | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Customer-support chatbot using a third-party LLM
...
*Enriched for interview drill · `customer-support-chatbot-llm`*
