# System Design: KYC / customer onboarding
...
|-----------|----------|----------|
| Job | Design **KYC / customer onboarding** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for KYC / customer onboarding | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for KYC / customer onboarding.
...
Domain note: tailor 2–3 cases specifically to **KYC / customer onboarding** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to KYC / customer onboarding.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **KYC / customer onboarding**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "KYC / customer onboarding".
...
Core components for **KYC / customer onboarding**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless kyccustomeronboardin Service]
...
**Failure modes checklist for KYC / customer onboarding**
...
### 5.4 Problem-specific deep notes for KYC / customer onboarding
...
1. **Invariants** for KYC / customer onboarding (what must never break).
...
| 16 | Which algorithms apply to KYC / customer onboarding? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **KYC / customer onboarding**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `kyc-customer-onboarding` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for KYC / customer onboarding
...
| Business SLI for KYC / customer onboarding | Product truth |
...
- Cost model at 100×
- Abuse cases unique to KYC / customer onboarding
...
*Enriched for interview drill · `kyc-customer-onboarding`*
