# System Design: Merchant payouts
...
|-----------|----------|----------|
| Job | Design **Merchant payouts** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Merchant payouts | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Merchant payouts.
...
Domain note: tailor 2–3 cases specifically to **Merchant payouts** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Merchant payouts.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Merchant payouts**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Merchant payouts".
...
Core components for **Merchant payouts**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless merchantpayouts Service]
...
**Failure modes checklist for Merchant payouts**
...
### 5.4 Problem-specific deep notes for Merchant payouts
...
1. **Invariants** for Merchant payouts (what must never break).
...
| 16 | Which algorithms apply to Merchant payouts? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Merchant payouts**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `merchant-payouts` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Merchant payouts
...
| Business SLI for Merchant payouts | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Merchant payouts
...
*Enriched for interview drill · `merchant-payouts`*
