...
> **Focus areas:** Onsales · Queues · Seat inventory · Holds · Checkout · Bots · Fairness · Hot events  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Ticketmaster**  
...
Goal: sell event tickets under extreme contention at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to sell event tickets under extreme contention | Unrelated product surface |
| Core noun | `ticket hold` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | waiting room, seat inventory, hold TTL, bot mitigation | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **waiting room**.
2. Deliver a production-quality path for **seat inventory**.
3. Deliver a production-quality path for **hold TTL**.
4. Deliver a production-quality path for **bot mitigation**.
5. Durable `ticket hold` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | hold < 200ms after admit; queue UX separate |
...
| N5 | Freshness? | Live location/ETA/status SLOs | seatmap deltas < 1s during onsale |
...
1. User/system initiates flow to sell event tickets under extreme contention → validation → core `ticket hold` created.
...
|--------|----------|-----|------|--------|
| Hold attempts/s/event | 5K | 50K | 500K | 5M |
...
> Design **Ticketmaster** to sell event tickets under extreme contention, emphasizing waiting room, seat inventory, hold TTL, bot mitigation. Start from baseline ~5K hold attempts/s/event and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: onsale 50K+
Entities (order-of): seats/events
Write class focus: holds
Typical payload: seat holds
...
Use seat holds as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **event inventory shard**, admission control, and key sharding.
...
```text
waiting room → admit → seat hold CAS → pay → finalize tickets
```
...
**Ticketmaster** interview signal: speak fluently about **waiting room** and **seat inventory**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Ticketmaster**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **waiting room** as a first-class subsystem.
2. Invest in **seat inventory** as a first-class subsystem.
3. Invest in **hold TTL** as a first-class subsystem.
4. Invest in **bot mitigation** as a first-class subsystem.
...
"For Ticketmaster, I'd guarantee **waiting room** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Ticketmaster?
...
### Q2. Why not SQL SELECT FOR UPDATE per seat under onsale?
...
For Ticketmaster: emphasize waiting room, seat inventory, hold TTL; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
baseline: {'Primary QPS': 'onsale 50K+', 'Entities': 'seats/events', 'Write QPS': 'holds', 'Payload': 'seat holds', 'Hot structure': 'event inventory shard'}
