# System Design: Append-only event store
...
|-----------|----------|----------|
| Job | Design **Append-only event store** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Append-only event store | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Append-only event store.
...
Domain note: tailor 2–3 cases specifically to **Append-only event store** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Append-only event store.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Append-only event store**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Append-only event store".
...
Core components for **Append-only event store**:
- **public API**
...
flowchart LR
  ...
  GW --> Svc[Stateless appendonlyeventstore Service]
...
**Failure modes checklist for Append-only event store**
...
### 5.4 Problem-specific deep notes for Append-only event store
...
1. **Invariants** for Append-only event store (what must never break).
...
| 16 | Which algorithms apply to Append-only event store? | Expect discussion of: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Append-only event store**: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping.
...
*Category: Lower-level and machine-coding system design · Slug: `append-only-event-store` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Append-only event store
...
| Business SLI for Append-only event store | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Append-only event store
...
*Enriched for interview drill · `append-only-event-store`*
