...
> **Focus areas:** Dispatch · Offer/accept · Exclusivity · Batching · Fairness · ETA · Cancel/reassign  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Rider-Driver Matching**  
...
Goal: match supply to demand without double-assign at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to match supply to demand without double-assign | Unrelated product surface |
| Core noun | `match offer` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | exclusive locks/leases, offer protocol, batch vs greedy, fairness | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **exclusive locks/leases**.
2. Deliver a production-quality path for **offer protocol**.
3. Deliver a production-quality path for **batch vs greedy**.
4. Deliver a production-quality path for **fairness**.
5. Durable `match offer` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | decision < 100–200ms once candidates ready |
...
1. User/system initiates flow to match supply to demand without double-assign → validation → core `match offer` created.
...
|--------|----------|-----|------|--------|
| Match decisions/s | 1K | 10K | 100K | 1M |
...
> Design **Rider-Driver Matching** to match supply to demand without double-assign, emphasizing exclusive locks/leases, offer protocol, batch vs greedy, fairness. Start from baseline ~1K match decisions/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 1K match/s city
Entities (order-of): demand+supply pools
Write class focus: offers/acks
Typical payload: offer events
...
Use offer events as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **dispatch leases**, admission control, and key sharding.
...
```text
candidate geo → score → exclusive lease → offer → accept/timeout → commit
```
...
**Rider-Driver Matching** interview signal: speak fluently about **exclusive locks/leases** and **offer protocol**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Rider-Driver Matching**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **exclusive locks/leases** as a first-class subsystem.
2. Invest in **offer protocol** as a first-class subsystem.
3. Invest in **batch vs greedy** as a first-class subsystem.
4. Invest in **fairness** as a first-class subsystem.
...
"For Rider-Driver Matching, I'd guarantee **exclusive locks/leases** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Rider-Driver Matching?
...
For Rider-Driver Matching: emphasize exclusive locks/leases, offer protocol, batch vs greedy; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Rider-Driver Matching
baseline: {'Primary QPS': '1K match/s city', 'Entities': 'demand+supply pools', 'Write QPS': 'offers/acks', 'Payload': 'offer events', 'Hot structure': 'dispatch leases'}
