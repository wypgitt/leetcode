...
|-----------|----------|----------|
| Job | Design **Matchmaking** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Matchmaking | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Matchmaking.
...
Domain note: tailor 2–3 cases specifically to **Matchmaking** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Matchmaking.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Matchmaking**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Matchmaking".
...
Core components for **Matchmaking**:
- **write API**
...
flowchart LR
  ...
  GW --> Svc[Stateless matchmaking Service]
...
**Failure modes checklist for Matchmaking**
...
### 5.4 Problem-specific deep notes for Matchmaking
...
1. **Invariants** for Matchmaking (what must never break).
...
| 16 | Which algorithms apply to Matchmaking? | Expect discussion of: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Matchmaking**: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat.
...
*Category: Social, communication, and collaboration · Slug: `matchmaking` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Matchmaking
...
| Business SLI for Matchmaking | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Matchmaking
...
*Enriched for interview drill · `matchmaking`*
