# System Design: Carbon-accounting ledger
...
|-----------|----------|----------|
| Job | Design **Carbon-accounting ledger** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Carbon-accounting ledger | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Carbon-accounting ledger.
...
Domain note: tailor 2–3 cases specifically to **Carbon-accounting ledger** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Carbon-accounting ledger.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Carbon-accounting ledger**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Carbon-accounting ledger".
...
Core components for **Carbon-accounting ledger**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless carbonaccountingledg Service]
...
**Failure modes checklist for Carbon-accounting ledger**
...
### 5.4 Problem-specific deep notes for Carbon-accounting ledger
...
1. **Invariants** for Carbon-accounting ledger (what must never break).
...
| 16 | Which algorithms apply to Carbon-accounting ledger? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Carbon-accounting ledger**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `carbon-accounting-ledger` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Carbon-accounting ledger
...
| Business SLI for Carbon-accounting ledger | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Carbon-accounting ledger
...
*Enriched for interview drill · `carbon-accounting-ledger`*
