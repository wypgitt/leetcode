# System Design: AI coding assistant
...
|-----------|----------|----------|
| Job | Design **AI coding assistant** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for AI coding assistant | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for AI coding assistant.
...
Domain note: tailor 2–3 cases specifically to **AI coding assistant** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to AI coding assistant.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **AI coding assistant**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "AI coding assistant".
...
Core components for **AI coding assistant**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless aicodingassistant Service]
...
**Failure modes checklist for AI coding assistant**
...
### 5.4 Problem-specific deep notes for AI coding assistant
...
1. **Invariants** for AI coding assistant (what must never break).
...
| 16 | Which algorithms apply to AI coding assistant? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **AI coding assistant**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `ai-coding-assistant` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for AI coding assistant
...
| Business SLI for AI coding assistant | Product truth |
...
- Cost model at 100×
- Abuse cases unique to AI coding assistant
...
*Enriched for interview drill · `ai-coding-assistant`*
