# System Design: Employee identity lifecycle management
...
|-----------|----------|----------|
| Job | Design **Employee identity lifecycle management** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Employee identity lifecycle management | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Employee identity lifecycle management.
...
Domain note: tailor 2–3 cases specifically to **Employee identity lifecycle management** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Employee identity lifecycle management.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Employee identity lifecycle management**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Employee identity lifecycle management".
...
Core components for **Employee identity lifecycle management**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless employeeidentitylife Service]
...
**Failure modes checklist for Employee identity lifecycle management**
...
### 5.4 Problem-specific deep notes for Employee identity lifecycle management
...
1. **Invariants** for Employee identity lifecycle management (what must never break).
...
| 16 | Which algorithms apply to Employee identity lifecycle management? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Employee identity lifecycle management**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `employee-identity-lifecycle` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Employee identity lifecycle management
...
| Business SLI for Employee identity lifecycle management | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Employee identity lifecycle management
...
*Enriched for interview drill · `employee-identity-lifecycle`*
