# System Design: Real-time bidding
...
|-----------|----------|----------|
| Job | Design **Real-time bidding** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Real-time bidding | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Real-time bidding.
...
Domain note: tailor 2–3 cases specifically to **Real-time bidding** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Real-time bidding.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Real-time bidding**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Real-time bidding".
...
Core components for **Real-time bidding**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless realtimebidding Service]
...
**Failure modes checklist for Real-time bidding**
...
### 5.4 Problem-specific deep notes for Real-time bidding
...
1. **Invariants** for Real-time bidding (what must never break).
...
| 16 | Which algorithms apply to Real-time bidding? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Real-time bidding**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `real-time-bidding` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Real-time bidding
...
| Business SLI for Real-time bidding | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Real-time bidding
...
*Enriched for interview drill · `real-time-bidding`*
