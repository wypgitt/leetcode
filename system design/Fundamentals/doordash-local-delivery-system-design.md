# System Design: DoorDash or Local-Delivery Service

> **Focus areas:** Three-sided marketplace · Orders · Dasher dispatch · Restaurant prep · ETA · Batching  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **DoorDash or Local-Delivery Service**  
...
Goal: orchestrate restaurant→dasher→customer delivery at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to orchestrate restaurant→dasher→customer delivery | Unrelated product surface |
| Core noun | `delivery order` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | three-sided sync, prep-time, dasher assign, promise ETAs | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **three-sided sync**.
2. Deliver a production-quality path for **prep-time**.
3. Deliver a production-quality path for **dasher assign**.
4. Deliver a production-quality path for **promise ETAs**.
5. Durable `delivery order` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | checkout < 500ms; assign minutes-scale |
...
1. User/system initiates flow to orchestrate restaurant→dasher→customer delivery → validation → core `delivery order` created.
...
> Design **DoorDash or Local-Delivery Service** to orchestrate restaurant→dasher→customer delivery, emphasizing three-sided sync, prep-time, dasher assign, promise ETAs. Start from baseline ~1K orders/min and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 1K orders
Entities (order-of): restaurants+dashers
Write class focus: order events
Typical payload: order+track
...
Use order+track as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **city marketplace**, admission control, and key sharding.
...
```text
order → merchant accept/prep → dasher assign (batch?) → pickup → dropoff
```
...
**DoorDash or Local-Delivery Service** interview signal: speak fluently about **three-sided sync** and **prep-time**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **DoorDash or Local-Delivery Service**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **three-sided sync** as a first-class subsystem.
2. Invest in **prep-time** as a first-class subsystem.
3. Invest in **dasher assign** as a first-class subsystem.
4. Invest in **promise ETAs** as a first-class subsystem.
...
"For DoorDash or Local-Delivery Service, I'd guarantee **three-sided sync** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in DoorDash or Local-Delivery Service?
...
For DoorDash or Local-Delivery Service: emphasize three-sided sync, prep-time, dasher assign; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: DoorDash or Local-Delivery Service
baseline: {'Primary QPS': '1K orders', 'Entities': 'restaurants+dashers', 'Write QPS': 'order events', 'Payload': 'order+track', 'Hot structure': 'city marketplace'}
