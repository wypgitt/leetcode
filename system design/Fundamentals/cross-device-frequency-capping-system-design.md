# System Design: Cross-device frequency capping
...
|-----------|----------|----------|
| Job | Design **Cross-device frequency capping** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Cross-device frequency capping | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Cross-device frequency capping.
...
Domain note: tailor 2–3 cases specifically to **Cross-device frequency capping** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Cross-device frequency capping.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Cross-device frequency capping**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Cross-device frequency capping".
...
Core components for **Cross-device frequency capping**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless crossdevicefrequency Service]
...
**Failure modes checklist for Cross-device frequency capping**
...
### 5.4 Problem-specific deep notes for Cross-device frequency capping
...
1. **Invariants** for Cross-device frequency capping (what must never break).
...
| 16 | Which algorithms apply to Cross-device frequency capping? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Cross-device frequency capping**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `cross-device-frequency-capping` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Cross-device frequency capping
...
| Business SLI for Cross-device frequency capping | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Cross-device frequency capping
...
*Enriched for interview drill · `cross-device-frequency-capping`*
