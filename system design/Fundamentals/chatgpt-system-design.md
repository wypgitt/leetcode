...
|-----------|----------|----------|
| Job | Design **ChatGPT** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for ChatGPT | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for ChatGPT.
...
Domain note: tailor 2–3 cases specifically to **ChatGPT** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to ChatGPT.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **ChatGPT**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "ChatGPT".
...
flowchart LR
  ...
  GW --> Svc[Stateless chatgpt Service]
...
**Failure modes checklist for ChatGPT**
...
### 5.4 Problem-specific deep notes for ChatGPT
...
1. **Invariants** for ChatGPT (what must never break).
...
| 16 | Which algorithms apply to ChatGPT? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **ChatGPT**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `chatgpt` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix A — ChatGPT-style system specifics

### Path
Client → gateway → conversation service → inference gateway → model fleet; stream tokens SSE/WS.

### Memory
Short-term context window; long-term memory optional retrieval; store messages durably before/aside inference.

### Safety
Input/output moderation; tool permissioning; rate limits & token budgets per user.

## Appendix — Deep dive notes for ChatGPT
...
| Business SLI for ChatGPT | Product truth |
...
*Enriched for interview drill · `chatgpt`*
