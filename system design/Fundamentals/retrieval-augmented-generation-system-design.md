...
|-----------|----------|----------|
| Job | Design **Retrieval-augmented generation** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Retrieval-augmented generation | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Retrieval-augmented generation.
...
Domain note: tailor 2–3 cases specifically to **Retrieval-augmented generation** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Retrieval-augmented generation.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Retrieval-augmented generation**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Retrieval-augmented generation".
...
Core components for **Retrieval-augmented generation**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless retrievalaugmentedge Service]
...
**Failure modes checklist for Retrieval-augmented generation**
...
### 5.4 Problem-specific deep notes for Retrieval-augmented generation
...
1. **Invariants** for Retrieval-augmented generation (what must never break).
...
| 16 | Which algorithms apply to Retrieval-augmented generation? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Retrieval-augmented generation**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `retrieval-augmented-generation` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix A — RAG specifics

### Pipeline
Ingest → chunk → embed → index → retrieve → rerank → prompt → generate → cite.

### Chunking
Overlap vs semantic splits; store offsets for citations. ACL filters **must** apply at retrieval.

### Freshness
Incremental re-embed on update; version embeddings with model id.

### Eval
Recall@k, faithfulness, citation precision; offline golden sets + online feedback.

## Appendix — Deep dive notes for Retrieval-augmented generation
...
| Business SLI for Retrieval-augmented generation | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Retrieval-augmented generation
...
*Enriched for interview drill · `retrieval-augmented-generation`*
