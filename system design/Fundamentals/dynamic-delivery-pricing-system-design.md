# System Design: Dynamic Delivery Pricing

> **Focus areas:** Fees · Incentives · Hex demand · Quote binding · Fairness · Experimentation  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Dynamic Delivery Pricing**  
...
Goal: price local deliveries dynamically at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to price local deliveries dynamically | Unrelated product surface |
| Core noun | `delivery quote` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | fee components, incentive coupling, quote TTL, marketplace balance | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **fee components**.
2. Deliver a production-quality path for **incentive coupling**.
3. Deliver a production-quality path for **quote TTL**.
4. Deliver a production-quality path for **marketplace balance**.
5. Durable `delivery quote` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | quote < 150ms |
...
1. User/system initiates flow to price local deliveries dynamically → validation → core `delivery quote` created.
...
| Domain-specific contention | Serialize on delivery quote keys; lease/TTL patterns |
...
> Design **Dynamic Delivery Pricing** to price local deliveries dynamically, emphasizing fee components, incentive coupling, quote TTL, marketplace balance. Start from baseline ~5K quote qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 5K quotes
Entities (order-of): hex fees
Write class focus: marketplace stats
Typical payload: quote
...
Use quote as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **pricing engines**, admission control, and key sharding.
...
```text
features→fee model→incentives→bind quote→order uses quote_id
```
...
**Dynamic Delivery Pricing** interview signal: speak fluently about **fee components** and **incentive coupling**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Dynamic Delivery Pricing**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **fee components** as a first-class subsystem.
2. Invest in **incentive coupling** as a first-class subsystem.
3. Invest in **quote TTL** as a first-class subsystem.
4. Invest in **marketplace balance** as a first-class subsystem.
...
"For Dynamic Delivery Pricing, I'd guarantee **fee components** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Dynamic Delivery Pricing?
...
For Dynamic Delivery Pricing: emphasize fee components, incentive coupling, quote TTL; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Dynamic Delivery Pricing
baseline: {'Primary QPS': '5K quotes', 'Entities': 'hex fees', 'Write QPS': 'marketplace stats', 'Payload': 'quote', 'Hot structure': 'pricing engines'}
