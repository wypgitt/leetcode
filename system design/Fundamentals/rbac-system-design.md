...
|-----------|----------|----------|
| Job | Design **Role-based access control** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Role-based access control | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Role-based access control.
...
Domain note: tailor 2–3 cases specifically to **Role-based access control** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Role-based access control.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Role-based access control**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Role-based access control".
...
Core components for **Role-based access control**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
**Failure modes checklist for Role-based access control**
...
### 5.4 Problem-specific deep notes for Role-based access control
...
1. **Invariants** for Role-based access control (what must never break).
...
| 16 | Which algorithms apply to Role-based access control? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Role-based access control**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `rbac` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Role-based access control
...
| Business SLI for Role-based access control | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Role-based access control
...
*Enriched for interview drill · `rbac`*
