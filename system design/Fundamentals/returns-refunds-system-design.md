# System Design: Returns and refunds
...
|-----------|----------|----------|
| Job | Design **Returns and refunds** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Returns and refunds | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Returns and refunds.
...
Domain note: tailor 2–3 cases specifically to **Returns and refunds** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Returns and refunds.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Returns and refunds**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Returns and refunds".
...
Core components for **Returns and refunds**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless returnsrefunds Service]
...
**Failure modes checklist for Returns and refunds**
...
### 5.4 Problem-specific deep notes for Returns and refunds
...
1. **Invariants** for Returns and refunds (what must never break).
...
| 16 | Which algorithms apply to Returns and refunds? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Returns and refunds**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `returns-refunds` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Returns and refunds
...
| Business SLI for Returns and refunds | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Returns and refunds
...
*Enriched for interview drill · `returns-refunds`*
