# System Design: Assignment submission and grading
...
|-----------|----------|----------|
| Job | Design **Assignment submission and grading** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Assignment submission and grading | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Assignment submission and grading.
...
Domain note: tailor 2–3 cases specifically to **Assignment submission and grading** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Assignment submission and grading.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Assignment submission and grading**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Assignment submission and grading".
...
Core components for **Assignment submission and grading**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless assignmentsubmission Service]
...
**Failure modes checklist for Assignment submission and grading**
...
### 5.4 Problem-specific deep notes for Assignment submission and grading
...
1. **Invariants** for Assignment submission and grading (what must never break).
...
| 16 | Which algorithms apply to Assignment submission and grading? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Assignment submission and grading**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `assignment-submission-grading` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Assignment submission and grading
...
| Business SLI for Assignment submission and grading | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Assignment submission and grading
...
*Enriched for interview drill · `assignment-submission-grading`*
