# System Design: Merchant-lending platform
...
|-----------|----------|----------|
| Job | Design **Merchant-lending platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Merchant-lending platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Merchant-lending platform.
...
Domain note: tailor 2–3 cases specifically to **Merchant-lending platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Merchant-lending platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Merchant-lending platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Merchant-lending platform".
...
Core components for **Merchant-lending platform**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless merchantlendingplatf Service]
...
**Failure modes checklist for Merchant-lending platform**
...
### 5.4 Problem-specific deep notes for Merchant-lending platform
...
1. **Invariants** for Merchant-lending platform (what must never break).
...
| 16 | Which algorithms apply to Merchant-lending platform? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Merchant-lending platform**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `merchant-lending-platform` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Merchant-lending platform
...
| Business SLI for Merchant-lending platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Merchant-lending platform
...
*Enriched for interview drill · `merchant-lending-platform`*
