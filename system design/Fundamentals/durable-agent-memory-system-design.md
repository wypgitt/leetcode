# System Design: Durable agent memory
...
|-----------|----------|----------|
| Job | Design **Durable agent memory** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Durable agent memory | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Durable agent memory.
...
Domain note: tailor 2–3 cases specifically to **Durable agent memory** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Durable agent memory.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Durable agent memory**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Durable agent memory".
...
Core components for **Durable agent memory**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless durableagentmemory Service]
...
**Failure modes checklist for Durable agent memory**
...
### 5.4 Problem-specific deep notes for Durable agent memory
...
1. **Invariants** for Durable agent memory (what must never break).
...
| 16 | Which algorithms apply to Durable agent memory? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Durable agent memory**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `durable-agent-memory` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Durable agent memory
...
| Business SLI for Durable agent memory | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Durable agent memory
...
*Enriched for interview drill · `durable-agent-memory`*
