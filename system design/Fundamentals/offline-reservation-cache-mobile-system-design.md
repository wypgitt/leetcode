# System Design: Offline Reservation Cache for Mobile Clients

> **Focus areas:** Offline-first · Local cache · Conflict resolution · Sync · Holds · Idempotency · UX  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Offline Reservation Cache for Mobile Clients**  
...
Goal: let mobile clients reserve with intermittent connectivity at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to let mobile clients reserve with intermittent connectivity | Unrelated product surface |
| Core noun | `offline reservation` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | local durable queue, sync protocol, conflict UX, server authority | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **local durable queue**.
2. Deliver a production-quality path for **sync protocol**.
3. Deliver a production-quality path for **conflict UX**.
4. Deliver a production-quality path for **server authority**.
5. Durable `offline reservation` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | local op < 16ms; sync when online |
...
1. User/system initiates flow to let mobile clients reserve with intermittent connectivity → validation → core `offline reservation` created.
...
|--------|----------|-----|------|--------|
| Sync sessions/s | 1K | 10K | 100K | 1M |
...
> Design **Offline Reservation Cache for Mobile Clients** to let mobile clients reserve with intermittent connectivity, emphasizing local durable queue, sync protocol, conflict UX, server authority. Start from baseline ~1K sync sessions/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 1K sync
Entities (order-of): mobile clients
Write class focus: reservation intents
Typical payload: sync batches
...
Use sync batches as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **client queue + server CAS**, admission control, and key sharding.
...
```text
local enqueue → sync push idempotent → server CAS → pull truth → resolve UX
```
...
**Offline Reservation Cache for Mobile Clients** interview signal: speak fluently about **local durable queue** and **sync protocol**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Offline Reservation Cache for Mobile Clients**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **local durable queue** as a first-class subsystem.
2. Invest in **sync protocol** as a first-class subsystem.
3. Invest in **conflict UX** as a first-class subsystem.
4. Invest in **server authority** as a first-class subsystem.
...
"For Offline Reservation Cache for Mobile Clients, I'd guarantee **local durable queue** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Offline Reservation Cache for Mobile Clients?
...
For Offline Reservation Cache for Mobile Clients: emphasize local durable queue, sync protocol, conflict UX; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Offline Reservation Cache for Mobile Clients
baseline: {'Primary QPS': '1K sync', 'Entities': 'mobile clients', 'Write QPS': 'reservation intents', 'Payload': 'sync batches', 'Hot structure': 'client queue + server CAS'}
...
| Baseline | 1K Sync sessions/s |
| Invariant | no double-allocate / no silent money loss |
| Scale endgame | cell mesh + edge + tiered stores |

---

*End of Offline Reservation Cache for Mobile Clients system design prep doc.*
