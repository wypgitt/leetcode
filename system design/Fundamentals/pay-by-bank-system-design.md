...
|-----------|----------|----------|
| Job | Design **Pay-by-bank** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Pay-by-bank | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Pay-by-bank.
...
Domain note: tailor 2–3 cases specifically to **Pay-by-bank** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Pay-by-bank.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Pay-by-bank**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Pay-by-bank".
...
Core components for **Pay-by-bank**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless paybybank Service]
...
**Failure modes checklist for Pay-by-bank**
...
### 5.4 Problem-specific deep notes for Pay-by-bank
...
1. **Invariants** for Pay-by-bank (what must never break).
...
| 16 | Which algorithms apply to Pay-by-bank? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Pay-by-bank**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `pay-by-bank` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Pay-by-bank
...
| Business SLI for Pay-by-bank | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Pay-by-bank
...
*Enriched for interview drill · `pay-by-bank`*
