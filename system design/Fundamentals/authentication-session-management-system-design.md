# System Design: Authentication and session management
...
|-----------|----------|----------|
| Job | Design **Authentication and session management** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Authentication and session management | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Authentication and session management.
...
Domain note: tailor 2–3 cases specifically to **Authentication and session management** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Authentication and session management.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Authentication and session management**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Authentication and session management".
...
Core components for **Authentication and session management**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless authenticationsessio Service]
...
**Failure modes checklist for Authentication and session management**
...
### 5.4 Problem-specific deep notes for Authentication and session management
...
1. **Invariants** for Authentication and session management (what must never break).
...
| 16 | Which algorithms apply to Authentication and session management? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Authentication and session management**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `authentication-session-management` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Authentication and session management
...
| Business SLI for Authentication and session management | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Authentication and session management
...
*Enriched for interview drill · `authentication-session-management`*
