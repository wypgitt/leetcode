...
|-----------|----------|----------|
| Job | Design **Course recommendations** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Course recommendations | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Course recommendations.
...
Domain note: tailor 2–3 cases specifically to **Course recommendations** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Course recommendations.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Course recommendations**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Course recommendations".
...
Core components for **Course recommendations**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless courserecommendation Service]
...
**Failure modes checklist for Course recommendations**
...
### 5.4 Problem-specific deep notes for Course recommendations
...
1. **Invariants** for Course recommendations (what must never break).
...
| 16 | Which algorithms apply to Course recommendations? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Course recommendations**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `course-recommendations` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Course recommendations
...
| Business SLI for Course recommendations | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Course recommendations
...
*Enriched for interview drill · `course-recommendations`*
