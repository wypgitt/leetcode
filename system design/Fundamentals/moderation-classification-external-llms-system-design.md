# System Design: Moderation or classification using external LLMs
...
|-----------|----------|----------|
| Job | Design **Moderation or classification using external LLMs** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Moderation or classification using external LLMs | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Moderation or classification using external LLMs.
...
Domain note: tailor 2–3 cases specifically to **Moderation or classification using external LLMs** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Moderation or classification using external LLMs.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Moderation or classification using external LLMs**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Moderation or classification using external LLMs".
...
Core components for **Moderation or classification using external LLMs**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless moderationclassifica Service]
...
**Failure modes checklist for Moderation or classification using external LLMs**
...
### 5.4 Problem-specific deep notes for Moderation or classification using external LLMs
...
1. **Invariants** for Moderation or classification using external LLMs (what must never break).
...
| 16 | Which algorithms apply to Moderation or classification using external LLMs? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Moderation or classification using external LLMs**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `moderation-classification-external-llms` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Moderation or classification using external LLMs
...
| Business SLI for Moderation or classification using external LLMs | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Moderation or classification using external LLMs
...
*Enriched for interview drill · `moderation-classification-external-llms`*
