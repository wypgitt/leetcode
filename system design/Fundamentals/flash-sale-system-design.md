...
|-----------|----------|----------|
| Job | Design **Flash sale** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Flash sale | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Flash sale.
...
Domain note: tailor 2–3 cases specifically to **Flash sale** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Flash sale.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Flash sale**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Flash sale".
...
Core components for **Flash sale**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless flashsale Service]
...
**Failure modes checklist for Flash sale**
...
### 5.4 Problem-specific deep notes for Flash sale
...
1. **Invariants** for Flash sale (what must never break).
...
| 16 | Which algorithms apply to Flash sale? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Flash sale**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `flash-sale` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Flash sale
...
| Business SLI for Flash sale | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Flash sale
...
*Enriched for interview drill · `flash-sale`*
