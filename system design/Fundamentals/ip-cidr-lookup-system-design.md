...
|-----------|----------|----------|
| Job | Design **IP/CIDR lookup** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for IP/CIDR lookup | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for IP/CIDR lookup.
...
Domain note: tailor 2–3 cases specifically to **IP/CIDR lookup** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to IP/CIDR lookup.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **IP/CIDR lookup**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "IP/CIDR lookup".
...
Core components for **IP/CIDR lookup**:
- **public API**
...
flowchart LR
  ...
  GW --> Svc[Stateless ipcidrlookup Service]
...
**Failure modes checklist for IP/CIDR lookup**
...
### 5.4 Problem-specific deep notes for IP/CIDR lookup
...
1. **Invariants** for IP/CIDR lookup (what must never break).
...
| 16 | Which algorithms apply to IP/CIDR lookup? | Expect discussion of: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **IP/CIDR lookup**: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping.
...
*Category: Lower-level and machine-coding system design · Slug: `ip-cidr-lookup` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for IP/CIDR lookup
...
| Business SLI for IP/CIDR lookup | Product truth |
...
- Cost model at 100×
- Abuse cases unique to IP/CIDR lookup
...
*Enriched for interview drill · `ip-cidr-lookup`*
