# System Design: Loyalty points and cashback
...
|-----------|----------|----------|
| Job | Design **Loyalty points and cashback** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Loyalty points and cashback | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Loyalty points and cashback.
...
Domain note: tailor 2–3 cases specifically to **Loyalty points and cashback** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Loyalty points and cashback.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Loyalty points and cashback**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Loyalty points and cashback".
...
Core components for **Loyalty points and cashback**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless loyaltypointscashbac Service]
...
**Failure modes checklist for Loyalty points and cashback**
...
### 5.4 Problem-specific deep notes for Loyalty points and cashback
...
1. **Invariants** for Loyalty points and cashback (what must never break).
...
| 16 | Which algorithms apply to Loyalty points and cashback? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Loyalty points and cashback**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `loyalty-points-cashback` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Loyalty points and cashback
...
| Business SLI for Loyalty points and cashback | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Loyalty points and cashback
...
*Enriched for interview drill · `loyalty-points-cashback`*
