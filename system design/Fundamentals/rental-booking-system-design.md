# System Design: Rental Booking

> **Focus areas:** Inventory calendar · Holds · Checkout · Payments · Idempotency · Overbooking prevention  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Rental Booking**  
...
Goal: reserve rental inventory safely at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to reserve rental inventory safely | Unrelated product surface |
| Core noun | `booking hold` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | calendar locks, hold TTL, payment auth, no double-book | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **calendar locks**.
2. Deliver a production-quality path for **hold TTL**.
3. Deliver a production-quality path for **payment auth**.
4. Deliver a production-quality path for **no double-book**.
5. Durable `booking hold` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | hold < 200ms |
...
1. User/system initiates flow to reserve rental inventory safely → validation → core `booking hold` created.
...
|--------|----------|-----|------|--------|
| Booking attempts/s | 300 | 3K | 30K | 300K |
...
> Design **Rental Booking** to reserve rental inventory safely, emphasizing calendar locks, hold TTL, payment auth, no double-book. Start from baseline ~300 booking attempts/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 300 booking
Entities (order-of): calendars
Write class focus: 300
Typical payload: booking tx
...
Use booking tx as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **row locks / leases**, admission control, and key sharding.
...
```text
conditional calendar update → hold TTL → payment → confirm CAS
```
...
**Rental Booking** interview signal: speak fluently about **calendar locks** and **hold TTL**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Rental Booking**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **calendar locks** as a first-class subsystem.
2. Invest in **hold TTL** as a first-class subsystem.
3. Invest in **payment auth** as a first-class subsystem.
4. Invest in **no double-book** as a first-class subsystem.
...
"For Rental Booking, I'd guarantee **calendar locks** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Rental Booking?
...
For Rental Booking: emphasize calendar locks, hold TTL, payment auth; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Rental Booking
baseline: {'Primary QPS': '300 booking', 'Entities': 'calendars', 'Write QPS': '300', 'Payload': 'booking tx', 'Hot structure': 'row locks / leases'}
