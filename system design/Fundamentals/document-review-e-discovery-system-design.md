# System Design: Document review and e-discovery
...
|-----------|----------|----------|
| Job | Design **Document review and e-discovery** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Document review and e-discovery | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Document review and e-discovery.
...
Domain note: tailor 2–3 cases specifically to **Document review and e-discovery** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Document review and e-discovery.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Document review and e-discovery**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Document review and e-discovery".
...
Core components for **Document review and e-discovery**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless documentreviewedisco Service]
...
**Failure modes checklist for Document review and e-discovery**
...
### 5.4 Problem-specific deep notes for Document review and e-discovery
...
1. **Invariants** for Document review and e-discovery (what must never break).
...
| 16 | Which algorithms apply to Document review and e-discovery? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Document review and e-discovery**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `document-review-e-discovery` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Document review and e-discovery
...
| Business SLI for Document review and e-discovery | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Document review and e-discovery
...
*Enriched for interview drill · `document-review-e-discovery`*
