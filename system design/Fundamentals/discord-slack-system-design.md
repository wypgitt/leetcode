# System Design: Discord or Slack
...
|-----------|----------|----------|
| Job | Design **Discord or Slack** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Discord or Slack | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Discord or Slack.
...
Domain note: tailor 2–3 cases specifically to **Discord or Slack** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Discord or Slack.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Discord or Slack**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Discord or Slack".
...
Core components for **Discord or Slack**:
- **write API**
...
flowchart LR
  ...
  GW --> Svc[Stateless discordslack Service]
...
**Failure modes checklist for Discord or Slack**
...
### 5.4 Problem-specific deep notes for Discord or Slack
...
1. **Invariants** for Discord or Slack (what must never break).
...
| 16 | Which algorithms apply to Discord or Slack? | Expect discussion of: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Discord or Slack**: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat.
...
*Category: Social, communication, and collaboration · Slug: `discord-slack` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Discord or Slack
...
| Business SLI for Discord or Slack | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Discord or Slack
...
*Enriched for interview drill · `discord-slack`*
