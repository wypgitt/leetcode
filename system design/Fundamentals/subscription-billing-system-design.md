# System Design: Subscription billing
...
|-----------|----------|----------|
| Job | Design **Subscription billing** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Subscription billing | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Subscription billing.
...
Domain note: tailor 2–3 cases specifically to **Subscription billing** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Subscription billing.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Subscription billing**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Subscription billing".
...
Core components for **Subscription billing**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless subscriptionbilling Service]
...
**Failure modes checklist for Subscription billing**
...
### 5.4 Problem-specific deep notes for Subscription billing
...
1. **Invariants** for Subscription billing (what must never break).
...
| 16 | Which algorithms apply to Subscription billing? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Subscription billing**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `subscription-billing` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Subscription billing
...
| Business SLI for Subscription billing | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Subscription billing
...
*Enriched for interview drill · `subscription-billing`*
