...
|-----------|----------|----------|
| Job | Design **Refunds** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Refunds | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Refunds.
...
Domain note: tailor 2–3 cases specifically to **Refunds** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Refunds.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Refunds**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Refunds".
...
flowchart LR
  ...
  GW --> Svc[Stateless refunds Service]
...
**Failure modes checklist for Refunds**
...
### 5.4 Problem-specific deep notes for Refunds
...
1. **Invariants** for Refunds (what must never break).
...
| 16 | Which algorithms apply to Refunds? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Refunds**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `refunds` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Refunds
...
| Business SLI for Refunds | Product truth |
...
*Enriched for interview drill · `refunds`*
