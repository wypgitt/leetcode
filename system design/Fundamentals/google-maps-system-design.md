...
> **Focus areas:** Tiles · Geocoding · Routing · Traffic · Places · Live location · Multi-layer serving  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Google Maps**  
...
Goal: serve maps, geocode, route, and traffic at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to serve maps, geocode, route, and traffic | Unrelated product surface |
| Core noun | `map session` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | tile pyramid, routing graph, traffic fusion, geocoder + places | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **tile pyramid**.
2. Deliver a production-quality path for **routing graph**.
3. Deliver a production-quality path for **traffic fusion**.
4. Deliver a production-quality path for **geocoder + places**.
5. Durable `map session` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | tiles CDN; directions p99 < 200–300ms |
...
1. User/system initiates flow to serve maps, geocode, route, and traffic → validation → core `map session` created.
...
| Domain-specific contention | Serialize on map session keys; lease/TTL patterns |
...
> Design **Google Maps** to serve maps, geocode, route, and traffic, emphasizing tile pyramid, routing graph, traffic fusion, geocoder + places. Start from baseline ~50K map api qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 50K tile+API
Entities (order-of): global road graph + POIs
Write class focus: traffic probes 100K+
Typical payload: tiles 10–50KB
...
Use tiles 10–50KB as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **tile CDN + edge**, admission control, and key sharding.
...
```text
tile pyramid + CH routing on traffic-weighted graph + geocoder retrieval
```
...
**Google Maps** interview signal: speak fluently about **tile pyramid** and **routing graph**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Google Maps**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **tile pyramid** as a first-class subsystem.
2. Invest in **routing graph** as a first-class subsystem.
3. Invest in **traffic fusion** as a first-class subsystem.
4. Invest in **geocoder + places** as a first-class subsystem.
...
"For Google Maps, I'd guarantee **tile pyramid** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Google Maps?
...
### Q4. How do you prevent hot partitions?
...
### Q5. Where is eventual consistency unacceptable?
...
### Q7. How do you test geo correctness?
...
```text
problem: Google Maps
baseline: {'Primary QPS': '50K tile+API', 'Entities': 'global road graph + POIs', 'Write QPS': 'traffic probes 100K+', 'Payload': 'tiles 10–50KB', 'Hot structure': 'tile CDN + edge'}
