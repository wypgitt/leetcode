# System Design: Policy-compliance and evidence-collection platform
...
|-----------|----------|----------|
| Job | Design **Policy-compliance and evidence-collection platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Policy-compliance and evidence-collection platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Policy-compliance and evidence-collection platform.
...
Domain note: tailor 2–3 cases specifically to **Policy-compliance and evidence-collection platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Policy-compliance and evidence-collection platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Policy-compliance and evidence-collection platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Policy-compliance and evidence-collection platform".
...
Core components for **Policy-compliance and evidence-collection platform**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless policycomplianceevid Service]
...
**Failure modes checklist for Policy-compliance and evidence-collection platform**
...
### 5.4 Problem-specific deep notes for Policy-compliance and evidence-collection platform
...
1. **Invariants** for Policy-compliance and evidence-collection platform (what must never break).
...
| 16 | Which algorithms apply to Policy-compliance and evidence-collection platform? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Policy-compliance and evidence-collection platform**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `policy-compliance-evidence-collection` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Policy-compliance and evidence-collection platform
...
| Business SLI for Policy-compliance and evidence-collection platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Policy-compliance and evidence-collection platform
...
*Enriched for interview drill · `policy-compliance-evidence-collection`*
