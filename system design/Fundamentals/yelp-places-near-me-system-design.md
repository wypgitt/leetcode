# System Design: Yelp or Places Near Me

> **Focus areas:** Geo indexes · POI recall · Ranking · Open-now · Viewport · Hot tiles · City sharding  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Yelp or Places Near Me**  
...
Goal: discover nearby businesses at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to discover nearby businesses | Unrelated product surface |
| Core noun | `place/POI` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | H3/S2 geo index, two-stage recall→rank, open-now correctness, map viewport caching | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **H3/S2 geo index**.
2. Deliver a production-quality path for **two-stage recall→rank**.
3. Deliver a production-quality path for **open-now correctness**.
4. Deliver a production-quality path for **map viewport caching**.
5. Durable `place/POI` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | p99 nearby < 100–150ms |
...
| N5 | Freshness? | Live location/ETA/status SLOs | hours minutes–hours; geo static |
...
1. User/system initiates flow to discover nearby businesses → validation → core `place/POI` created.
...
| Domain-specific contention | Serialize on place/POI keys; lease/TTL patterns |
...
> Design **Yelp or Places Near Me** to discover nearby businesses, emphasizing H3/S2 geo index, two-stage recall→rank, open-now correctness, map viewport caching. Start from baseline ~5K nearby qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 5K nearby
Entities (order-of): 50M POIs
Write class focus: 100 catalog
...
Use 5 KB as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **geohash tile cache**, admission control, and key sharding.
...
```text
H3 k-ring recall → filter → haversine refine → score → page
```
...
**Yelp or Places Near Me** interview signal: speak fluently about **H3/S2 geo index** and **two-stage recall→rank**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Yelp or Places Near Me**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **H3/S2 geo index** as a first-class subsystem.
2. Invest in **two-stage recall→rank** as a first-class subsystem.
3. Invest in **open-now correctness** as a first-class subsystem.
4. Invest in **map viewport caching** as a first-class subsystem.
...
"For Yelp or Places Near Me, I'd guarantee **H3/S2 geo index** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Yelp or Places Near Me?
...
For Yelp or Places Near Me: emphasize H3/S2 geo index, two-stage recall→rank, open-now correctness; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Yelp or Places Near Me
baseline: {'Primary QPS': '5K nearby', 'Entities': '50M POIs', 'Write QPS': '100 catalog', 'Payload': '5 KB', 'Hot structure': 'geohash tile cache'}
