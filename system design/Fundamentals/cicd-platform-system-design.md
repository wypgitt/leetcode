...
|-----------|----------|----------|
| Job | Design **CI/CD platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for CI/CD platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for CI/CD platform.
...
Domain note: tailor 2–3 cases specifically to **CI/CD platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to CI/CD platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **CI/CD platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "CI/CD platform".
...
Core components for **CI/CD platform**:
- **API/control plane**
- **job scheduler**
- **artifact store**
- **runners/workers**
...
flowchart LR
  ...
  GW --> Svc[Stateless cicdplatform Service]
...
**Failure modes checklist for CI/CD platform**
...
### 5.4 Problem-specific deep notes for CI/CD platform
...
1. **Invariants** for CI/CD platform (what must never break).
...
| 16 | Which algorithms apply to CI/CD platform? | Expect discussion of: content-addressed storage, DAG scheduling, fair share / DRF, Merkle trees, copy-on-write. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **CI/CD platform**: content-addressed storage, DAG scheduling, fair share / DRF, Merkle trees, copy-on-write.
...
*Category: Developer tools and cloud platforms · Slug: `cicd-platform` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for CI/CD platform
...
| Business SLI for CI/CD platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to CI/CD platform
...
*Enriched for interview drill · `cicd-platform`*
