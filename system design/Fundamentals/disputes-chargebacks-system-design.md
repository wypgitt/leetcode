# System Design: Disputes and chargebacks
...
|-----------|----------|----------|
| Job | Design **Disputes and chargebacks** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Disputes and chargebacks | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Disputes and chargebacks.
...
Domain note: tailor 2–3 cases specifically to **Disputes and chargebacks** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Disputes and chargebacks.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Disputes and chargebacks**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Disputes and chargebacks".
...
Core components for **Disputes and chargebacks**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless disputeschargebacks Service]
...
**Failure modes checklist for Disputes and chargebacks**
...
### 5.4 Problem-specific deep notes for Disputes and chargebacks
...
1. **Invariants** for Disputes and chargebacks (what must never break).
...
| 16 | Which algorithms apply to Disputes and chargebacks? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Disputes and chargebacks**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `disputes-chargebacks` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Disputes and chargebacks
...
| Business SLI for Disputes and chargebacks | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Disputes and chargebacks
...
*Enriched for interview drill · `disputes-chargebacks`*
