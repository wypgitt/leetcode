...
|-----------|----------|----------|
| Job | Design **Clinical alerts** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Clinical alerts | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Clinical alerts.
...
Domain note: tailor 2–3 cases specifically to **Clinical alerts** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Clinical alerts.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Clinical alerts**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Clinical alerts".
...
Core components for **Clinical alerts**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless clinicalalerts Service]
...
**Failure modes checklist for Clinical alerts**
...
### 5.4 Problem-specific deep notes for Clinical alerts
...
1. **Invariants** for Clinical alerts (what must never break).
...
| 16 | Which algorithms apply to Clinical alerts? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Clinical alerts**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `clinical-alerts` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Clinical alerts
...
| Business SLI for Clinical alerts | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Clinical alerts
...
*Enriched for interview drill · `clinical-alerts`*
