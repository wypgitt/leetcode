# System Design: Expense-management platform
...
|-----------|----------|----------|
| Job | Design **Expense-management platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Expense-management platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Expense-management platform.
...
Domain note: tailor 2–3 cases specifically to **Expense-management platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Expense-management platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Expense-management platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Expense-management platform".
...
Core components for **Expense-management platform**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless expensemanagementpla Service]
...
**Failure modes checklist for Expense-management platform**
...
### 5.4 Problem-specific deep notes for Expense-management platform
...
1. **Invariants** for Expense-management platform (what must never break).
...
| 16 | Which algorithms apply to Expense-management platform? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Expense-management platform**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `expense-management-platform` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Expense-management platform
...
| Business SLI for Expense-management platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Expense-management platform
...
*Enriched for interview drill · `expense-management-platform`*
