# System Design: Public status page
...
|-----------|----------|----------|
| Job | Design **Public status page** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Public status page | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Public status page.
...
Domain note: tailor 2–3 cases specifically to **Public status page** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Public status page.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Public status page**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Public status page".
...
Core components for **Public status page**:
- **agents/collectors**
- **ingest pipeline**
- **TSDB/log index**
- **alert evaluator**
...
flowchart LR
  ...
  GW --> Svc[Stateless publicstatuspage Service]
...
**Failure modes checklist for Public status page**
...
### 5.4 Problem-specific deep notes for Public status page
...
1. **Invariants** for Public status page (what must never break).
...
| 16 | Which algorithms apply to Public status page? | Expect discussion of: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Public status page**: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate.
...
*Category: Observability, reliability, security, and operations · Slug: `public-status-page` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Public status page
...
| Business SLI for Public status page | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Public status page
...
*Enriched for interview drill · `public-status-page`*
