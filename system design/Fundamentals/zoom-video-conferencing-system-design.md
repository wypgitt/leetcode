# System Design: Zoom or video conferencing
...
|-----------|----------|----------|
| Job | Design **Zoom or video conferencing** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Zoom or video conferencing | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Zoom or video conferencing.
...
Domain note: tailor 2–3 cases specifically to **Zoom or video conferencing** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Zoom or video conferencing.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Zoom or video conferencing**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Zoom or video conferencing".
...
Core components for **Zoom or video conferencing**:
- **write API**
...
flowchart LR
  ...
  GW --> Svc[Stateless zoomvideoconferencin Service]
...
**Failure modes checklist for Zoom or video conferencing**
...
### 5.4 Problem-specific deep notes for Zoom or video conferencing
...
1. **Invariants** for Zoom or video conferencing (what must never break).
...
| 16 | Which algorithms apply to Zoom or video conferencing? | Expect discussion of: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Zoom or video conferencing**: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat.
...
*Category: Social, communication, and collaboration · Slug: `zoom-video-conferencing` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Zoom or video conferencing
...
| Business SLI for Zoom or video conferencing | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Zoom or video conferencing
...
*Enriched for interview drill · `zoom-video-conferencing`*
