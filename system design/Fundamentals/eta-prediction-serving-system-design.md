# System Design: ETA Prediction and Serving

> **Focus areas:** Features · Models · Online serving · Traffic · Uncertainty · Cache · Feedback loops  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **ETA Prediction and Serving**  
...
Goal: predict and serve arrival times at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to predict and serve arrival times | Unrelated product surface |
| Core noun | `ETA prediction` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | feature store, model tiers, calibration, feedback from completions | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **feature store**.
2. Deliver a production-quality path for **model tiers**.
3. Deliver a production-quality path for **calibration**.
4. Deliver a production-quality path for **feedback from completions**.
5. Durable `ETA prediction` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | predict p99 < 50–100ms |
...
| N5 | Freshness? | Live location/ETA/status SLOs | features seconds-stale OK if calibrated |
...
1. User/system initiates flow to predict and serve arrival times → validation → core `ETA prediction` created.
...
| Domain-specific contention | Serialize on ETA prediction keys; lease/TTL patterns |
...
> Design **ETA Prediction and Serving** to predict and serve arrival times, emphasizing feature store, model tiers, calibration, feedback from completions. Start from baseline ~10K eta qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 10K predict
Entities (order-of): feature vectors
Write class focus: feedback 2K
Typical payload: 1 KB features
...
Use 1 KB features as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **model cache + features**, admission control, and key sharding.
...
```text
fetch features → model infer → calibrate → cache → async feedback train
```
...
**ETA Prediction and Serving** interview signal: speak fluently about **feature store** and **model tiers**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **ETA Prediction and Serving**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **feature store** as a first-class subsystem.
2. Invest in **model tiers** as a first-class subsystem.
3. Invest in **calibration** as a first-class subsystem.
4. Invest in **feedback from completions** as a first-class subsystem.
...
"For ETA Prediction and Serving, I'd guarantee **feature store** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in ETA Prediction and Serving?
...
For ETA Prediction and Serving: emphasize feature store, model tiers, calibration; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: ETA Prediction and Serving
baseline: {'Primary QPS': '10K predict', 'Entities': 'feature vectors', 'Write QPS': 'feedback 2K', 'Payload': '1 KB features', 'Hot structure': 'model cache + features'}
