# System Design: Seller ratings and reputation
...
|-----------|----------|----------|
| Job | Design **Seller ratings and reputation** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Seller ratings and reputation | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Seller ratings and reputation.
...
Domain note: tailor 2–3 cases specifically to **Seller ratings and reputation** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Seller ratings and reputation.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Seller ratings and reputation**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Seller ratings and reputation".
...
Core components for **Seller ratings and reputation**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless sellerratingsreputat Service]
...
**Failure modes checklist for Seller ratings and reputation**
...
### 5.4 Problem-specific deep notes for Seller ratings and reputation
...
1. **Invariants** for Seller ratings and reputation (what must never break).
...
| 16 | Which algorithms apply to Seller ratings and reputation? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Seller ratings and reputation**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `seller-ratings-reputation` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Seller ratings and reputation
...
| Business SLI for Seller ratings and reputation | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Seller ratings and reputation
...
*Enriched for interview drill · `seller-ratings-reputation`*
