# System Design: Shipping-Label and Carrier-Integration Service

> **Focus areas:** Rate shop · Labels · Carrier APIs · Idempotency · Retries · Manifests · Webhooks  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Shipping-Label and Carrier-Integration Service**  
...
Goal: rate-shop and purchase carrier labels at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to rate-shop and purchase carrier labels | Unrelated product surface |
| Core noun | `shipment label` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | carrier adapters, idempotent purchase, label storage, webhooks | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **carrier adapters**.
2. Deliver a production-quality path for **idempotent purchase**.
3. Deliver a production-quality path for **label storage**.
4. Deliver a production-quality path for **webhooks**.
5. Durable `shipment label` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | rate < 500ms; purchase < 2s + carrier |
...
1. User/system initiates flow to rate-shop and purchase carrier labels → validation → core `shipment label` created.
...
|--------|----------|-----|------|--------|
| Label purchases/s | 500 | 5K | 50K | 500K |
...
> Design **Shipping-Label and Carrier-Integration Service** to rate-shop and purchase carrier labels, emphasizing carrier adapters, idempotent purchase, label storage, webhooks. Start from baseline ~500 label purchases/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 500 purchases
Entities (order-of): shipments
Write class focus: 500
Typical payload: label PDF/ZPL
...
Use label PDF/ZPL as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **carrier adapters**, admission control, and key sharding.
...
```text
rate shop → purchase idempotent → store label → manifest → track webhooks
```
...
**Shipping-Label and Carrier-Integration Service** interview signal: speak fluently about **carrier adapters** and **idempotent purchase**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Shipping-Label and Carrier-Integration Service**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **carrier adapters** as a first-class subsystem.
2. Invest in **idempotent purchase** as a first-class subsystem.
3. Invest in **label storage** as a first-class subsystem.
4. Invest in **webhooks** as a first-class subsystem.
...
"For Shipping-Label and Carrier-Integration Service, I'd guarantee **carrier adapters** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Shipping-Label and Carrier-Integration Service?
...
For Shipping-Label and Carrier-Integration Service: emphasize carrier adapters, idempotent purchase, label storage; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Shipping-Label and Carrier-Integration Service
baseline: {'Primary QPS': '500 purchases', 'Entities': 'shipments', 'Write QPS': '500', 'Payload': 'label PDF/ZPL', 'Hot structure': 'carrier adapters'}
...
| Baseline | 500 Label purchases/s |
| Invariant | no double-allocate / no silent money loss |
| Scale endgame | cell mesh + edge + tiered stores |

---

*End of Shipping-Label and Carrier-Integration Service system design prep doc.*
