# System Design: Alerting platform
...
|-----------|----------|----------|
| Job | Design **Alerting platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Alerting platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Alerting platform.
...
Domain note: tailor 2–3 cases specifically to **Alerting platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Alerting platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Alerting platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Alerting platform".
...
Core components for **Alerting platform**:
- **agents/collectors**
- **ingest pipeline**
- **TSDB/log index**
- **alert evaluator**
...
flowchart LR
  ...
  GW --> Svc[Stateless alertingplatform Service]
...
**Failure modes checklist for Alerting platform**
...
### 5.4 Problem-specific deep notes for Alerting platform
...
1. **Invariants** for Alerting platform (what must never break).
...
| 16 | Which algorithms apply to Alerting platform? | Expect discussion of: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Alerting platform**: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate.
...
*Category: Observability, reliability, security, and operations · Slug: `alerting-platform` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Alerting platform
...
| Business SLI for Alerting platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Alerting platform
...
*Enriched for interview drill · `alerting-platform`*
