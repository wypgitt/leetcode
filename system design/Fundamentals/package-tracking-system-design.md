# System Design: Package Tracking

> **Focus areas:** Scan events · Milestones · Carrier ingest · ETA · Notifications · Multi-leg  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Package Tracking**  
...
Goal: ingest scans and present package journeys at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to ingest scans and present package journeys | Unrelated product surface |
| Core noun | `tracking event` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | event ordering, milestone SM, carrier adapters, ETA | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **event ordering**.
2. Deliver a production-quality path for **milestone SM**.
3. Deliver a production-quality path for **carrier adapters**.
4. Deliver a production-quality path for **ETA**.
5. Durable `tracking event` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | scan ingest < 100ms; read < 150ms |
...
| N5 | Freshness? | Live location/ETA/status SLOs | scan visible < few seconds after ingest |
...
1. User/system initiates flow to ingest scans and present package journeys → validation → core `tracking event` created.
...
| Domain-specific contention | Serialize on tracking event keys; lease/TTL patterns |
...
> Design **Package Tracking** to ingest scans and present package journeys, emphasizing event ordering, milestone SM, carrier adapters, ETA. Start from baseline ~30K scan events/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 30K scans
Entities (order-of): packages
Write class focus: 30K
Typical payload: scan event
...
Use scan event as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **tracking log**, admission control, and key sharding.
...
```text
ingest scan → order/dedup → milestone SM → ETA update → notify
```
...
**Package Tracking** interview signal: speak fluently about **event ordering** and **milestone SM**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Package Tracking**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **event ordering** as a first-class subsystem.
2. Invest in **milestone SM** as a first-class subsystem.
3. Invest in **carrier adapters** as a first-class subsystem.
4. Invest in **ETA** as a first-class subsystem.
...
"For Package Tracking, I'd guarantee **event ordering** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Package Tracking?
...
For Package Tracking: emphasize event ordering, milestone SM, carrier adapters; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
baseline: {'Primary QPS': '30K scans', 'Entities': 'packages', 'Write QPS': '30K', 'Payload': 'scan event', 'Hot structure': 'tracking log'}
