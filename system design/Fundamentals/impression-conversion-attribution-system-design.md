# System Design: Impression and conversion attribution

> **Focus areas:** catalog · cart · checkout · inventory · ads auction · attribution
> **Style:** End-to-end design with progressive scale (10× → 100× → 1,000×)
> **Category:** E-commerce and advertising
...
|-----------|----------|----------|
| Job | Design **Impression and conversion attribution** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Impression and conversion attribution | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Impression and conversion attribution.
...
Domain note: tailor 2–3 cases specifically to **Impression and conversion attribution** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Impression and conversion attribution.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Impression and conversion attribution**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Impression and conversion attribution".
...
Section family `06-ecommerce-advertising` emphasizes: catalog · cart · checkout · inventory · ads auction · attribution.
...
Core components for **Impression and conversion attribution**:
- **product read path**
- **cart/checkout**
- **inventory reservation**
- **ad decisioning**
- **click/impression pipeline**
...
**Algorithms / structures likely discussed:** optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern

### 3.5 Deal-breakers for this problem family
- oversell without reservation
- auction >100ms RTB budget
- click stream in OLTP
...
flowchart LR
  ...
  GW --> Svc[Stateless impressionconversion Service]
...
**Failure modes checklist for Impression and conversion attribution**
...
- Key algorithms: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern
...
### 5.4 Problem-specific deep notes for Impression and conversion attribution
...
1. **Invariants** for Impression and conversion attribution (what must never break).
...
| 16 | Which algorithms apply to Impression and conversion attribution? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Impression and conversion attribution**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `impression-conversion-attribution` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Impression and conversion attribution
...
| Business SLI for Impression and conversion attribution | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Impression and conversion attribution
...
*Enriched for interview drill · `impression-conversion-attribution`*
