# System Design: Assigned-Seat Reservation

> **Focus areas:** Seat maps · Atomic holds · Section inventory · Checkout · Releases · Fairness  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Assigned-Seat Reservation**  
...
Goal: reserve specific seats without conflicts at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to reserve specific seats without conflicts | Unrelated product surface |
| Core noun | `seat reservation` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | seat-level locks, map UX, atomic multi-seat hold, release paths | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **seat-level locks**.
2. Deliver a production-quality path for **map UX**.
3. Deliver a production-quality path for **atomic multi-seat hold**.
4. Deliver a production-quality path for **release paths**.
5. Durable `seat reservation` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | hold < 150–200ms |
...
1. User/system initiates flow to reserve specific seats without conflicts → validation → core `seat reservation` created.
...
|--------|----------|-----|------|--------|
| Seat hold ops/s | 10K | 100K | 1M | 10M |
...
> Design **Assigned-Seat Reservation** to reserve specific seats without conflicts, emphasizing seat-level locks, map UX, atomic multi-seat hold, release paths. Start from baseline ~10K seat hold ops/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 10K seat ops
Entities (order-of): venue seats
Write class focus: holds
Typical payload: seatmap diffs
...
Use seatmap diffs as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **seat bitmaps/rows**, admission control, and key sharding.
...
```text
select seats → atomic multi-seat conditional update → TTL → checkout
```
...
**Assigned-Seat Reservation** interview signal: speak fluently about **seat-level locks** and **map UX**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Assigned-Seat Reservation**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **seat-level locks** as a first-class subsystem.
2. Invest in **map UX** as a first-class subsystem.
3. Invest in **atomic multi-seat hold** as a first-class subsystem.
4. Invest in **release paths** as a first-class subsystem.
...
"For Assigned-Seat Reservation, I'd guarantee **seat-level locks** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Assigned-Seat Reservation?
...
For Assigned-Seat Reservation: emphasize seat-level locks, map UX, atomic multi-seat hold; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Assigned-Seat Reservation
baseline: {'Primary QPS': '10K seat ops', 'Entities': 'venue seats', 'Write QPS': 'holds', 'Payload': 'seatmap diffs', 'Hot structure': 'seat bitmaps/rows'}
