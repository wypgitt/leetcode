# System Design: Data-labeling platform
...
|-----------|----------|----------|
| Job | Design **Data-labeling platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Data-labeling platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Data-labeling platform.
...
Domain note: tailor 2–3 cases specifically to **Data-labeling platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Data-labeling platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Data-labeling platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Data-labeling platform".
...
Core components for **Data-labeling platform**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless datalabelingplatform Service]
...
**Failure modes checklist for Data-labeling platform**
...
### 5.4 Problem-specific deep notes for Data-labeling platform
...
1. **Invariants** for Data-labeling platform (what must never break).
...
| 16 | Which algorithms apply to Data-labeling platform? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Data-labeling platform**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `data-labeling-platform` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Data-labeling platform
...
| Business SLI for Data-labeling platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Data-labeling platform
...
*Enriched for interview drill · `data-labeling-platform`*
