# System Design: Reviews, Ratings, Voting, and Reviewer Rewards

> **Focus areas:** Reviews · Aggregates · Voting · Fraud · Rewards · Ranking impact · Moderation  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Reviews, Ratings, Voting, and Reviewer Rewards**  
...
Goal: capture ratings/reviews and reward quality at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to capture ratings/reviews and reward quality | Unrelated product surface |
| Core noun | `review` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | aggregate pipelines, vote fraud, moderation, reward ledger | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **aggregate pipelines**.
2. Deliver a production-quality path for **vote fraud**.
3. Deliver a production-quality path for **moderation**.
4. Deliver a production-quality path for **reward ledger**.
5. Durable `review` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | read p99 < 150ms; write < 300ms |
...
1. User/system initiates flow to capture ratings/reviews and reward quality → validation → core `review` created.
...
| Domain-specific contention | Serialize on review keys; lease/TTL patterns |
...
|--------|----------|-----|------|--------|
| Review writes/s | 500 | 5K | 50K | 500K |
...
> Design **Reviews, Ratings, Voting, and Reviewer Rewards** to capture ratings/reviews and reward quality, emphasizing aggregate pipelines, vote fraud, moderation, reward ledger. Start from baseline ~500 review writes/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 3K reads
Entities (order-of): reviews
Write class focus: 500 reviews
Typical payload: review text
...
Use review text as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **aggregates cache**, admission control, and key sharding.
...
```text
write review → moderate → update aggregates → optional reward → rank feed
```
...
**Reviews, Ratings, Voting, and Reviewer Rewards** interview signal: speak fluently about **aggregate pipelines** and **vote fraud**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Reviews, Ratings, Voting, and Reviewer Rewards**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **aggregate pipelines** as a first-class subsystem.
2. Invest in **vote fraud** as a first-class subsystem.
3. Invest in **moderation** as a first-class subsystem.
4. Invest in **reward ledger** as a first-class subsystem.
...
"For Reviews, Ratings, Voting, and Reviewer Rewards, I'd guarantee **aggregate pipelines** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Reviews, Ratings, Voting, and Reviewer Rewards?
...
For Reviews, Ratings, Voting, and Reviewer Rewards: emphasize aggregate pipelines, vote fraud, moderation; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Reviews, Ratings, Voting, and Reviewer Rewards
baseline: {'Primary QPS': '3K reads', 'Entities': 'reviews', 'Write QPS': '500 reviews', 'Payload': 'review text', 'Hot structure': 'aggregates cache'}
...
| Baseline | 500 Review writes/s |
| Invariant | no double-allocate / no silent money loss |
| Scale endgame | cell mesh + edge + tiered stores |

---

*End of Reviews, Ratings, Voting, and Reviewer Rewards system design prep doc.*
