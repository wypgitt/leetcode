...
|-----------|----------|----------|
| Job | Design **Expiring cache** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Expiring cache | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Expiring cache.
...
Domain note: tailor 2–3 cases specifically to **Expiring cache** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Expiring cache.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Expiring cache**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Expiring cache".
...
Core components for **Expiring cache**:
- **public API**
...
flowchart LR
  ...
  GW --> Svc[Stateless expiringcache Service]
...
**Failure modes checklist for Expiring cache**
...
### 5.4 Problem-specific deep notes for Expiring cache
...
1. **Invariants** for Expiring cache (what must never break).
...
| 16 | Which algorithms apply to Expiring cache? | Expect discussion of: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Expiring cache**: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping.
...
*Category: Lower-level and machine-coding system design · Slug: `expiring-cache` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Expiring cache
...
| Business SLI for Expiring cache | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Expiring cache
...
*Enriched for interview drill · `expiring-cache`*
