# System Design: Price tracking and price-change alerts
...
|-----------|----------|----------|
| Job | Design **Price tracking and price-change alerts** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Price tracking and price-change alerts | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Price tracking and price-change alerts.
...
Domain note: tailor 2–3 cases specifically to **Price tracking and price-change alerts** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Price tracking and price-change alerts.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Price tracking and price-change alerts**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Price tracking and price-change alerts".
...
Core components for **Price tracking and price-change alerts**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless pricetrackingalerts Service]
...
**Failure modes checklist for Price tracking and price-change alerts**
...
### 5.4 Problem-specific deep notes for Price tracking and price-change alerts
...
1. **Invariants** for Price tracking and price-change alerts (what must never break).
...
| 16 | Which algorithms apply to Price tracking and price-change alerts? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Price tracking and price-change alerts**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `price-tracking-alerts` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Price tracking and price-change alerts
...
| Business SLI for Price tracking and price-change alerts | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Price tracking and price-change alerts
...
*Enriched for interview drill · `price-tracking-alerts`*
