# System Design: Zero-downtime database migration
...
|-----------|----------|----------|
| Job | Design **Zero-downtime database migration** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Zero-downtime database migration | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Zero-downtime database migration.
...
Domain note: tailor 2–3 cases specifically to **Zero-downtime database migration** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Zero-downtime database migration.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Zero-downtime database migration**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Zero-downtime database migration".
...
Core components for **Zero-downtime database migration**:
- **API/control plane**
- **job scheduler**
- **artifact store**
- **runners/workers**
...
flowchart LR
  ...
  GW --> Svc[Stateless zerodowntimedatabase Service]
...
**Failure modes checklist for Zero-downtime database migration**
...
### 5.4 Problem-specific deep notes for Zero-downtime database migration
...
1. **Invariants** for Zero-downtime database migration (what must never break).
...
| 16 | Which algorithms apply to Zero-downtime database migration? | Expect discussion of: content-addressed storage, DAG scheduling, fair share / DRF, Merkle trees, copy-on-write. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Zero-downtime database migration**: content-addressed storage, DAG scheduling, fair share / DRF, Merkle trees, copy-on-write.
...
*Category: Developer tools and cloud platforms · Slug: `zero-downtime-database-migration` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Zero-downtime database migration
...
| Business SLI for Zero-downtime database migration | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Zero-downtime database migration
...
*Enriched for interview drill · `zero-downtime-database-migration`*
