# System Design: Inventory reservation
...
|-----------|----------|----------|
| Job | Design **Inventory reservation** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Inventory reservation | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Inventory reservation.
...
Domain note: tailor 2–3 cases specifically to **Inventory reservation** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Inventory reservation.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Inventory reservation**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Inventory reservation".
...
Core components for **Inventory reservation**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless inventoryreservation Service]
...
**Failure modes checklist for Inventory reservation**
...
### 5.4 Problem-specific deep notes for Inventory reservation
...
1. **Invariants** for Inventory reservation (what must never break).
...
| 16 | Which algorithms apply to Inventory reservation? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Inventory reservation**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `inventory-reservation` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Inventory reservation
...
| Business SLI for Inventory reservation | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Inventory reservation
...
*Enriched for interview drill · `inventory-reservation`*
