...
|-----------|----------|----------|
| Job | Design **Ad-click aggregator** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Ad-click aggregator | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Ad-click aggregator.
...
Domain note: tailor 2–3 cases specifically to **Ad-click aggregator** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Ad-click aggregator.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Ad-click aggregator**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Ad-click aggregator".
...
Core components for **Ad-click aggregator**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless adclickaggregator Service]
...
**Failure modes checklist for Ad-click aggregator**
...
### 5.4 Problem-specific deep notes for Ad-click aggregator
...
1. **Invariants** for Ad-click aggregator (what must never break).
...
| 16 | Which algorithms apply to Ad-click aggregator? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Ad-click aggregator**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `ad-click-aggregator` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Ad-click aggregator
...
| Business SLI for Ad-click aggregator | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Ad-click aggregator
...
*Enriched for interview drill · `ad-click-aggregator`*
