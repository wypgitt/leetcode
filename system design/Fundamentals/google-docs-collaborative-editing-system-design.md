...
|-----------|----------|----------|
| Job | Design **Google Docs collaborative editing** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Google Docs collaborative editing | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Google Docs collaborative editing.
...
Domain note: tailor 2–3 cases specifically to **Google Docs collaborative editing** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Google Docs collaborative editing.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Google Docs collaborative editing**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Google Docs collaborative editing".
...
Core components for **Google Docs collaborative editing**:
- **write API**
...
flowchart LR
  ...
  GW --> Svc[Stateless googledocscollaborat Service]
...
**Failure modes checklist for Google Docs collaborative editing**
...
### 5.4 Problem-specific deep notes for Google Docs collaborative editing
...
1. **Invariants** for Google Docs collaborative editing (what must never break).
...
| 16 | Which algorithms apply to Google Docs collaborative editing? | Expect discussion of: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Google Docs collaborative editing**: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat.
...
*Category: Social, communication, and collaboration · Slug: `google-docs-collaborative-editing` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix A — Collaborative editing specifics

### OT vs CRDT
OT: central ordering, smaller metadata, complex transform. CRDT: P2P-friendly, larger metadata, simpler merge. Many products use CRDT-like with central sequencer.

### Presence & carets
Ephemeral channel (WS); not durable. Docs: durable op log with snapshots for catch-up.

### Hot documents
Partition by doc_id; sticky WS gateway; snapshot every N ops; tombstone GC.

### Offline
Buffer ops; rebase/merge on reconnect; conflict UX for non-textual if needed.

## Appendix — Deep dive notes for Google Docs collaborative editing
...
| Business SLI for Google Docs collaborative editing | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Google Docs collaborative editing
...
*Enriched for interview drill · `google-docs-collaborative-editing`*
