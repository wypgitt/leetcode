# System Design: Model routing across providers
...
|-----------|----------|----------|
| Job | Design **Model routing across providers** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Model routing across providers | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Model routing across providers.
...
Domain note: tailor 2–3 cases specifically to **Model routing across providers** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Model routing across providers.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Model routing across providers**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Model routing across providers".
...
Core components for **Model routing across providers**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless modelroutingprovider Service]
...
**Failure modes checklist for Model routing across providers**
...
### 5.4 Problem-specific deep notes for Model routing across providers
...
1. **Invariants** for Model routing across providers (what must never break).
...
| 16 | Which algorithms apply to Model routing across providers? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Model routing across providers**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `model-routing-providers` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Model routing across providers
...
| Business SLI for Model routing across providers | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Model routing across providers
...
*Enriched for interview drill · `model-routing-providers`*
