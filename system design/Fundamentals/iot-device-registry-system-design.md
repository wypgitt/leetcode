# System Design: IoT device registry
...
|-----------|----------|----------|
| Job | Design **IoT device registry** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for IoT device registry | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for IoT device registry.
...
Domain note: tailor 2–3 cases specifically to **IoT device registry** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to IoT device registry.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **IoT device registry**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "IoT device registry".
...
Core components for **IoT device registry**:
- **device auth**
- **telemetry ingest**
- **command plane**
- **OTA rollout**
...
flowchart LR
  ...
  GW --> Svc[Stateless iotdeviceregistry Service]
...
**Failure modes checklist for IoT device registry**
...
### 5.4 Problem-specific deep notes for IoT device registry
...
1. **Invariants** for IoT device registry (what must never break).
...
| 16 | Which algorithms apply to IoT device registry? | Expect discussion of: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **IoT device registry**: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching.
...
*Category: Devices, mobile, edge, and IoT · Slug: `iot-device-registry` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for IoT device registry
...
| Business SLI for IoT device registry | Product truth |
...
- Cost model at 100×
- Abuse cases unique to IoT device registry
...
*Enriched for interview drill · `iot-device-registry`*
