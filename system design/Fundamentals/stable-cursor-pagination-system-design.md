# System Design: Stable cursor pagination
...
|-----------|----------|----------|
| Job | Design **Stable cursor pagination** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Stable cursor pagination | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Stable cursor pagination.
...
Domain note: tailor 2–3 cases specifically to **Stable cursor pagination** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Stable cursor pagination.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Stable cursor pagination**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Stable cursor pagination".
...
Core components for **Stable cursor pagination**:
- **public API**
...
flowchart LR
  ...
  GW --> Svc[Stateless stablecursorpaginati Service]
...
**Failure modes checklist for Stable cursor pagination**
...
### 5.4 Problem-specific deep notes for Stable cursor pagination
...
1. **Invariants** for Stable cursor pagination (what must never break).
...
| 16 | Which algorithms apply to Stable cursor pagination? | Expect discussion of: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Stable cursor pagination**: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping.
...
*Category: Lower-level and machine-coding system design · Slug: `stable-cursor-pagination` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Stable cursor pagination
...
| Business SLI for Stable cursor pagination | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Stable cursor pagination
...
*Enriched for interview drill · `stable-cursor-pagination`*
