# System Design: Online Auction

> **Focus areas:** Bids · Soft close · Leader election of price · Idempotency · Notifications · Fraud  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Online Auction**  
...
Goal: run correct high-contention auctions at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to run correct high-contention auctions | Unrelated product surface |
| Core noun | `bid` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | bid ordering, soft close, winner correctness, anti-sniping | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **bid ordering**.
2. Deliver a production-quality path for **soft close**.
3. Deliver a production-quality path for **winner correctness**.
4. Deliver a production-quality path for **anti-sniping**.
5. Durable `bid` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | bid ack < 100–150ms |
...
1. User/system initiates flow to run correct high-contention auctions → validation → core `bid` created.
...
> Design **Online Auction** to run correct high-contention auctions, emphasizing bid ordering, soft close, winner correctness, anti-sniping. Start from baseline ~5K bids/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 5K bids
Entities (order-of): auctions
Write class focus: 5K
Typical payload: bid
...
Use bid as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **auction leader row**, admission control, and key sharding.
...
```text
validate bid → CAS high mark → extend soft close → notify → settle winner
```
...
**Online Auction** interview signal: speak fluently about **bid ordering** and **soft close**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Online Auction**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **bid ordering** as a first-class subsystem.
2. Invest in **soft close** as a first-class subsystem.
3. Invest in **winner correctness** as a first-class subsystem.
4. Invest in **anti-sniping** as a first-class subsystem.
...
"For Online Auction, I'd guarantee **bid ordering** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Online Auction?
...
For Online Auction: emphasize bid ordering, soft close, winner correctness; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
baseline: {'Primary QPS': '5K bids', 'Entities': 'auctions', 'Write QPS': '5K', 'Payload': 'bid', 'Hot structure': 'auction leader row'}
...
*End of Online Auction system design prep doc.*
