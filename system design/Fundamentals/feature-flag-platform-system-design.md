<!-- Adapted into Fundamentals bank from Stripe/feature-flag-system-design.md for cross-company prep. -->

# System Design: Feature Flag Service

> **Focus areas:** Targeting · Rollouts · Stickiness · Kill switch  
...
|---|---|---|---|
| F1 | Flags? | Boolean multivariate | Typed variants |
| F2 | Targeting? | Rules on attrs | merchant_id env |
| F3 | Rollout? | % with stickiness | Hash subject |
| F4 | Kill switch? | Instant off | Global flag |
| F5 | Defaults? | Safe on outage | Money OFF |
| F6 | Audit? | Prod changes | Who when why |
| F7 | Cache? | Edge+local | Version poll |
| F8 | Eval latency? | Sub-ms cached | Budget rule eval |
| F9 | Tenant scope? | Per merchant | No cross bleed |
| F10 | SDK? | Server-side enforce | Not client only security |
| F11 | Testing? | Override in staging | Separate env |
| F12 | Version? | Flag config version | Monotonic |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Feature Flag Service
4. Admin recon/replay hooks for Feature Flag Service
...
1. Flags? → Boolean multivariate → durable write + audit
2. Targeting? → Rules on attrs → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Defaults? → safe degradation documented
...
> Design feature flags ~500K eval/s sub-ms cached safe money defaults.
...
### 3.11 Domain model (Feature Flag Service)
...
### 4.1 End-to-end (Feature Flag Service)
...
```text
API Edge → Feature Flag Service → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of feature flag service system design.*

### 8.19 Feature Flag Service — rollout checklist
