# System Design: LRU or LFU cache
...
|-----------|----------|----------|
| Job | Design **LRU or LFU cache** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for LRU or LFU cache | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for LRU or LFU cache.
...
Domain note: tailor 2–3 cases specifically to **LRU or LFU cache** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to LRU or LFU cache.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **LRU or LFU cache**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "LRU or LFU cache".
...
Core components for **LRU or LFU cache**:
- **public API**
...
flowchart LR
  ...
  GW --> Svc[Stateless lrulfucache Service]
...
**Failure modes checklist for LRU or LFU cache**
...
### 5.4 Problem-specific deep notes for LRU or LFU cache
...
1. **Invariants** for LRU or LFU cache (what must never break).
...
| 16 | Which algorithms apply to LRU or LFU cache? | Expect discussion of: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **LRU or LFU cache**: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping.
...
*Category: Lower-level and machine-coding system design · Slug: `lru-lfu-cache` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix A — LRU/LFU implementation specifics

### LRU
HashMap + doubly linked list; O(1) get/put. Thread safety: single mutex vs sharded locks.

### LFU
Frequency lists (O(1) with freq buckets) or approx with aging. TinyLFU / W-TinyLFU for production.

### Memory
Bound by entries or bytes; weight-based eviction; avoid unbounded stale maps.

## Appendix — Deep dive notes for LRU or LFU cache
...
| Business SLI for LRU or LFU cache | Product truth |
...
- Cost model at 100×
- Abuse cases unique to LRU or LFU cache
...
*Enriched for interview drill · `lru-lfu-cache`*
