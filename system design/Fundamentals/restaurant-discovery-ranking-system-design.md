# System Design: Restaurant Discovery and Ranking

> **Focus areas:** Feed ranking · Geo · Availability · Personalization · Diversity · Latency · Experimentation  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Restaurant Discovery and Ranking**  
...
Goal: rank restaurants for discovery feeds at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to rank restaurants for discovery feeds | Unrelated product surface |
| Core noun | `restaurant impression` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | geo+availability recall, LTR ranking, diversity, open/busy signals | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **geo+availability recall**.
2. Deliver a production-quality path for **LTR ranking**.
3. Deliver a production-quality path for **diversity**.
4. Deliver a production-quality path for **open/busy signals**.
5. Durable `restaurant impression` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | feed p99 < 200ms |
...
1. User/system initiates flow to rank restaurants for discovery feeds → validation → core `restaurant impression` created.
...
| Domain-specific contention | Serialize on restaurant impression keys; lease/TTL patterns |
...
> Design **Restaurant Discovery and Ranking** to rank restaurants for discovery feeds, emphasizing geo+availability recall, LTR ranking, diversity, open/busy signals. Start from baseline ~8K feed qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 8K feed
Entities (order-of): 1M restaurants/region
Write class focus: availability 5K
Typical payload: feed page 10KB
...
Use feed page 10KB as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **geo recall + LTR**, admission control, and key sharding.
...
```text
geo+hours recall → filters → LTR → diversity → page
```
...
**Restaurant Discovery and Ranking** interview signal: speak fluently about **geo+availability recall** and **LTR ranking**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Restaurant Discovery and Ranking**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **geo+availability recall** as a first-class subsystem.
2. Invest in **LTR ranking** as a first-class subsystem.
3. Invest in **diversity** as a first-class subsystem.
4. Invest in **open/busy signals** as a first-class subsystem.
...
"For Restaurant Discovery and Ranking, I'd guarantee **geo+availability recall** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Restaurant Discovery and Ranking?
...
For Restaurant Discovery and Ranking: emphasize geo+availability recall, LTR ranking, diversity; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Restaurant Discovery and Ranking
baseline: {'Primary QPS': '8K feed', 'Entities': '1M restaurants/region', 'Write QPS': 'availability 5K', 'Payload': 'feed page 10KB', 'Hot structure': 'geo recall + LTR'}
