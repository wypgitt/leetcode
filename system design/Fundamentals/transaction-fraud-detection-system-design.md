# System Design: Transaction fraud detection
...
|-----------|----------|----------|
| Job | Design **Transaction fraud detection** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Transaction fraud detection | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Transaction fraud detection.
...
Domain note: tailor 2–3 cases specifically to **Transaction fraud detection** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Transaction fraud detection.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Transaction fraud detection**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Transaction fraud detection".
...
Core components for **Transaction fraud detection**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless transactionfrauddete Service]
...
**Failure modes checklist for Transaction fraud detection**
...
### 5.4 Problem-specific deep notes for Transaction fraud detection
...
1. **Invariants** for Transaction fraud detection (what must never break).
...
| 16 | Which algorithms apply to Transaction fraud detection? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Transaction fraud detection**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `transaction-fraud-detection` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Transaction fraud detection
...
| Business SLI for Transaction fraud detection | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Transaction fraud detection
...
*Enriched for interview drill · `transaction-fraud-detection`*
