# System Design: Recurring invoicing
...
|-----------|----------|----------|
| Job | Design **Recurring invoicing** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Recurring invoicing | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Recurring invoicing.
...
Domain note: tailor 2–3 cases specifically to **Recurring invoicing** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Recurring invoicing.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Recurring invoicing**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Recurring invoicing".
...
Core components for **Recurring invoicing**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless recurringinvoicing Service]
...
**Failure modes checklist for Recurring invoicing**
...
### 5.4 Problem-specific deep notes for Recurring invoicing
...
1. **Invariants** for Recurring invoicing (what must never break).
...
| 16 | Which algorithms apply to Recurring invoicing? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Recurring invoicing**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `recurring-invoicing` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Recurring invoicing
...
| Business SLI for Recurring invoicing | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Recurring invoicing
...
*Enriched for interview drill · `recurring-invoicing`*
