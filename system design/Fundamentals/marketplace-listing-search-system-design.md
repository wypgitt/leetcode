# System Design: Marketplace Listing and Search

> **Focus areas:** Listing CRUD · Search index · Geo+filters · Ranking · Freshness · Photos  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Marketplace Listing and Search**  
...
Goal: publish listings and power search at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to publish listings and power search | Unrelated product surface |
| Core noun | `listing document` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | index pipeline, geo+facet filters, ranking, photo CDN | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **index pipeline**.
2. Deliver a production-quality path for **geo+facet filters**.
3. Deliver a production-quality path for **ranking**.
4. Deliver a production-quality path for **photo CDN**.
5. Durable `listing document` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | search p99 < 200ms |
...
1. User/system initiates flow to publish listings and power search → validation → core `listing document` created.
...
| Domain-specific contention | Serialize on listing document keys; lease/TTL patterns |
...
> Design **Marketplace Listing and Search** to publish listings and power search, emphasizing index pipeline, geo+facet filters, ranking, photo CDN. Start from baseline ~8K search qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 8K search
Entities (order-of): 10M listings
Write class focus: 1K index updates
Typical payload: 8 KB
...
Use 8 KB as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **search cluster**, admission control, and key sharding.
...
```text
CRUD → CDC → analyze/index → query rewrite → retrieve → rank
```
...
**Marketplace Listing and Search** interview signal: speak fluently about **index pipeline** and **geo+facet filters**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Marketplace Listing and Search**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **index pipeline** as a first-class subsystem.
2. Invest in **geo+facet filters** as a first-class subsystem.
3. Invest in **ranking** as a first-class subsystem.
4. Invest in **photo CDN** as a first-class subsystem.
...
"For Marketplace Listing and Search, I'd guarantee **index pipeline** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Marketplace Listing and Search?
...
For Marketplace Listing and Search: emphasize index pipeline, geo+facet filters, ranking; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Marketplace Listing and Search
baseline: {'Primary QPS': '8K search', 'Entities': '10M listings', 'Write QPS': '1K index updates', 'Payload': '8 KB', 'Hot structure': 'search cluster'}
