# System Design: Approval-workflow engine
...
|-----------|----------|----------|
| Job | Design **Approval-workflow engine** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Approval-workflow engine | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Approval-workflow engine.
...
Domain note: tailor 2–3 cases specifically to **Approval-workflow engine** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Approval-workflow engine.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Approval-workflow engine**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Approval-workflow engine".
...
Core components for **Approval-workflow engine**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless approvalworkflowengi Service]
...
**Failure modes checklist for Approval-workflow engine**
...
### 5.4 Problem-specific deep notes for Approval-workflow engine
...
1. **Invariants** for Approval-workflow engine (what must never break).
...
| 16 | Which algorithms apply to Approval-workflow engine? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Approval-workflow engine**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `approval-workflow-engine` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Approval-workflow engine
...
| Business SLI for Approval-workflow engine | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Approval-workflow engine
...
*Enriched for interview drill · `approval-workflow-engine`*
