...
|-----------|----------|----------|
| Job | Design **Vector database** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Vector database | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Vector database.
...
Domain note: tailor 2–3 cases specifically to **Vector database** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Vector database.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Vector database**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Vector database".
...
Core components for **Vector database**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless vectordatabase Service]
...
**Failure modes checklist for Vector database**
...
### 5.4 Problem-specific deep notes for Vector database
...
1. **Invariants** for Vector database (what must never break).
...
| 16 | Which algorithms apply to Vector database? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Vector database**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `vector-database` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Vector database
...
| Business SLI for Vector database | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Vector database
...
*Enriched for interview drill · `vector-database`*
