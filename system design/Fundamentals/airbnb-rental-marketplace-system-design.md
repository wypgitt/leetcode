# System Design: Airbnb or Rental Marketplace

> **Focus areas:** Listings · Search · Calendar availability · Booking · Payments · Trust · Messaging  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Airbnb or Rental Marketplace**  
...
Goal: operate a short-term rental marketplace at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to operate a short-term rental marketplace | Unrelated product surface |
| Core noun | `stay booking` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | calendar contention, search+availability, booking SM, trust/safety | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **calendar contention**.
2. Deliver a production-quality path for **search+availability**.
3. Deliver a production-quality path for **booking SM**.
4. Deliver a production-quality path for **trust/safety**.
5. Durable `stay booking` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | search < 300ms; book < 500ms |
...
1. User/system initiates flow to operate a short-term rental marketplace → validation → core `stay booking` created.
...
| Domain-specific contention | Serialize on stay booking keys; lease/TTL patterns |
...
> Design **Airbnb or Rental Marketplace** to operate a short-term rental marketplace, emphasizing calendar contention, search+availability, booking SM, trust/safety. Start from baseline ~5K search qps and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 5K search
Entities (order-of): 5M listings
Write class focus: booking 200
Typical payload: search results
...
Use search results as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **search+calendar**, admission control, and key sharding.
...
```text
search filter by availability → hold nights → pay → confirm / release
```
...
**Airbnb or Rental Marketplace** interview signal: speak fluently about **calendar contention** and **search+availability**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Airbnb or Rental Marketplace**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **calendar contention** as a first-class subsystem.
2. Invest in **search+availability** as a first-class subsystem.
3. Invest in **booking SM** as a first-class subsystem.
4. Invest in **trust/safety** as a first-class subsystem.
...
"For Airbnb or Rental Marketplace, I'd guarantee **calendar contention** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Airbnb or Rental Marketplace?
...
For Airbnb or Rental Marketplace: emphasize calendar contention, search+availability, booking SM; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Airbnb or Rental Marketplace
baseline: {'Primary QPS': '5K search', 'Entities': '5M listings', 'Write QPS': 'booking 200', 'Payload': 'search results', 'Hot structure': 'search+calendar'}
