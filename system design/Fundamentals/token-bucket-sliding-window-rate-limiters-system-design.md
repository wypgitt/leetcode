# System Design: Token-bucket and sliding-window rate limiters
...
|-----------|----------|----------|
| Job | Design **Token-bucket and sliding-window rate limiters** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Token-bucket and sliding-window rate limiters | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Token-bucket and sliding-window rate limiters.
...
Domain note: tailor 2–3 cases specifically to **Token-bucket and sliding-window rate limiters** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Token-bucket and sliding-window rate limiters.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Token-bucket and sliding-window rate limiters**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Token-bucket and sliding-window rate limiters".
...
Core components for **Token-bucket and sliding-window rate limiters**:
- **public API**
...
flowchart LR
  ...
  GW --> Svc[Stateless tokenbucketslidingwi Service]
...
**Failure modes checklist for Token-bucket and sliding-window rate limiters**
...
### 5.4 Problem-specific deep notes for Token-bucket and sliding-window rate limiters
...
1. **Invariants** for Token-bucket and sliding-window rate limiters (what must never break).
...
| 16 | Which algorithms apply to Token-bucket and sliding-window rate limiters? | Expect discussion of: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Token-bucket and sliding-window rate limiters**: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping.
...
*Category: Lower-level and machine-coding system design · Slug: `token-bucket-sliding-window-rate-limiters` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Token-bucket and sliding-window rate limiters
...
| Business SLI for Token-bucket and sliding-window rate limiters | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Token-bucket and sliding-window rate limiters
...
*Enriched for interview drill · `token-bucket-sliding-window-rate-limiters`*
