# System Design: Food-Order Tracking

> **Focus areas:** Order state machine · Live maps · Push · Merchant/dasher events · ETA refresh  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Food-Order Tracking**  
...
Goal: track order progress in realtime at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to track order progress in realtime | Unrelated product surface |
| Core noun | `order track session` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | state machine, event fanout, map tracking, ETA revisions | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **state machine**.
2. Deliver a production-quality path for **event fanout**.
3. Deliver a production-quality path for **map tracking**.
4. Deliver a production-quality path for **ETA revisions**.
5. Durable `order track session` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | event visible < 1–2s |
...
| N5 | Freshness? | Live location/ETA/status SLOs | state < 1–2s |
...
1. User/system initiates flow to track order progress in realtime → validation → core `order track session` created.
...
| Domain-specific contention | Serialize on order track session keys; lease/TTL patterns |
...
|--------|----------|-----|------|--------|
| Track events/s | 20K | 200K | 2M | 20M |
...
> Design **Food-Order Tracking** to track order progress in realtime, emphasizing state machine, event fanout, map tracking, ETA revisions. Start from baseline ~20K track events/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 20K events
Entities (order-of): open orders
Write class focus: 20K
Typical payload: track push 1KB
...
Use track push 1KB as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **order event log**, admission control, and key sharding.
...
```text
append event → SM transition → notify subscribers → refresh ETA
```
...
**Food-Order Tracking** interview signal: speak fluently about **state machine** and **event fanout**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Food-Order Tracking**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **state machine** as a first-class subsystem.
2. Invest in **event fanout** as a first-class subsystem.
3. Invest in **map tracking** as a first-class subsystem.
4. Invest in **ETA revisions** as a first-class subsystem.
...
"For Food-Order Tracking, I'd guarantee **state machine** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Food-Order Tracking?
...
For Food-Order Tracking: emphasize state machine, event fanout, map tracking; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Food-Order Tracking
baseline: {'Primary QPS': '20K events', 'Entities': 'open orders', 'Write QPS': '20K', 'Payload': 'track push 1KB', 'Hot structure': 'order event log'}
