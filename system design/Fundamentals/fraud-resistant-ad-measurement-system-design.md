# System Design: Fraud-resistant ad measurement
...
|-----------|----------|----------|
| Job | Design **Fraud-resistant ad measurement** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Fraud-resistant ad measurement | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Fraud-resistant ad measurement.
...
Domain note: tailor 2–3 cases specifically to **Fraud-resistant ad measurement** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Fraud-resistant ad measurement.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Fraud-resistant ad measurement**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Fraud-resistant ad measurement".
...
Core components for **Fraud-resistant ad measurement**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless fraudresistantadmeas Service]
...
**Failure modes checklist for Fraud-resistant ad measurement**
...
### 5.4 Problem-specific deep notes for Fraud-resistant ad measurement
...
1. **Invariants** for Fraud-resistant ad measurement (what must never break).
...
| 16 | Which algorithms apply to Fraud-resistant ad measurement? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Fraud-resistant ad measurement**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `fraud-resistant-ad-measurement` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Fraud-resistant ad measurement
...
| Business SLI for Fraud-resistant ad measurement | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Fraud-resistant ad measurement
...
*Enriched for interview drill · `fraud-resistant-ad-measurement`*
