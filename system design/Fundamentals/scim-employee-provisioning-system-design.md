# System Design: SCIM employee provisioning
...
|-----------|----------|----------|
| Job | Design **SCIM employee provisioning** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for SCIM employee provisioning | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for SCIM employee provisioning.
...
Domain note: tailor 2–3 cases specifically to **SCIM employee provisioning** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to SCIM employee provisioning.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **SCIM employee provisioning**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "SCIM employee provisioning".
...
Core components for **SCIM employee provisioning**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless scimemployeeprovisio Service]
...
**Failure modes checklist for SCIM employee provisioning**
...
### 5.4 Problem-specific deep notes for SCIM employee provisioning
...
1. **Invariants** for SCIM employee provisioning (what must never break).
...
| 16 | Which algorithms apply to SCIM employee provisioning? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **SCIM employee provisioning**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `scim-employee-provisioning` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for SCIM employee provisioning
...
| Business SLI for SCIM employee provisioning | Product truth |
...
- Cost model at 100×
- Abuse cases unique to SCIM employee provisioning
...
*Enriched for interview drill · `scim-employee-provisioning`*
