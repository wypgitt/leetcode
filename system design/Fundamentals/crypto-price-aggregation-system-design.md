# System Design: Crypto-price aggregation across inconsistent providers

> **Focus areas:** ledger · idempotency · reconciliation · risk · settlement
> **Style:** End-to-end design with progressive scale (10× → 100× → 1,000×)
> **Category:** Payments, banking, fintech, and crypto
...
|-----------|----------|----------|
| Job | Design **Crypto-price aggregation across inconsistent providers** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Crypto-price aggregation across inconsistent providers | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Crypto-price aggregation across inconsistent providers.
...
Domain note: tailor 2–3 cases specifically to **Crypto-price aggregation across inconsistent providers** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Crypto-price aggregation across inconsistent providers.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Crypto-price aggregation across inconsistent providers**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Crypto-price aggregation across inconsistent providers".
...
Section family `07-payments-fintech` emphasizes: ledger · idempotency · reconciliation · risk · settlement.
...
Core components for **Crypto-price aggregation across inconsistent providers**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
**Algorithms / structures likely discussed:** double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features

### 3.5 Deal-breakers for this problem family
- mutable balances without journal
- no idempotency keys
...
flowchart LR
  ...
  GW --> Svc[Stateless cryptopriceaggregati Service]
...
**Failure modes checklist for Crypto-price aggregation across inconsistent providers**
...
- Key algorithms: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features
...
### 5.4 Problem-specific deep notes for Crypto-price aggregation across inconsistent providers
...
1. **Invariants** for Crypto-price aggregation across inconsistent providers (what must never break).
...
| 16 | Which algorithms apply to Crypto-price aggregation across inconsistent providers? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Crypto-price aggregation across inconsistent providers**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `crypto-price-aggregation` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Crypto-price aggregation across inconsistent providers
...
| Business SLI for Crypto-price aggregation across inconsistent providers | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Crypto-price aggregation across inconsistent providers
...
*Enriched for interview drill · `crypto-price-aggregation`*
