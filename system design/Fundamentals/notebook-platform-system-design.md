# System Design: Notebook platform
...
|-----------|----------|----------|
| Job | Design **Notebook platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Notebook platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Notebook platform.
...
Domain note: tailor 2–3 cases specifically to **Notebook platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Notebook platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Notebook platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Notebook platform".
...
Core components for **Notebook platform**:
- **API/control plane**
- **job scheduler**
- **artifact store**
- **runners/workers**
...
flowchart LR
  ...
  GW --> Svc[Stateless notebookplatform Service]
...
**Failure modes checklist for Notebook platform**
...
### 5.4 Problem-specific deep notes for Notebook platform
...
1. **Invariants** for Notebook platform (what must never break).
...
| 16 | Which algorithms apply to Notebook platform? | Expect discussion of: content-addressed storage, DAG scheduling, fair share / DRF, Merkle trees, copy-on-write. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Notebook platform**: content-addressed storage, DAG scheduling, fair share / DRF, Merkle trees, copy-on-write.
...
*Category: Developer tools and cloud platforms · Slug: `notebook-platform` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Notebook platform
...
| Business SLI for Notebook platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Notebook platform
...
*Enriched for interview drill · `notebook-platform`*
