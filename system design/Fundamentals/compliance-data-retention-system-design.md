# System Design: Compliance data retention
...
|-----------|----------|----------|
| Job | Design **Compliance data retention** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Compliance data retention | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Compliance data retention.
...
Domain note: tailor 2–3 cases specifically to **Compliance data retention** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Compliance data retention.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Compliance data retention**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Compliance data retention".
...
Core components for **Compliance data retention**:
- **agents/collectors**
- **ingest pipeline**
- **TSDB/log index**
- **alert evaluator**
...
flowchart LR
  ...
  GW --> Svc[Stateless compliancedataretent Service]
...
**Failure modes checklist for Compliance data retention**
...
### 5.4 Problem-specific deep notes for Compliance data retention
...
1. **Invariants** for Compliance data retention (what must never break).
...
| 16 | Which algorithms apply to Compliance data retention? | Expect discussion of: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Compliance data retention**: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate.
...
*Category: Observability, reliability, security, and operations · Slug: `compliance-data-retention` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Compliance data retention
...
| Business SLI for Compliance data retention | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Compliance data retention
...
*Enriched for interview drill · `compliance-data-retention`*
