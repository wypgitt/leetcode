...
|-----------|----------|----------|
| Job | Design **Checkout** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Checkout | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Checkout.
...
Domain note: tailor 2–3 cases specifically to **Checkout** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Checkout.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Checkout**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Checkout".
...
flowchart LR
  ...
  GW --> Svc[Stateless checkout Service]
...
**Failure modes checklist for Checkout**
...
### 5.4 Problem-specific deep notes for Checkout
...
1. **Invariants** for Checkout (what must never break).
...
| 16 | Which algorithms apply to Checkout? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Checkout**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `checkout` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Checkout
...
| Business SLI for Checkout | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Checkout
...
*Enriched for interview drill · `checkout`*
