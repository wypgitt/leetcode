# System Design: Hybrid lexical and semantic retrieval
...
|-----------|----------|----------|
| Job | Design **Hybrid lexical and semantic retrieval** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Hybrid lexical and semantic retrieval | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Hybrid lexical and semantic retrieval.
...
Domain note: tailor 2–3 cases specifically to **Hybrid lexical and semantic retrieval** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Hybrid lexical and semantic retrieval.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Hybrid lexical and semantic retrieval**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Hybrid lexical and semantic retrieval".
...
Core components for **Hybrid lexical and semantic retrieval**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless hybridlexicalsemanti Service]
...
**Failure modes checklist for Hybrid lexical and semantic retrieval**
...
### 5.4 Problem-specific deep notes for Hybrid lexical and semantic retrieval
...
1. **Invariants** for Hybrid lexical and semantic retrieval (what must never break).
...
| 16 | Which algorithms apply to Hybrid lexical and semantic retrieval? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Hybrid lexical and semantic retrieval**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `hybrid-lexical-semantic-retrieval` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Hybrid lexical and semantic retrieval
...
| Business SLI for Hybrid lexical and semantic retrieval | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Hybrid lexical and semantic retrieval
...
*Enriched for interview drill · `hybrid-lexical-semantic-retrieval`*
