# System Design: Approval and evidence workflows
...
|-----------|----------|----------|
| Job | Design **Approval and evidence workflows** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Approval and evidence workflows | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Approval and evidence workflows.
...
Domain note: tailor 2–3 cases specifically to **Approval and evidence workflows** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Approval and evidence workflows.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Approval and evidence workflows**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Approval and evidence workflows".
...
Core components for **Approval and evidence workflows**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless approvalevidencework Service]
...
**Failure modes checklist for Approval and evidence workflows**
...
### 5.4 Problem-specific deep notes for Approval and evidence workflows
...
1. **Invariants** for Approval and evidence workflows (what must never break).
...
| 16 | Which algorithms apply to Approval and evidence workflows? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Approval and evidence workflows**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `approval-evidence-workflows` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Approval and evidence workflows
...
| Business SLI for Approval and evidence workflows | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Approval and evidence workflows
...
*Enriched for interview drill · `approval-evidence-workflows`*
