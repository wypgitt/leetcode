# System Design: Electronic signatures
...
|-----------|----------|----------|
| Job | Design **Electronic signatures** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Electronic signatures | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Electronic signatures.
...
Domain note: tailor 2–3 cases specifically to **Electronic signatures** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Electronic signatures.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Electronic signatures**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Electronic signatures".
...
Core components for **Electronic signatures**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless electronicsignatures Service]
...
**Failure modes checklist for Electronic signatures**
...
### 5.4 Problem-specific deep notes for Electronic signatures
...
1. **Invariants** for Electronic signatures (what must never break).
...
| 16 | Which algorithms apply to Electronic signatures? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Electronic signatures**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `electronic-signatures` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Electronic signatures
...
| Business SLI for Electronic signatures | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Electronic signatures
...
*Enriched for interview drill · `electronic-signatures`*
