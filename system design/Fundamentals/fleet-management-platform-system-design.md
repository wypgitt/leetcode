# System Design: Fleet-Management Platform

> **Focus areas:** Vehicles · Telemetry · Maintenance · Assignment · Geofences · Compliance · Alerts  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Fleet-Management Platform**  
...
Goal: manage vehicle fleets and telemetry at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to manage vehicle fleets and telemetry | Unrelated product surface |
| Core noun | `fleet asset` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | telemetry ingest, asset registry, geofence alerts, maintenance | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **telemetry ingest**.
2. Deliver a production-quality path for **asset registry**.
3. Deliver a production-quality path for **geofence alerts**.
4. Deliver a production-quality path for **maintenance**.
5. Durable `fleet asset` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | telem ingest < 50ms; alert < 5s |
...
| N5 | Freshness? | Live location/ETA/status SLOs | map refresh 5–15s; critical alerts faster |
...
1. User/system initiates flow to manage vehicle fleets and telemetry → validation → core `fleet asset` created.
...
| Domain-specific contention | Serialize on fleet asset keys; lease/TTL patterns |
...
> Design **Fleet-Management Platform** to manage vehicle fleets and telemetry, emphasizing telemetry ingest, asset registry, geofence alerts, maintenance. Start from baseline ~100K telemetry points/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 100K telemetry
Entities (order-of): vehicles
Write class focus: 100K
Typical payload: telem 200B
...
Use telem 200B as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **fleet geo + rules**, admission control, and key sharding.
...
```text
ingest telem → rules/geofence → update live map → maintenance triggers
```
...
**Fleet-Management Platform** interview signal: speak fluently about **telemetry ingest** and **asset registry**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Fleet-Management Platform**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **telemetry ingest** as a first-class subsystem.
2. Invest in **asset registry** as a first-class subsystem.
3. Invest in **geofence alerts** as a first-class subsystem.
4. Invest in **maintenance** as a first-class subsystem.
...
"For Fleet-Management Platform, I'd guarantee **telemetry ingest** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Fleet-Management Platform?
...
For Fleet-Management Platform: emphasize telemetry ingest, asset registry, geofence alerts; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Fleet-Management Platform
baseline: {'Primary QPS': '100K telemetry', 'Entities': 'vehicles', 'Write QPS': '100K', 'Payload': 'telem 200B', 'Hot structure': 'fleet geo + rules'}
