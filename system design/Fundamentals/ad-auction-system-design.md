...
|-----------|----------|----------|
| Job | Design **Ad auction** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Ad auction | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Ad auction.
...
Domain note: tailor 2–3 cases specifically to **Ad auction** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Ad auction.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Ad auction**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Ad auction".
...
Core components for **Ad auction**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless adauction Service]
...
**Failure modes checklist for Ad auction**
...
### 5.4 Problem-specific deep notes for Ad auction
...
1. **Invariants** for Ad auction (what must never break).
...
| 16 | Which algorithms apply to Ad auction? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Ad auction**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `ad-auction` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Ad auction
...
| Business SLI for Ad auction | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Ad auction
...
*Enriched for interview drill · `ad-auction`*
