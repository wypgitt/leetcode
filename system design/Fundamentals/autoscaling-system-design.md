...
|-----------|----------|----------|
| Job | Design **Autoscaling** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Autoscaling | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Autoscaling.
...
Domain note: tailor 2–3 cases specifically to **Autoscaling** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Autoscaling.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Autoscaling**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Autoscaling".
...
flowchart LR
  ...
  GW --> Svc[Stateless autoscaling Service]
...
**Failure modes checklist for Autoscaling**
...
### 5.4 Problem-specific deep notes for Autoscaling
...
1. **Invariants** for Autoscaling (what must never break).
...
| 16 | Which algorithms apply to Autoscaling? | Expect discussion of: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Autoscaling**: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate.
...
*Category: Observability, reliability, security, and operations · Slug: `autoscaling` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Autoscaling
...
| Business SLI for Autoscaling | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Autoscaling
...
*Enriched for interview drill · `autoscaling`*
