# System Design: Peer-to-peer payments
...
|-----------|----------|----------|
| Job | Design **Peer-to-peer payments** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Peer-to-peer payments | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Peer-to-peer payments.
...
Domain note: tailor 2–3 cases specifically to **Peer-to-peer payments** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Peer-to-peer payments.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Peer-to-peer payments**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Peer-to-peer payments".
...
Core components for **Peer-to-peer payments**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless peertopeerpayments Service]
...
**Failure modes checklist for Peer-to-peer payments**
...
### 5.4 Problem-specific deep notes for Peer-to-peer payments
...
1. **Invariants** for Peer-to-peer payments (what must never break).
...
| 16 | Which algorithms apply to Peer-to-peer payments? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Peer-to-peer payments**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `peer-to-peer-payments` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Peer-to-peer payments
...
| Business SLI for Peer-to-peer payments | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Peer-to-peer payments
...
*Enriched for interview drill · `peer-to-peer-payments`*
