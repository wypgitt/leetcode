# System Design: Ranking or recommendation-serving platform
...
|-----------|----------|----------|
| Job | Design **Ranking or recommendation-serving platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Ranking or recommendation-serving platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Ranking or recommendation-serving platform.
...
Domain note: tailor 2–3 cases specifically to **Ranking or recommendation-serving platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Ranking or recommendation-serving platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Ranking or recommendation-serving platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Ranking or recommendation-serving platform".
...
Core components for **Ranking or recommendation-serving platform**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless rankingrecommendatio Service]
...
**Failure modes checklist for Ranking or recommendation-serving platform**
...
### 5.4 Problem-specific deep notes for Ranking or recommendation-serving platform
...
1. **Invariants** for Ranking or recommendation-serving platform (what must never break).
...
| 16 | Which algorithms apply to Ranking or recommendation-serving platform? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Ranking or recommendation-serving platform**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `ranking-recommendation-serving` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Ranking or recommendation-serving platform
...
| Business SLI for Ranking or recommendation-serving platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Ranking or recommendation-serving platform
...
*Enriched for interview drill · `ranking-recommendation-serving`*
