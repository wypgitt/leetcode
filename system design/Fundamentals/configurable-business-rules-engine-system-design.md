# System Design: Configurable business-rules engine
...
|-----------|----------|----------|
| Job | Design **Configurable business-rules engine** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Configurable business-rules engine | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Configurable business-rules engine.
...
Domain note: tailor 2–3 cases specifically to **Configurable business-rules engine** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Configurable business-rules engine.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Configurable business-rules engine**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Configurable business-rules engine".
...
Core components for **Configurable business-rules engine**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless configurablebusiness Service]
...
**Failure modes checklist for Configurable business-rules engine**
...
### 5.4 Problem-specific deep notes for Configurable business-rules engine
...
1. **Invariants** for Configurable business-rules engine (what must never break).
...
| 16 | Which algorithms apply to Configurable business-rules engine? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Configurable business-rules engine**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `configurable-business-rules-engine` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Configurable business-rules engine
...
| Business SLI for Configurable business-rules engine | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Configurable business-rules engine
...
*Enriched for interview drill · `configurable-business-rules-engine`*
