# System Design: Dispatch Across Geographic Regions

> **Focus areas:** City cells · Region routing · Cross-border · Failover · Consistency · Cell ownership  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Dispatch Across Geographic Regions**  
...
Goal: route and own dispatch by geography at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to route and own dispatch by geography | Unrelated product surface |
| Core noun | `dispatch cell` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | cell ownership, cross-region handoff, blast radius, directory service | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **cell ownership**.
2. Deliver a production-quality path for **cross-region handoff**.
3. Deliver a production-quality path for **blast radius**.
4. Deliver a production-quality path for **directory service**.
5. Durable `dispatch cell` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | directory lookup < 20ms |
...
1. User/system initiates flow to route and own dispatch by geography → validation → core `dispatch cell` created.
...
| Domain-specific contention | Serialize on dispatch cell keys; lease/TTL patterns |
...
|--------|----------|-----|------|--------|
| Cities/cells | 200 | 2K | 20K | 200K microcells |
...
> Design **Dispatch Across Geographic Regions** to route and own dispatch by geography, emphasizing cell ownership, cross-region handoff, blast radius, directory service. Start from baseline ~200 cities/cells and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 2K dispatch
Entities (order-of): 200 city cells
Write class focus: ownership heartbeats
Typical payload: dispatch msgs
...
Use dispatch msgs as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **cell leader**, admission control, and key sharding.
...
```text
geo→cell directory → cell-owned matching → handoff protocol on borders
```
...
**Dispatch Across Geographic Regions** interview signal: speak fluently about **cell ownership** and **cross-region handoff**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Dispatch Across Geographic Regions**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **cell ownership** as a first-class subsystem.
2. Invest in **cross-region handoff** as a first-class subsystem.
3. Invest in **blast radius** as a first-class subsystem.
4. Invest in **directory service** as a first-class subsystem.
...
"For Dispatch Across Geographic Regions, I'd guarantee **cell ownership** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Dispatch Across Geographic Regions?
...
For Dispatch Across Geographic Regions: emphasize cell ownership, cross-region handoff, blast radius; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Dispatch Across Geographic Regions
baseline: {'Primary QPS': '2K dispatch', 'Entities': '200 city cells', 'Write QPS': 'ownership heartbeats', 'Payload': 'dispatch msgs', 'Hot structure': 'cell leader'}
