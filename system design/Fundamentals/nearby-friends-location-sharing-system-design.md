# System Design: Nearby-Friends Location Sharing

> **Focus areas:** Live location · Privacy · Friend graph · Geo fanout · Battery · Consent · TTL  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Nearby-Friends Location Sharing**  
...
Goal: share and query friend locations at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to share and query friend locations | Unrelated product surface |
| Core noun | `location share` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | consent & visibility, sparse live updates, friend-scoped geo, battery budgets | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **consent & visibility**.
2. Deliver a production-quality path for **sparse live updates**.
3. Deliver a production-quality path for **friend-scoped geo**.
4. Deliver a production-quality path for **battery budgets**.
5. Durable `location share` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | update ack < 200ms; friend query < 150ms |
...
| N5 | Freshness? | Live location/ETA/status SLOs | friend loc age policy 10–60s |
...
1. User/system initiates flow to share and query friend locations → validation → core `location share` created.
...
| Domain-specific contention | Serialize on location share keys; lease/TTL patterns |
...
|--------|----------|-----|------|--------|
| Location updates/s | 20K | 200K | 2M | 20M |
...
> Design **Nearby-Friends Location Sharing** to share and query friend locations, emphasizing consent & visibility, sparse live updates, friend-scoped geo, battery budgets. Start from baseline ~20K location updates/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 20K loc updates
Entities (order-of): 5M sharers
Write class focus: 20K
Typical payload: 200B ping
...
Use 200B ping as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **friend loc cache**, admission control, and key sharding.
...
```text
policy check → write live loc → friend-scoped geo index → push deltas
```
...
**Nearby-Friends Location Sharing** interview signal: speak fluently about **consent & visibility** and **sparse live updates**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Nearby-Friends Location Sharing**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **consent & visibility** as a first-class subsystem.
2. Invest in **sparse live updates** as a first-class subsystem.
3. Invest in **friend-scoped geo** as a first-class subsystem.
4. Invest in **battery budgets** as a first-class subsystem.
...
"For Nearby-Friends Location Sharing, I'd guarantee **consent & visibility** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Nearby-Friends Location Sharing?
...
For Nearby-Friends Location Sharing: emphasize consent & visibility, sparse live updates, friend-scoped geo; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Nearby-Friends Location Sharing
baseline: {'Primary QPS': '20K loc updates', 'Entities': '5M sharers', 'Write QPS': '20K', 'Payload': '200B ping', 'Hot structure': 'friend loc cache'}
...
| Baseline | 20K Location updates/s |
| Invariant | no double-allocate / no silent money loss |
| Scale endgame | cell mesh + edge + tiered stores |

---

*End of Nearby-Friends Location Sharing system design prep doc.*
