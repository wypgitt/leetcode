# System Design: Location-sharing service
...
|-----------|----------|----------|
| Job | Design **Location-sharing service** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Location-sharing service | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Location-sharing service.
...
Domain note: tailor 2–3 cases specifically to **Location-sharing service** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Location-sharing service.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Location-sharing service**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Location-sharing service".
...
Core components for **Location-sharing service**:
- **device auth**
- **telemetry ingest**
- **command plane**
- **OTA rollout**
...
flowchart LR
  ...
  GW --> Svc[Stateless locationsharingservi Service]
...
**Failure modes checklist for Location-sharing service**
...
### 5.4 Problem-specific deep notes for Location-sharing service
...
1. **Invariants** for Location-sharing service (what must never break).
...
| 16 | Which algorithms apply to Location-sharing service? | Expect discussion of: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Location-sharing service**: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching.
...
*Category: Devices, mobile, edge, and IoT · Slug: `location-sharing-service` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Location-sharing service
...
| Business SLI for Location-sharing service | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Location-sharing service
...
*Enriched for interview drill · `location-sharing-service`*
