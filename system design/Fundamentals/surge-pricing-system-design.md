...
> **Focus areas:** Demand/supply hexes · Multipliers · Fairness · Stability · Fraud · Realtime compute  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Surge Pricing**  
...
Goal: compute and apply dynamic prices at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to compute and apply dynamic prices | Unrelated product surface |
| Core noun | `surge multiplier` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | hex demand/supply, hysteresis, quote binding, gaming resistance | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **hex demand/supply**.
2. Deliver a production-quality path for **hysteresis**.
3. Deliver a production-quality path for **quote binding**.
4. Deliver a production-quality path for **gaming resistance**.
5. Durable `surge multiplier` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | quote path < 100ms including mult fetch |
...
| N5 | Freshness? | Live location/ETA/status SLOs | hex mult age < 5–15s |
...
1. User/system initiates flow to compute and apply dynamic prices → validation → core `surge multiplier` created.
...
|--------|----------|-----|------|--------|
| Pricing quotes/s | 5K | 50K | 500K | 5M |
...
> Design **Surge Pricing** to compute and apply dynamic prices, emphasizing hex demand/supply, hysteresis, quote binding, gaming resistance. Start from baseline ~5K pricing quotes/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 5K quote
Entities (order-of): hex multipliers
Write class focus: hex compute 1K
Typical payload: multiplier map
...
Use multiplier map as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **hex state Redis**, admission control, and key sharding.
...
```text
aggregate demand/supply → smoothed mult → bind into quote TTL
```
...
**Surge Pricing** interview signal: speak fluently about **hex demand/supply** and **hysteresis**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Surge Pricing**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **hex demand/supply** as a first-class subsystem.
2. Invest in **hysteresis** as a first-class subsystem.
3. Invest in **quote binding** as a first-class subsystem.
4. Invest in **gaming resistance** as a first-class subsystem.
...
"For Surge Pricing, I'd guarantee **hex demand/supply** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Surge Pricing?
...
For Surge Pricing: emphasize hex demand/supply, hysteresis, quote binding; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Surge Pricing
baseline: {'Primary QPS': '5K quote', 'Entities': 'hex multipliers', 'Write QPS': 'hex compute 1K', 'Payload': 'multiplier map', 'Hot structure': 'hex state Redis'}
