# System Design: Uber or Lyft

> **Focus areas:** Marketplace · Matching · Dispatch · Live location · ETA · Surge · Payments · City cells  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Uber or Lyft**  
...
Goal: match riders and drivers end-to-end at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to match riders and drivers end-to-end | Unrelated product surface |
| Core noun | `trip` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | trip state machine, exclusive dispatch, surge, city isolation | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **trip state machine**.
2. Deliver a production-quality path for **exclusive dispatch**.
3. Deliver a production-quality path for **surge**.
4. Deliver a production-quality path for **city isolation**.
5. Durable `trip` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | quote < 500ms; match p50 < 15s dense |
...
1. User/system initiates flow to match riders and drivers end-to-end → validation → core `trip` created.
...
|--------|----------|-----|------|--------|
| Trip requests/min/city | 2K | 20K | 200K | 2M global |
...
> Design **Uber or Lyft** to match riders and drivers end-to-end, emphasizing trip state machine, exclusive dispatch, surge, city isolation. Start from baseline ~2K trip requests/min/city and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 2K trip requests
Entities (order-of): 50K drivers/city peak
Write class focus: loc 100K
Typical payload: trip events
...
Use trip events as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **city dispatch cell**, admission control, and key sharding.
...
```text
quote → pay hold → dispatch exclusive match → trip SM → capture
```
...
**Uber or Lyft** interview signal: speak fluently about **trip state machine** and **exclusive dispatch**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Uber or Lyft**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **trip state machine** as a first-class subsystem.
2. Invest in **exclusive dispatch** as a first-class subsystem.
3. Invest in **surge** as a first-class subsystem.
4. Invest in **city isolation** as a first-class subsystem.
...
"For Uber or Lyft, I'd guarantee **trip state machine** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Uber or Lyft?
...
### Q2. How do city cells fail independently?
...
For Uber or Lyft: emphasize trip state machine, exclusive dispatch, surge; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Uber or Lyft
baseline: {'Primary QPS': '2K trip requests', 'Entities': '50K drivers/city peak', 'Write QPS': 'loc 100K', 'Payload': 'trip events', 'Hot structure': 'city dispatch cell'}
