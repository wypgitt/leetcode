...
|-----------|----------|----------|
| Job | Design **Stock exchange** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Stock exchange | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Stock exchange.
...
Domain note: tailor 2–3 cases specifically to **Stock exchange** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Stock exchange.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Stock exchange**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Stock exchange".
...
Core components for **Stock exchange**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless stockexchange Service]
...
**Failure modes checklist for Stock exchange**
...
### 5.4 Problem-specific deep notes for Stock exchange
...
1. **Invariants** for Stock exchange (what must never break).
...
| 16 | Which algorithms apply to Stock exchange? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Stock exchange**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `stock-exchange` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Stock exchange
...
| Business SLI for Stock exchange | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Stock exchange
...
*Enriched for interview drill · `stock-exchange`*
