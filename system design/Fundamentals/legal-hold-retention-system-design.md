# System Design: Legal hold and retention
...
|-----------|----------|----------|
| Job | Design **Legal hold and retention** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Legal hold and retention | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Legal hold and retention.
...
Domain note: tailor 2–3 cases specifically to **Legal hold and retention** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Legal hold and retention.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Legal hold and retention**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Legal hold and retention".
...
Core components for **Legal hold and retention**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless legalholdretention Service]
...
**Failure modes checklist for Legal hold and retention**
...
### 5.4 Problem-specific deep notes for Legal hold and retention
...
1. **Invariants** for Legal hold and retention (what must never break).
...
| 16 | Which algorithms apply to Legal hold and retention? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Legal hold and retention**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `legal-hold-retention` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Legal hold and retention
...
| Business SLI for Legal hold and retention | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Legal hold and retention
...
*Enriched for interview drill · `legal-hold-retention`*
