# System Design: WAF or DDoS-protection service
...
|-----------|----------|----------|
| Job | Design **WAF or DDoS-protection service** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for WAF or DDoS-protection service | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for WAF or DDoS-protection service.
...
Domain note: tailor 2–3 cases specifically to **WAF or DDoS-protection service** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to WAF or DDoS-protection service.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **WAF or DDoS-protection service**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "WAF or DDoS-protection service".
...
Core components for **WAF or DDoS-protection service**:
- **agents/collectors**
- **ingest pipeline**
- **TSDB/log index**
- **alert evaluator**
...
flowchart LR
  ...
  GW --> Svc[Stateless wafddosprotection Service]
...
**Failure modes checklist for WAF or DDoS-protection service**
...
### 5.4 Problem-specific deep notes for WAF or DDoS-protection service
...
1. **Invariants** for WAF or DDoS-protection service (what must never break).
...
| 16 | Which algorithms apply to WAF or DDoS-protection service? | Expect discussion of: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **WAF or DDoS-protection service**: DDSketch/hdr histograms, trace sampling, alert grouping, consistent hashing shard, SLO burn-rate.
...
*Category: Observability, reliability, security, and operations · Slug: `waf-ddos-protection` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for WAF or DDoS-protection service
...
| Business SLI for WAF or DDoS-protection service | Product truth |
...
- Cost model at 100×
- Abuse cases unique to WAF or DDoS-protection service
...
*Enriched for interview drill · `waf-ddos-protection`*
