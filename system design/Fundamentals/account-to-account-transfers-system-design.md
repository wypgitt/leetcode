# System Design: Account-to-account transfers
...
|-----------|----------|----------|
| Job | Design **Account-to-account transfers** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Account-to-account transfers | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Account-to-account transfers.
...
Domain note: tailor 2–3 cases specifically to **Account-to-account transfers** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Account-to-account transfers.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Account-to-account transfers**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Account-to-account transfers".
...
Core components for **Account-to-account transfers**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless accounttoaccounttran Service]
...
**Failure modes checklist for Account-to-account transfers**
...
### 5.4 Problem-specific deep notes for Account-to-account transfers
...
1. **Invariants** for Account-to-account transfers (what must never break).
...
| 16 | Which algorithms apply to Account-to-account transfers? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Account-to-account transfers**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `account-to-account-transfers` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Account-to-account transfers
...
| Business SLI for Account-to-account transfers | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Account-to-account transfers
...
*Enriched for interview drill · `account-to-account-transfers`*
