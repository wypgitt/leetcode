...
|-----------|----------|----------|
| Job | Design **EV charging network** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for EV charging network | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for EV charging network.
...
Domain note: tailor 2–3 cases specifically to **EV charging network** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to EV charging network.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **EV charging network**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "EV charging network".
...
Core components for **EV charging network**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless evchargingnetwork Service]
...
**Failure modes checklist for EV charging network**
...
### 5.4 Problem-specific deep notes for EV charging network
...
1. **Invariants** for EV charging network (what must never break).
...
| 16 | Which algorithms apply to EV charging network? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **EV charging network**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `ev-charging-network` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for EV charging network
...
| Business SLI for EV charging network | Product truth |
...
- Cost model at 100×
- Abuse cases unique to EV charging network
...
*Enriched for interview drill · `ev-charging-network`*
