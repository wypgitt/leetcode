...
|-----------|----------|----------|
| Job | Design **Ad targeting** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Ad targeting | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Ad targeting.
...
Domain note: tailor 2–3 cases specifically to **Ad targeting** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Ad targeting.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Ad targeting**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Ad targeting".
...
Core components for **Ad targeting**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless adtargeting Service]
...
**Failure modes checklist for Ad targeting**
...
### 5.4 Problem-specific deep notes for Ad targeting
...
1. **Invariants** for Ad targeting (what must never break).
...
| 16 | Which algorithms apply to Ad targeting? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Ad targeting**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `ad-targeting` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Ad targeting
...
| Business SLI for Ad targeting | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Ad targeting
...
*Enriched for interview drill · `ad-targeting`*
