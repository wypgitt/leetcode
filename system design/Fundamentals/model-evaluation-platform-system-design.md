# System Design: Model-evaluation platform
...
|-----------|----------|----------|
| Job | Design **Model-evaluation platform** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Model-evaluation platform | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Model-evaluation platform.
...
Domain note: tailor 2–3 cases specifically to **Model-evaluation platform** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Model-evaluation platform.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Model-evaluation platform**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Model-evaluation platform".
...
Core components for **Model-evaluation platform**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless modelevaluationplatf Service]
...
**Failure modes checklist for Model-evaluation platform**
...
### 5.4 Problem-specific deep notes for Model-evaluation platform
...
1. **Invariants** for Model-evaluation platform (what must never break).
...
| 16 | Which algorithms apply to Model-evaluation platform? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Model-evaluation platform**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `model-evaluation-platform` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Model-evaluation platform
...
| Business SLI for Model-evaluation platform | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Model-evaluation platform
...
*Enriched for interview drill · `model-evaluation-platform`*
