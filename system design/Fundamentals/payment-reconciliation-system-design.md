# System Design: Payment reconciliation
...
|-----------|----------|----------|
| Job | Design **Payment reconciliation** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Payment reconciliation | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Payment reconciliation.
...
Domain note: tailor 2–3 cases specifically to **Payment reconciliation** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Payment reconciliation.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Payment reconciliation**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Payment reconciliation".
...
Core components for **Payment reconciliation**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless paymentreconciliatio Service]
...
**Failure modes checklist for Payment reconciliation**
...
### 5.4 Problem-specific deep notes for Payment reconciliation
...
1. **Invariants** for Payment reconciliation (what must never break).
...
| 16 | Which algorithms apply to Payment reconciliation? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Payment reconciliation**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `payment-reconciliation` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Payment reconciliation
...
| Business SLI for Payment reconciliation | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Payment reconciliation
...
*Enriched for interview drill · `payment-reconciliation`*
