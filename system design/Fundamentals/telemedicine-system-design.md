...
|-----------|----------|----------|
| Job | Design **Telemedicine** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Telemedicine | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Telemedicine.
...
Domain note: tailor 2–3 cases specifically to **Telemedicine** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Telemedicine.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Telemedicine**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Telemedicine".
...
Core components for **Telemedicine**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless telemedicine Service]
...
**Failure modes checklist for Telemedicine**
...
### 5.4 Problem-specific deep notes for Telemedicine
...
1. **Invariants** for Telemedicine (what must never break).
...
| 16 | Which algorithms apply to Telemedicine? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Telemedicine**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `telemedicine` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Telemedicine
...
| Business SLI for Telemedicine | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Telemedicine
...
*Enriched for interview drill · `telemedicine`*
