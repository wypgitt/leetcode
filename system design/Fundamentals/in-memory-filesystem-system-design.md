# System Design: In-memory filesystem
...
|-----------|----------|----------|
| Job | Design **In-memory filesystem** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for In-memory filesystem | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for In-memory filesystem.
...
Domain note: tailor 2–3 cases specifically to **In-memory filesystem** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to In-memory filesystem.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **In-memory filesystem**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "In-memory filesystem".
...
Core components for **In-memory filesystem**:
- **public API**
...
flowchart LR
  ...
  GW --> Svc[Stateless inmemoryfilesystem Service]
...
**Failure modes checklist for In-memory filesystem**
...
### 5.4 Problem-specific deep notes for In-memory filesystem
...
1. **Invariants** for In-memory filesystem (what must never break).
...
| 16 | Which algorithms apply to In-memory filesystem? | Expect discussion of: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **In-memory filesystem**: LRU/LFU, token bucket, ring buffer, skip lists / B+trees, lock striping.
...
*Category: Lower-level and machine-coding system design · Slug: `in-memory-filesystem` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for In-memory filesystem
...
| Business SLI for In-memory filesystem | Product truth |
...
- Cost model at 100×
- Abuse cases unique to In-memory filesystem
...
*Enriched for interview drill · `in-memory-filesystem`*
