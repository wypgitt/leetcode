# System Design: Remote proctoring
...
|-----------|----------|----------|
| Job | Design **Remote proctoring** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Remote proctoring | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Remote proctoring.
...
Domain note: tailor 2–3 cases specifically to **Remote proctoring** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Remote proctoring.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Remote proctoring**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Remote proctoring".
...
Core components for **Remote proctoring**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless remoteproctoring Service]
...
**Failure modes checklist for Remote proctoring**
...
### 5.4 Problem-specific deep notes for Remote proctoring
...
1. **Invariants** for Remote proctoring (what must never break).
...
| 16 | Which algorithms apply to Remote proctoring? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Remote proctoring**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `remote-proctoring` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Remote proctoring
...
| Business SLI for Remote proctoring | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Remote proctoring
...
*Enriched for interview drill · `remote-proctoring`*
