# System Design: Real-time stock-market data
...
|-----------|----------|----------|
| Job | Design **Real-time stock-market data** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Real-time stock-market data | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Real-time stock-market data.
...
Domain note: tailor 2–3 cases specifically to **Real-time stock-market data** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Real-time stock-market data.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Real-time stock-market data**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Real-time stock-market data".
...
Core components for **Real-time stock-market data**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless realtimestockmarketd Service]
...
**Failure modes checklist for Real-time stock-market data**
...
### 5.4 Problem-specific deep notes for Real-time stock-market data
...
1. **Invariants** for Real-time stock-market data (what must never break).
...
| 16 | Which algorithms apply to Real-time stock-market data? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Real-time stock-market data**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `realtime-stock-market-data` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Real-time stock-market data
...
| Business SLI for Real-time stock-market data | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Real-time stock-market data
...
*Enriched for interview drill · `realtime-stock-market-data`*
