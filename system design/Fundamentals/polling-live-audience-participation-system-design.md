# System Design: Polling or live audience-participation system
...
|-----------|----------|----------|
| Job | Design **Polling or live audience-participation system** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Polling or live audience-participation system | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Polling or live audience-participation system.
...
Domain note: tailor 2–3 cases specifically to **Polling or live audience-participation system** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Polling or live audience-participation system.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Polling or live audience-participation system**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Polling or live audience-participation system".
...
Core components for **Polling or live audience-participation system**:
- **write API**
...
flowchart LR
  ...
  GW --> Svc[Stateless pollingliveaudiencep Service]
...
**Failure modes checklist for Polling or live audience-participation system**
...
### 5.4 Problem-specific deep notes for Polling or live audience-participation system
...
1. **Invariants** for Polling or live audience-participation system (what must never break).
...
| 16 | Which algorithms apply to Polling or live audience-participation system? | Expect discussion of: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Polling or live audience-participation system**: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat.
...
*Category: Social, communication, and collaboration · Slug: `polling-live-audience-participation` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Polling or live audience-participation system
...
| Business SLI for Polling or live audience-participation system | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Polling or live audience-participation system
...
*Enriched for interview drill · `polling-live-audience-participation`*
