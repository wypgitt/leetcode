...
|-----------|----------|----------|
| Job | Design **Product search** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Product search | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Product search.
...
Domain note: tailor 2–3 cases specifically to **Product search** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Product search.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Product search**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Product search".
...
Core components for **Product search**:
- **product read path**
- **cart/checkout**
...
flowchart LR
  ...
  GW --> Svc[Stateless productsearch Service]
...
**Failure modes checklist for Product search**
...
### 5.4 Problem-specific deep notes for Product search
...
1. **Invariants** for Product search (what must never break).
...
| 16 | Which algorithms apply to Product search? | Expect discussion of: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Product search**: optimistic inventory, second-price/GSP, bloom for freq cap, sharded counters, outbox pattern.
...
*Category: E-commerce and advertising · Slug: `product-search` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Product search
...
| Business SLI for Product search | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Product search
...
*Enriched for interview drill · `product-search`*
