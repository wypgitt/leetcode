# System Design: Notification-as-a-service product
...
|-----------|----------|----------|
| Job | Design **Notification-as-a-service product** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Notification-as-a-service product | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Notification-as-a-service product.
...
Domain note: tailor 2–3 cases specifically to **Notification-as-a-service product** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Notification-as-a-service product.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Notification-as-a-service product**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Notification-as-a-service product".
...
Core components for **Notification-as-a-service product**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless notificationasaservi Service]
...
**Failure modes checklist for Notification-as-a-service product**
...
### 5.4 Problem-specific deep notes for Notification-as-a-service product
...
1. **Invariants** for Notification-as-a-service product (what must never break).
...
| 16 | Which algorithms apply to Notification-as-a-service product? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Notification-as-a-service product**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `notification-as-a-service` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Notification-as-a-service product
...
| Business SLI for Notification-as-a-service product | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Notification-as-a-service product
...
*Enriched for interview drill · `notification-as-a-service`*
