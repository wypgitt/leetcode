...
|-----------|----------|----------|
| Job | Design **Live online classes** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Live online classes | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Live online classes.
...
Domain note: tailor 2–3 cases specifically to **Live online classes** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Live online classes.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Live online classes**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Live online classes".
...
Core components for **Live online classes**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless liveonlineclasses Service]
...
**Failure modes checklist for Live online classes**
...
### 5.4 Problem-specific deep notes for Live online classes
...
1. **Invariants** for Live online classes (what must never break).
...
| 16 | Which algorithms apply to Live online classes? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Live online classes**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `live-online-classes` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Live online classes
...
| Business SLI for Live online classes | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Live online classes
...
*Enriched for interview drill · `live-online-classes`*
