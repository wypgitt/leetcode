# System Design: Listing Watchlists and Availability Alerts

> **Focus areas:** Watchlists · Change detection · Alert fanout · Dedup · Preference matching · Push  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Listing Watchlists and Availability Alerts**  
...
Goal: notify users when watched inventory opens at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to notify users when watched inventory opens | Unrelated product surface |
| Core noun | `availability alert` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | watch inverted index, change CDC, alert dedup, quiet hours | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **watch inverted index**.
2. Deliver a production-quality path for **change CDC**.
3. Deliver a production-quality path for **alert dedup**.
4. Deliver a production-quality path for **quiet hours**.
5. Durable `availability alert` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | alert after change < 30–60s |
...
1. User/system initiates flow to notify users when watched inventory opens → validation → core `availability alert` created.
...
| Domain-specific contention | Serialize on availability alert keys; lease/TTL patterns |
...
|--------|----------|-----|------|--------|
| Alert evaluations/s | 5K | 50K | 500K | 5M |
...
> Design **Listing Watchlists and Availability Alerts** to notify users when watched inventory opens, emphasizing watch inverted index, change CDC, alert dedup, quiet hours. Start from baseline ~5K alert evaluations/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 1K watch writes
Entities (order-of): watches
Write class focus: availability CDC 5K
Typical payload: push alerts
...
Use push alerts as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **inverted watches**, admission control, and key sharding.
...
```text
CDC availability → match watches → dedup → respect quiet hours → push
```
...
**Listing Watchlists and Availability Alerts** interview signal: speak fluently about **watch inverted index** and **change CDC**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Listing Watchlists and Availability Alerts**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **watch inverted index** as a first-class subsystem.
2. Invest in **change CDC** as a first-class subsystem.
3. Invest in **alert dedup** as a first-class subsystem.
4. Invest in **quiet hours** as a first-class subsystem.
...
"For Listing Watchlists and Availability Alerts, I'd guarantee **watch inverted index** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Listing Watchlists and Availability Alerts?
...
For Listing Watchlists and Availability Alerts: emphasize watch inverted index, change CDC, alert dedup; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Listing Watchlists and Availability Alerts
baseline: {'Primary QPS': '1K watch writes', 'Entities': 'watches', 'Write QPS': 'availability CDC 5K', 'Payload': 'push alerts', 'Hot structure': 'inverted watches'}
...
| Baseline | 5K Alert evaluations/s |
| Invariant | no double-allocate / no silent money loss |
| Scale endgame | cell mesh + edge + tiered stores |

---

*End of Listing Watchlists and Availability Alerts system design prep doc.*
