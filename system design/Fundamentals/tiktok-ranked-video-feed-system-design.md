# System Design: TikTok-style ranked video feed
...
|-----------|----------|----------|
| Job | Design **TikTok-style ranked video feed** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for TikTok-style ranked video feed | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for TikTok-style ranked video feed.
...
Domain note: tailor 2–3 cases specifically to **TikTok-style ranked video feed** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to TikTok-style ranked video feed.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **TikTok-style ranked video feed**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "TikTok-style ranked video feed".
...
Core components for **TikTok-style ranked video feed**:
- **write API**
...
flowchart LR
  ...
  GW --> Svc[Stateless tiktokrankedvideofee Service]
...
**Failure modes checklist for TikTok-style ranked video feed**
...
### 5.4 Problem-specific deep notes for TikTok-style ranked video feed
...
1. **Invariants** for TikTok-style ranked video feed (what must never break).
...
| 16 | Which algorithms apply to TikTok-style ranked video feed? | Expect discussion of: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **TikTok-style ranked video feed**: push/pull hybrid fanout, OT/CRDT, graph BFS sampling, score ranking, ring buffers for live chat.
...
*Category: Social, communication, and collaboration · Slug: `tiktok-ranked-video-feed` · Use this as a living checklist; customize numbers with the interviewer.*
...
*Enriched · `tiktok-ranked-video-feed`*
