# System Design: Crypto order routing across exchanges
...
|-----------|----------|----------|
| Job | Design **Crypto order routing across exchanges** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Crypto order routing across exchanges | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Crypto order routing across exchanges.
...
Domain note: tailor 2–3 cases specifically to **Crypto order routing across exchanges** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Crypto order routing across exchanges.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Crypto order routing across exchanges**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Crypto order routing across exchanges".
...
Core components for **Crypto order routing across exchanges**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless cryptoorderrouting Service]
...
**Failure modes checklist for Crypto order routing across exchanges**
...
### 5.4 Problem-specific deep notes for Crypto order routing across exchanges
...
1. **Invariants** for Crypto order routing across exchanges (what must never break).
...
| 16 | Which algorithms apply to Crypto order routing across exchanges? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Crypto order routing across exchanges**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `crypto-order-routing` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Crypto order routing across exchanges
...
| Business SLI for Crypto order routing across exchanges | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Crypto order routing across exchanges
...
*Enriched for interview drill · `crypto-order-routing`*
