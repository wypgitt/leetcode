# System Design: Employee onboarding and offboarding
...
|-----------|----------|----------|
| Job | Design **Employee onboarding and offboarding** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Employee onboarding and offboarding | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Employee onboarding and offboarding.
...
Domain note: tailor 2–3 cases specifically to **Employee onboarding and offboarding** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Employee onboarding and offboarding.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Employee onboarding and offboarding**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Employee onboarding and offboarding".
...
Core components for **Employee onboarding and offboarding**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless employeeonboardingof Service]
...
**Failure modes checklist for Employee onboarding and offboarding**
...
### 5.4 Problem-specific deep notes for Employee onboarding and offboarding
...
1. **Invariants** for Employee onboarding and offboarding (what must never break).
...
| 16 | Which algorithms apply to Employee onboarding and offboarding? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Employee onboarding and offboarding**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `employee-onboarding-offboarding` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Employee onboarding and offboarding
...
| Business SLI for Employee onboarding and offboarding | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Employee onboarding and offboarding
...
*Enriched for interview drill · `employee-onboarding-offboarding`*
