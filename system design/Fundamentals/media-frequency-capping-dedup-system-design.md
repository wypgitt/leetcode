# System Design: Media-frequency capping and deduplication across devices
...
|-----------|----------|----------|
| Job | Design **Media-frequency capping and deduplication across devices** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Media-frequency capping and deduplication across devices | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Media-frequency capping and deduplication across devices.
...
Domain note: tailor 2–3 cases specifically to **Media-frequency capping and deduplication across devices** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Media-frequency capping and deduplication across devices.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Media-frequency capping and deduplication across devices**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Media-frequency capping and deduplication across devices".
...
Core components for **Media-frequency capping and deduplication across devices**:
- **upload ingress**
- **transcode farm**
- **origin/CDN**
- **manifest/playlist**
...
flowchart LR
  ...
  GW --> Svc[Stateless mediafrequencycappin Service]
...
**Failure modes checklist for Media-frequency capping and deduplication across devices**
...
### 5.4 Problem-specific deep notes for Media-frequency capping and deduplication across devices
...
1. **Invariants** for Media-frequency capping and deduplication across devices (what must never break).
...
| 16 | Which algorithms apply to Media-frequency capping and deduplication across devices? | Expect discussion of: ABR ladder, consistent hashing for cache, perceptual hash, embedding ANN, token bucket for encoding slots. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Media-frequency capping and deduplication across devices**: ABR ladder, consistent hashing for cache, perceptual hash, embedding ANN, token bucket for encoding slots.
...
*Category: Media and content platforms · Slug: `media-frequency-capping-dedup` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Media-frequency capping and deduplication across devices
...
| Business SLI for Media-frequency capping and deduplication across devices | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Media-frequency capping and deduplication across devices
...
*Enriched for interview drill · `media-frequency-capping-dedup`*
