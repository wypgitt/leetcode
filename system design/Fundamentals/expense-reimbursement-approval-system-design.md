# System Design: Expense reimbursement and approval
...
|-----------|----------|----------|
| Job | Design **Expense reimbursement and approval** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Expense reimbursement and approval | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Expense reimbursement and approval.
...
Domain note: tailor 2–3 cases specifically to **Expense reimbursement and approval** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Expense reimbursement and approval.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Expense reimbursement and approval**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Expense reimbursement and approval".
...
Core components for **Expense reimbursement and approval**:
- **payment API**
- **orchestrator**
- **ledger**
- **processor adapters**
...
flowchart LR
  ...
  GW --> Svc[Stateless expensereimbursement Service]
...
**Failure modes checklist for Expense reimbursement and approval**
...
### 5.4 Problem-specific deep notes for Expense reimbursement and approval
...
1. **Invariants** for Expense reimbursement and approval (what must never break).
...
| 16 | Which algorithms apply to Expense reimbursement and approval? | Expect discussion of: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Expense reimbursement and approval**: double-entry, saga/state machine, fencing, deterministic matching engine, HLL for fraud features.
...
*Category: Payments, banking, fintech, and crypto · Slug: `expense-reimbursement-approval` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Expense reimbursement and approval
...
| Business SLI for Expense reimbursement and approval | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Expense reimbursement and approval
...
*Enriched for interview drill · `expense-reimbursement-approval`*
