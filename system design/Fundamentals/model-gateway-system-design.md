...
|-----------|----------|----------|
| Job | Design **Model gateway** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Model gateway | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Model gateway.
...
Domain note: tailor 2–3 cases specifically to **Model gateway** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Model gateway.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Model gateway**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Model gateway".
...
Core components for **Model gateway**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless modelgateway Service]
...
**Failure modes checklist for Model gateway**
...
### 5.4 Problem-specific deep notes for Model gateway
...
1. **Invariants** for Model gateway (what must never break).
...
| 16 | Which algorithms apply to Model gateway? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Model gateway**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `model-gateway` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Model gateway
...
| Business SLI for Model gateway | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Model gateway
...
*Enriched for interview drill · `model-gateway`*
