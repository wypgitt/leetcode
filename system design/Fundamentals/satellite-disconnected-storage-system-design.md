# System Design: Satellite or highly disconnected storage
...
|-----------|----------|----------|
| Job | Design **Satellite or highly disconnected storage** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Satellite or highly disconnected storage | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Satellite or highly disconnected storage.
...
Domain note: tailor 2–3 cases specifically to **Satellite or highly disconnected storage** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Satellite or highly disconnected storage.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Satellite or highly disconnected storage**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Satellite or highly disconnected storage".
...
Core components for **Satellite or highly disconnected storage**:
- **device auth**
- **telemetry ingest**
- **command plane**
- **OTA rollout**
...
flowchart LR
  ...
  GW --> Svc[Stateless satellitedisconnecte Service]
...
**Failure modes checklist for Satellite or highly disconnected storage**
...
### 5.4 Problem-specific deep notes for Satellite or highly disconnected storage
...
1. **Invariants** for Satellite or highly disconnected storage (what must never break).
...
| 16 | Which algorithms apply to Satellite or highly disconnected storage? | Expect discussion of: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Satellite or highly disconnected storage**: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching.
...
*Category: Devices, mobile, edge, and IoT · Slug: `satellite-disconnected-storage` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Satellite or highly disconnected storage
...
| Business SLI for Satellite or highly disconnected storage | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Satellite or highly disconnected storage
...
*Enriched for interview drill · `satellite-disconnected-storage`*
