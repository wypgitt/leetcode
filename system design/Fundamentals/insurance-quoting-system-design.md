# System Design: Insurance quoting
...
|-----------|----------|----------|
| Job | Design **Insurance quoting** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Insurance quoting | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Insurance quoting.
...
Domain note: tailor 2–3 cases specifically to **Insurance quoting** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Insurance quoting.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Insurance quoting**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Insurance quoting".
...
Core components for **Insurance quoting**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless insurancequoting Service]
...
**Failure modes checklist for Insurance quoting**
...
### 5.4 Problem-specific deep notes for Insurance quoting
...
1. **Invariants** for Insurance quoting (what must never break).
...
| 16 | Which algorithms apply to Insurance quoting? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Insurance quoting**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `insurance-quoting` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Insurance quoting
...
| Business SLI for Insurance quoting | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Insurance quoting
...
*Enriched for interview drill · `insurance-quoting`*
