# System Design: Immutable or tamper-evident audit log
...
|-----------|----------|----------|
| Job | Design **Immutable or tamper-evident audit log** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Immutable or tamper-evident audit log | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Immutable or tamper-evident audit log.
...
Domain note: tailor 2–3 cases specifically to **Immutable or tamper-evident audit log** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Immutable or tamper-evident audit log.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Immutable or tamper-evident audit log**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Immutable or tamper-evident audit log".
...
Core components for **Immutable or tamper-evident audit log**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless immutabletamperevide Service]
...
**Failure modes checklist for Immutable or tamper-evident audit log**
...
### 5.4 Problem-specific deep notes for Immutable or tamper-evident audit log
...
1. **Invariants** for Immutable or tamper-evident audit log (what must never break).
...
| 16 | Which algorithms apply to Immutable or tamper-evident audit log? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Immutable or tamper-evident audit log**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `immutable-tamper-evident-audit-log` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Immutable or tamper-evident audit log
...
| Business SLI for Immutable or tamper-evident audit log | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Immutable or tamper-evident audit log
...
*Enriched for interview drill · `immutable-tamper-evident-audit-log`*
