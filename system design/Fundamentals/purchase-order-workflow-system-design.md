# System Design: Purchase-order workflow
...
|-----------|----------|----------|
| Job | Design **Purchase-order workflow** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Purchase-order workflow | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Purchase-order workflow.
...
Domain note: tailor 2–3 cases specifically to **Purchase-order workflow** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Purchase-order workflow.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Purchase-order workflow**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Purchase-order workflow".
...
Core components for **Purchase-order workflow**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless purchaseorderworkflo Service]
...
**Failure modes checklist for Purchase-order workflow**
...
### 5.4 Problem-specific deep notes for Purchase-order workflow
...
1. **Invariants** for Purchase-order workflow (what must never break).
...
| 16 | Which algorithms apply to Purchase-order workflow? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Purchase-order workflow**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `purchase-order-workflow` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Purchase-order workflow
...
| Business SLI for Purchase-order workflow | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Purchase-order workflow
...
*Enriched for interview drill · `purchase-order-workflow`*
