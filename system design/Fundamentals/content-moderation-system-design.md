...
|-----------|----------|----------|
| Job | Design **Content moderation** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Content moderation | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Content moderation.
...
Domain note: tailor 2–3 cases specifically to **Content moderation** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Content moderation.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Content moderation**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Content moderation".
...
Core components for **Content moderation**:
- **upload ingress**
- **transcode farm**
- **origin/CDN**
- **manifest/playlist**
...
flowchart LR
  ...
  GW --> Svc[Stateless contentmoderation Service]
...
**Failure modes checklist for Content moderation**
...
### 5.4 Problem-specific deep notes for Content moderation
...
1. **Invariants** for Content moderation (what must never break).
...
| 16 | Which algorithms apply to Content moderation? | Expect discussion of: ABR ladder, consistent hashing for cache, perceptual hash, embedding ANN, token bucket for encoding slots. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Content moderation**: ABR ladder, consistent hashing for cache, perceptual hash, embedding ANN, token bucket for encoding slots.
...
*Category: Media and content platforms · Slug: `content-moderation` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Content moderation
...
| Business SLI for Content moderation | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Content moderation
...
*Enriched for interview drill · `content-moderation`*
