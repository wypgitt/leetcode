# System Design: Smart-meter event pipeline
...
|-----------|----------|----------|
| Job | Design **Smart-meter event pipeline** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Smart-meter event pipeline | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Smart-meter event pipeline.
...
Domain note: tailor 2–3 cases specifically to **Smart-meter event pipeline** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Smart-meter event pipeline.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Smart-meter event pipeline**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Smart-meter event pipeline".
...
Core components for **Smart-meter event pipeline**:
- **device auth**
- **telemetry ingest**
- **command plane**
- **OTA rollout**
...
flowchart LR
  ...
  GW --> Svc[Stateless smartmetereventpipel Service]
...
**Failure modes checklist for Smart-meter event pipeline**
...
### 5.4 Problem-specific deep notes for Smart-meter event pipeline
...
1. **Invariants** for Smart-meter event pipeline (what must never break).
...
| 16 | Which algorithms apply to Smart-meter event pipeline? | Expect discussion of: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Smart-meter event pipeline**: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching.
...
*Category: Devices, mobile, edge, and IoT · Slug: `smart-meter-event-pipeline` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Smart-meter event pipeline
...
| Business SLI for Smart-meter event pipeline | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Smart-meter event pipeline
...
*Enriched for interview drill · `smart-meter-event-pipeline`*
