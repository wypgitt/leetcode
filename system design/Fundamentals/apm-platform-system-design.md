<!-- Adapted into Fundamentals bank from Stripe/apm-system-design.md for cross-company prep. -->

# System Design: Application Performance Monitoring

> **Focus areas:** Tracing · Sampling · Service map · PII scrub · SLOs  
...
|---|---|---|---|
| F1 | Spans? | HTTP+RPC auto | SDK inject |
| F2 | Sampling? | Head+tail error bias | Keep errors |
| F3 | Storage? | Hot+cold traces | Retention tiers |
| F4 | PII? | Scrub rules | No secrets in tags |
| F5 | Service map? | Edge aggregation | Dependency graph |
| F6 | Query? | Trace by id latency | Indexed |
| F7 | Critical path? | Never block request | Async export |
| F8 | Cardinality? | Limit tag keys | Deny high |
| F9 | SLO? | Latency error budget | Dashboards |
| F10 | Multi-region? | Regional collectors | Global query federate |
| F11 | Alerts? | Tail sample anomalies | Page on SLO burn |
| F12 | Rollout? | SDK version | Feature flag |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Application Performance Monitoring
4. Admin recon/replay hooks for Application Performance Monitoring
...
- Replacing all Stripe production systems in one Application Performance Monitoring design
...
1. Spans? → HTTP+RPC auto → durable write + audit
2. Sampling? → Head+tail error bias → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Service map? → safe degradation documented
...
> Design APM ~50K spans/s to 1000x with head/tail sampling.
...
### 3.11 Domain model (Application Performance Monitoring)
...
### 4.1 End-to-end (Application Performance Monitoring)
...
```text
API Edge → Application Performance Monitoring → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of application performance monitoring system design.*

### 8.19 Application Performance Monitoring — rollout checklist
