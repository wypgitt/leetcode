# System Design: Project/task management
...
|-----------|----------|----------|
| Job | Design **Project/task management** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Project/task management | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Project/task management.
...
Domain note: tailor 2–3 cases specifically to **Project/task management** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Project/task management.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Project/task management**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Project/task management".
...
Core components for **Project/task management**:
- **authN/authZ**
- **tenant router**
- **workflow engine**
- **audit log**
...
flowchart LR
  ...
  GW --> Svc[Stateless projecttaskmanagemen Service]
...
**Failure modes checklist for Project/task management**
...
### 5.4 Problem-specific deep notes for Project/task management
...
1. **Invariants** for Project/task management (what must never break).
...
| 16 | Which algorithms apply to Project/task management? | Expect discussion of: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Project/task management**: policy evaluation, hierarchical ACLs, outbox, cursor pagination, SCIM sync.
...
*Category: Enterprise SaaS, identity, and workflow products · Slug: `project-task-management` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Project/task management
...
| Business SLI for Project/task management | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Project/task management
...
*Enriched for interview drill · `project-task-management`*
