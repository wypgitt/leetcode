# System Design: Factory telemetry
...
|-----------|----------|----------|
| Job | Design **Factory telemetry** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Factory telemetry | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Factory telemetry.
...
Domain note: tailor 2–3 cases specifically to **Factory telemetry** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Factory telemetry.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Factory telemetry**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Factory telemetry".
...
Core components for **Factory telemetry**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless factorytelemetry Service]
...
**Failure modes checklist for Factory telemetry**
...
### 5.4 Problem-specific deep notes for Factory telemetry
...
1. **Invariants** for Factory telemetry (what must never break).
...
| 16 | Which algorithms apply to Factory telemetry? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Factory telemetry**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `factory-telemetry` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Factory telemetry
...
| Business SLI for Factory telemetry | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Factory telemetry
...
*Enriched for interview drill · `factory-telemetry`*
