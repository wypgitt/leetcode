...
|-----------|----------|----------|
| Job | Design **Digital twin** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Digital twin | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Digital twin.
...
Domain note: tailor 2–3 cases specifically to **Digital twin** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Digital twin.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Digital twin**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Digital twin".
...
Core components for **Digital twin**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless digitaltwin Service]
...
**Failure modes checklist for Digital twin**
...
### 5.4 Problem-specific deep notes for Digital twin
...
1. **Invariants** for Digital twin (what must never break).
...
| 16 | Which algorithms apply to Digital twin? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Digital twin**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `digital-twin` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Digital twin
...
| Business SLI for Digital twin | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Digital twin
...
*Enriched for interview drill · `digital-twin`*
