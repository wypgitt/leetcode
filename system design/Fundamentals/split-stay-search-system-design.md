# System Design: Split-Stay Search

> **Focus areas:** Multi-listing itineraries · Combinatorial search · Availability · Ranking · Latency budgets  
> **Style:** End-to-end product design with progressive scale (10× → 100× → 1,000×)  
> **Interview theme:** Maps / marketplaces / mobility / logistics — **Split-Stay Search**  
...
Goal: find multi-listing stay combinations at marketplace/geo scale—with progressive architecture from one city/region to global cells—while protecting **correctness under contention** (inventory, seats, dispatch leases, money).

### 1.0 What this is / is not

| Dimension | This doc | Not this |
|-----------|----------|----------|
| Job | System to find multi-listing stay combinations | Unrelated product surface |
| Core noun | `split-stay itinerary` lifecycle | Every adjacent company product |
| Depth | Senior/staff HLD + deep dives | Only UI mockups or only ML papers |
| Emphasis | combinatorial explosion, pruning, availability joins, latency budgets | Generic CRUD without geo/contention |
...
1. Deliver a production-quality path for **combinatorial explosion**.
2. Deliver a production-quality path for **pruning**.
3. Deliver a production-quality path for **availability joins**.
4. Deliver a production-quality path for **latency budgets**.
5. Durable `split-stay itinerary` state machine with idempotent writes.
...
|---|----------|-----------------|--------|
| N1 | Latency SLO? | Interactive where user waits | search < 1–2s with budgets |
...
1. User/system initiates flow to find multi-listing stay combinations → validation → core `split-stay itinerary` created.
...
|--------|----------|-----|------|--------|
| Split searches/s | 200 | 2K | 20K | 200K |
...
> Design **Split-Stay Search** to find multi-listing stay combinations, emphasizing combinatorial explosion, pruning, availability joins, latency budgets. Start from baseline ~200 split searches/s and evolve through 10×/100×/1,000× with geo/cell partitioning. MVP protects contention invariants with idempotent APIs; defer global omniscient optimizers.
...
```text
Baseline primary: 200 complex searches
Entities (order-of): listing pairs+
Write class focus: calendar reads
Typical payload: itineraries
...
Use itineraries as mean response/event size for order-of math.
...
Expect hot keys around popular cells, onsale `event_id`, viral listings, downtown lunch hexes. Mitigate with **search workers**, admission control, and key sharding.
...
```text
split date continuum → candidate listings/segment → prune → score combos
```
...
**Split-Stay Search** interview signal: speak fluently about **combinatorial explosion** and **pruning**, then connect to failure modes (TTL expiry, CAS conflicts, cell failover).
...
Golden signals for **Split-Stay Search**: QPS/latency/errors on APIs; **freshness/lag** on geo/index; **CAS conflict rate**; **lease expiry rate**; business KPIs (match rate, book conversion, onsale checkout, on-time delivery).
...
1. Invest in **combinatorial explosion** as a first-class subsystem.
2. Invest in **pruning** as a first-class subsystem.
3. Invest in **availability joins** as a first-class subsystem.
4. Invest in **latency budgets** as a first-class subsystem.
...
"For Split-Stay Search, I'd guarantee **combinatorial explosion** correctness with leases/CAS, serve discovery via approximate geo + refine, isolate peaks in cells, and bind user quotes. Everything else—SOTA ranking, global OR solvers—is iterative on that skeleton."

---

## 7. Deeper / Related Interview Questions

### Q1. What is the single hardest correctness invariant in Split-Stay Search?
...
For Split-Stay Search: emphasize combinatorial explosion, pruning, availability joins; progressive cell scale; strong on contention keys; eventual on derived indexes; idempotent mobile retries.
...
```text
problem: Split-Stay Search
baseline: {'Primary QPS': '200 complex searches', 'Entities': 'listing pairs+', 'Write QPS': 'calendar reads', 'Payload': 'itineraries', 'Hot structure': 'search workers'}
