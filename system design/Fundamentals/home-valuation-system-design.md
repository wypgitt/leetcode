...
|-----------|----------|----------|
| Job | Design **Home valuation** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Home valuation | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Home valuation.
...
Domain note: tailor 2–3 cases specifically to **Home valuation** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Home valuation.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Home valuation**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Home valuation".
...
Core components for **Home valuation**:
- **domain API**
...
flowchart LR
  ...
  GW --> Svc[Stateless homevaluation Service]
...
**Failure modes checklist for Home valuation**
...
### 5.4 Problem-specific deep notes for Home valuation
...
1. **Invariants** for Home valuation (what must never break).
...
| 16 | Which algorithms apply to Home valuation? | Expect discussion of: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Home valuation**: state machines, outbox, temporal workflows, evidence hashing, calendar/interval indexes.
...
*Category: Domain-specific startup prompts · Slug: `home-valuation` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Home valuation
...
| Business SLI for Home valuation | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Home valuation
...
*Enriched for interview drill · `home-valuation`*
