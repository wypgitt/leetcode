# System Design: Resumable upload over unreliable networks
...
|-----------|----------|----------|
| Job | Design **Resumable upload over unreliable networks** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Resumable upload over unreliable networks | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Resumable upload over unreliable networks.
...
Domain note: tailor 2–3 cases specifically to **Resumable upload over unreliable networks** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Resumable upload over unreliable networks.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Resumable upload over unreliable networks**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Resumable upload over unreliable networks".
...
Core components for **Resumable upload over unreliable networks**:
- **device auth**
- **telemetry ingest**
- **command plane**
- **OTA rollout**
...
flowchart LR
  ...
  GW --> Svc[Stateless resumableuploadunrel Service]
...
**Failure modes checklist for Resumable upload over unreliable networks**
...
### 5.4 Problem-specific deep notes for Resumable upload over unreliable networks
...
1. **Invariants** for Resumable upload over unreliable networks (what must never break).
...
| 16 | Which algorithms apply to Resumable upload over unreliable networks? | Expect discussion of: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Resumable upload over unreliable networks**: exponential backoff, CRDT/LWW, Merkle sync, chunked resumable upload, geofenced batching.
...
*Category: Devices, mobile, edge, and IoT · Slug: `resumable-upload-unreliable-networks` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Resumable upload over unreliable networks
...
| Business SLI for Resumable upload over unreliable networks | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Resumable upload over unreliable networks
...
*Enriched for interview drill · `resumable-upload-unreliable-networks`*
