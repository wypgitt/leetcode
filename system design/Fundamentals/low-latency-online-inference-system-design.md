# System Design: Low-latency online inference
...
|-----------|----------|----------|
| Job | Design **Low-latency online inference** as a production system | A toy single-process demo without failure modes |
...
|---|-----------------|---------------------------|--------------------|
| F1 | Who are the users / clients? | End users, internal services, or both for Low-latency online inference | Authn model, SLAs, multi-tenant vs single-tenant |
...
1. Core create/read (and critical update/delete) path for Low-latency online inference.
...
Domain note: tailor 2–3 cases specifically to **Low-latency online inference** in the interview (the table above is the baseline set).
...
- Compliance? PII/PHI/PCI as applicable to Low-latency online inference.
- Team size / time? Design for evolution, not big-bang perfection.
- Read-your-writes required? Default yes for owner reads.

**Scope statement to repeat back:**

> Design **Low-latency online inference**: MVP correctness and durability first, then a clean path through 10× / 100× / 1,000× with explicit trade-offs on consistency, caching, sharding, and failure modes. Defer adjacent products unless requested.
...
```text
Assume baseline peak read QPS R, write QPS W for "Low-latency online inference".
...
Core components for **Low-latency online inference**:
- **API gateway**
- **model router**
- **GPU schedulers**
- **vector index**
...
flowchart LR
  ...
  GW --> Svc[Stateless lowlatencyonlineinfe Service]
...
**Failure modes checklist for Low-latency online inference**
...
### 5.4 Problem-specific deep notes for Low-latency online inference
...
1. **Invariants** for Low-latency online inference (what must never break).
...
| 16 | Which algorithms apply to Low-latency online inference? | Expect discussion of: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers. Explain complexity and failure modes, not just names. |
...
Expect to whiteboard or verbally justify structures relevant to **Low-latency online inference**: ANN (HNSW/IVF), KV-cache reuse, bin-packing GPUs, speculative decoding, circuit breakers.
...
*Category: AI, LLM, and machine-learning systems · Slug: `low-latency-online-inference` · Use this as a living checklist; customize numbers with the interviewer.*

## Appendix — Deep dive notes for Low-latency online inference
...
| Business SLI for Low-latency online inference | Product truth |
...
- Cost model at 100×
- Abuse cases unique to Low-latency online inference
...
*Enriched for interview drill · `low-latency-online-inference`*
