# System Design: Bookstore Pricing API with Batch Fetches

> **Focus areas:** Batch price API · Coalescing · Cache tiers · Partner feeds · Staleness labels · Rate limits
...
> **Interview theme:** Databricks — commerce API with batch semantics and amplification control
...
1. `POST /v1/prices` — fan-out to sellers for ISBN, aggregate within overall deadline.
...
> Design a batch bookstore pricing API that coalesces upstream ISBN fetches under a hard latency budget, aggregates partial results, caches/coalesces aggressively, and places orders via a crash-safe charge→order saga with idempotency—evolving from ~500 quote QPS through 10× / 100× / 1,000× by reducing fan-out amplification.
...
BatchPriceRequest       → isbn, bid_cents, customer_id, idempotency_key, deadline
SellerOffer        → seller_id, price_cents, in_stock, retrieved_at, freshness
BatchPriceResult        → UNAVAILABLE | ABOVE_BID | ORDER_PLACED
...
Two-phase optional: `POST /v1/prices` then `POST /orders` with `quote_id` for clearer UX.
...
1. Amplification melting seller APIs  
...
### 7.1 Batch API follow-ups

**Q: Batch of 100 with one slow ISBN?**  
A: Per-ISBN deadline inside batch budget; return UNAVAILABLE for slow item; do not block siblings.

**Q: Same ISBN different batches concurrent?**  
A: Singleflight on isbn key merges upstream either way.

**Q: GraphQL vs REST batch?**  
A: REST POST batch simpler for rate limit cost = len(isbns); GraphQL optional later.

**Q: Webhook on price change?**  
A: Phase 2: feed processor emits events; premium tier subscribers.
