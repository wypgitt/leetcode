...
> **Focus areas:** Object storage · Table transaction log · Medallion layers · Metadata/data plane split · Multipart · ACID commits
...
> **Interview theme:** Databricks — lakehouse foundation combining S3-like blob storage with Delta-style table commits on object storage
...
### 7.1 Lakehouse-specific

**Q: Why separate object storage from table log?**  
A: Blobs scale cheaply with EC; table log gives atomic version boundaries and time travel without mutating Parquet files.

**Q: What breaks with concurrent writers without OCC?**  
A: Lost updates — two writers both publish overlapping file sets; readers see inconsistent snapshots.

**Q: How handle vacuum/delete files still referenced?**  
A: Retention window; log retains remove actions; vacuum only deletes files unreferenced by any version ≥ retention cutoff.

**Q: S3 listing at scale?**  
A: Avoid full prefix list per query; checkpoint + incremental log tail; partition pruning via stats in log metadata.

**Q: Compare to Hive-style partition directories?**  
A: Open table format adds transactional commit boundary; Hive lacks atomic multi-file commits across partitions.
