...
> **Focus areas:** Sync CRUD · Async job enqueue · Status polling · Webhooks · Outbox · Idempotent workers
...
> **Interview theme:** Databricks — CRUD API with durable async jobs, status polling, and webhook delivery
...
Goal: bound **async financial workflows**—state machines, timers, compensations, idempotent steps.

### 1.1 Functional Requirements

| # | Question to ask | Expected / typical interviewer answer | Design implication |
|---|---|---|---|
| F1 | Workflow? | Durable state machine | Not in-memory |
| F2 | Steps? | Idempotent by step key | Attempt epoch |
| F3 | Timers? | At-least-once fire | Dedupe lease |
| F4 | Compensation? | Reverse prior steps | Idempotent undo |
| F5 | Human task? | Manual approve | Dual control |
| F6 | Ledger? | Step posts journal | Linked |
| F7 | Visibility? | Status query API | History append |
| F8 | Version? | Workflow definition ver | Migrate carefully |
| F9 | Pause? | Admin signal | Audit |
| F10 | Multi-region? | SW workflow home | Fence timers |
| F11 | Poison? | DLQ step | Don't block |
| F12 | Batch payout? | Fan-out child workflows | Parent tracks |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Async Financial Workflow Scheduling
4. Admin recon/replay hooks for Async Financial Workflow Scheduling
...
- Replacing all Stripe production systems in one Async Financial Workflow Scheduling design
...
1. Workflow? → Durable state machine → durable write + audit
2. Steps? → Idempotent by step key → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Human task? → safe degradation documented
...
> Design financial workflows ~10K events/s payouts fees sweeps durable.
...
### 3.11 Domain model (Async Financial Workflow Scheduling)
...
### 4.1 End-to-end (Async Financial Workflow Scheduling)
...
```text
API Edge → Async Financial Workflow Scheduling → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of async financial workflow scheduling system design.*

### 8.19 Async Financial Workflow Scheduling — rollout checklist
...
```
*End of crud async jobs HLD prep.*
