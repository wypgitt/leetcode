...
> **Focus areas:** Token bucket · Hierarchical limits · Hypervisor enforcer · Burst · Fairness · Control plane push
...
> **Interview theme:** Databricks — per-VM bandwidth enforcement on host/hypervisor with hierarchical quotas
...
Goal: bound **rate limiting** for Stripe APIs—algorithms, distribution, fairness, fail-closed money routes.

### 1.1 Functional Requirements

| # | Question to ask | Expected / typical interviewer answer | Design implication |
|---|---|---|---|
| F1 | Keys? | API key+IP+route | Composite key |
| F2 | Algorithm? | Token bucket+sliding | Hybrid |
| F3 | Burst? | Allow N burst | Configurable |
| F4 | Distributed? | Edge+central reconcile | Eventual fair |
| F5 | Money routes? | Fail closed if uncertain | 429 not unlimited |
| F6 | Overrides? | Support plans | Quota admin API |
| F7 | Response? | 429 Retry-After | Headers |
| F8 | Multi-region? | Local counters sync | Merge windows |
| F9 | Accuracy? | ~exact per key | Not global single counter |
| F10 | Observability? | Usage metrics | Per tenant |
| F11 | Testing? | Load test fairness | Noisy neighbor |
| F12 | Rollout? | Shadow limit | Compare 429 rate |

**MVP functional scope (lock with interviewer):**

1. Internal allow(key,cost) at edge
2. `POST /v1/limits/rules admin` — idempotent where mutating.
3. `GET /v1/limits/usage/{tenant}` — idempotent where mutating.
4. Token bucket + sliding window hybrid
5. Per-route override API
6. Usage metrics export
...
1. Keys? → API key+IP+route → durable write + audit
2. Algorithm? → Token bucket+sliding → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Money routes? → safe degradation documented
...
> Design distributed rate limiter ~100K decisions/s to 1000x fair per tenant.
...
```text
100K decisions/s baseline API edge
...
```text
Counter state ephemeral+snapshots
...
```text
Money routes 429 when limiter unavailable
```
...
```text
Money-critical routes return 429 when limiter uncertain—never fail open unlimited
```
...
| API | Semantics |
|---|---|
| `Internal: allow(key,cost)` | Internal protocol |
| `POST /v1/limits/rules` | Idempotent / typed errors / RYW where applicable |
| `GET /v1/limits/usage/{tenant}` | Idempotent / typed errors / RYW where applicable |

Stripe-flavored: `Idempotency-Key`, `Request-Id`, typed errors, expandable objects.

### 3.11 Domain model (Rate Limiter)
...
A: Bucket for burst; sliding for strict RPM contracts — hybrid common.

### 7.22

**Q: Global counter Redis?**  
A: No at scale — shard by hash(key) mod N.
...
```text
API Edge → Rate Limiter → Domain Store
                              │
                              ├─► Idempotency / Audit
...
Related docs: transaction-apis-log, metrics-service, payment-processing.
...
### 8.19 Rate Limiter — rollout checklist
...
```
*End of vm bandwidth rate limiter HLD prep.*
