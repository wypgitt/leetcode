# System Design: Stock Order Manager

> **Focus areas:** Order lifecycle · GTC/GTD · Cancel vs execute · Idempotency · Venue gateway · Timers · Audit
...
> **Interview theme:** Databricks — high-reliability order lifecycle manager connecting merchants, issuers, and acquirers
...
Goal: bound **end-to-end payments**—Order, rails, ledger, uncertain windows.
...
| F10 | Partial capture? | Amount <= place orderd | Multiple captures policy |
...
1. Create Order → requires_confirmation → confirm → processing → succeeded
...
| Capture > place orderd | 400 invalid amount |
...
Capture: confirm amount ≤ place orderd — may be async batch
...
**Q: Partial capture?**  
A: Multiple capture calls; track captured_sum ≤ place orderd.
