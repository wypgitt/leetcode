# System Design: Safe Server Restart Mechanism (N Servers)

> **Focus areas:** Drain · quorum · rollout waves · health · sticky sessions · Apple SRE · zero/near-zero user-visible downtime  
...
Goal: **bound the product**—a control plane that safely restarts or redeploys N servers (stateless + lightly stateful) without dropping critical in-flight work.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Server roles? | Stateless API + some sticky/session + queues | Classify restart classes |
| F2 | Traffic? | L7 LB / mesh | Drain via readiness |
| F3 | In-flight? | Finish or handoff within deadline | Grace period + reject new |
| F4 | Batching? | Max % concurrent restart | Wave controller |
| F5 | Health? | Deep health + synthetic probes | Gate next wave |
| F6 | Stateful? | Leader election / lease rebalance | Fence old leaders |
| F7 | Config/secrets? | Inject before ready | Fail closed if missing |
| F8 | Rollback? | Auto on error budget burn | Fast previous artifact |
| F9 | Dependencies? | Respect downstream capacity | Stagger + jitter |
| F10 | Human gates? | Optional for prod high-risk | Change tickets |
| F11 | Multi-region? | Region-serial or AZ-serial | Never global simultaneous |
...
|---|----------|-----------------|--------|
| N1 | User-visible errors during restart? | Near zero | Error budget gate on waves |
...
|--------|----------|-----|------|--------|
| Servers N | 100 | 1K | 10K | 100K |
| Services | 20 | 100 | 500 | 2K |
| Restarts / day | 50 | 500 | 5K | 50K |
| Max concurrent draining | 5% | 5% | 2–5% | 1–2% |
| Synthetic probe QPS | 1K | 5K | 20K | 50K |
| Regions | 2 | 3 | 5 | 10 |
...
> Design safe restart/rolling upgrade for a fleet of N servers: drain, readiness, batching, blast-radius limits, rollback—with Apple-style availability and privacy of ops telemetry.
...
```text
N=10K, 2% waves → 200 at a time
If each drain 60s + bake 120s → wave ~3 min; full fleet ~3min × 50 ≈ 150 min (+ jitter)
Error budget: halt if burn > threshold
```
...
3. **Primary service(s) for Safe Server Restart Mechanism (N Servers)** — business state machines & APIs.  
...
### 3.4a Restart state machine

```text
SELECTED → DRAINING (fail readiness) → WAIT_INFLIGHT → STOP → UPGRADE → START → HEALTH → BAKE → DONE
                                  ↘ ABORT_ROLLBACK
```

Waves gated by error budget, dependency capacity, and PDB-like constraints.
...
```text
 Orchestrator -> select wave -> LB drain -> instance stop/upgrade -> health -> bake
       |                                      |
       +------ error budget gate / rollback --+
 Dependencies capacity checker ----/ 
```
...
1. Durability before ACK for accepted critical writes in Safe Server Restart Mechanism (N Servers).
...
**1× — correct MVP for Safe Server Restart Mechanism (N Servers)**
...
Never restart all regions; respect pod disruption; quorum services restart followers first; leaders with transfer.
...
### 6.2 Top risks for Safe Server Restart Mechanism (N Servers)
...
> Design Safe Server Restart Mechanism (N Servers) with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Safe Server Restart Mechanism (N Servers)-specific

**Q: Sticky sessions?**  
A: Drain until session ends or reauth; prefer session externalization.

**Q: Stateful leader?**  
A: Transfer leadership before restart; fence old.
...
Topic accents for **Safe Server Restart Mechanism (N Servers)** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
## 9. Extended Deep Dive — Safe Server Restart

### 9.1 Apple platform context

Apple interviewers expect explicit treatment of **privacy** (what is logged/stored), **authentication** (human SSO + service mTLS), **offline/degraded behavior**, and **on-device vs server** boundaries. This section maps those lenses onto safe server restart.
...
For safe server restart, classify every field before adding to schemas (public / internal / confidential / restricted).
...
Name the **first bottleneck** you expect for safe server restart (usually hot keys, write QPS, or cross-region latency).
...
## 9. Extra deep dive — Safe restart (Apple SRE)

### 9.1 Restart classes
| Class | Examples | Strategy |
|-------|----------|----------|
| Stateless API | edge microservices | % waves, readiness drain |
| Sticky session | legacy web | externalize session or drain to zero |
| Queue consumers | Kafka/SQS workers | stop lease → finish → exit |
| Leaders | schedulers, block primary | transfer + fence epoch first |
| Stateful datastores | Redis/PG | follow operator runbook; not naive SIGTERM |

### 9.2 Wave math worked example
```text
N=10_000 instances, maxUnavailable=2%
wave_size=200
drain=45s, stop/start=30s, bake=120s → ~195s/wave
waves=50 → ~162 minutes (+ jitter 10–20%)
halt if burn rate of 5xx > 2× baseline for 5m
```

### 9.3 Health vs readiness
- **liveness**: process stuck → kill/restart locally (careful with deadlock)
- **readiness**: remove from LB; used for drain
- **deep health**: dependency checks during bake, not on every probe

### 9.4 Quorum services
Restart followers first; for leaders: step-down → wait catch-up → fence old → restart.
Never simultaneous AZ-wide restart of a quorum.

### 9.5 Sticky sessions & Apple clients
Prefer tokenized sessions in shared store. If sticky unavoidable: connection draining with long grace; clients reconnect with jitter.

### 9.6 Secrets/config on restart
Fail closed if secrets missing; config version must match artifact; no “start then fetch later” for auth keys.

### 9.7 Multi-region policy
Serialize region restarts for shared-fate control planes; independent for cell-local services.
Canary region first.

### 9.8 Observability during rollout
Track: in-flight gauge, drain duration histogram, wave error budget, synthetic probe success, restart epoch.

### 9.9 Failure scenarios
| Scenario | Response |
|----------|----------|
| Bake fails | auto-rollback wave; freeze controller |
| LB flap | pause waves; investigate health hysteresis |
| Dependency brownout | reduce wave size; extend bake |
| Human abort | controller enters ABORTED; leave remaining on old |

### 9.10 Interview traps
- Restarting all regions at once
- Treating leader like stateless
- Using only liveness for drain
- No error-budget gate
- Ignoring in-flight non-idempotent writes

### 9.11 60s summary
> Classify servers; drain via readiness; wave with maxUnavailable and error-budget gates; transfer/fence leaders; serialize blast radius across AZ/region; rollback fast from immutable artifacts.

### 9.12 API sketch — restart controller
```text
POST /v1/rollouts {service, artifact, strategy, maxUnavailable}
GET  /v1/rollouts/{id}
POST /v1/rollouts/{id}/abort
POST /v1/rollouts/{id}/resume
```

### 9.13 State machine
```text
PENDING → WAVE_I → DRAIN → RESTART → BAKE → WAVE_I+1 → SUCCEEDED
                 ↘ FAILED → ROLLING_BACK → ABORTED/ROLLED_BACK
```

### 9.14 PDB / capacity checker
Refuse wave if remaining healthy < min_capable for peak QPS. Include dependency capacity (DB connections).

### 9.15 Chaos drills
1. Kill instance mid-drain
2. Blackhole dependency during bake
3. Split-brain old leader after transfer
4. Clock skew on bake timers
