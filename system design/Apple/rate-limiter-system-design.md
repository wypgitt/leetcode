...
> **Focus areas:** Token bucket · sliding window · distributed counters · fail-open/closed · Apple API gateway · privacy of keys · offline clients  
...
Goal: **bound the product**—a distributed rate-limiting service used in front of Apple-style public/partner APIs and internal microservices.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Who is limited? | API keys, users, IPs, devices | Key taxonomy + authn |
| F2 | Algorithms? | Token bucket + sliding window | Pluggable limiters |
| F3 | Where enforced? | Edge gateway + service mesh sidecar | Defense in depth |
| F4 | Consistency? | Approx OK within ε; optional strong for payments | Redis cluster / local+sync |
| F5 | Response? | 429 + Retry-After + headers | Standard client contract |
| F6 | Quotas vs rate? | Both: sustained + burst | Separate counters |
| F7 | Overrides? | Admin allowlist / incident raise | Audited config |
| F8 | Fail mode? | Fail-closed for abuse-sensitive; fail-open for reads optional | Per-route policy |
| F9 | Multi-region? | Per-region budgets + global soft caps | Avoid cross-region sync on hot path |
| F10 | Client offline? | Device may self-throttle; server authoritative when online | On-device token cache advisory |
| F11 | Analytics? | Aggregates only | No raw key→user join in logs |
...
|---|----------|-----------------|--------|
| N1 | Decide latency? | Inline on API | p99 < 5–10ms in-region add |
...
|--------|----------|-----|------|--------|
| Distinct keys | 10M | 100M | 1B | 10B |
| Peak check QPS | 100K | 1M | 10M | 100M |
| Config updates / day | 10K | 100K | 1M | 10M |
| Regions | 3 | 5 | 10 | 20 |
| Routes/policies | 1K | 5K | 20K | 50K |
| Hot key QPS (single) | 10K | 50K | 200K | 1M |
...
**Topic accent:** 10×: Redis cluster local region; 100×: key sharding + hot-key split; 1000×: hierarchical local enforcement + global soft reconciler.
...
> Design a rate limiter supporting per-key algorithms, distributed accuracy trade-offs, fail-open vs fail-closed policy by tier, and privacy-safe keying—scaled 10×/100×/1000×.
...
```text
Peak 100M checks/s at 1000×
If each check = 1 Redis RTT locally: need massive sharded fleet
Budget: p99 <+10ms → mostly in-region memory; no cross-region sync on check
```

### 2.2 State size

```text
Counter key ~64–128 B including metadata
10B keys × 100 B = 1 TB raw (plus replicas) → sharding + TTL mandatory
Hot keys: replicate locally (L1) on gateway shards
```

### 2.3 Config propagation

```text
50K policies: push via versioned config snapshots; clients subscribe deltas
Don't read policy from SQL on every request
```
...
|-------|----------------|-------------|
| Policy config | Limits & routes | Versioned eventual |
| Counter plane | Token/windows | Approx or strong per SKU |
| Enforcement | Gateway/sidecar | Fail policy explicit |
| Admin/audit | Overrides | Strong audit log |
...
3. **Primary service(s) for Rate Limiter** — business state machines & APIs.  
...
| Sliding window log/counter | Smooth abuse |
| Fixed window | Simple; boundary spikes |
| GCRA / leaky | Telecom-smooth |

**Distributed:** sharded counters by `hash(key)`; hot keys use local gateway token caches reconciled asynchronously with bounded error **or** dedicated hot-key shards.
...
```text
 Client -> Edge GW --check--> Limiter shard (hash key)
              |                    |
              | deny 429           +--> hot-key local token cache
              v
           Service
 Config control plane --> versioned snapshots --> GW/sidecars
```
...
1. Durability before ACK for accepted critical writes in Rate Limiter.
...
**1× — correct MVP for Rate Limiter**
...
### 5.5 Accuracy vs availability

Document ε error under async reconciliation. Payments/abuse: fail-closed. Public read fanout: may fail-open with local limits. Hot key: split into `key#0..k` or sticky shard.
...
### 6.2 Top risks for Rate Limiter
...
> Design Rate Limiter with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Rate Limiter-specific

**Q: Central Redis or local?**  
A: Local+reconcile for mega QPS; central for strong enforcement SKUs.

**Q: How to handle hot API keys?**  
A: Dedicated shards / key splitting / hierarchical limits.
...
Topic accents for **Rate Limiter** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Rate Limiter
         -> Durable SoT
         -> Async workers
...
> I'll split **Rate Limiter** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Rate Limiter |
|-------|------------------------|
...
*End of Rate Limiter system design.*

### 8.18 Deep topic notes — Rate Limiter


##### Check API
```text
Check(key, cost, labels) -> {allow, remaining, reset_ms, retry_after}
Reserve/Settle optional for multi-step admissions (LLM-like TPM)
```

##### Algorithms
Token bucket (burst); sliding window counter (smooth); GCRA optional.
Hierarchical: user → app → org → global.

##### Hot keys
Local gateway allotment with async reconciliation; bounded overshoot ε documented.
Fail-closed for auth/abuse routes; fail-open optional for public GETs with tiny local cap.

##### Privacy-safe keys
Prefer `hash(api_key_id)` / opaque account id — never email/IP raw as primary key in durable analytics.
