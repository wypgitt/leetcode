# System Design: Legacy Storage → New Storage Migration

> **Focus areas:** Dual-read/write · checksums · backfill · cutover · rollback · Apple iCloud-class data · encryption key continuity  
...
Goal: **bound the product**—a migration system that moves durable data from a legacy storage system to a new one with correctness, privacy, and minimal user-visible disruption.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Data types? | Objects + metadata DB | Separate planes |
| F2 | Downtime budget? | Near-zero preferred | Dual-read/write |
| F3 | Consistency? | No silent divergence | Checksum verify |
| F4 | Encryption? | Keys must continue to decrypt | Key migration plan |
| F5 | Backfill? | Bulk copy + catch-up | Watermarks |
| F6 | Cutover? | Per-shard / per-tenant | Feature flags |
| F7 | Rollback? | Keep legacy writable window | Reverse sync optional |
| F8 | Deletes? | Tombstones replicate both ways in dual window | Privacy correctness |
| F9 | Perf? | p99 not regress >X% | Shadow traffic |
| F10 | Ownership? | Migration controller owns progress | Single orchestrator |
| F11 | Audit? | Per-object verification reports | Evidence for compliance |
...
| Migration bandwidth | 10 Gbps | 50 Gbps | 200 Gbps | 1 Tbps |
| Dual-write QPS | 10K | 100K | 1M | 10M |
| Verify rate objects/s | 5K | 50K | 200K | 1M |
| Tenants/shards | 100 | 1K | 10K | 100K |
...
> Design storage migration: inventory, dual-write/dual-read, bulk backfill, verification, cutover, rollback—with encryption and deletion semantics preserved.

---

## 2. Back-of-the-Envelope Estimation

### 2.1 Migration window

```text
100 PB at 50 Gbps ≈ 100e15 bits / 50e9 ≈ 2e6 seconds ≈ 23 days raw
⇒ need parallelism, prioritization (hot tenants first), and checksum cycles
```
...
3. **Primary service(s) for Legacy Storage → New Storage Migration** — business state machines & APIs.  
...
### 3.4a Migration phases

```text
0 Inventory  1 Dual-write  2 Backfill  3 Dual-read (shadow verify)
4 Cutover per shard  5 Legacy drain  6 Decommission
```

Checksums + sampled byte compares; deletes/tombstones in both systems during dual window.
...
```text
 App -> Migration proxy (dual-write/dual-read flags) -> Legacy + New stores
                |                              ^
                +--> Backfill workers ----------+
                +--> Verifier (checksums)
```
...
1. Durability before ACK for accepted critical writes in Legacy Storage → New Storage Migration.
...
**1× — correct MVP for Legacy Storage → New Storage Migration**
...
### 5.5 Correctness protocol

Per-object: size + strong hash; dual-read compare sampled %; continuous catch-up consumer from legacy changelog; cutover only when error budget of mismatches met.
...
### 6.2 Top risks for Legacy Storage → New Storage Migration
...
> Design Legacy Storage → New Storage Migration with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Legacy Storage → New Storage Migration-specific

**Q: When to cut over?**  
A: Mismatch rate below threshold; dual-read green; rollback rehearsed.

**Q: Encryption keys?**  
A: Re-wrap DEKs; never leave plaintext windows unlogged.
...
Topic accents for **Legacy Storage → New Storage Migration** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
## 9. Extended Deep Dive — Storage System Migration

### 9.1 Apple platform context

Apple interviewers expect explicit treatment of **privacy** (what is logged/stored), **authentication** (human SSO + service mTLS), **offline/degraded behavior**, and **on-device vs server** boundaries. This section maps those lenses onto storage system migration.
...
For storage system migration, classify every field before adding to schemas (public / internal / confidential / restricted).
...
Name the **first bottleneck** you expect for storage system migration (usually hot keys, write QPS, or cross-region latency).
...
## 9. Extra deep dive — Storage migration

### 9.1 Phases
Inventory → Dual-write → Backfill → Dual-read verify → Per-shard cutover → Drain legacy → Decommission

### 9.2 Dual-write matrix
Puts/deletes hit both; reads shadow-compare; privacy deletes must tombstone both.

### 9.3 Verifier
size + cryptographic hash; sampled byte compare; continuous changelog catch-up; mismatch budget gates cutover.

### 9.4 Encryption continuity
Re-wrap DEKs to new KMS; never plaintext bulk export windows without audit.

### 9.5 Bandwidth planning
```text
bytes / throughput = window; parallelize by shard/tenant; prioritize hot keys
```

### 9.6 Rollback
Keep legacy writable until verify green; feature-flag read primary; reverse sync optional.

### 9.7 Apple privacy angle
Migration logs contain object ids/hashes—not contents; residency preserved (EU stays EU).

### 9.8 Deal-breakers
Cutover without verifier; ACK only on one side during dual-write for durable user data; losing deletes.

### 9.9 60s summary
> Dual-write + backfill + checksum verify + shard cutover with rollback window; keys and deletes first-class; residency honored.
