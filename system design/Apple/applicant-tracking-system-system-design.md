# System Design: Applicant Tracking System

> **Focus areas:** Pipelines · PII · RBAC · audit · interview loops · Apple hiring privacy · retention · GDPR/CCPA-style deletion  
...
Goal: **bound the product**—an applicant-tracking system (ATS) for recruiting: applications, pipelines, interviews, offers—with strict PII controls.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Users? | Candidates, recruiters, interviewers, admins | RBAC |
| F2 | Core objects? | Requisition, application, interview, offer | State machines |
| F3 | Documents? | Resumes/PDFs | Object store + virus scan |
| F4 | PII? | High sensitivity | Field-level access + encryption |
| F5 | Pipeline? | Configurable stages | Per-req templates |
| F6 | Scheduling? | Hooks to calendar | Out of deep MVP ok |
| F7 | Search? | Candidate search by skills | Indexed fields with ACL |
| F8 | Audit? | Who viewed/exported PII | Mandatory audit |
| F9 | Retention? | Legal holds + deletion | Privacy workflows |
| F10 | Integrations? | Job boards, email | Inbound parsers |
| F11 | Global? | Residency for EU candidates | Region cells |
...
|--------|----------|-----|------|--------|
| Applications / year | 2M | 20M | 100M | 500M |
| Concurrent recruiters | 5K | 20K | 50K | 100K |
| Docs stored | 5M | 50M | 200M | 1B |
| Peak API QPS | 200 | 2K | 10K | 50K |
| Audit events / day | 1M | 10M | 50M | 200M |
| Regions | 2 | 3 | 5 | 8 |
...
> Design ATS: candidate profiles, job reqs, stage transitions, interviewer scheduling hooks, documents, audit, retention/deletion—privacy-first.

---

## 2. Back-of-the-Envelope Estimation

### 2.1 PII-heavy storage

```text
50M docs × 500KB avg = 25 PB — object store
Metadata DB much smaller but ACL/audit heavy
```
...
3. **Primary service(s) for Applicant Tracking System** — business state machines & APIs.  
...
```text
 Candidates/Recruiters -> API -> ATS services -> PII-hardened DB
                              |                    |
                         Doc store (scan)      Audit log
                              |
...
1. Durability before ACK for accepted critical writes in Applicant Tracking System.
...
**1× — correct MVP for Applicant Tracking System**
...
Field-level RBAC; watermark exports; mandatory audit; residency; legal hold vs deletion conflict resolution workflow.
...
### 6.2 Top risks for Applicant Tracking System
...
> Design Applicant Tracking System with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Applicant Tracking System-specific

**Q: Right to be forgotten vs hiring records?**  
A: Legal hold workflow; minimize; timed retention schedules.

**Q: Interviewer bias tooling?**  
A: Out of scope unless asked; don't log protected attrs casually.
...
Topic accents for **Applicant Tracking System** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Applicant Tracking System
         -> Durable SoT
         -> Async workers
...
> I'll split **Applicant Tracking System** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Applicant Tracking System |
|-------|------------------------|
...
*End of Applicant Tracking System system design.*

### 8.18 Deep topic notes — Applicant Tracking System


##### Entities
Req → Application → StageTransition → Interview → Feedback → Offer.
RBAC: recruiter, hiring manager, interviewer (blinded fields), admin, compliance.

##### PII
Field-level encrypt; export watermark; audit every view; EU residency cells.
