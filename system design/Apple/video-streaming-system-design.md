# System Design: Video Streaming System (Netflix-class / Apple TV+ vibes)

> **Focus areas:** Ingest · ABR · CDN · DRM · offline downloads · privacy of watch history · Apple TV+ style product constraints  
...
Goal: **bound the product**—a consumer video streaming service: catalog, playback, ABR, CDN, DRM, recommendations hooks, offline downloads.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Content? | VOD MVP; live Phase 2 | Pipeline differences |
| F2 | Ingest? | Mezzanine upload → transcode ladder | Job scheduler |
| F3 | Playback? | HLS/DASH ABR | Manifest service |
| F4 | CDN? | Multi-CDN / edge cache | Origin shield |
| F5 | DRM? | FairPlay/Widevine-class | License service |
| F6 | Offline? | Download rental/purchase | Secure local store |
| F7 | Accounts? | Profiles; kids mode | Authz |
| F8 | Progress? | Resume across devices | Sync with privacy |
| F9 | Recs? | Optional; on-device signals preferred | Boundary |
| F10 | Subtitles? | Sidecar timed text | Packaging |
| F11 | Analytics? | QoE aggregates; DP for taste | Minimize watch dossiers |
| F12 | Privacy / logging? | Minimize PII; retention caps; deletion APIs | Privacy budget + scrubbing pipelines |

**MVP functional scope (lock with interviewer):**

1. VOD ingest→ladder→CDN→DRM→player→progress sync.
...
- Global live sports ultra-low-latency as MVP
...
|---|----------|-----------------|--------|
| N1 | Startup time? | Play fast | TTFF p50 < 1–2s on good net |
...
|--------|----------|-----|------|--------|
| Subscribers | 10M | 100M | 500M | 1B |
| Peak concurrent streams | 1M | 10M | 50M | 200M |
| Catalog titles | 10K | 50K | 200K | 500K |
| Egress Gbps | 10Tbit peak class | ×10 | ×100 | ×1000 class |
| Transcode jobs / day | 5K | 50K | 200K | 1M |
| Watch progress QPS | 20K | 200K | 1M | 5M |
...
> Design video streaming: upload/transcode, manifest, CDN edge, DRM license, client player, watch progress sync with privacy controls.
...
```text
10M concurrent × 5 Mbps ≈ 50 Tbps class — CDN problem, not origin
Origin shield hit ratio dominates cost
```

### 2.2 Packaging

```text
Ladder: 6–8 rungs; segment 2–6s
Transcode: minutes of mezzanine → many CPU/GPU hours; queue + priority
```

### 2.3 Progress sync

```text
1M concurrent × heartbeat every 10s ≈ 100K QPS — batch/coalesce; home cell per user
```
...
3. **Primary service(s) for Video Streaming System (Netflix-class / Apple TV+ vibes)** — business state machines & APIs.  
...
```text
Client → license + manifest → CDN segments → decrypt/decode
Progress: coalesced writes to user home cell; privacy controls for history
```
...
```text
 Studio upload -> Transcode ladder -> Packager -> Origin -> CDN -> Player
                     |                               |
                  Job scheduler                 DRM license service
 User progress sync -> home cell (privacy controls)
```
...
1. Durability before ACK for accepted critical writes in Video Streaming System (Netflix-class / Apple TV+ vibes).
...
**1× — correct MVP for Video Streaming System (Netflix-class / Apple TV+ vibes)**
...
Multi-CDN failover; license short TTL; offline content encrypted with device-bound keys; watch history minimization / limit sync fields.
...
### 6.2 Top risks for Video Streaming System (Netflix-class / Apple TV+ vibes)
...
> Design Video Streaming System (Netflix-class / Apple TV+ vibes) with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Video Streaming System (Netflix-class / Apple TV+ vibes)-specific

**Q: Origin vs CDN responsibility?**  
A: CDN egress; origin shield protects packager.

**Q: Live vs VOD?**  
A: Live needs lower-latency packager & different failure modes—Phase 2.
...
Topic accents for **Video Streaming System (Netflix-class / Apple TV+ vibes)** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Video Streaming System (Netflix-class / Apple TV+ vibes)
         -> Durable SoT
         -> Async workers
...
> I'll split **Video Streaming System (Netflix-class / Apple TV+ vibes)** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Video Streaming System (Netflix-class / Apple TV+ vibes) |
|-------|------------------------|
...
*End of Video Streaming System (Netflix-class / Apple TV+ vibes) system design.*

### 8.18 Deep topic notes — Video Streaming System (Netflix-class / Apple TV+ vibes)


##### Ladder & ABR
Rungs 240p–4K; 2–6s segments; player picks by bandwidth/buffer.
DRM license short-lived; offline content device-bound keys.
Progress coalesced; history minimization controls.
