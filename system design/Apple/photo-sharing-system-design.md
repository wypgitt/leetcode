# System Design: Scalable Photo-Sharing Service

> **Focus areas:** Upload · derivatives · feed · ACL · iCloud Photos vibes · on-device processing · HEIC · privacy  
...
Goal: **bound the product**—a scalable photo-sharing service (albums, share links, feeds) with Apple-like privacy and on-device processing hooks.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Upload? | Resumable chunked | Dedup by hash |
| F2 | Derivatives? | Thumbs/web/HEIC variants | Async workers |
| F3 | Library? | Albums + library view | Metadata index |
| F4 | Sharing? | Link / people ACL | Capability URLs + authz |
| F5 | Feed? | Friends activity optional | Fanout or pull |
| F6 | On-device? | Memories/faces on-device preferred | Server never needs faces raw |
| F7 | EXIF? | Strip or encrypt sensitive GPS per policy | Privacy defaults |
| F8 | Deletion? | Immediate unlink + async purge | Legal retention exceptions |
| F9 | CDN? | Authz'd signed URLs | Short TTL |
| F10 | Conflicts? | Multi-device library sync | Vector clocks / sync tokens |
| F11 | Search? | On-device index + optional server | Boundary |
...
|--------|----------|-----|------|--------|
| MAU | 50M | 500M | 1B | 2B |
| Uploads / day | 100M | 1B | 5B | 20B |
| Library photos stored | 10B | 100B | 1T | 5T |
| Peak upload Gbps | 50 | 500 | 2K | 10K |
| Feed read QPS | 100K | 1M | 5M | 20M |
| Derivative jobs / day | 300M | 3B | 15B | 60B |
...
> Design photo sharing: upload pipeline, derivatives, metadata index, sharing ACL, fetch/CDN, deletion—clear on-device vs server processing.

---

## 2. Back-of-the-Envelope Estimation

### 2.1 Upload & derivatives

```text
100M uploads/day × 3 derivatives ≈ 300M jobs
Bytes: avg 3MB original → 300 TB/day ingress class at baseline — CDN/object store
```
...
3. **Primary service(s) for Scalable Photo-Sharing Service** — business state machines & APIs.  
...
```text
 Camera roll -> Upload (resumable) -> Object store -> Derivative workers
                      |                    |
                 Metadata home cell     Signed CDN URLs
                      |
                 Sharing ACL / links
 On-device ML (memories/faces) optional; server minimization
```
...
1. Durability before ACK for accepted critical writes in Scalable Photo-Sharing Service.
...
**1× — correct MVP for Scalable Photo-Sharing Service**
...
### 5.5 Delete & share revoke

Capability revoke + CDN cache purge; async byte purge with verifier; strip GPS by default on public shares.
...
### 6.2 Top risks for Scalable Photo-Sharing Service
...
> Design Scalable Photo-Sharing Service with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Scalable Photo-Sharing Service-specific

**Q: Faces on server?**  
A: Prefer on-device; if server, isolate & encrypt & retention-tight.

**Q: Viral share storm?**  
A: CDN + capability tokens; rate limit create.
...
Topic accents for **Scalable Photo-Sharing Service** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Scalable Photo-Sharing Service
         -> Durable SoT
         -> Async workers
...
> I'll split **Scalable Photo-Sharing Service** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Scalable Photo-Sharing Service |
|-------|------------------------|
...
*End of Scalable Photo-Sharing Service system design.*

### 8.18 Deep topic notes — Scalable Photo-Sharing Service


##### Upload
Resumable chunks; hash dedup; async derivatives; metadata home cell.
Share capabilities revoke → CDN purge.
Faces/memories on-device preferred; GPS stripped on public links by default.
