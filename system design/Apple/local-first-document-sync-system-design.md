# System Design: Local-First Document Synchronization

> **Focus areas:** iCloud Drive · File versions · CRDT metadata · Conflict copies · E2E · On-demand fetch · Battery  
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Granularity? | Files + folders + packages | Tree + blob refs |
| F2 | Conflicts? | Keep both copies | Conflict file naming UX |
| F3 | Versions? | History N deep | Snapshot chain |
| F4 | Partial sync? | On-demand download | Placeholder + fetch |
| F5 | E2E? | Optional ADP | Ciphertext blobs |
| F6 | Apps? | Bundle packages atomic | Temp + commit |
| F7 | Delete? | Trash window | Tombstones |
| F8 | Sharing? | Link/folder share | ACL + share keys |
| F9 | Offline? | Edit local copy | Upload on reconnect |
| F10 | Large files? | Chunked resumable | Upload sessions |
| F11 | Watchers? | FSEvents local | Debounce cloud push |
| F12 | Desktop sync? | Mac full tree | Same protocol |

**MVP functional scope (lock with interviewer):**

1. Local-first open
2. Metadata log + blob store
3. Conflict copies
4. On-demand fetch
5. Trash + restore
6. Resumable upload
7. Debounced directory sync

**Out of MVP (explicitly defer):**

- Real-time multi-user OT
- Deduup across users at 1,000×
- Block-level sync inside files

### 1.2 Non-Functional Requirements

| # | Question | Expected answer | Target |
|---|----------|-----------------|--------|
| N1 | Privacy | E2E option | Opaque blobs |
| N2 | Open local file | Instant | Local path < 100ms |
| N3 | Hydrate cloud-only | User-visible | p50 < 3s Wi‑Fi |
| N4 | Battery | Defer bulk | Wi‑Fi/charging bias |
| N5 | Durability | No silent loss | ACK before eviction |
| N6 | Consistency | Eventual metadata | Conflict copies on write clash |
| N7 | Availability | 99.9% | Offline OK |
| N8 | Quota | Enforced | Pause uploads |

### 1.3 Cases (Flows & Edge Cases)

**Happy paths**

1. Create doc offline → save local → upload → iPad sees file
2. Edit same file offline on two Macs → two conflict copies
3. Evict local → cloud placeholder → fetch on open
4. Move folder → tree ops batch sync
...
| Case | Behavior |
|------|----------|
| Package half-written | Atomic commit marker |
| Huge video project | Chunked |
| Rename vs edit race | Separate ops |
| Symlinks | Policy: no or materialize |
| iCloud strip on low disk | Never evict before ACK |
| Path case sensitivity | Normalize |
| Malware scan | Out of band optional |
| Share link revoke | Rekey |

### 1.4 Scales (Progressive)

| Metric | Baseline | 10× | 100× | 1,000× |
|--------|----------|-----|------|--------|
| Users | 100M | 400M | 800M | 800M+ |
| Files/user avg | 5K | 8K | 12K | 15K |
| Uploads/day | 200M | 2B | 20B | 200B |
| Avg file | 2 MB | 3 MB | 4 MB | 5 MB |
| Metadata ops/day | 1B | 10B | 100B | 1T |
| On-demand fetches/day | 500M | 5B | 50B | 500B |
| Peak upload QPS | 100K | 1M | 10M | 100M |
| Containers | 100M | 400M | 800M | 800M+ |

**What each jump forces:**

- **10×:** Shard containers; upload sessions
- **100×:** Regional blobs; directory snapshots
- **1,000×:** Cold tier; hierarchical cursors

### 1.5 Etc. (Constraints & Assumptions)

- Not block-level sync inside PDF
- Packages must commit atomically

**Scope statement:**

> Design local-first document sync (Pages/Files/iCloud Drive class): encrypted file blobs, metadata log, conflict copies, on-demand hydration, and battery-aware transfers at progressive scale.
...
```text
File uploads operations: 200M/day × 2 MB ≈ 400 PB/day? → 200e6 × 2e6 = 4e14 B = 400 TB/day
Avg payload: 2–5 MB avg
...
|--------|------|------|-------------------|
| A. Block-level sync | Efficient | Complex | MVP |
| B. Whole-file versioned | Simple | Bandwidth | — chosen |
| C. Last writer only | Easy | Data loss | Never |
| D. Central lock server | No conflicts | Offline bad | Mobile |

**Chosen path:** Whole-file content-addressed blobs with metadata log; write conflicts → sibling conflict copies.

### 3.4 Privacy, trust, and encryption

File keys in client; server stores ciphertext; paths may be encrypted in E2E mode.

### 3.5 Consistency, sync, and conflict resolution

Per-file version vector; conflict if concurrent writes without knowledge.

### 3.6 Offline-first behavior

Full local tree; outbox for ops; upload queue for bytes.

### 3.7 Battery, radio, and scheduling

Debounce tree ops; defer large files; coalesce metadata.

### 3.8 Multi-region / cell model

Regional blob placement; home cell metadata.

### 3.9 Abuse, auth, and quotas

Quota; rate limit download; share link TTL.
...
1. Local durable write before UI ACK for user mutations in **Local-First Document Synchronization**.
...
| Dual-writer split brain | Single-writer home cell per `container_id / file_id` |

### 5.2 Scalability & sharding

```text
container_id / file_id → home_cell (consistent hash / directory)
File metadata log + content-addressed blobs partitioned by container_id / file_id
...
- Observability: sizes, latencies, error codes—not plaintext **Local-First Document Synchronization** content.
...
Local-first document sync with versioned ciphertext blobs, tree metadata log, conflict copies, on-demand hydration, and resumable transfers.

### 6.2 Top trade-offs

| Trade-off | Choice | Why |
|-----------|--------|-----|
| Conflict copies vs merge | Copies | Simplicity |
| Whole file vs blocks | Whole file | MVP |
| Eager sync all | On-demand | Disk/battery |
| E2E vs server preview | E2E | Privacy |

### 6.3 MVP → scale path

MVP: file versions + conflicts. Scale: shards, regional blobs, snapshots.
...
**Q: What must never leave the device plaintext in Local-First Document Synchronization?**  
...
**Q: How is Local-First Document Synchronization different from generic cloud file sync?**  
...
1. Offline write for 24h then sync—no loss in Local-First Document Synchronization.
...
> For Local-First Document Synchronization: keep a crisp local-first story, put encryption and conflict rules on the client, let the server order opaque sync operations and store ciphertext, schedule radios for battery, and scale with per-user cells plus cursors/snapshots. Call out privacy mode forks explicitly.
...
1. Is Advanced Data Protection / E2E required or optional for Local-First Document Synchronization?
...
| Anti-pattern | Why it hurts Local-First Document Synchronization |
|--------------|----------------------|
...
*End of local-first document synchronization system design.*
