...
> **Focus areas:** Consistent hashing · replication · invalidation · thundering herd · Apple edge/API cache · privacy of cached PII  
...
Goal: **bound the product**—a distributed in-memory cache service for application data with strong operational controls.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | API? | GET/SET/DEL/MGET, TTL | Memcached/Redis-like |
| F2 | Consistency? | Per-key linearizable optional; usually eventual replica | Modes |
| F3 | Placement? | Consistent hashing + virtual nodes | Rebalance |
| F4 | Replication? | N replicas async/sync | SLA tiers |
| F5 | Invalidation? | TTL + explicit del + pubsub | Avoid stale forever |
| F6 | Stampede? | Singleflight / early refresh | Thundering herd |
| F7 | Persistence? | Optional AOF/snapshot | Cache ≠ SoT usually |
| F8 | Multi-tenant? | Namespaces + max memory | Noisy neighbor |
| F9 | PII? | Encrypt values / short TTL / no dump | Privacy |
| F10 | Hot keys? | Local L1 + negative cache | Split keys |
| F11 | Ops? | Autoscaling, cold start warmup | Slosh control |
...
|--------|----------|-----|------|--------|
| Keys | 1B | 10B | 100B | 1T |
| Peak QPS | 1M | 10M | 100M | 1B |
| Memory fleet | 10 TB | 100 TB | 1 PB | 10 PB |
| Nodes | 100 | 1K | 5K | 20K |
| Hot key QPS | 100K | 500K | 2M | 10M |
...
> Design distributed cache: get/set/delete, TTL, replication, consistency modes, stampede control, multi-tenant isolation, and PII handling.
...
```text
1B QPS at extreme with 20K nodes → 50K QPS/node average; hot keys break averages
Value size: 1KB avg × 1T keys cannot all be resident → admission + TTL + tiers
```
...
3. **Primary service(s) for Distributed Cache** — business state machines & APIs.  
...
```text
 App -> Proxy/Smart client -> Consistent-hash nodes (N replicas)
              |                     |
           singleflight          pubsub invalidation
              |
           optional L1 process cache
```
...
1. Durability before ACK for accepted critical writes in Distributed Cache.
...
**1× — correct MVP for Distributed Cache**
...
### 5.5 Stampede & consistency

Soft TTL + singleflight; replay protection on reconnect storms; don't use cache as SoT for balances/authz decisions without version checks.
...
### 6.2 Top risks for Distributed Cache
...
> Design Distributed Cache with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Distributed Cache-specific

**Q: Replication sync or async?**  
A: Async for latency; sync for small strong namespaces.

**Q: Memory pressure?**  
A: LRU/LFU + tenant caps; admission control.
...
Topic accents for **Distributed Cache** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Distributed Cache
         -> Durable SoT
         -> Async workers
...
> I'll split **Distributed Cache** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Distributed Cache |
|-------|------------------------|
...
### 8.18 Deep topic notes — Distributed Cache


##### Placement
Consistent hash + vnodes; replication factor R; rack-aware.
Stampede: singleflight + soft TTL probabilistic early refresh.
PII values: encrypt or short TTL; disable unauth dumps.
