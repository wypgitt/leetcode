# System Design: Deep-Dive / Redesign a System You Built

> **Focus areas:** Interview meta · ownership · metrics · failure stories · redesign trade-offs · Apple privacy/API lens · progressive hardening  
...
Goal: **bound the product**—an interview framework for defending and optionally redesigning a system you personally built (use a concrete example template).
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | What system? | Pick one you owned end-to-end | Concrete > generic |
| F2 | Users/SLOs? | State real numbers | Anchor estimation |
| F3 | Invariants? | Money/privacy/durability? | List hard invariants |
| F4 | Biggest outage? | Tell truth + fix | Reliability depth |
| F5 | Scale path? | What breaks at 10×? | Progressive redesign |
| F6 | APIs? | Show contracts | Versioning story |
| F7 | Storage? | SoT and why | Trade-offs |
| F8 | Privacy? | What never logged | Apple lens |
| F9 | Offline? | If mobile involved | Client boundary |
| F10 | What you'd redesign? | Honest 2–3 changes | Not rewrite fantasy |
| F11 | Ownership? | Oncall, metrics, cost | Operability |
...
|--------|----------|-----|------|--------|
| Example: QPS | 1K | 10K | 100K | 1M |
| Example: data size | 10 TB | 100 TB | 1 PB | 10 PB |
| Example: services | 5 | 20 | 50 | 200 |
| Example: oncall pages/w | 2 | 5 | 15 | 40 |
| Example: regions | 1 | 2 | 3 | 5 |
| Interview depth mins | 45 | 45 | 60 | 60 |
...
> Prepare to deep-dive a system you built: clarify scope, expose invariants, walk failures, propose redesign under 10×/100×/1000× with Apple-weighted privacy/API/storage concerns.

---

## 2. Back-of-the-Envelope Estimation

### 2.1 How to estimate in-interview

```text
Pick 3 numbers you know: DAU, QPS peak, data size
Derive: storage growth/day, fan-out, bottleneck
Show unit checks; Apple interviewers punish sloppy PB/TB mistakes
```
...
3. **Primary service(s) for Deep-Dive / Redesign a System You Built** — business state machines & APIs.  
...
### 3.4a Interview structure for “your system”

Use a real system: draw planes, list invariants, pick one redesign (e.g., home cells, privacy logs, lease fencing). Defend why MVP was right then and what 100× forces.
...
```text
 Interview narrative board:

  [Users/SLOs] -> [Planes] -> [Invariants] -> [Failure story]
        |                         |
        +-----> [10x/100x/1000x redesign] <----+
        +-----> [Privacy/Auth/Offline boundary]
```
...
1. Durability before ACK for accepted critical writes in Deep-Dive / Redesign a System You Built.
...
**1× — correct MVP for Deep-Dive / Redesign a System You Built**
...
### 5.5 How to score high

Be specific with numbers you owned; admit a real outage; propose a redesign that addresses a named bottleneck; map Apple themes (privacy/offline/auth) onto your system honestly.
...
### 6.2 Top risks for Deep-Dive / Redesign a System You Built
...
> Design Deep-Dive / Redesign a System You Built with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Deep-Dive / Redesign a System You Built-specific

**Q: What if I can't share proprietary details?**  
A: Generalize names; keep real numbers order-of-magnitude; focus on invariants.

**Q: Rewrite vs evolve?**  
A: Prefer strangler/facade; justify big-bang only with hard constraint.
...
Topic accents for **Deep-Dive / Redesign a System You Built** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Deep-Dive / Redesign a System You Built
         -> Durable SoT
         -> Async workers
...
> I'll split **Deep-Dive / Redesign a System You Built** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Deep-Dive / Redesign a System You Built |
|-------|------------------------|
...
*End of Deep-Dive / Redesign a System You Built system design.*

### 8.18 Deep topic notes — Deep-Dive / Redesign a System You Built


##### Narrative template
1. Mission & users 2. Architecture then vs now 3. Invariants 4. Hardest outage
5. Metrics you owned 6. Redesign for 100× 7. Privacy/auth/offline mapping 8. What you'd not change.

Bring one real diagram and three real numbers.
