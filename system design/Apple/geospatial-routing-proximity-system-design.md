# System Design: Geospatial Shortest-Route / Proximity Service

> **Focus areas:** Graphs · tiles · S2/H3 · proximity · offline maps · Apple Maps vibes · privacy of location  
...
Goal: **bound the product**—a geospatial service providing proximity search and shortest-route computation with location privacy.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | APIs? | Nearby search + route A→B | Separate indexes |
| F2 | Map data? | Road graph tiles | Versioned datasets |
| F3 | Proximity? | S2/H3 + POI index | Radius/kNN |
| F4 | Routing? | Dijkstra/A* / contraction hierarchies | Preprocessing |
| F5 | Traffic? | Optional live speeds | Overlay |
| F6 | Offline? | Region packs on device | On-device router |
| F7 | Privacy? | Snap-to-grid; no raw trace store by default | Minimization |
| F8 | Latency? | Interactive routes | Edge + cache |
| F9 | Update cadence? | Map weekly; traffic seconds | Planes |
| F10 | Multi-modal? | Out of MVP or stub | Extensibility |
| F11 | Abuse? | Scraping POI | Auth + rate limit |
...
|--------|----------|-----|------|--------|
| Route QPS peak | 10K | 100K | 1M | 5M |
| Nearby QPS peak | 20K | 200K | 2M | 10M |
| POIs | 100M | 500M | 2B | 5B |
| Graph edges | 1B | 2B | 5B | 10B |
| Offline pack downloads / day | 1M | 10M | 50M | 100M |
| Map regions | global | global | global+ | global+HD |
...
> Design geospatial routing/proximity: map data, indexing, route API, traffic hooks, offline packs, and privacy of user location queries.
...
```text
1B edges × 20 B = 20 GB bare; with CH preprocessing more memory/shard
Tile cache at edge for common corridors
```
...
3. **Primary service(s) for Geospatial Shortest-Route / Proximity Service** — business state machines & APIs.  
...
```text
 Route API -> Graph shard (CH/A*) -> traffic overlay
 Nearby API -> H3/S2 index -> POI store
 Offline packs -> on-device router
 Location queries minimized / snapped
```
...
1. Durability before ACK for accepted critical writes in Geospatial Shortest-Route / Proximity Service.
...
**1× — correct MVP for Geospatial Shortest-Route / Proximity Service**
...
Snap/fuzz; retain traces only with consent; offline packs reduce server sight; rate-limit scrapers.
...
### 6.2 Top risks for Geospatial Shortest-Route / Proximity Service
...
> Design Geospatial Shortest-Route / Proximity Service with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Geospatial Shortest-Route / Proximity Service-specific

**Q: Preprocess vs online?**  
A: CH/HL for speed; traffic as overlay deltas.

**Q: Multi-stop?**  
A: Heuristic TSP Phase 2; keep MVP A→B.
...
Topic accents for **Geospatial Shortest-Route / Proximity Service** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Geospatial Shortest-Route / Proximity Service
         -> Durable SoT
         -> Async workers
...
> I'll split **Geospatial Shortest-Route / Proximity Service** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Geospatial Shortest-Route / Proximity Service |
|-------|------------------------|
...
*End of Geospatial Shortest-Route / Proximity Service system design.*

### 8.18 Deep topic notes — Geospatial Shortest-Route / Proximity Service


##### Indexes
H3/S2 for POI; CH/HL graph for routes; traffic overlay deltas.
Offline packs: on-device router; privacy via snap/fuzz & minimization.
