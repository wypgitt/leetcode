# System Design: Blackjack Gaming Site

> **Focus areas:** Fair RNG · ledger · anti-cheat · realtime tables · wallets · audit · regulatory · Apple pay/auth patterns for identity  
...
Goal: **bound the product**—an online blackjack gaming site with fair dealing, wallets, multi-player tables, and strong auditability.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Modes? | Multiplayer tables + RNG shoe | Table service |
| F2 | Money? | Real or play chips? Assume real-money-capable ledger | Double-entry ledger |
| F3 | Fairness? | Provably auditable RNG | Commit-reveal or certified RNG |
| F4 | Realtime? | WebSocket/state sync | Authoritative server |
| F5 | Anti-cheat? | No client-dealt cards | Server authority |
| F6 | Matchmaking? | Table stakes tiers | Seat allocator |
| F7 | KYC/geo? | Jurisdiction gates | Compliance service |
| F8 | Disconnect? | Auto-action timers | State machine |
| F9 | Audit? | Immutable hand logs | Forensic replay |
| F10 | Scale? | Many parallel tables | Shard by table_id |
| F11 | Auth? | Strong account binding | Device signals for fraud |
...
7. Player seats, bets reserved in ledger, hand completes, payout settles.
8. Fair shoe deal audited.
...
| Disconnect mid-hand | Timer auto-action |
| Double settle | Idempotent hand_id |
| RNG suspicion | Replay from audit log |

### 1.4 Scales (Progressive)

| Metric | Baseline | 10× | 100× | 1,000× |
|--------|----------|-----|------|--------|
| Concurrent tables | 10K | 100K | 1M | 5M |
| Players online | 100K | 1M | 10M | 50M |
| Hands / day | 20M | 200M | 2B | 20B |
| Ledger TPS peak | 5K | 50K | 500K | 2M |
| WS messages / s | 200K | 2M | 20M | 100M |
| Fraud checks / s | 5K | 50K | 500K | 2M |
...
> Design blackjack: matchmaking/tables, shoe/RNG, betting ledger, realtime sync, fraud, KYC hooks—correctness of money and fairness first.

---

## 2. Back-of-the-Envelope Estimation

### 2.1 Ledger vs realtime

```text
Separate: authoritative table state (memory+log) vs money ledger (strong SQL/Spanner)
Hands/day 20B × tiny events → stream archive; ledger only money transitions
```

### 2.2 RNG

```text
Don't generate cards on clients
Server shoe with audited entropy; persist shoe commit hash per hand
```
...
3. **Primary service(s) for Blackjack Gaming Site** — business state machines & APIs.  
...
### 3.4a Money vs game state

Game engine is authoritative for cards; **ledger service** is authoritative for balances. Bet placement = ledger reserve; settle on hand end with idempotent `hand_id`.
...
```text
 Player <-> Gateway <-> Table shard (authoritative shoe/state)
                           |
                      Ledger service (funds)
                           |
                      Audit/hand log immutable store
```
...
1. Durability before ACK for accepted critical writes in Blackjack Gaming Site.
...
**1× — correct MVP for Blackjack Gaming Site**
...
Commit shoe hash; ledger reserve/settle idempotent; disconnect timers; jurisdiction geo-fence at gateway; immutable hand logs for disputes.
...
### 6.2 Top risks for Blackjack Gaming Site
...
> Design Blackjack Gaming Site with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Blackjack Gaming Site-specific

**Q: Can clients deal cards?**  
A: Never.

**Q: How to prove fairness?**  
A: Publish commit hashes; third-party RNG audit.
...
Topic accents for **Blackjack Gaming Site** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Blackjack Gaming Site
         -> Durable SoT
         -> Async workers
...
> I'll split **Blackjack Gaming Site** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Blackjack Gaming Site |
|-------|------------------------|
...
*End of Blackjack Gaming Site system design.*

### 8.18 Deep topic notes — Blackjack Gaming Site


##### Hand state machine
WAITING_BETS → DEALING → PLAYER_TURNS → DEALER → SETTLE → WAITING_BETS
Ledger: Reserve(bet) → Settle(win/lose/push) idempotent on hand_id.
RNG: server shoe; commit hash published per shoe/hand.
