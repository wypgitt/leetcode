...
> **Focus areas:** ID generation · redirects · analytics privacy · abuse · Apple app-link vibes · geo routing  
...
Goal: **bound the product**—a URL shortener with reliable redirects, optional privacy-safe analytics, and abuse controls.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Create? | Long URL → short code | Idempotent create |
| F2 | Redirect? | 302/301 policy | Hot path cache |
| F3 | Custom aliases? | Optional vanity | Reservation |
| F4 | Expiry? | TTL + max clicks | Sweeper |
| F5 | Analytics? | Aggregate counts; no fingerprint dossiers | Privacy |
| F6 | Abuse? | Malware/phishing scans | Async classifier |
| F7 | Auth? | API keys for create | Quotas |
| F8 | Predictability? | Non-guessable codes | 128-bit space / hash |
| F9 | Geo? | Optional regional landing | Edge keys |
| F10 | Delete? | Owner revoke | Cache purge |
| F11 | SLA? | Redirect always-on | Multi-region read |
...
|--------|----------|-----|------|--------|
| Creates / day | 10M | 100M | 1B | 5B |
| Redirects / day | 1B | 10B | 100B | 1T |
| Peak redirect QPS | 50K | 500K | 5M | 50M |
| Active links | 5B | 50B | 200B | 1T |
| Abuse scans / day | 10M | 100M | 1B | 5B |
...
> Design URL shortener: create, redirect, expiry, custom aliases, analytics with privacy, and scale of redirect QPS.
...
```text
50M QPS redirects: edge cache + regional KV; creates are tiny fraction
ID space: 62^8 ≈ 2.1e14 for 8-char base62 — still add unguessability via longer or keyed hash
```
...
3. **Primary service(s) for URL Shortener** — business state machines & APIs.  
...
```text
 Create API -> ID gen + KV store -> (async) abuse scan
 Redirect: Edge cache -> KV -> 302 Location
...
1. Durability before ACK for accepted critical writes in URL Shortener.
...
**1× — correct MVP for URL Shortener**
...
### 5.5 Open redirect / malware

Async scan; blocklists; safe interstitial for unknown; aggregate analytics only.
...
### 6.2 Top risks for URL Shortener
...
> Design URL Shortener with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 URL Shortener-specific

**Q: Enumeration attacks?**  
A: Long codes + rate limit + anomaly detection.

**Q: 301 vs 302?**  
A: 302/307 for controllable analytics & revocation; 301 caches hard.
...
Topic accents for **URL Shortener** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for URL Shortener
         -> Durable SoT
         -> Async workers
...
> I'll split **URL Shortener** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to URL Shortener |
|-------|------------------------|
...
### 8.18 Deep topic notes — URL Shortener


##### ID generation
Hash(long+salt)[:n] or Snowflake-like → base62; reject collisions.
Redirect hot path: edge cache; 302 default for control.
Analytics aggregates only; abuse async scan.
