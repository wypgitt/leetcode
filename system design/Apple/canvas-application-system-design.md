# System Design: Canvas Application (Shapes, Layers, Undo/Redo, Collaboration)

> **Focus areas:** CRDT/OT · layers · undo stacks · realtime collab · offline · Apple Freeform/Keynote-ish · privacy of shared docs  
...
Goal: **bound the product**—a collaborative canvas app supporting shapes, layers, undo/redo, and multi-user editing with offline support.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Objects? | Shapes, text, images, layers | Scene graph |
| F2 | Editing? | Transform, style, z-order | Op log |
| F3 | Undo/redo? | Per-user stacks; collab-aware | OT/CRDT + inverse |
| F4 | Collab? | Multi-cursor realtime | Presence channel |
| F5 | Offline? | Edit offline; sync later | Local DB + conflict |
| F6 | Authz? | Owner/editor/viewer | ACL on doc |
| F7 | Media? | Images in object store | Deduped blobs |
| F8 | Export? | PNG/PDF | Render workers |
| F9 | History? | Version snapshots | Periodic compact |
| F10 | Scale of doc? | 10k–100k objects | Spatial index client |
| F11 | Privacy? | E2EE optional mode | Server minimizes content access |
| F12 | Privacy / logging? | Minimize PII; retention caps; deletion APIs | Privacy budget + scrubbing pipelines |

**MVP functional scope (lock with interviewer):**

1. Realtime collab ops + undo/redo + offline journal + ACL + media refs.
...
- Full Illustrator feature parity
- Pixel-perfect cross-platform fonts as MVP
...
|--------|----------|-----|------|--------|
| MAU | 5M | 50M | 200M | 500M |
| Concurrent collab docs | 50K | 500K | 2M | 10M |
| Ops / sec peak | 100K | 1M | 5M | 20M |
| Avg objects / doc | 500 | 1K | 2K | 5K |
| Media PB | 1 | 10 | 50 | 200 |
| Offline clients | 1M | 10M | 50M | 100M |
...
> Design canvas: document model, ops, presence, undo/redo, conflict resolution, media blobs, ACL—on-device + sync boundary clear.
...
```text
1M ops/s peak: shard by doc_id; presence separate channel
Snapshots every N ops or M MB to bound replay
```
...
3. **Primary service(s) for Canvas Application (Shapes, Layers, Undo/Redo, Collaboration)** — business state machines & APIs.  
...
### 3.4a Collaboration model

Prefer **CRDT** (or OT with central sequencer) for shapes; **per-doc sequencer** home cell for MVP simplicity.

Undo: local inverse ops that transform against concurrent remote ops; don't break others' stacks silently.
...
```text
 Clients <Realtime ops> Doc sequencer (home) -> Op log + snapshots
    |                         |
  Local undo                  +-> Presence service
    |                         |
 Offline journal ----sync---->+-> Media object store
```
...
1. Durability before ACK for accepted critical writes in Canvas Application (Shapes, Layers, Undo/Redo, Collaboration).
...
**1× — correct MVP for Canvas Application (Shapes, Layers, Undo/Redo, Collaboration)**
...
### 5.5 Undo under concurrency

Transform inverses; snapshot compaction; large media by reference; privacy ACL on every subscribe.
...
### 6.2 Top risks for Canvas Application (Shapes, Layers, Undo/Redo, Collaboration)
...
> Design Canvas Application (Shapes, Layers, Undo/Redo, Collaboration) with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Canvas Application (Shapes, Layers, Undo/Redo, Collaboration)-specific

**Q: OT vs CRDT?**  
A: Central sequencer OT simpler MVP; CRDT better offline multi-master.

**Q: Huge documents?**  
A: Spatial indexing client-side; paginate ops; snapshot.
...
Topic accents for **Canvas Application (Shapes, Layers, Undo/Redo, Collaboration)** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Canvas Application (Shapes, Layers, Undo/Redo, Collaboration)
         -> Durable SoT
         -> Async workers
...
> I'll split **Canvas Application (Shapes, Layers, Undo/Redo, Collaboration)** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Canvas Application (Shapes, Layers, Undo/Redo, Collaboration) |
|-------|------------------------|
...
*End of Canvas Application (Shapes, Layers, Undo/Redo, Collaboration) system design.*

### 8.18 Deep topic notes — Canvas Application (Shapes, Layers, Undo/Redo, Collaboration)


##### Doc model
Scene graph nodes with transform, style, z, parent.
Ops: create/update/delete/reorder; presence separate.
CRDT or sequencer; undo transforms against concurrent ops; snapshots every N KB.
