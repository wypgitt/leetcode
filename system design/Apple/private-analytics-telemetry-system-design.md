# System Design: Private Analytics / Telemetry

> **Focus areas:** Differential privacy · on-device aggregation · opt-in · Apple-like privacy · sketch/histograms · anti-fingerprinting  
...
Goal: **bound the product**—a private analytics and telemetry pipeline that answers product questions without building invasive user dossiers.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Who emits? | Apps/OS components with consent | SDK + policy |
| F2 | Goals? | Funnels, crash rates, feature adoption—not ads dossiers | Question catalog |
| F3 | On-device? | Aggregate locally first | Periodic sketches |
| F4 | DP? | ε budgets per domain | Noise + accounting |
| F5 | Transport? | Batched, delayed, coalesced | Anti-timing |
| F6 | Identity? | Rotating cohort IDs; no stable ad-id | Unlinkability |
| F7 | Server? | Aggregate only; access controlled | Clean rooms |
| F8 | Opt-out? | Hard off; delete | Compliance |
| F9 | Debugging? | Separate opt-in diagnostics channel | Don't mix |
| F10 | Schema? | Versioned event taxonomy | Allowlist fields |
| F11 | Abuse? | Fraudulent telemetry | Attestation + rate |
...
|---|----------|-----------------|--------|
| N1 | Collection delay OK? | Hours OK | Privacy > realtime |
...
7. Device aggregates locally, applies noise, uplinks batch.
8. Analyst query answered under ε budget.
...
| ε exhausted | Stop uplink domain |
| Attestation fail | Reject |
| Attempt to join identities | Architecture prevents |

### 1.4 Scales (Progressive)

| Metric | Baseline | 10× | 100× | 1,000× |
|--------|----------|-----|------|--------|
| Devices reporting | 50M | 500M | 2B | 3B |
| Raw-ish events before on-device agg / day | 20B | 200B | 1T | 5T |
| Uplink aggregates / day | 200M | 2B | 10B | 50B |
| DP queries / day | 1K | 5K | 20K | 50K |
| ε domains | 20 | 50 | 100 | 200 |
| Analysts | 100 | 500 | 2K | 5K |
...
**Topic accent:** 10×: on-device batching; 100×: DP accounting service; 1000×: clean-room query layer, strict ε budgets, attestation.
...
> Design private analytics: event SDK, on-device aggregation, DP noise, collection minimization, server aggregation, access controls.

---

## 2. Back-of-the-Envelope Estimation

### 2.1 Compression via on-device agg

```text
20B raw events → 200M uplinks (100× reduction) via local counts/sketches
Further DP may drop rare keys entirely (good)
```

### 2.2 ε accounting

```text
Each domain daily budget ε=1 split across reports
Server rejects/drops when device reports exceed — fail closed for privacy
```
...
|-------|----------------|-------------|
| Consent/policy | What may leave device | Strong |
| On-device agg | Sketches/counts | Local |
| Transport | Batched uplink | Best-effort |
| DP accounting | ε budgets | Strong ledger |
| Query clean room | Analyst answers | Controlled |
...
3. **Primary service(s) for Private Analytics / Telemetry** — business state machines & APIs.  
...
### 3.4a Privacy pipeline

```text
Event allowlist → on-device aggregate → delay/coalesce → DP noise → attested uplink
  → regional aggregator → clean-room query (ε accounted)
```
...
```text
 App --policy--> On-device aggregator --DP+delay--> Ingest
                                              |
                                        ε accountant
                                              v
                                        Regional aggregates
                                              v
                                        Clean-room query API
```
...
1. Durability before ACK for accepted critical writes in Private Analytics / Telemetry.
...
**1× — correct MVP for Private Analytics / Telemetry**
...
Rotating cohort keys; batch delays defeat timing join; no forever device id in clear; separate crash diagnostics opt-in channel.
...
### 6.2 Top risks for Private Analytics / Telemetry
...
> Design Private Analytics / Telemetry with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Private Analytics / Telemetry-specific

**Q: Is DP enough alone?**  
A: No—minimization, unlinkability, access control, purpose limitation.

**Q: What if ε exhausted?**  
A: Stop collecting that domain; don't silently remove noise.
...
Topic accents for **Private Analytics / Telemetry** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Private Analytics / Telemetry
         -> Durable SoT
         -> Async workers
...
> I'll split **Private Analytics / Telemetry** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Private Analytics / Telemetry |
|-------|------------------------|
...
*End of Private Analytics / Telemetry system design.*

### 8.18 Deep topic notes — Private Analytics / Telemetry


##### Pipeline
Allowlist → on-device count/sketch → delay → DP noise → attest → regional agg → clean room.
ε ledger per domain; fail closed when exhausted; unlinkable cohort ids.
