# System Design: Globally Replicated Block Storage (US/EU, Strong Consistency)

> **Focus areas:** Quorum · RPO/RTO · US/EU residency · strong consistency · fencing · Apple enterprise/iCloud hooks · encryption  
...
Goal: **bound the product**—a globally replicated block storage service with strong consistency across US and EU regions (or clear residency modes).
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | API? | Attach volume, read/write blocks, snapshots | Block device abstraction |
| F2 | Consistency? | Strong (linearizable) for primary mode | Leader/quorum |
| F3 | Regions? | US + EU; residency options | Placement policy |
| F4 | RPO/RTO? | RPO=0 sync quorum in mode A; async DR mode B | Explicit SKUs |
| F5 | Encryption? | Customer + service keys | Envelope encryption |
| F6 | Snapshots? | Crash-consistent + optional app-consistent | Copy-on-write |
| F7 | Multi-attach? | Optional read-only; RW single attach MVP | Fencing |
| F8 | Perf? | IOPS/latency SLOs by tier | Placement + caching |
| F9 | Failure? | Disk/node/AZ/region | Rebuild + failover |
| F10 | Tenancy? | Strong isolation | QoS + encryption domains |
| F11 | Compliance? | EU data stays in EU when selected | Control plane metadata rules |
...
|---|----------|-----------------|--------|
| N1 | IO latency? | Primary region | Sub-ms–few ms local; cross-region cost explicit |
...
7. Create volume in EU residency; attach; write; snapshot.
8. AZ failure: quorum continues.
...
| Stale attacher | Gen fence |
| Rebuild storm | Paced rebuild |
| Wrong-region write | Policy deny |

### 1.4 Scales (Progressive)

| Metric | Baseline | 10× | 100× | 1,000× |
|--------|----------|-----|------|--------|
| Volumes | 100K | 1M | 10M | 100M |
| Provisioned PB | 10 | 100 | 1K | 10K |
| Peak IOPS cluster | 10M | 100M | 1B | 10B |
| Snapshots / day | 100K | 1M | 10M | 100M |
| Cross-region sync GB/s | 1 | 10 | 50 | 200 |
| Nodes | 1K | 5K | 20K | 100K |
...
**Topic accent:** 10×: AZ quorum; 100×: US/EU placement SKUs; 1000×: cell-based storage fabrics + autonomous rebuild fleets.
...
> Design global block storage: volume API, replication, quorum/leader, failure domains, encryption, and US/EU placement with strong consistency where promised.
...
```text
10M IOPS cluster baseline → per-node budgets + QoS tokens
4KB random read vs 1MB sequential: size tiers differently
Cross-region sync 200 GB/s at extreme: dedicated backbone assumption
```

### 2.2 Capacity

```text
10 EB at 1000× with 3-way repl → ~30 EB raw (+ coding overhead trade-offs)
Rebuild bandwidth must be reserved or customer IO collapses
```

### 2.3 Consistency cost

```text
Sync quorum US-EU adds tens of ms — productize as separate SKU
Don't silently sell “strong + local latency + cross-ocean” as free
```
...
|-------|----------------|-------------|
| Control plane | Volume metadata | Strong |
| Data plane | Block IO | Leader/quorum |
| Placement | US/EU policies | Policy engine |
| Snapshot | COW history | Consistent epoch |
| Key management | Encryption | HSM-backed |
...
3. **Primary service(s) for Globally Replicated Block Storage (US/EU, Strong Consistency)** — business state machines & APIs.  
...
### 3.4a Replication SKUs

| SKU | Consistency | Placement |
|-----|-------------|-----------|
| LocalStrong | Linearizable | 3 AZ one region |
| UniResidency | Linearizable | US-only or EU-only |
| GlobalSync | Linearizable cross-region | High latency cost |
| GlobalAsyncDR | RPO>0 | DR lag explicit |
...
```text
 Attach API -> Control Plane (volume meta, placement)
                    |
        +-----------+------------+
        v                        v
   US cell quorum             EU cell quorum
   (leader + followers)       (residency SKU)
        |                        |
        +-------- async DR ------+   (only if SKU allows)
 Client VM/agent <-- block IO via data plane endpoint
```
...
1. Durability before ACK for accepted critical writes in Globally Replicated Block Storage (US/EU, Strong Consistency).
...
**1× — correct MVP for Globally Replicated Block Storage (US/EU, Strong Consistency)**
...
Attach generation numbers; IO with stale gen rejected. Rebuild pacing reserved. Never allow two RW attachers without cluster capability + fencing.
...
### 6.2 Top risks for Globally Replicated Block Storage (US/EU, Strong Consistency)
...
> Design Globally Replicated Block Storage (US/EU, Strong Consistency) with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Globally Replicated Block Storage (US/EU, Strong Consistency)-specific

**Q: Why not sync-always US-EU?**  
A: Latency & partition story; offer SKUs.

**Q: How do snapshots work?**  
A: COW epoch; freeze sequence; async copyout.
...
Topic accents for **Globally Replicated Block Storage (US/EU, Strong Consistency)** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
## 9. Extended Deep Dive — Global Block Storage

### 9.1 Apple platform context

Apple interviewers expect explicit treatment of **privacy** (what is logged/stored), **authentication** (human SSO + service mTLS), **offline/degraded behavior**, and **on-device vs server** boundaries. This section maps those lenses onto global block storage.
...
For global block storage, classify every field before adding to schemas (public / internal / confidential / restricted).
...
Name the **first bottleneck** you expect for global block storage (usually hot keys, write QPS, or cross-region latency).
...
## 9. Extra deep dive — Global block storage

### 9.1 SKUs
LocalStrong (3 AZ), UniResidency (US or EU), GlobalSync (expensive), GlobalAsyncDR (RPO>0).

### 9.2 Fencing
`attach_gen` on every IO; stale attacher rejected; single RW attach MVP.

### 9.3 Quorum & failover
Primary + journal; followers; AZ loss tolerated; region loss per SKU.

### 9.4 Snapshots
COW epoch; crash-consistent; async copyout; encrypted.

### 9.5 Rebuild pacing
Reserve bandwidth; never starve customer IO; scrub for bitrot.

### 9.6 Encryption
Envelope DEK/KEK; customer-managed optional; HSM root.

### 9.7 Honesty about physics
Sync US–EU adds tens of ms—productize, don’t hide.

### 9.8 Deal-breakers
Two RW attachers without fencing; silent cross-residency; claiming free global linearizability + local latency.

### 9.9 60s summary
> Block volumes with quorum, attach fencing, explicit residency/consistency SKUs, paced rebuild, envelope encryption.
