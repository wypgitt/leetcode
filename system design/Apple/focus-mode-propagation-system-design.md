# System Design: Focus Mode Propagation Across Devices

> **Focus areas:** State sync · Do Not Disturb · Filters · Privacy · Low latency · Calendar triggers · Battery · Family  
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | State? | Active focus id + manual/auto | Small payload |
| F2 | Config? | Allowlists, schedules, calendars | Encrypted sync |
| F3 | Latency? | Other devices within seconds | Push priority |
| F4 | Triggers? | Time, location, app, calendar | Local evaluate |
| F5 | Override? | Manual always wins temporarily | TTL override |
| F6 | DND integration? | Notification center filters | OS hook |
| F7 | Share Focus status? | iMessage optional | Separate |
| F8 | Work/personal? | Multiple profiles | List sync |
| F9 | Privacy? | App allowlists sensitive | E2E option |
| F10 | Watch? | Mirror phone or independent | Policy |
| F11 | CarPlay? | Driving focus | Auto |
| F12 | Family? | Not auto parent control | Separate Screen Time |

**MVP functional scope (lock with interviewer):**

1. Focus profile config sync
2. Active focus state broadcast
3. Push priority channel
4. Manual override with TTL
5. Local trigger evaluation
6. Notification filter apply hook
7. Encrypted allowlists

**Out of MVP (explicitly defer):**

- Server-side notification filtering for all apps globally
- Employer remote force focus

### 1.2 Non-Functional Requirements

| # | Question | Expected answer | Target |
|---|----------|-----------------|--------|
| N1 | Propagate latency | < 5s p95 | Push + small payload |
| N2 | Battery | Tiny updates | No poll |
| N3 | Privacy | Allowlists encrypted option | Sensitive |
| N4 | Consistency | Eventual | Manual override rules |
| N5 | Offline | Local focus works | Sync later |
| N6 | Availability | 99.9% | Local filters cached |
| N7 | UX | No flapping | Debounce auto triggers |
| N8 | Security | No remote force by third party | User controlled |

### 1.3 Cases (Flows & Edge Cases)

**Happy paths**

1. Enable Work Focus on iPhone → Mac/iPad switch within seconds
2. Calendar trigger starts Sleep focus locally on all devices
3. Manual override disables auto until TTL
4. Edit allowlist on Mac → sync → phone applies
5. Watch mirrors phone focus state

**Edge / failure cases**

| Case | Behavior |
|------|----------|
| Conflicting manual on two devices | LWW + manual priority |
| Auto trigger race | Debounce 30s |
| Push delay | Pull on unlock |
| Large allowlist | Compress diff |
| Delete profile | Tombstone + default |
| Timezone schedule | Local eval |
| CarPlay enter/exit | Driving focus |
| MDM do not disturb | Separate channel |

### 1.4 Scales (Progressive)

| Metric | Baseline | 10× | 100× | 1,000× |
|--------|----------|-----|------|--------|
| Users with Focus | 100M | 200M | 300M | 400M |
| Profiles/user | 5 | 8 | 10 | 12 |
| State changes/day | 500M | 2B | 5B | 10B |
| Config edits/day | 50M | 200M | 500M | 1B |
| Devices/user | 3 | 3.5 | 4 | 4 |
| Avg state payload | 200 B | 250 B | 300 B | 300 B |
| Avg config | 5 KB | 8 KB | 10 KB | 12 KB |
| Push priority/day | 500M | 2B | 5B | 10B |

**What each jump forces:**

- **10×:** Coalesce config diffs
- **100×:** Edge state cache
- **1,000×:** Binary diff profiles

### 1.5 Etc. (Constraints & Assumptions)

- Focus is OS-integrated; server doesn't filter notifications directly
- User consent for sharing status

**Scope statement:**

> Design Focus Mode propagation: focus state and configuration sync across devices, integration with notifications/calendar/location triggers, privacy of app allowlists, low-latency sync, and battery-minimal updates.
...
```text
Focus state changes operations: 500M/day × 200 B ≈ 100 GB/day
Avg payload: 200–300 B state
Peak ≈ 5× context switches average → size sharding + coalescing mandatory at 100×
```
...
|--------|------|------|-------------------|
| A. Server filters notifications | Central | Privacy/latency | No |
| B. Sync state+config; local apply | Fast private | Requires push | — chosen |
| C. Poll every 10s | Simple | Battery | No |
| D. iCloud file drop profiles | Rare updates OK | State slow | No |

**Chosen path:** Low-latency encrypted sync of focus profiles and active state; each device applies filters locally; priority push for state changes.

### 3.4 Privacy, trust, and encryption

Allowlists reveal app usage—encrypt configs; don't log plaintext lists server-side under E2E.

### 3.5 Consistency, sync, and conflict resolution

State LWW with manual override precedence; config op-log merge.

### 3.6 Offline-first behavior

Local focus continues; state reconciles on reconnect.

### 3.7 Battery, radio, and scheduling

Tiny payloads; push not poll; debounce auto triggers.

### 3.8 Multi-region / cell model

Home cell per user; global anycast API.

### 3.9 Abuse, auth, and quotas

Only user's devices can set state; rate limit.
...
1. Local durable write before UI ACK for user mutations in **Focus Mode Propagation Across Devices**.
...
| Dual-writer split brain | Single-writer home cell per `focus_profile_id` |

### 5.2 Scalability & sharding

```text
focus_profile_id → home_cell (consistent hash / directory)
Focus state + config mutation sync partitioned by focus_profile_id
...
- Observability: sizes, latencies, error codes—not plaintext **Focus Mode Propagation Across Devices** content.
...
Focus Mode propagation via encrypted profile sync and priority state broadcast; local trigger evaluation and notification filtering; manual override rules prevent flapping.

### 6.2 Top trade-offs

| Trade-off | Choice | Why |
|-----------|--------|-----|
| Latency vs battery | Priority push small | Energy OK |
| Mirror vs independent Watch | Mirror default | UX |
| E2E allowlists | Yes option | Server blind |
| Auto vs manual | Manual wins | User trust |

### 6.3 MVP → scale path

MVP: state broadcast + config sync. Scale: diff compression, edge cache.

### 6.4 Risks

- Flapping auto triggers
- Push delays
...
**Q: What must never leave the device plaintext in Focus Mode Propagation Across Devices?**  
...
**Q: How is Focus Mode Propagation Across Devices different from generic cloud file sync?**  
...
1. Offline write for 24h then sync—no loss in Focus Mode Propagation Across Devices.
...
> For Focus Mode Propagation Across Devices: keep a crisp local-first story, put encryption and conflict rules on the client, let the server order opaque sync operations and store ciphertext, schedule radios for battery, and scale with per-user cells plus cursors/snapshots. Call out privacy mode forks explicitly.
...
1. Is Advanced Data Protection / E2E required or optional for Focus Mode Propagation Across Devices?
...
| Anti-pattern | Why it hurts Focus Mode Propagation Across Devices |
|--------------|----------------------|
...
*End of focus mode propagation across devices system design.*
