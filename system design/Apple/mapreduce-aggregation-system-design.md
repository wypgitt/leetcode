# System Design: MapReduce-Style Aggregation

> **Focus areas:** Map · shuffle · reduce · combiners · skew · checkpoint · privacy of inputs · Apple analytics batch vibes  
...
Goal: **bound the product**—a MapReduce-style batch aggregation framework for large-scale analytics over privacy-sensitive inputs.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | API? | Submit job: inputs, map, reduce, combiners | Job control plane |
| F2 | Data? | Object store datasets | Split planning |
| F3 | Faults? | Task retry; speculative exec | Lineage |
| F4 | Shuffle? | Pull/push; encrypted | Privacy of intermediates |
| F5 | Skew? | Key salting / range split | Runtime detectors |
| F6 | Exactly-once? | Effectively once via commit | Task commit protocol |
| F7 | Multi-tenancy? | Queues + quotas | Fair scheduler |
| F8 | UDFs? | Sandboxed | cgroup/Wasm/JVM security |
| F9 | Privacy? | Column ACL; DP aggregations optional | Policy engine |
| F10 | Observability? | Stage timelines | Counters |
| F11 | Output? | Commit to dirs with manifest | Atomic rename/_SUCCESS |
...
|--------|----------|-----|------|--------|
| Jobs / day | 5K | 50K | 200K | 1M |
| Peak concurrent tasks | 50K | 500K | 5M | 20M |
| Shuffle bytes / day | 1 PB | 10 PB | 100 PB | 1 EB |
| Input datasets | 10K | 100K | 1M | 5M |
| Tenants | 50 | 200 | 1K | 5K |
| Workers | 5K | 50K | 200K | 1M |
...
> Design MapReduce aggregation: job API, map/shuffle/reduce, fault tolerance, skew handling, and privacy controls on intermediate data.
...
```text
Shuffle often dominates: 100 PB/day → network & disk careful
Combiners reduce shuffle; skew kills stragglers
```
...
3. **Primary service(s) for MapReduce-Style Aggregation** — business state machines & APIs.  
...
```text
 Submit -> Job master -> Map tasks -> Shuffle (encrypted) -> Reduce -> Commit output
              |                         |
           Fair scheduler            Speculative exec / skew handlers
```
...
1. Durability before ACK for accepted critical writes in MapReduce-Style Aggregation.
...
**1× — correct MVP for MapReduce-Style Aggregation**
...
### 5.5 Shuffle privacy & skew

Encrypt shuffle; ACL intermediates; detect heavy keys; speculative execution; atomic output commit.
...
### 6.2 Top risks for MapReduce-Style Aggregation
...
> Design MapReduce-Style Aggregation with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 MapReduce-Style Aggregation-specific

**Q: Stragglers?**  
A: Speculative tasks; skew split; progress timeouts.

**Q: UDF escape?**  
A: Sandbox + egress deny; memory/cpu caps.
...
Topic accents for **MapReduce-Style Aggregation** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for MapReduce-Style Aggregation
         -> Durable SoT
         -> Async workers
...
> I'll split **MapReduce-Style Aggregation** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to MapReduce-Style Aggregation |
|-------|------------------------|
...
*End of MapReduce-Style Aggregation system design.*

### 8.18 Deep topic notes — MapReduce-Style Aggregation


##### Commit protocol
Task outputs to temp; reduce commit renames into final + `_SUCCESS` manifest.
Shuffle encrypted; combiners; skew detection; speculative exec.
