# System Design: Family Sharing Permissions

> **Focus areas:** Apple ID family · ACL · Purchase sharing · Screen Time · Key wrapping · Privacy between members · Child accounts  
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Roles? | Organizer, parent, child, member | RBAC |
| F2 | Share what? | Purchases, subscriptions, locations opt-in | Not personal photos by default |
| F3 | Child privacy? | Hide messages/health from parents by default | Policy exceptions Screen Time |
| F4 | Purchase? | Organizer pays | Entitlement propagation |
| F5 | Leave family? | Revoke entitlements | Rekey |
| F6 | Invite? | Email/iMessage | Pending tokens |
| F7 | Screen Time? | Parental controls | Separate subsystem sync |
| F8 | Location sharing? | Opt-in Find My circle | Not automatic |
| F9 | Age up? | Promote child account | Policy transition |
| F10 | Region? | Store region alignment | Compliance |
| F11 | Audit? | Parent sees activity summaries not content | Aggregates |
| F12 | Death/transfer? | Organizer transfer | Support flow |

**MVP functional scope (lock with interviewer):**

1. Family group CRUD
2. Role-based ACL
3. Shared entitlements service
4. Personal partition isolation
5. Invite/leave flows
6. Revoke on leave
7. Child age policies

**Out of MVP (explicitly defer):**

- Full legal custody workflows
- Cross-platform family beyond Apple

### 1.2 Non-Functional Requirements

| # | Question | Expected answer | Target |
|---|----------|-----------------|--------|
| N1 | Privacy between members | Strong default walls | No read others' iCloud |
| N2 | ACL check latency | < 50ms | Cached entitlements |
| N3 | Consistency | Eventual entitlements | Minutes OK |
| N4 | Security | Rekey on membership change | Forward secrecy lite |
| N5 | Availability | 99.99% entitlement reads | CDN cache |
| N6 | Compliance | COPPA | Child defaults |
| N7 | Battery | Low churn sync | Push on change |
| N8 | Abuse | Invite spam limits | Rate limits |

### 1.3 Cases (Flows & Edge Cases)

**Happy paths**

1. Organizer enables Apple One → entitlements propagate to members
2. Child tries purchase → ask to buy flow
3. Member leaves → subscriptions revert → rekey shared keys
4. Parent sets Screen Time → syncs to child devices
5. Teen age-up → privacy defaults shift

**Edge / failure cases**

| Case | Behavior |
|------|----------|
| Organizer leaves | Forced transfer |
| Region mismatch | Block join |
| Stale entitlement cache | TTL + push invalidate |
| Shared app hidden personal data | Partition keys |
| Compromised child device | Local controls |
| Duplicate invites | Idempotent |
| Court order | Support legal process |
...
|--------|----------|-----|------|--------|
| Families | 50M | 80M | 100M | 120M |
| Members/family avg | 3 | 3.5 | 4 | 4 |
| Entitlement checks/day | 500M | 5B | 50B | 500B |
| Policy changes/day | 5M | 50M | 500M | 5B |
| Invites/day | 500K | 5M | 50M | 500M |
| Leave/revoke/day | 200K | 2M | 20M | 200M |
| Child accounts | 30M | 50M | 70M | 80M |
| Cached entitlement QPS peak | 100K | 1M | 10M | 100M |

**What each jump forces:**

- **10×:** Entitlement CDN; family graph shard
- **100×:** Regional policy stores
- **1,000×:** Edge ACL cache with signed bundles

### 1.5 Etc. (Constraints & Assumptions)

- Not all iCloud data is family-shared
- COPPA/privacy walls are non-negotiable

**Scope statement:**

> Design Family Sharing permissions: family group graph, role-based ACLs, shared entitlements, child privacy walls, encrypted personal data separation, and sync of family-scoped settings across devices.
...
```text
Entitlement checks operations: 500M/day
Avg payload: tiny ACL blob
Peak ≈ 10× peak average → size sharding + coalescing mandatory at 100×
```
...
|--------|------|------|-------------------|
| A. Shared family vault for all data | Simple sharing | Privacy disaster | No |
| B. Entitlements + isolated partitions | Safe | Complex ACL | — chosen |
| C. Parent can read all child content | Easy monitoring | Trust/legal fail | Default off |
| D. No server family graph | P2P only | Doesn't scale | No |

**Chosen path:** Family graph for entitlements and policies; cryptographic/logical partitions keep personal data isolated; rekey on membership changes.

### 3.4 Privacy, trust, and encryption

Family shares SKUs not messages; child data walled; location opt-in; aggregates for Screen Time not raw content.

### 3.5 Consistency, sync, and conflict resolution

Entitlement propagation eventual; membership changes trigger revoke + cache bust.

### 3.6 Offline-first behavior

Cached entitlements allow offline app launch within grace TTL.

### 3.7 Battery, radio, and scheduling

Push on policy change only; no polling.

### 3.8 Multi-region / cell model

Family region locked for store compliance; graph home cell.

### 3.9 Abuse, auth, and quotas

Invite rate limits; child purchase friction; audit organizer actions.
...
1. Local durable write before UI ACK for user mutations in **Family Sharing Permissions**.
...
| Dual-writer split brain | Single-writer home cell per `family_id / member_id` |

### 5.2 Scalability & sharding

```text
family_id / member_id → home_cell (consistent hash / directory)
Family policy graph + encrypted personal partitions partitioned by family_id / member_id
...
- Observability: sizes, latencies, error codes—not plaintext **Family Sharing Permissions** content.
...
Family Sharing with RBAC entitlements, isolated personal partitions, child privacy defaults, rekey on leave, and cached entitlement distribution.

### 6.2 Top trade-offs

| Trade-off | Choice | Why |
|-----------|--------|-----|
| Sharing vs privacy | Isolated partitions | Trust |
| Cache TTL vs freshness | Short TTL + push | Latency |
| Parent visibility | Aggregates not content | Ethics |
| Rekey cost | On leave | Security |

### 6.3 MVP → scale path

MVP: group + entitlements. Scale: edge cache, sharded graph, policy sync.

### 6.4 Risks

- Overbroad parent access UX
- Stale entitlements piracy
- Region compliance
- Rekey failures
...
**Q: What must never leave the device plaintext in Family Sharing Permissions?**  
...
**Q: How is Family Sharing Permissions different from generic cloud file sync?**  
...
1. Offline write for 24h then sync—no loss in Family Sharing Permissions.
...
> For Family Sharing Permissions: keep a crisp local-first story, put encryption and conflict rules on the client, let the server order opaque sync operations and store ciphertext, schedule radios for battery, and scale with per-user cells plus cursors/snapshots. Call out privacy mode forks explicitly.
...
1. Is Advanced Data Protection / E2E required or optional for Family Sharing Permissions?
...
| Anti-pattern | Why it hurts Family Sharing Permissions |
|--------------|----------------------|
...
*End of family sharing permissions system design.*
