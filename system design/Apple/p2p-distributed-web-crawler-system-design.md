# System Design: Peer-to-Peer Distributed Web Crawler

> **Focus areas:** P2P coordination · politeness · dedupe · privacy of crawl logs · on-device vs coordinator · rate limits · CAP-ish trade-offs  
...
Goal: **bound the product**—a peer-to-peer distributed web crawler that coordinates URL frontier work across many peers without a single choke-point coordinator (or with a thin optional bootstrap), while remaining polite to origin servers and privacy-conscious about what is logged.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Who runs peers? | Many volunteer/company peers; optional trusted seeds | Trust model + peer identity/attest |
| F2 | What is crawled? | Public web; respect robots.txt | Politeness + robots cache |
| F3 | Frontier ownership? | Partition URL space across peers | Consistent hash / DHT of URL keys |
| F4 | Dedupe? | URL + content hash | Simhash/content-addressed store |
| F5 | Freshness? | Recrawl policies by site class | Priority queue with revisit scores |
| F6 | Output? | WARC/segments to object store or index pipeline | Immutable segments + manifests |
| F7 | Coordination? | P2P gossip + optional bootstrap directory | Avoid single master for frontier |
| F8 | Auth between peers? | mTLS / peer keys | Signed gossip messages |
| F9 | Rate limits? | Per-host QPS budgets global-ish | Token budgets per host key |
| F10 | JS rendering? | Out of MVP or limited pool | Separate render workers |
| F11 | Ban/blocklist? | Legal/abuse blocklist distributed | Signed deny lists |
...
7. Peer joins, receives frontier slice, fetches politely, publishes segment.
8. Host budget enforced across peers.
9. robots.txt deny stops fetch.
...
| Malicious peer over-fetch | Reputation slash + eviction |
| Duplicate content | CAS dedupe |
| Legal takedown | Signed blocklist epoch |

### 1.4 Scales (Progressive)

| Metric | Baseline | 10× | 100× | 1,000× |
|--------|----------|-----|------|--------|
| Peers | 1K | 10K | 100K | 1M |
| URLs discovered / day | 100M | 1B | 10B | 100B |
| Fetches / day | 50M | 500M | 5B | 50B |
| Peak fetch QPS (global) | 2K | 20K | 200K | 2M |
| Unique hosts active | 1M | 5M | 20M | 50M |
| Segment bytes / day | 20 TB | 200 TB | 2 PB | 20 PB |
| robots.txt cache QPS | 500 | 5K | 50K | 500K |
...
**Topic accent:** 10×: DHT/frontier shards + per-host budgets; 100×: segment lakes + peer reputation; 1000×: hierarchical aggregators, specialized render pools, legal geo fences.
...
> Design a P2P web crawler: peer discovery/bootstrap, shared URL frontier with ownership, politeness (robots.txt + per-host budgets), content fetch + content-hash dedupe, optional indexing handoff—with progressive scale and explicit privacy of crawl telemetry.
...
| Class | Baseline order | 1000× order | Notes |
|-------|----------------|-------------|-------|
| URL discover/enqueue | ~5K/s | ~5M/s | Frontier writes |
| Fetch | ~2K/s | ~2M/s | Politeness-bound |
| Content hash / dedupe | ~2K/s | ~2M/s | CAS lookups |
| Gossip / peer heartbeats | ~10K/s | ~10M/s | Must be lossy-ok |
| Segment flush | batch | batch | Object store |

**Anti-pattern:** one “QPS” mixing fetch and gossip.

### 2.2 Politeness math

```text
If global fetch 2M/s across 50M hosts → average 0.04/s/host
But hot hosts need hard caps: e.g. 1–10 QPS/host globally coordinated
Coordination key: hash(host) → budget owner peer set (shuffle shard)
```

### 2.3 Storage

```text
URL metadata ~100–200 B
100B URLs × 150 B = 15 TB metadata class (plus indexes)
Content: most stored as segments compressed; unique HTML avg 20KB → enormous
⇒ store selectively / sample / index extracts; don't keep all forever
```

### 2.4 Bandwidth

```text
2M fetch/s × 20KB avg ≈ 40 GB/s ingress globally (1000×) → ISP/peer capacity planning
```
...
|-------|----------------|-------------|
| Peer membership | Identity, gossip, failure detection | Eventual |
| Frontier | URL ownership & priorities | Owner peer strong for key |
| Fetch/politeness | Per-host budgets | Budget owner strong |
| Content/CAS | Segments & hashes | Immutable objects |
| Privacy/telemetry | Aggregates only | Eventual + DP |
...
3. **Primary service(s) for Peer-to-Peer Distributed Web Crawler** — business state machines & APIs.  
...
### 3.4a Frontier ownership

```text
url_key = hash(canonical_url)
owner_peers = shuffle_shard(url_key)  # e.g. 2 of N
enqueue → owner; duplicates merge by highest priority / earliest discovery
fetch lease: owner grants lease_id to fetcher peer; fencing on segment publish
```

Politeness budget for `host` owned similarly—**global host QPS is a leased budget**, not honor-system only.
...
```text
        ...
        +------+------+                          +------+------+
               ...
               v                                        v
        +-------------+     segments/WARC        +--------------+
        | Object Store|<------------------------>| Index / sink |
        +-------------+                          +--------------+
               ^
               | bootstrap (optional thin directory)
        +------+------+
        | Seed / Dir  |
        +-------------+
```
...
1. Durability before ACK for accepted critical writes in Peer-to-Peer Distributed Web Crawler.
...
**1× — correct MVP for Peer-to-Peer Distributed Web Crawler**
...
### 5.5 Politeness & adversarial peers

- Signed budgets; peers exceeding budgets lose reputation / partitions.
- Legal blocklists distributed with signatures and epochs.
- Content poisoning: multi-peer fetch agreement optional for high-value hosts.
- Privacy: don't ship raw cookies/PII found in pages into central logs; redact pipelines.
...
### 6.2 Top risks for Peer-to-Peer Distributed Web Crawler
...
> Design Peer-to-Peer Distributed Web Crawler with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Peer-to-Peer Distributed Web Crawler-specific

**Q: Who owns robots.txt truth?**  
A: Cached with TTL by host-budget owners; signed updates; err polite.

**Q: How to stop peer freeloaders?**  
A: Attestation + contribution scoring + encrypted segment credits.

**Q: Canonicalization bugs?**  
A: Strict URL canon pipeline; duplicate clusters via content hash.
...
Topic accents for **Peer-to-Peer Distributed Web Crawler** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Peer-to-Peer Distributed Web Crawler
         -> Durable SoT
         -> Async workers
...
> I'll split **Peer-to-Peer Distributed Web Crawler** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Peer-to-Peer Distributed Web Crawler |
|-------|------------------------|
...
*End of Peer-to-Peer Distributed Web Crawler system design.*

### 8.18 Deep topic notes — Peer-to-Peer Distributed Web Crawler


##### Frontier DHT details
```text
key = blake3(canonical_url)
vnodes = 128 per peer
replication = 2 (owner + successor)
enqueue merges: max(priority), min(next_fetch_at)
lease: 30s fetch lease with peer_id + fence
```

##### Politeness budget algorithm
Token bucket per host owned by `hash(host)%budget_shards`.
Global QPS = sum of shard grants; peers must request tokens (batched).
Starvation: fairness across peers; emergency legal stop epochs.

##### Segment format
WARC-like records with content-hash; manifest Merkle root published.
Dedup: URL key + content hash inverted index (approx with Bloom at peers).

##### Privacy of crawl telemetry
Export only: host-level success ratios, latency histograms, blocklist hits.
Never: cookies, Authorization headers, form bodies, personal pages if classified.

##### Peer churn
SWIM-like failure detection; re-replicate frontier keys on leave.
Bootstrap directory is optional convenience—not SoT for frontier.

##### Progressive scale
1×: few peers + shared Redis frontier (honest interview MVP shortcut) → evolve to DHT.
10×: host budget shards.
100×: segment lake + indexer separation.
1000×: hierarchical aggregators + specialized render fleet.
