# System Design: CI/CD System

> **Focus areas:** Build cache · artifact signing · progressive delivery · secrets · Apple code signing · SBOM · multi-platform (iOS/macOS/services)  
...
Goal: **bound the product**—a CI/CD platform that builds, tests, signs, and progressively deploys services and client apps in an Apple-like environment.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Triggers? | PR, merge, tag, manual, schedule | Event bus |
| F2 | Build targets? | Services + iOS/macOS apps | Matrix + signing |
| F3 | Tests? | Unit/integration/UI/device farm | Fan-out shards |
| F4 | Artifacts? | Immutable, content-addressed | CAS + SBOM |
| F5 | Signing? | Code sign + provenance attestations | HSM/key hierarchy |
| F6 | Deploy? | Canary → staged → full | Progressive delivery |
| F7 | Secrets? | Short-lived OIDC to vault | No long-lived in logs |
| F8 | Cache? | Remote build cache | Hermetic inputs |
| F9 | Policy? | Required checks + codeowners | Admission control |
| F10 | Multi-tenant? | Orgs/teams isolation | Quota + sandbox |
| F11 | Audit? | Who deployed what when | Tamper-evident log |
| F12 | Privacy / logging? | Minimize PII; retention caps; deletion APIs | Privacy budget + scrubbing pipelines |

**MVP functional scope (lock with interviewer):**

1. Hermetic build, test fan-out, sign, canary deploy, audit.
2. Clarify actors, trust boundaries, and on-device vs server responsibilities.
3. Define primary APIs with authn/authz and idempotency.
4. Durable SoT + explicit state machine(s).
5. Happy path + critical failure/fencing behavior.
6. Baseline estimation with split load classes and unit checks.
7. Privacy: retention, deletion, log minimization.
8. Observability & audit for sensitive actions.
9. Progressive scale plan 10×/100×/1000× with the first bottleneck named.

**Out of MVP (explicitly defer):**

- Multi-cloud exotic exotic exotic
- GUI IDE inside CI
...
|--------|----------|-----|------|--------|
| Builds / day | 10K | 100K | 1M | 10M |
| Peak concurrent builds | 200 | 2K | 20K | 100K |
| Test shards / build avg | 20 | 40 | 80 | 100 |
| Artifact GB / day | 5 TB | 50 TB | 500 TB | 5 PB |
| Deploys / day | 500 | 5K | 50K | 200K |
| Developers | 2K | 10K | 50K | 200K |
...
> Design CI/CD: SCM triggers, hermetic builds, test fan-out, artifact store + signing, deploy pipelines with canary/staged rollout, secrets, and audit—privacy and least-privilege baked in.
...
```text
1M builds/day × 80 shards = 80M tasks/day
Cache hit rate dominates cluster size; hermeticity prevents poison caches
```
...
3. **Primary service(s) for CI/CD System** — business state machines & APIs.  
...
```text
VCS event → Orchestrator → Build (hermetic) → Test matrix → Sign/SBOM → Artifact CAS
  → Deploy controller (canary/stage) → Verify → Promote / Rollback
```

Secrets via OIDC federation; provenance attestations required for prod deploy admission.
...
```text
 VCS -> CI Orchestrator -> Build cache/workers -> Tests -> Sign/SBOM -> CAS
                              |
                         Deploy controller -> Canary -> Stage -> Prod
                              |
                         Policy admission + audit log
```
...
1. Durability before ACK for accepted critical writes in CI/CD System.
...
**1× — correct MVP for CI/CD System**
...
Hermetic builds; verified provenance; signing keys in HSM; blocked unsigned prod; cache poisoning defenses (input root hashes).
...
### 6.2 Top risks for CI/CD System
...
> Design CI/CD System with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 CI/CD System-specific

**Q: How to prevent prod unsigned deploy?**  
A: Admission webhook requires provenance attestation.

**Q: Flaky tests?**  
A: Quarantine + rerun budgets; don't hide with infinite reruns.
...
Topic accents for **CI/CD System** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
## 9. Extended Deep Dive — CI/CD System

### 9.1 Apple platform context

Apple interviewers expect explicit treatment of **privacy** (what is logged/stored), **authentication** (human SSO + service mTLS), **offline/degraded behavior**, and **on-device vs server** boundaries. This section maps those lenses onto ci/cd system.
...
For ci/cd system, classify every field before adding to schemas (public / internal / confidential / restricted).
...
Name the **first bottleneck** you expect for ci/cd system (usually hot keys, write QPS, or cross-region latency).
...
## 9. Extra deep dive — CI/CD (Apple signing & supply chain)

### 9.1 Pipeline stages
VCS → Orchestrator → Hermetic build → Test matrix → Sign/SBOM → CAS → Deploy admission → Canary → Stage → Prod

### 9.2 Hermeticity
Pinned toolchain; input Merkle root; remote cache keyed by root; non-hermetic steps quarantined.

### 9.3 Code signing
HSM-backed keys; separate Apple app signing vs service artifact signing; provenance attestations required for prod admission webhook.

### 9.4 Multi-platform matrix
| Target | Notes |
|--------|-------|
| Linux services | container + SBOM |
| iOS/macOS | device farm / simulators; signing identities |
| ML artifacts | model cards + hash pins |

### 9.5 Progressive delivery
1%→10%→50%→100%; automatic halt on SLO burn; links to safe-restart controller.

### 9.6 Secrets
OIDC federation to vault; job-scoped short-lived creds; never echo in logs.

### 9.7 Flaky tests
Quarantine + bounded reruns; don’t hide failures with infinite retry.

### 9.8 Multi-tenant isolation
Org/team quotas; sandboxed runners; network egress allowlists.

### 9.9 Audit
Who deployed what digest when; immutable event log; break-glass tracked.

### 9.10 Scale jumps
10×: remote cache + shard tests; 100×: runner autoscaling pools; 1000×: regional build caches + hierarchical schedulers.

### 9.11 Deal-breakers
Unsigned prod; cache poisoning without input roots; long-lived cloud keys in CI; deploy without canary for user-facing.

### 9.12 60s summary
> Hermetic builds, signed provenance, attested deploy admission, progressive delivery with autohalt, short-lived secrets, Apple multi-platform matrix awareness.
