...
> **Focus areas:** Leases · heartbeats · retries · priorities · DAG/workflows · cron/delay · multi-tenant fairness · Apple privacy for payloads  
...
Goal: **bound the product**—a multi-tenant distributed job scheduler used by Apple-scale internal services (and potentially developer platforms) for one-shot, delayed, cron, and DAG workflows.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Who submits jobs? | Internal services + multi-tenant teams | tenant_id everywhere; quotas |
| F2 | Job types? | One-shot, delayed, cron, DAG | Separate job vs workflow |
| F3 | Payload size? | <256KB inline; else object pointer | Cap + encrypt blobs |
| F4 | Delivery? | At-least-once; idempotent effects | Leases + fencing |
| F5 | Retries? | Backoff + DLQ | Attempt history |
| F6 | Leases? | Heartbeat required | Server-owned expiry |
| F7 | Priorities? | H/N/L + fairness | WFQ / shuffle shard |
| F8 | Dependencies? | DAG fan-out/in | Workflow engine advances |
| F9 | Cancel? | Pending sure; running cooperative | Cancel token |
| F10 | Cron? | TZ-aware; catch-up policy | Materializers + jitter |
| F11 | Observability? | Timeline per job | Structured events |
...
> Design a distributed job scheduler with lease-based execution, at-least-once semantics, retries/DLQ, priorities/fairness, delayed/cron, and simple DAGs—home-cell single-writer state, privacy for job payloads.

---

## 2. Back-of-the-Envelope Estimation

### 2.1 Split write classes

| Class | Baseline peak | 1000× | Notes |
|-------|---------------|-------|-------|
| Submit | 500 QPS | 500K | Durable + idempotency |
| Lease | 500 | 500K | CAS hot path |
| Heartbeat | 2K | 2M | Dominates — separate plane |
| Complete | 500 | 500K | Unlock children async |

### 2.2 Concurrent work

```text
500 jobs/s × 30s ≈ 15K concurrent (mean); provision for p99 duration tails
1000×: 500K/s × 30s ≈ 15M concurrent → elastic workers
```

### 2.3 Storage

```text
Job ~1.5KB metadata; 10M/day × 7 × 1.5KB ≈ 105 GB hot baseline
1000×: 10B/day × 7 × 1.5KB ≈ 105 TB hot/warm — archive completes
Unit check: 10B×1.5KB = 15 TB/day metadata, not PB.
```
...
|-------|----------------|-------------|
| Job store | Durable jobs/workflows | Strong per home cell |
| Scheduler/lease | Ready sets + leases | Owner shard strong |
| Heartbeat | Lease extend | Sharded; batched |
| Workers | Execute effects | At-least-once |
| Crypto/privacy | Payload encryption | KMS envelopes |
...
3. **Primary service(s) for Distributed Job Scheduler** — business state machines & APIs.  
...
```text
READY → RUNNING with lease_id, lease_until
Heartbeat extends if lease_id matches
Complete requires lease_id; else 409 fencing
Reaper steals on expiry → at-least-once; handlers must be idempotent
Workflow engine (not workers) advances DAG
```
...
```text
 Clients -> API GW -> Submit/Query -> Job Store (home cell)
                         |               ^
                         v               |
                   Scheduler shards <----+
                         |
                   Lease/Heartbeat plane
                         |
                      Workers --> idempotent side effects
                         |
                   Workflow engine --> unlock children
```
...
1. Durability before ACK for accepted critical writes in Distributed Job Scheduler.
...
**1× — correct MVP for Distributed Job Scheduler**
...
Heartbeats dominate at scale—batch extends, lengthen leases, shard owners, never update monolithic OLTP per pulse. Workflow unlock via outbox.
...
### 6.2 Top risks for Distributed Job Scheduler
...
> Design Distributed Job Scheduler with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Distributed Job Scheduler-specific

**Q: Exactly-once?**  
A: At-least-once + idempotent effects + fencing.

**Q: Heartbeat meltdown?**  
A: Separate plane; batch; longer leases.
...
Topic accents for **Distributed Job Scheduler** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Distributed Job Scheduler
         -> Durable SoT
         -> Async workers
...
> I'll split **Distributed Job Scheduler** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Distributed Job Scheduler |
|-------|------------------------|
...
### 8.18 Deep topic notes — Distributed Job Scheduler


##### Lease protocol (normative)
```text
Lease: CAS state=READY→RUNNING, lease_id=uuid, attempt++
Heartbeat: UPDATE lease_until WHERE lease_id=?
Complete: CAS terminal WHERE lease_id=? ELSE 409
Reaper: RUNNING∧now>lease_until → READY (new attempt policy)
```

##### Workflow advancement
Single-threaded actor per `workflow_id` partition; outbox unlocks children in pages.

##### Heartbeat scaling
Batch 100 leases/RPC; heartbeat every L/3; lease plane ≠ job OLTP.

##### Fairness
Weighted fair queue per tenant; shuffle shard noisy tenants to 2-of-32 shards.

##### Payload privacy
Inline ≤256KB encrypted; larger object URL + DEK wrap; logs store ids only.
