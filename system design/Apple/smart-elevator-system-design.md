# System Design: Smart Elevator System

> **Focus areas:** Destination dispatch · accessibility · energy · realtime control · safety · offline building mode · Apple Maps-building vibes  
...
Goal: **bound the product**—a smart elevator control + destination-dispatch system for a multi-bank building, optimizing wait time, energy, and accessibility.
...
|---|-----------------|----------------------------------------|--------------------|
| F1 | Inputs? | Hall call, destination entry, car call | Unified request model |
| F2 | Banks? | Multiple shafts/banks | Group controller |
| F3 | Objectives? | Wait time, journey time, energy, fairness | Multi-objective scheduler |
| F4 | Accessibility? | Extended door, priority | Mode flags |
| F5 | Safety? | Hard interlocks independent of cloud | Edge safety PLC |
| F6 | Offline building? | Works without cloud | Local controller SoT |
| F7 | Traffic patterns? | Up-peak, lunch, down-peak | Policy profiles |
| F8 | Telemetry? | Anonymized counts | Privacy of movement |
| F9 | Fire/emergency? | Recall modes | Override hierarchy |
| F10 | UI? | Kiosk + mobile optional | Latency budgets |
| F11 | Maintenance? | Out-of-service cars | Dynamic topology |
...
|---|----------|-----------------|--------|
| N1 | Assignment latency? | Immediate UI | p99 < 200ms local controller |
...
7. Destination entered; car assigned; accessible mode honored.
8. Up-peak policy groups floors.
...
| Car out of service | Reassign dynamically |
| Fire recall | Override hierarchy |
| Cloud offline | Local controller continues |

### 1.4 Scales (Progressive)

| Metric | Baseline | 10× | 100× | 1,000× |
|--------|----------|-----|------|--------|
| Buildings | 1 | 10 | 100 | 1K |
| Cars / building | 16 | 16–64 | 64 | 128 |
| Requests / peak min | 200 | 2K | 20K | 200K |
| Controller decision QPS | 50 | 500 | 5K | 50K |
| Telemetry events / day | 1M | 10M | 100M | 1B |
| Cloud vs edge | edge primary | edge primary | fleet mgmt cloud | fleet + ML cloud |
...
> Design smart elevators: hall call / destination entry, bank assignment, car scheduling, accessibility modes, energy optimization, failure/safety—edge controller vs cloud boundary.
...
```text
Decisions every 100–500ms per bank
Cloud RTT too high/unreliable for door interlocks → edge SoT
Cloud: fleet learning, software update, anonymized stats
```
...
3. **Primary service(s) for Smart Elevator System** — business state machines & APIs.  
...
Safety + dispatch **on building controller**. Cloud optional for optimization hints and fleet updates—never door permission.
...
```text
 Kiosk/App -> Building Group Controller (dispatch + safety link)
                    |
         +----------+----------+
         v          v          v
        Car A      Car B      Car C   (embedded controllers)
                    |
            Cloud fleet mgr (optional hints, updates, anon stats)
```
...
1. Durability before ACK for accepted critical writes in Smart Elevator System.
...
**1× — correct MVP for Smart Elevator System**
...
### 5.5 Safety independence

Dispatch optimizer may die; cars still move safely under degraded local policy. Fire recall overrides all ML/energy goals.
...
### 6.2 Top risks for Smart Elevator System
...
> Design Smart Elevator System with explicit planes, durable ACK semantics, authz-before-data, privacy-minimized telemetry, and a progressive path from correct MVP to cell-based scale—calling out on-device/server boundaries and residency where Apple interviewers probe.
...
### 7.5 Smart Elevator System-specific

**Q: Cloud down?**  
A: Local controller continues; cloud hints ignored.

**Q: Energy vs wait time?**  
A: Weighted objective by time-of-day profile.
...
Topic accents for **Smart Elevator System** live in interview whiteboarding—map fields to your entities (Job, Volume, File, Hand, RouteRequest, etc.).
...
Clients / Devices
   -> Edge GW (authn, RL)
      -> Service planes for Smart Elevator System
         -> Durable SoT
         -> Async workers
...
> I'll split **Smart Elevator System** into planes, lock invariants (durability, fencing, authz, privacy), design APIs with idempotency, show an ASCII architecture, then scale via shards→cells→hierarchy while keeping on-device/server boundaries explicit.
...
| Theme | Application to Smart Elevator System |
|-------|------------------------|
...
*End of Smart Elevator System system design.*

### 8.18 Deep topic notes — Smart Elevator System


##### Cost function
```text
J = a*Σwait + b*Σjourney + c*energy + d*unfairness - accessibility_boost
```
Recompute each 100–500ms; safety PLC independent.

##### Modes
Up-peak, lunch, down-peak, fire recall, independent service, VIP.
