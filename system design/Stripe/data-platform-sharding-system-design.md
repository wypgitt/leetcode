# System Design: Production Data Platform Sharding

> **Focus areas:** Shard keys · Directory · Reshard · Dual-write  
...
|---|---|---|---|
| F1 | Shard key? | Stable high cardinality | tenant_id etc |
| F2 | Directory? | SoT routing | Epoch fencing |
| F3 | Reshard? | Dual-write range move | Verify backfill |
| F4 | Cross-shard query? | Avoid hot path | Scatter-gather async |
| F5 | Analytics? | Replicas/warehouse | Not primary OLTP |
| F6 | Split? | Range or hash | Plan capacity |
| F7 | Rollback? | Directory pointer revert | Dual-write window |
| F8 | Consistency? | Single-shard TX | No 2PC hot path |
| F9 | Observability? | Shard heat | Rebalance jobs |
| F10 | Tenant move? | Planned migration | Maintenance window optional |
| F11 | Global indexes? | Async | Not synchronous |
| F12 | Testing? | Shadow routing | Compare rows |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Production Data Platform Sharding
4. Admin recon/replay hooks for Production Data Platform Sharding
...
- Replacing all Stripe production systems in one Production Data Platform Sharding design
...
1. Shard key? → Stable high cardinality → durable write + audit
2. Directory? → SoT routing → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Analytics? → safe degradation documented
...
> Design OLTP sharding ~100K row writes/s zero-downtime resharding.
...
### 3.11 Domain model (Production Data Platform Sharding)
...
### 4.1 End-to-end (Production Data Platform Sharding)
...
```text
API Edge → Production Data Platform Sharding → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of production data platform sharding system design.*

### 8.19 Production Data Platform Sharding — rollout checklist
