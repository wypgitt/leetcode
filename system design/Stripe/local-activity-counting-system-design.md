# System Design: Near-Real-Time Local Activity Counting

> **Focus areas:** Geo windows · Edge agg · Late events  
...
|---|---|---|---|
| F1 | Input? | Activity events | Stream ingest |
| F2 | Geo? | Cell hierarchy | City block |
| F3 | Window? | 1m 5m 1h sliding | Tumbling option |
| F4 | Accuracy? | Approx OK | Error bound stated |
| F5 | Late events? | Watermark buffer | Drop or adjust |
| F6 | Privacy? | Aggregate only | No raw PII buckets |
| F7 | Use? | Ops dashboards | Not payout decisions |
| F8 | Edge? | Pre-aggregate | Reduce central load |
| F9 | Query? | count by cell window | Low latency |
| F10 | Scale? | Shard by geo | Hot city isolate |
| F11 | Uniques? | HyperLogLog | Not exact global |
| F12 | Backfill? | Replay kafka | Idempotent windows |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Near-Real-Time Local Activity Counting
4. Admin recon/replay hooks for Near-Real-Time Local Activity Counting
...
- Replacing all Stripe production systems in one Near-Real-Time Local Activity Counting design
...
1. Input? → Activity events → durable write + audit
2. Geo? → Cell hierarchy → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Late events? → safe degradation documented
...
> Design local activity counts ~100K events/s operational not money SoT.
...
### 3.11 Domain model (Near-Real-Time Local Activity Counting)
...
### 4.1 End-to-end (Near-Real-Time Local Activity Counting)
...
```text
API Edge → Near-Real-Time Local Activity Counting → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of near-real-time local activity counting system design.*

### 8.19 Near-Real-Time Local Activity Counting — rollout checklist
