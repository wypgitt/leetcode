# System Design: Distributed Metrics Counter

> **Focus areas:** Sharded counters · Flush · Exact vs approximate  
...
|---|---|---|---|
| F1 | Modes? | Exact vs approximate | Billing exact |
| F2 | Increment? | Sharded atomic | Local agg |
| F3 | Flush? | Periodic durable | WAL before ack |
| F4 | Read? | Sum shards+local | Staleness bound |
| F5 | Hot key? | Split counter | Per shard subkeys |
| F6 | Idempotency? | Flush replay safe | Dedupe batch |
| F7 | Billing? | Must be exact | No CRDT money |
| F8 | Dashboard? | HLL OK | Error bound |
| F9 | Multi-region? | Single writer shard | Or CRDT analytics only |
| F10 | Backfill? | Replay log | Idempotent |
| F11 | API? | inc get flush | Internal |
| F12 | Alerts? | Drift vs source | Recon |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Distributed Metrics Counter
4. Admin recon/replay hooks for Distributed Metrics Counter
...
- Replacing all Stripe production systems in one Distributed Metrics Counter design
...
1. Modes? → Exact vs approximate → durable write + audit
2. Increment? → Sharded atomic → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Hot key? → safe degradation documented
...
> Design distributed counters ~1M inc/s exact for billing approximate for analytics.
...
### 3.11 Domain model (Distributed Metrics Counter)
...
### 4.1 End-to-end (Distributed Metrics Counter)
...
```text
API Edge → Distributed Metrics Counter → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of distributed metrics counter system design.*

### 8.19 Distributed Metrics Counter — rollout checklist
