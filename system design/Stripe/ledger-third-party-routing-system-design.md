# System Design: Consistent Ledger + Unreliable Third-Party Routing

> **Focus areas:** Ledger SW · Outbox route · Inquiry · Saga  
...
|---|---|---|---|
| F1 | Ledger? | Strong consistent local | Commit first |
| F2 | Router? | Unreliable HTTP | Async outbox |
| F3 | External key? | Stable per operation | Never per retry |
| F4 | Timeout? | Inquiry not blind retry | Classify |
| F5 | Recon? | Break queue | Ops tickets |
| F6 | Ordering? | Local serial per book | External best effort |
| F7 | Compensation? | Reverse journal | If external failed confirmed |
| F8 | Idempotency? | Both sides keys | Linked |
| F9 | Availability? | Ledger not blocked | Decouple |
| F10 | Multi-region? | SW ledger home | Router regional |
| F11 | Audit? | Route attempts log | Append |
| F12 | API? | Post journal inquiry | Admin recon |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Consistent Ledger + Unreliable Third-Party Routing
4. Admin recon/replay hooks for Consistent Ledger + Unreliable Third-Party Routing
...
- Replacing all Stripe production systems in one Consistent Ledger + Unreliable Third-Party Routing design
...
1. Post balanced journal locally → commit → outbox route attempt
2. External router 200 → mark route COMPLETE
3. Router timeout → inquiry → found → COMPLETE without re-post journal
4. Router fail confirmed → compensation journal via workflow
5. Duplicate route external key → partner returns original → idempotent success
6. Recon job finds mismatch → open break → ops ticket
...
> Design consistent ledger with unreliable external router ~500 post/s.
...
### 3.11 Domain model (Consistent Ledger + Unreliable Third-Party Routing)
...
### 4.1 End-to-end (Consistent Ledger + Unreliable Third-Party Routing)

```text
Clients / Services → API Gateway (auth, rate limit)
        → Consistent Ledger + Unreliable Third-Party Routing API → Directory → Home Cell
...
```text
API Edge → Consistent Ledger + Unreliable Third-Party Routing → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of consistent ledger + unreliable third-party routing system design.*

### 8.19 Consistent Ledger + Unreliable Third-Party Routing — rollout checklist
...
### 8.20 Consistent Ledger + Unreliable Third-Party Routing — load test profile
