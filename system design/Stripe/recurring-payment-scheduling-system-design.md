# System Design: Recurring Payment Scheduling

> **Focus areas:** Billing cycles · Dunning · Timezone · Idempotent charge  
...
|---|---|---|---|
| F1 | Scheduler? | Durable timer queue | Not cron single box |
| F2 | Cycle key? | subscription+cycle_id | Idempotency |
| F3 | Timezone? | Merchant billing TZ | DST policy |
| F4 | Dunning? | Retry schedule | State machine |
| F5 | Cancel? | Stop future cycles | Race with charge TX |
| F6 | Proration? | Mid-cycle change | Ledger journals |
| F7 | Invoice? | Generate before charge | Immutable |
| F8 | Webhook? | invoice.paid failed | Outbox |
| F9 | Leap day? | Explicit rules | Feb 29 policy |
| F10 | Pause? | Skip cycles | Resume date |
| F11 | Ledger? | Charge journal | journal_key |
| F12 | Scale? | Shard by merchant | Peak midnight bursts |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Recurring Payment Scheduling
4. Admin recon/replay hooks for Recurring Payment Scheduling
...
- Replacing all Stripe production systems in one Recurring Payment Scheduling design
...
1. Scheduler? → Durable timer queue → durable write + audit
2. Cycle key? → subscription+cycle_id → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Cancel? → safe degradation documented
...
> Design recurring billing ~1K bills/s peak idempotent per cycle.
...
### 3.11 Domain model (Recurring Payment Scheduling)
...
### 4.1 End-to-end (Recurring Payment Scheduling)
...
```text
API Edge → Recurring Payment Scheduling → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of recurring payment scheduling system design.*

### 8.19 Recurring Payment Scheduling — rollout checklist
