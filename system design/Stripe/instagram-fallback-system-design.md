# System Design: Instagram-Like Feed (Stripe Fallback)

> **Focus areas:** Fanout · Rank · Media · Idempotent post · Degrade  
...
|---|---|---|---|
| F1 | Post? | Idempotent publish_id | Never duplicate |
| F2 | Feed read? | Home timeline | Ranked |
| F3 | Fanout? | Hybrid celeb pull | Avoid write amp |
| F4 | Media? | CDN object store | Separate upload idem |
| F5 | Follow? | Graph edges | Consistent enough |
| F6 | Rank? | Precompute scores | Approx stale OK |
| F7 | Like? | Counter sharded | At-least-once inc |
| F8 | Delete? | Tombstone | Eventual remove |
| F9 | Celebrity? | Pull model | No sync fanout |
| F10 | Regional? | Read replicas | Post home cell |
| F11 | Degrade? | Chronological fallback | If ranker down |
| F12 | Consistency? | Eventual feed | Strong post ACK |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Instagram-Like Feed (Stripe Fallback)
4. Admin recon/replay hooks for Instagram-Like Feed (Stripe Fallback)
...
- Replacing all Stripe production systems in one Instagram-Like Feed (Stripe Fallback) design
...
1. POST /posts with publish_id → durable post → 201
2. Celebrity post → pull model fans (no sync fanout)
3. Normal user post → hybrid fanout to active followers cache
4. GET /feed → merge followees → rank → return (stale OK 30s)
5. Ranker down → chronological fallback
6. Media upload idempotent object key → attach to post
...
> Design social feed ~500K read/s 10K post/s graceful degrade.
...
### 3.11 Domain model (Instagram-Like Feed (Stripe Fallback))
...
### 4.1 End-to-end (Instagram-Like Feed)
...
```text
API Edge → Instagram-Like Feed (Stripe Fallback) → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of instagram-like feed (stripe fallback) system design.*

### 8.19 Instagram-Like Feed (Stripe Fallback) — rollout checklist
