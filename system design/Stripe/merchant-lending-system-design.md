...
> **Focus areas:** Underwriting · Disbursement · Repayment sweep · Ledger  
...
|---|---|---|---|
| F1 | Underwriting? | Risk model decision | PII protect |
| F2 | Loan account? | Ledger books | Dedicated accounts |
| F3 | Disbursement? | Idempotent payout | journal_key |
| F4 | Repayment? | Sweep from captures | Hook idempotent |
| F5 | Interest? | Accrual schedule | Integer minor |
| F6 | Compliance? | Reg reporting | Audit |
| F7 | Default? | Collections state | Workflow |
| F8 | Offers? | Merchant accept | Time bound |
| F9 | Multi-currency? | Per loan currency | Explicit FX |
| F10 | Limits? | Max exposure | Platform caps |
| F11 | Recon? | Loan balance vs ledger | Daily |
| F12 | Regional? | Home per loan | SW |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Merchant Lending Platform
4. Admin recon/replay hooks for Merchant Lending Platform
...
- Replacing all Stripe production systems in one Merchant Lending Platform design
...
1. Underwriting? → Risk model decision → durable write + audit
2. Loan account? → Ledger books → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. Interest? → safe degradation documented
...
> Design merchant lending ~100 events/s with ledger-coupled idempotent money.
...
### 3.11 Domain model (Merchant Lending Platform)
...
### 4.1 End-to-end (Merchant Lending Platform)
...
```text
API Edge → Merchant Lending Platform → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of merchant lending platform system design.*

### 8.19 Merchant Lending Platform — rollout checklist
