...
> **Focus areas:** Consistent hash · Leases · Invalidation · Hot keys  
...
|---|---|---|---|
| F1 | Use case? | Read-heavy internal | Not money SoT |
| F2 | Eviction? | LRU per shard | Memory cap |
| F3 | Routing? | Consistent hash | Virtual nodes |
| F4 | Invalidation? | Pub/sub lease | Version bump |
| F5 | TTL? | Per key class | Short money hints |
| F6 | Hot key? | Replica fanout | Detect skew |
| F7 | Stale? | Explicit SLO | Bypass for payout |
| F8 | Multi-region? | Regional replicas | No active-active write |
| F9 | Size limit? | Max entry bytes | Reject huge |
| F10 | Near cache? | Local L1 | Combined L2 |
| F11 | Warmup? | On deploy | Gradual |
| F12 | Observability? | Hit rate evictions | Per shard |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Distributed LRU Cache
4. Admin recon/replay hooks for Distributed LRU Cache
...
1. Use case? → Read-heavy internal → durable write + audit
2. Eviction? → LRU per shard → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. TTL? → safe degradation documented
...
> Design distributed LRU cache ~200K get/s; never SoT for money balances.
...
### 3.11 Domain model (Distributed LRU Cache)
...
### 4.1 End-to-end (Distributed LRU Cache)
...
```text
API Edge → Distributed LRU Cache → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of distributed lru cache system design.*

### 8.19 Distributed LRU Cache — rollout checklist
