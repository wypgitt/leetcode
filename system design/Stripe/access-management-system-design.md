# System Design: Access Management System

> **Focus areas:** SSO · SCIM · Dashboard roles · Sessions · MFA  
...
|---|---|---|---|
| F1 | Users? | Merchant team+employees | Directory sync |
| F2 | SSO? | SAML OIDC | Enterprise |
| F3 | Roles? | Admin developer etc | RBAC dashboard |
| F4 | Sessions? | HttpOnly cookies | Rotation |
| F5 | MFA? | Required admin | TOTP/WebAuthn |
| F6 | SCIM? | Provision users | HR sync |
| F7 | Audit? | Login role changes | Immutable |
| F8 | API keys? | Separate machine creds | Rotation policy |
| F9 | Revoke? | Global session invalidate | <60s SLO |
| F10 | Cross-merchant? | Strict isolation | Never bleed |
| F11 | Break-glass? | Internal support | Ticket+jump |
| F12 | Compliance? | SOC evidence | Access reviews |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Access Management System
4. Admin recon/replay hooks for Access Management System
...
1. Users? → Merchant team+employees → durable write + audit
2. SSO? → SAML OIDC → correct domain behavior
3. Retry with same Idempotency-Key → stored response (no double effect)
4. Read-after-write from home cell → RYW consistent state
5. MFA? → safe degradation documented
...
> Design access mgmt ~10K auth/s for Dashboard and internal tools.
...
### 3.11 Domain model (Access Management System)
...
### 4.1 End-to-end (Access Management System)
...
```text
API Edge → Access Management System → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of access management system system design.*

### 8.19 Access Management System — rollout checklist
