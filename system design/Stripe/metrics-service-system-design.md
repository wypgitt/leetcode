# System Design: Metrics Service

> **Focus areas:** Time-series · Cardinality control · Rollups · Query · Retention  
...
|---|---|---|---|
| F1 | Emitters? | SDK sidecars | Auth per service |
| F2 | Types? | Counter gauge histogram | Typed agg |
| F3 | Labels? | Low cardinality | Governor deny |
| F4 | Query? | Range queries dashboards | Query engine |
| F5 | Retention? | Raw short rollups long | Tiered |
| F6 | Multi-tenant? | Team isolation | Quotas |
| F7 | Exactness? | Billing stricter | Separate path |
| F8 | Alerts? | Threshold rules | Eval service |
| F9 | Late data? | Watermark window | Bound lateness |
| F10 | Fan-in? | Cross-cell agg | Hierarchy |
| F11 | Registry? | Metric names | Governance |
| F12 | API? | Write+query HTTP | Versioned |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/metrics/write batch ingest` — idempotent where mutating.
2. `POST /v1/query/range PromQL-like` — idempotent where mutating.
3. Cardinality governor at ingest
4. Rollups 1m/5m/1h automatic
5. Tenant quota enforcement
6. Alert rule evaluation hook
...
1. SDK batch push → ingest validate labels → TSDB write
2. High-cardinality label rejected at edge → 400 metric_rejected
3. Query range → rollup tier selected by time span
4. Late sample within watermark → accepted into window
5. Cross-cell fan-in → hierarchical aggregator
6. Billing counter exact path separate from analytics HLL
...
> Design internal metrics platform ~500K samples/s to 1000x cardinality-safe.
...
```text
Ingest never blocks payment hot path; cardinality limits enforced at edge
```
...
| API | Semantics |
|---|---|
| `POST /v1/metrics/write` | Idempotent / typed errors / RYW where applicable |
| `POST /v1/query/range` | Idempotent / typed errors / RYW where applicable |
| `GET /v1/series` | Idempotent / typed errors / RYW where applicable |

Stripe-flavored: `Idempotency-Key`, `Request-Id`, typed errors, expandable objects.

### 3.11 Domain model (Metrics Service)
...
### 4.1 End-to-end (Metrics Service)
...
```text
API Edge → Metrics Service → Domain Store
                              │
                              ├─► Idempotency / Audit
...
Related docs: distributed-metrics-counter, apm-system-design, rate-limiter.
...
*End of metrics service system design.*

### 8.19 Metrics Service — rollout checklist
