# System Design: Superhero / Incident Dispatch Marketplace

> **Focus areas:** Geo match · SLA · Idempotent assign  
...
Goal: bound **dispatch marketplace**—geo matching, offers, SLA escalation, idempotent assignment.

### 1.1 Functional Requirements

| # | Question to ask | Expected / typical interviewer answer | Design implication |
|---|---|---|---|
| F1 | Supply? | Heroes on duty | Location heartbeat |
| F2 | Demand? | Incidents | Priority severity |
| F3 | Match? | Geo+skills | Rank score |
| F4 | Assign? | First accept wins | TX idempotent |
| F5 | SLA? | Escalation timers | Auto re-offer |
| F6 | Audit? | Who assigned when | Immutable |
| F7 | Cancel? | Incident resolved | Release hero |
| F8 | Fairness? | Round robin geo | Anti starvation |
| F9 | Pay? | Optional ledger hook | Idempotent bounty |
| F10 | Regional? | Geo shard | Partition outages |
| F11 | Offers? | Expire TTL | No ghost accept |
| F12 | Retry? | Same incident_id key | No double assign |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Superhero / Incident Dispatch Marketplace
4. Admin recon/replay hooks for Superhero / Incident Dispatch Marketplace
...
- Replacing all Stripe production systems in one Superhero / Incident Dispatch Marketplace design
...
> Design incident dispatch marketplace ~500/s no double-assign.
...
### 3.11 Domain model (Superhero / Incident Dispatch Marketplace)
...
### 4.1 End-to-end (Superhero / Incident Dispatch Marketplace)
...
```text
API Edge → Superhero / Incident Dispatch Marketplace → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of superhero / incident dispatch marketplace system design.*

### 8.19 Superhero / Incident Dispatch Marketplace — rollout checklist
