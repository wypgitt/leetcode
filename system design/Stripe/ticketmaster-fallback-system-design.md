# System Design: Ticketmaster-Like Ticketing (Stripe Fallback)

> **Focus areas:** Seat holds · Inventory · Idempotent purchase · Queue  
...
|---|---|---|---|
| F1 | Inventory? | Seat map shards | Linearizable sold |
| F2 | Hold? | TTL token | Expire release |
| F3 | Purchase? | checkout_key idempotent | One sold state |
| F4 | Oversell? | TX lock seat row | Reject double |
| F5 | Queue? | Virtual waiting room | Fair token |
| F6 | Payment? | After seat TX | Hold token link |
| F7 | Partial cart? | Multi-seat atomic | All or none |
| F8 | Refund? | Release inventory | Ledger optional |
| F9 | Hot event? | Shard by section | Not one row |
| F10 | Regional? | Home per event shard | SW |
| F11 | Scan? | Ticket barcode unique | Fraud check |
| F12 | Resale? | Transfer lock | Policy |

**MVP functional scope (lock with interviewer):**

1. `POST /v1/resources` — idempotent where mutating.
2. `GET /v1/resources/{id}` — idempotent where mutating.
3. Domain audit trail for Ticketmaster-Like Ticketing (Stripe Fallback)
4. Admin recon/replay hooks for Ticketmaster-Like Ticketing (Stripe Fallback)
...
- Replacing all Stripe production systems in one Ticketmaster-Like Ticketing (Stripe Fallback) design
...
1. POST hold → seat row lock TX → hold token TTL 10m
2. Hold expires → sweeper releases inventory
3. POST purchase checkout_key → verify hold → mark SOLD → payment
4. Duplicate purchase key → 200 original order
5. Waiting room token → admit to checkout shard
6. Payment fails → release hold or extend policy
...
> Design ticketing ~50K hold/s 5K purchase/s anti-oversell.
...
### 3.11 Domain model (Ticketmaster-Like Ticketing (Stripe Fallback))
...
### 4.1 End-to-end (Ticketmaster-Like Ticketing)
...
```text
API Edge → Ticketmaster-Like Ticketing (Stripe Fallback) → Domain Store
                              │
                              ├─► Idempotency / Audit
...
*End of ticketmaster-like ticketing (stripe fallback) system design.*

### 8.19 Ticketmaster-Like Ticketing (Stripe Fallback) — rollout checklist
