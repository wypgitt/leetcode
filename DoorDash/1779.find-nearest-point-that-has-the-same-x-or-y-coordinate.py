...
# points[i] = [a_i, b_i] represents that a point exists at (a_i, b_i). A point
# is valid if it shares the same x-coordinate or the same y-coordinate as your
# location.
#
...
class Solution:
    def nearestValidPoint(self, x: int, y: int, points: List[List[int]]) -> int:
        ...
        Valid point shares x or y with (x,y). Among valid, pick smallest
        Manhattan distance; ties → smallest index.

        Algorithm:
        - Scan points; if valid: update best (dist, index).
