...
class Solution:
    def __init__(self, w: List[int]):
        ...
        Build prefix sums of weights so each index occupies a contiguous
        segment of the total mass. pickIndex samples uniformly in [1, total]
        and binary-searches the segment.

        Algorithm:
        - prefix[i] = sum(w[0..i]); total = prefix[-1].

        Complexity: O(n) preprocess, O(1) space beyond prefix.
        """
        self.prefix = []
        running = 0
        for weight in w:
    ...
    def pickIndex(self) -> int:
        """
        Interview explanation:
        Draw target in [1, total] and bisect_left on prefix sums to map back
        to an index proportional to its weight.

        Algorithm:
        - target = random.randint(1, total); return bisect_left(prefix, target).
