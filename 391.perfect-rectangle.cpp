...
public:
    bool isRectangleCover(vector<vector<int>>& rectangles) {
        long long minX = LLONG_MAX, minY = LLONG_MAX, maxX = LLONG_MIN, maxY = LLONG_MIN, area = 0;
        ...
        for (auto& r : rectangles) {
            ...
            minX = min<long long>(minX, x1);
            minY = min<long long>(minY, y1);
            maxX = max<long long>(maxX, x2);
            maxY = max<long long>(maxY, y2);
        ...
        set<pair<int, int>> want{{(int)minX, (int)minY}, {(int)minX, (int)maxY}, {(int)maxX, (int)minY}, {(int)maxX, (int)maxY}};
