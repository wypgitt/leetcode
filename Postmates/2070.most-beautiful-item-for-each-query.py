...
class Solution:
    def maximumBeauty(self, items: List[List[int]], queries: List[int]) -> List[int]:
        ...
        Each item has [price, beauty]. For each query price, max beauty among
        items with price <= query (0 if none).

        Algorithm:
        - Sort items by price; keep running max beauty; for each query
          bisect_right on prices.
        ...
        for q in queries:
            ...
            ans.append(beauties[i] if i >= 0 else 0)
        return ans
# @lc code=end
