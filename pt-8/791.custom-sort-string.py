...
# arrangements like "dbca" or "bcda" would also be valid, as long as "b", "c",
...
class Solution:
    def customSortString(self, order: str, s: str) -> str:
        ...
        Permute s so characters in `order` appear in that relative order; others
        anywhere. Count frequencies in s; emit chars in order sequence, then
        remaining chars.

        Algorithm:
        - cnt = Counter(s)
        - Build result: for c in order: append c * cnt[c]; cnt[c]=0
        - Append remaining counts.

        Complexity: O(|s| + |order|) time, O(σ) space.
        """
        cnt = Counter(s)
        parts = []
        for c in order:
            if cnt[c]:
                parts.append(c * cnt[c])
                cnt[c] = 0
