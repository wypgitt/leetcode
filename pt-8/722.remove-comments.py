...
class Solution:
    def removeComments(self, source: List[str]) -> List[str]:
        ...
        Scan the source as a stream, tracking whether we are inside a block
        comment. Line comments discard the rest of the line; block comments may
        span lines. Non-comment characters form output lines (empty lines drop).

        Algorithm:
        - For each line, i = 0; while i < len(line):
          - If in block and see "*/": leave block, i += 2
          - Elif in block: i += 1
          - Elif see "//": break (rest of line ignored)
          - Elif see "/*": enter block, i += 2
          - Else append char, i += 1
        - When not in block and buffer nonempty after a line, flush as a result line.

        Complexity: O(N) time and O(N) space for total source length N.
        """
        ans: List[str] = []
        buf: List[str] = []
        in_block = False
