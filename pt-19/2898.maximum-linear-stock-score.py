#
# @lc app=leetcode id=2898 lang=python3
#
...
# Testcase Example:  "[1,5,3,7,8]"
#
#
# Given a 1-indexed integer array prices, where prices[i] is the price of
# a particular stock on the i^th day, your task is to select some of the
...
# selection is linear.
# The sum of all the elements is 35 which is the maximum possible some out
# of every selection.
#
# Constraints:
#
...
class Solution:
    def maxScore(self, prices: List[int]) -> int:
        ...
        Premium: a subsequence of (1-indexed) days is linear if consecutive
        selected prices differ by exactly their index difference. Score is sum
        of selected prices; maximize it.

        Algorithm:
        - prices[j]-prices[i] == j-i iff prices[i]-i is constant (0-based same).
        - Sum prices by key (price - index); return the maximum group sum.
