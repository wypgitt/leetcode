#
# @lc app=leetcode id=3056 lang=python3
#
# [3056] Snaps Analysis
#
...
# Testcase Example:  "{\"headers\":{\"Activities\":[\"activity_id\",\"user_id\",\"activity_type\",\"time_spent\"],\"Age\":[\"user_id\",\"age_bucket\"]},\"rows\":{\"Activities\":[[7274,123,\"open\",4.50],[2425,123,\"send\",3.50],[1413,456,\"send\",5.67],[2536,456,\"open\",3.00],[8564,456,\"send\",8.24],[5235,789,\"send\",6.24],[4251,123,\"open\",1.25],[1435,789,\"open\",5.25]],\"Age\":[[123,\"31-35\"],[789,\"21-25\"],[456,\"26-30\"]]}}"
#
#
# Table: Activities
#
...
class Solution:
    def solve(self) -> str:
        ...
        Premium SQL: Activities(activity_id, user_id, activity_type, time_spent)
        and Age(user_id, age_bucket). Per age_bucket, % of time on send vs open,
        rounded to 2 decimals.

        Algorithm:
        - Join on user_id; GROUP BY age_bucket; conditional SUMs / total * 100.
...
SELECT
    ...
    ROUND(
        100 * SUM(IF(activity_type = 'open', time_spent, 0)) / SUM(time_spent),
        2
    ) AS open_perc
FROM
    Activities
