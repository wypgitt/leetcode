...
# them. For example, the key 2 is mapped with ["a","b","c"], we need to
# push the key one time to type "a", two times to type "b", and three
# times to type "c" .
#
...
class Solution:
    def minimumPushes(self, word: str) -> int:
        ...
        Remap 8 keys; letters may repeat. Assign highest-frequency letters to the
        cheapest key positions (first push slots, then second, ...).

        Algorithm:
        - Count letter frequencies; sort descending.
        - i-th most frequent letter costs (i // 8 + 1) * frequency.

        Complexity: O(n + Σ log Σ) time with Σ<=26, O(Σ) space.
        """
