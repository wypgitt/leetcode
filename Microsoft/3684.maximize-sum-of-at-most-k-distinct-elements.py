...
#
# Return an array containing the chosen numbers in strictly descending
# order.
#
# Example 1:
#
...
# Explanation:
#
# The maximum sum is 3, which is attained by choosing 1 and 2. We
# rearrange them in strictly descending order as [2, 1].
#
# Constraints:
#
# 1 <= nums.length <= 100
...
class Solution:
    def maxKDistinct(self, nums: List[int], k: int) -> List[int]:
        ...
        Distinctness means each value at most once, so take the k largest
        unique numbers and emit them descending.

        Algorithm:
        - Deduplicate, sort descending, return the first min(k, unique) values.
        - Alternate: put into a set then sorted(..., reverse=True)[:k].
