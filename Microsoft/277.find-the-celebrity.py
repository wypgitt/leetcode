...
# not one. You are only allowed to ask questions like: "Hi, A. Do you know
# B?" to get information about whether A knows B. You need to find out the
...
#
# Note that the n x n 2D array graph given as input is not directly
# available to you, and instead only accessible through the helper
# function knows. graph[i][j] == 1 represents person i knows person j,
...
# means person i does not know person j. The celebrity is the person
...
class Solution:
    def findCelebrity(self, n: int) -> int:
        ...
        A celebrity is known by everyone and knows no one. Candidate elimination:
        if a knows b, a cannot be celebrity; else b cannot.

        Algorithm:
        - Start candidate = 0; for i in 1..n-1, if knows(candidate, i): candidate = i.
        - Verify candidate: for all i != candidate, not knows(candidate, i) and knows(i, candidate).
        - Return candidate or -1.

        Complexity: O(n) calls to knows, O(1) space.
        """
        candidate = 0
        for i in range(1, n):
