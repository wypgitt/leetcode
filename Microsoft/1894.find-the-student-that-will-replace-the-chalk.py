...
class Solution:
    def chalkReplacer(self, chalk: List[int], k: int) -> int:
        ...
        Students use chalk[i] in cycle; find who replaces when remaining chalk
        insufficient. Reduce k %= sum(chalk); scan once.

        Algorithm:
        - total = sum(chalk); k %= total; for i,c: if k<c return i; k-=c.
    ...
    def chalkReplacer_prefix(self, chalk: List[int], k: int) -> int:
        """
        Interview explanation:
        Alternate: prefix sums + linear/binary search for first prefix > k%sum.

        Algorithm:
        - Build pref; target = k % pref[-1]; find first i with pref[i] > target.
