...
class Solution:
    def groupThePeople(self, groupSizes: List[int]) -> List[List[int]]:
        ...
        Person i must be in a group of size groupSizes[i]. Bucket people by
        required size; whenever a bucket reaches that size, emit a group.

        Algorithm:
        - buckets defaultdict(list); ans=[].
        - For i,sz: append i; if len==sz: ans.append(bucket); clear.
