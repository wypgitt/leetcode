...
class Solution {
    ...
    public List<String> wordSubsets(String[] words1, String[] words2) {
        int[] req = new int[26];
        for (String b : words2) {
            int[] cntB = letterCounts(b);
            for (int i = 0; i < 26; i++) {
                req[i] = Math.max(req[i], cntB[i]);
            }
        }

        List<String> ans = new ArrayList<>();
        for (String a : words1) {
            int[] cntA = letterCounts(a);
            if (covers(cntA, req)) {
                ans.add(a);
            }
        }
    ...
    private static int[] letterCounts(String s) {
        int[] c = new int[26];
        int base = 'a';
    ...
    private static boolean covers(int[] cntA, int[] req) {
        for (int i = 0; i < 26; i++) {
            if (cntA[i] < req[i]) {
                return false;
            }
