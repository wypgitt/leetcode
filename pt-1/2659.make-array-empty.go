...
//
// @lc app=leetcode id=2659 lang=golang
//
...
// CountOperationsToEmptyArray2659 returns the number of operations required
// to empty nums under the "remove if front is min else rotate" rule.
func CountOperationsToEmptyArray2659(nums []int) int64 {
	n := len(nums)
	idx := make([]int, n)
	for i := 0; i < n; i++ {
		idx[i] = i
