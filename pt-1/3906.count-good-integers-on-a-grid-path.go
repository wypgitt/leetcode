...
//
// @lc app=leetcode id=3906 lang=golang
//
...
// CountGoodIntegersOnPath3906 counts good integers in [l, r] for the given path.
func CountGoodIntegersOnPath3906(l, r int, directions string) int {
	key := make([]bool, 16)
	row, col := 0, 0
	key[0] = true
	...
	calc := func(x int) int {
		...
		s := fmt.Sprintf("%016d", x)

		memo := make(map[key3906]int)
		var dfs func(pos, last int, lim bool) int
		dfs = func(pos, last int, lim bool) int {
			k := key3906{pos, last, lim}
			if v, ok := memo[k]; ok {
				return v
			}
			if pos == 16 {
			...
			for i := start; i <= end; i++ {
				...
				nextLim := lim && i == end
				res += dfs(pos+1, nextLast, nextLim)
			}
			memo[k] = res
			return res
		}
