...
func IsEscapePossible1036(blocked [][]int, source []int, target []int) bool {
	const n = 1000000
	...
	for _, b := range blocked {
		blockedSet[pack(b[0], b[1])] = struct{}{}
	}

	dx := [4]int{-1, 0, 1, 0}
	dy := [4]int{0, 1, 0, -1}

	walk := func(sx, sy, tx, ty int) bool {
		stack := make([]int64, 0, 1024)
		stack = append(stack, pack(sx, sy))
		vis := make(map[int64]struct{}, lim+8)
	...
	return walk(sx, sy, tx, ty) && walk(tx, ty, sx, sy)
}
