...
//
// @lc app=leetcode id=3910 lang=golang
//
...
// - Integer bitmasks for subset and vis: O(1) space, fast bit ops for n <= 13.
...
// EvenSumSubgraphs3910 counts nonempty connected induced subgraphs with even node-value sum.
func EvenSumSubgraphs3910(nums []int, edges [][]int) int {
	n := len(nums)
	g := make([][]int, n)
	for _, e := range edges {
		u, v := e[0], e[1]
