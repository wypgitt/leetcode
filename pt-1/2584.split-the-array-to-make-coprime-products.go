...
func primeFactors2584(num int) []int {
...
// FindValidSplit2584 returns the smallest split index i such that left/right products are coprime, else -1.
func FindValidSplit2584(nums []int) int {
	right := make(map[int]int)
	for _, v := range nums {
		for _, p := range primeFactors2584(v) {
			right[p]++
		}
	}

	leftBridge := make(map[int]int)

	for i := 0; i < len(nums)-1; i++ {
		for _, p := range primeFactors2584(nums[i]) {
			right[p]--
			if right[p] == 0 {
				delete(right, p)
				delete(leftBridge, p)
