...
// =============================================================================
//
// • **Slice as stack** — push/pop at end are **O(1)**.
...
// StockSpanner901 provides amortized O(1) next(price) using a monotone decreasing stack.
//
...
type StockSpanner901 struct {
	// Stack of (price, aggregated_span_for_that_block); prices strictly decrease bottom-up.
	stack [][2]int // [price, span]
}

// NewStockSpanner901 constructs an empty spanner.
func NewStockSpanner901() StockSpanner901 {
	return StockSpanner901{stack: nil}
}

// Next returns the span for today's price.
func (s *StockSpanner901) Next(price int) int {
	span := 1
