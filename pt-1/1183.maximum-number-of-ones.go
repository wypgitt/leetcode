...
// MaximumNumberOfOnes1183 returns the maximum number of ones under the sideLength window constraint.
func MaximumNumberOfOnes1183(width int, height int, sideLength int, maxOnes int) int {
	L := sideLength
	rowOrColCount := func(dim, residue int) int {
		if residue >= dim {
			return 0
		}
	...
	freq := make([]int, 0, L*L)
	for a := 0; a < L; a++ {
		ra := rowOrColCount(height, a)
		for b := 0; b < L; b++ {
			rb := rowOrColCount(width, b)
			freq = append(freq, ra*rb)
		}
	}
