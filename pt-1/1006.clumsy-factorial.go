...
// Clumsy1006 computes the clumsy factorial described in the problem.
func Clumsy1006(n int) int {
	stk := []int{n}
	k := 0
	for x := n - 1; x >= 1; x-- {
		top := stk[len(stk)-1]
		...
		if k == 0 {
			stk = append(stk, top*x)
		} else if k == 1 {
			// Go integer division truncates toward zero, matching LeetCode semantics.
			stk = append(stk, top/x)
		} else if k == 2 {
			stk = append(stk, top, x)
		} else {
