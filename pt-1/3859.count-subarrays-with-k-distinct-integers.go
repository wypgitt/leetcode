...
//
// @lc app=leetcode id=3859 lang=golang
//
...
//
// Data structures
// - map[int]int for per-value frequencies in the current window [l..r].
// - Integer counters:
...
// CountSubarrays3859 counts subarrays with exactly k distinct values where each appears at least m times.
func CountSubarrays3859(nums []int, k, m int) int {
	if k <= 0 {
		return 0
	}
	return atMost3859(nums, k, m) - atMost3859(nums, k-1, m)
}

// atMost3859 counts subarrays with <= K distinct values and every present value appearing >= m times.
func atMost3859(nums []int, K, m int) int {
	if K <= 0 {
		return 0
	}
	n := len(nums)
