...
public:
    ...
    void add(int i, int delta) { while (i <= n) { tree[i] += delta; i += i & -i; } }
    int sum(int i) { int total = 0; while (i > 0) { total += tree[i]; i -= i & -i; } return total; }
};
...
public:
    long long minInversionCount(vector<int>& nums, int k) {
        ...
        for (int i = 0; i < (int)nums.size(); ++i) rank[i] = lower_bound(sorted.begin(), sorted.end(), nums[i]) - sorted.begin() + 1;
        FenwickTree bit(sorted.size());
        long long cur = 0;
        for (int i = 0; i < k; ++i) {
            cur += i - bit.sum(rank[i]);
            bit.add(rank[i], 1);
        }
        long long ans = cur;
        for (int right = k; right < (int)nums.size(); ++right) {
            int left = right - k;
            cur -= bit.sum(rank[left] - 1);
            bit.add(rank[left], -1);
            cur += (k - 1) - bit.sum(rank[right]);
            bit.add(rank[right], 1);
            ans = min(ans, cur);
        }
        return ans;
    }
