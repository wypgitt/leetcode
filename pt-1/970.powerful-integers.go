...
// PowerfulIntegers970 returns all integers of the form x^i + y^j <= bound.
func PowerfulIntegers970(x int, y int, bound int) []int {
	powers := func(base int) []int {
		out := make([]int, 0, 32)
		v := 1
		for v <= bound {
			out = append(out, v)
	...
	seen := make(map[int]struct{}, len(px)*len(py))
	for _, a := range px {
		for _, b := range py {
			s := a + b
			if s <= bound {
