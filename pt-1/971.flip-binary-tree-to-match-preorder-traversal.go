...
// FlipMatchVoyage971 returns the values of nodes flipped to match voyage preorder, or [-1].
func FlipMatchVoyage971(root *TreeNode, voyage []int) []int {
	i := 0
	flips := make([]int, 0)
	...
	dfs = func(node *TreeNode) bool {
		...
		if i >= len(voyage) || node.Val != voyage[i] {
			return false
		}
		i++
		...
		if node.Left != nil && voyage[i] == node.Left.Val {
			return dfs(node.Left) && dfs(node.Right)
		}
		if node.Right != nil && voyage[i] == node.Right.Val {
